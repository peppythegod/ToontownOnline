...
class DistributedSellbotBossAI(DistributedBossCogAI.DistributedBossCogAI,
                               FSM.FSM):
    ...
    def hitBoss(self, bossDamage):
        ...
        if self.bossDamage >= self.bossMaxDamage:
            ...
            self._DistributedSellbotBossAI__recordHit()
    ...
    def touchCage(self):
        ...
        if toon:
            ...
            toon._DistributedSellbotBossAI__touchedCage = 1
            self._DistributedSellbotBossAI__goodJump(avId)
    ...
    def doNextAttack(self, task):
        ...
        if attackCode == ToontownGlobals.BossCogAreaAttack:
            self._DistributedSellbotBossAI__doAreaAttack()
        elif attackCode == ToontownGlobals.BossCogDirectedAttack:
            self._DistributedSellbotBossAI__doDirectedAttack()
        else:
            self.b_setAttackCode(attackCode)

    def _DistributedSellbotBossAI__doAreaAttack(self):
    ...
    def _DistributedSellbotBossAI__doDirectedAttack(self):
        if self.nearToons:
    ...
    def _DistributedSellbotBossAI__sendDooberIds(self):
        dooberIds = []
    ...
    def enterRollToBattleTwo(self):
        ...
        self.barrier = self.beginBarrier(
            ...
            self._DistributedSellbotBossAI__doneRollToBattleTwo)

    def _DistributedSellbotBossAI__doneRollToBattleTwo(self, avIds):
    ...
    def enterPrepareBattleTwo(self):
        self.barrier = self.beginBarrier(
            ...
            self._DistributedSellbotBossAI__donePrepareBattleTwo)
        self.makeBattleTwoBattles()

    def _DistributedSellbotBossAI__donePrepareBattleTwo(self, avIds):
    ...
    def enterPrepareBattleThree(self):
        self.barrier = self.beginBarrier(
            ...
            self._DistributedSellbotBossAI__donePrepareBattleThree)

    def _DistributedSellbotBossAI__donePrepareBattleThree(self, avIds):
    ...
    def enterBattleThree(self):
        ...
        for toonId in self.involvedToons:
            ...
            if toon:
                toon._DistributedSellbotBossAI__touchedCage = 0
                continue

        self.waitForNextAttack(5)
        self.waitForNextStrafe(9)
        self.cagedToonDialogIndex = 100
        self._DistributedSellbotBossAI__saySomethingLater()

    def _DistributedSellbotBossAI__saySomething(self, task=None):
        index = None
        avId = 0
        ...
        toon = simbase.air.doId2do.get(avId)
        if toon._DistributedSellbotBossAI__touchedCage:
    ...
    def _DistributedSellbotBossAI__saySomethingLater(self, delayTime=15):
        taskName = self.uniqueName('CagedToonSaySomething')
        taskMgr.remove(taskName)
        taskMgr.doMethodLater(
            delayTime, self._DistributedSellbotBossAI__saySomething, taskName)

    def _DistributedSellbotBossAI__goodJump(self, avId):
        currState = self.getCurrentOrNextState()
        if currState != 'BattleThree':
            return None
    ...
    def enterVictory(self):
        ...
        self.barrier = self.beginBarrier(
            ...
            self._DistributedSellbotBossAI__doneVictory)

    def _DistributedSellbotBossAI__doneVictory(self, avIds):
    ...
    def _DistributedSellbotBossAI__resetDoobers(self):
        for suit in self.doobers:
            suit.requestDelete()
    ...
    def _DistributedSellbotBossAI__recordHit(self):
        now = globalClock.getFrameTime()
        self.hitCount += 1
