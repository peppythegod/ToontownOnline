...
class DistributedCashbotBossGoonAI(
    ...
    legLength = 10
    directionTable = [(0, 15), (10, 10), (-10, 10), (20, 8), (-20, 8), (40, 5),
                      (-40, 5), (60, 4), (-60, 4), (80, 3), (-80, 3), (120, 2),
                      (-120, 2), (180, 1)]
    offMask = BitMask32(0)
    ...
    def _DistributedCashbotBossGoonAI__chooseTarget(self, extraDelay=0):
        direction = self._DistributedCashbotBossGoonAI__chooseDirection()
    ...
    def _DistributedCashbotBossGoonAI__chooseDirection(self):
    ...
    def _DistributedCashbotBossGoonAI__startWalk(self):
        ...
        if availableTime > 0:
            ...
            taskMgr.doMethodLater(
                availableTime,
                self._DistributedCashbotBossGoonAI__reachedTarget,
                self.uniqueName('reachedTarget'))
            self.isWalking = 1
        else:
            self._DistributedCashbotBossGoonAI__reachedTarget(None)

    def _DistributedCashbotBossGoonAI__stopWalk(self, pauseTime=None):
        if self.isWalking:
            taskMgr.remove(self.uniqueName('reachedTarget'))
            if pauseTime is None:
                now = globalClock.getFrameTime()
                t = (now - self.departureTime) / \
    ...
    def _DistributedCashbotBossGoonAI__reachedTarget(self, task):
        self._DistributedCashbotBossGoonAI__stopWalk()
        self._DistributedCashbotBossGoonAI__chooseTarget()
        self._DistributedCashbotBossGoonAI__startWalk()

    def _DistributedCashbotBossGoonAI__recoverWalk(self, task):
        self.demand('Walk')
        return Task.done

    def doFree(self, task):
        DistributedCashbotBossObjectAI.DistributedCashbotBossObjectAI.doFree(
            self, task)
        self.demand('Walk')
        return Task.done
    ...
    def requestStunned(self, pauseTime):
        ...
        self._DistributedCashbotBossGoonAI__stopWalk(pauseTime)
    ...
    def enterEmergeA(self):
        ...
        taskMgr.doMethodLater(walkTime,
                              self._DistributedCashbotBossGoonAI__recoverWalk,
                              self.uniqueName('recoverWalk'))
