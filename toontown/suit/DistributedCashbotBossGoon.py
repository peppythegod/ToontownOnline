...
class DistributedCashbotBossGoon(
    ...
    walkGrabZ = -3.6000000000000001
    stunGrabZ = -2.2000000000000002
    wiggleFreeTime = 2
    craneFrictionCoef = 0.14999999999999999
    craneSlideSpeed = 10
    craneRotateSpeed = 20

    def __init__(self, cr):
        DistributedCashbotBossObject.DistributedCashbotBossObject.__init__(
            self, cr)
        DistributedGoon.DistributedGoon.__init__(self, cr)
        self.target = None
        self.arrivalTime = None
        self.flyToMagnetSfx = loader.loadSfx(
            'phase_5/audio/sfx/TL_rake_throw_only.mp3')
        ...
        self.toMagnetSoundInterval = Sequence(
            SoundInterval(
                ...
                node=self), SoundInterval(self.hitMagnetSfx, node=self))
        self.hitFloorSfx = loader.loadSfx(
            'phase_5/audio/sfx/AA_drop_flowerpot.mp3')
    ...
    def generate(self):
        DistributedCashbotBossObject.DistributedCashbotBossObject.generate(
            self)
        DistributedGoon.DistributedGoon.generate(self)

    def announceGenerate(self):
        DistributedCashbotBossObject.DistributedCashbotBossObject.announceGenerate(
            self)
    ...
    def _DistributedCashbotBossGoon__startWalk(self):
        self._DistributedCashbotBossGoon__stopWalk()
        if self.target:
    ...
    def _DistributedCashbotBossGoon__stopWalk(self):
        if self.walkTrack:
            self.walkTrack.pause()
            self.walkTrack = None

    def _DistributedCashbotBossGoon__wiggleTask(self, task):
    ...
    def _DistributedCashbotBossGoon__wiggleFree(self, task):
    ...
    def prepareGrab(self):
        DistributedCashbotBossObject.DistributedCashbotBossObject.prepareGrab(
            self)
        if self.isStunned or self.boss.localToonIsSafe:
            ...
            taskMgr.add(self._DistributedCashbotBossGoon__wiggleTask,
                        self.wiggleTaskName)
            base.sfxPlayer.playSfx(self.wiggleSfx, node=self)
            if self.avId == localAvatar.doId:
                taskMgr.doMethodLater(
                    self.wiggleFreeTime,
                    self._DistributedCashbotBossGoon__wiggleFree,
                    self.wiggleFreeName)

        self.radar.hide()

    def prepareRelease(self):
        DistributedCashbotBossObject.DistributedCashbotBossObject.prepareRelease(
            self)
    ...
    def setTarget(self, x, y, h, arrivalTime):
        ...
        self.targetH = h
        now = globalClock.getFrameTime()
        self.arrivalTime = globalClockDelta.networkToLocalTime(
            arrivalTime, now)
        if self.state == 'Walk':
            self._DistributedCashbotBossGoon__startWalk()
    ...
    def enterOff(self):
        ...
        DistributedCashbotBossObject.DistributedCashbotBossObject.enterOff(
            self)
