...
from toontown.dna.DNAParser import DNAStorage, loadDNAFileAI, DNAInteractiveProp, DNASuitPoint
    ...
    def initDNAInfo(self):
        ...
        for i in range(self.dnaStore.getNumDNAVisGroupsAI()):
            ...
            if vg.getNumBattleCells() == 1:
                ...
                self.notify.warning(
                    'multiple battle cells for zone: %d' % zoneId)
                self.battlePosDict[zoneId] = vg.getBattleCell(0).getPos()
            ...
            if True:
                for i in range(vg.getNumChildren()):
                    ...
                    if isinstance(childDnaGroup, DNAInteractiveProp):
                        self.notify.debug(
                            'got interactive prop %s' % childDnaGroup)
        ...
        for i in range(numPoints):
            ...
            ) == DNASuitPoint.COGHQINPOINT or point.getPointType(
            ) == DNASuitPoint.COGHQOUTPOINT:
    ...
    def performPathTest(self):
        ...
        for i in range(numPathPoints - 1):
            ...
            travelTime = self.dnaStore.getSuitEdgeTravelTime(
                path.getPointIndex(i), path.getPointIndex(i + 1),
                self.suitWalkSpeed)
            self.notify.debug('edge from point ' + repr(i) + ' to point ' +
                              repr(i + 1) + ' is in zone: ' + repr(zone) +
