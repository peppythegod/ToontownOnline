...
class DistributedLawbotBossSuit(DistributedSuitBase.DistributedSuitBase):
    ...
    timeToShow = 1.0
    timeToRelease = 3.1499999999999999
    throwPaperEndTime = 4.3300000000000001
    ...
    def _DistributedLawbotBossSuit__handleToonCollision(self, collEntry):
    ...
    def enterWalk(self):
        ...
        self.enableBattleDetect(
            'walk', self._DistributedLawbotBossSuit__handleToonCollision)
        self.loop('walk', 0)
        pathPoints = [
            Vec3(50, 15, 0),
            Vec3(50, 25, 0),
    ...
    def doAttack(self, x1, y1, z1, x2, y2, z2):
        self.notify.debug('x1=%.2f y1=%.2f z2=%.2f x2=%.2f y2=%.2f z2=%.2f' %
                          (x1, y1, z1, x2, y2, z2))
        ...
        throwingSeq = self.makeAttackThrowingTrack(attackEvidence, duration,
                                                   Point3(x2, y2, z2))
    ...
    def doProsecute(self):
        ...
        throwingSeq = self.makeProsecuteThrowingTrack(attackEvidence, duration,
                                                      prosecutionPanPos)
    ...
    def makeProsecuteThrowingTrack(self, evidence, inFlightDuration, hitPos):
        ...
        propTrack = Sequence(
            Func(evidence.hide),
            Func(evidence.setPos, 0, 0.5, -0.29999999999999999),
            Func(evidence.reparentTo, self.getRightHand()),
            Wait(self.timeToShow), Func(evidence.show),
            Wait(self.timeToRelease - self.timeToShow),
            Func(evidence.wrtReparentTo, render), Func(self.makeDummySequence),
            inFlight, Func(evidence.detachNode))
    ...
    def makeAttackThrowingTrack(self, evidence, inFlightDuration, hitPos):
        ...
        propTrack = Sequence(
            ...
            Wait(self.timeToShow), Func(evidence.show),
            Wait(self.timeToRelease - self.timeToShow),
            Func(evidence.wrtReparentTo, render), Func(evidence.setZ, 1.3),
            evidence.posInterval(inFlightDuration, hitPos, fluid=1),
    ...
    def makePreThrowAttackTrack(self, evidence, inFlightDuration, hitPos):
        suitTrack = Sequence()
        suitTrack.append(
            ActorInterval(self, 'throw-paper', endTime=self.timeToRelease))
        throwPaperDuration = suitTrack.getDuration()
        origHpr = self.getHpr()
        self.headsUp(hitPos)
        newHpr = self.getHpr()
        ...
        propTrack = Sequence(
            Func(evidence.hide),
            Func(evidence.setPos, 0, 0.5, -0.29999999999999999),
            Func(evidence.setScale, 1), Func(evidence.setHpr, 0, 0, 0),
            Func(evidence.reparentTo, self.getRightHand()),
            Wait(self.timeToShow), Func(evidence.show),
            Wait(self.timeToRelease - self.timeToShow))
        throwingTrack = Parallel(suitTrack, propTrack, rotateTrack)
    ...
    def makePostThrowAttackTrack(self, evidence, inFlightDuration, hitPos):
        ...
        propTrack = Sequence(
            Func(evidence.wrtReparentTo, render), Func(evidence.setScale, 1),
            Func(evidence.show), Func(evidence.setZ, 1.3),
            evidence.posInterval(inFlightDuration, hitPos, fluid=1),
            Func(evidence.hide))
    ...
    def getEvidence(self, usedForAttack=False):
        ...
        if usedForAttack:
            ...
            colNode.setTag('attackCode',
                           str(ToontownGlobals.BossCogLawyerAttack))
    ...
    def enterPostThrowProsecute(self):
        ...
        (postThrowTrack,
         self.flyingEvidenceTrack) = self.makePostThrowProsecuteTrack(
             self.prosecuteEvidence, duration, self.curTargetPt)
        fullSequence = Sequence(
            postThrowTrack,
        ...
        self.flyingEvidenceTrack.append(
            Func(self.finishedWithFlying, 'prosecute'))
    ...
    def enterPostThrowAttack(self):
        ...
        (postThrowTrack,
         self.flyingEvidenceTrack) = self.makePostThrowAttackTrack(
             self.attackEvidence, duration, self.curTargetPt)
        fullSequence = Sequence(
            postThrowTrack,
            Func(self.requestStateIfNotInFlux, 'neutral'),
        ...
        self.activeIntervals[flyName] = self.flyingEvidenceTrack
        self.flyingEvidenceTrack.append(
            Func(self.finishedWithFlying, 'attack'))
        self.flyingEvidenceTrack.start()
