...
class DistributedLawbotBossSuit(DistributedSuitBase.DistributedSuitBase):
    ...
    def _DistributedLawbotBossSuit__handleToonCollision(self, collEntry):
        ...
        self.notify.debug(
            'Distributed suit: requesting a Battle with ' +
            'toon: %d' %
            toonId)
    ...
    def enterWalk(self):
        ...
        pathPoints = [
            ...
            Vec3(20, 15, 0),
            Vec3(50, 15, 0)]
        self.tutWalkTrack = self.makePathTrack(
            self, pathPoints, 4.5, 'tutFlunkyWalk')
    ...
    def doAttack(self, x1, y1, z1, x2, y2, z2):
        self.notify.debug(
            'x1=%.2f y1=%.2f z2=%.2f x2=%.2f y2=%.2f z2=%.2f' %
            (x1, y1, z1, x2, y2, z2))
    ...
    def doProsecute(self):
        ...
        fullSequence = Sequence(
            throwingSeq, Func(
                self.boss.flashGreen), Func(
                self.clearInterval, throwName), name=throwName)
        self.activeIntervals[throwName] = fullSequence
        fullSequence.start()
    ...
    def makeProsecuteThrowingTrack(self, evidence, inFlightDuration, hitPos):
        ...
        propTrack = Sequence(
            Func(
                evidence.hide), Func(
                evidence.setPos, 0, 0.5, -0.29999999999999999), Func(
                evidence.reparentTo, self.getRightHand()), Wait(
                    self.timeToShow), Func(
                        evidence.show), Wait(
                            self.timeToRelease - self.timeToShow), Func(
                                evidence.wrtReparentTo, render), Func(
                                    self.makeDummySequence), inFlight, Func(
                                        evidence.detachNode))
    ...
    def makePreThrowAttackTrack(self, evidence, inFlightDuration, hitPos):
        ...
        propTrack = Sequence(
            Func(
                evidence.hide), Func(
                evidence.setPos, 0, 0.5, -0.29999999999999999), Func(
                evidence.setScale, 1), Func(
                    evidence.setHpr, 0, 0, 0), Func(
                        evidence.reparentTo, self.getRightHand()), Wait(
                            self.timeToShow), Func(
                                evidence.show), Wait(
                                    self.timeToRelease - self.timeToShow))
        throwingTrack = Parallel(suitTrack, propTrack, rotateTrack)
        return throwingTrack
    ...
    def getEvidence(self, usedForAttack=False):
        ...
        if usedForAttack:
            ...
            colNode.setTag(
                'attackCode', str(
                    ToontownGlobals.BossCogLawyerAttack))
    ...
    def enterPostThrowAttack(self):
        ...
        fullSequence = Sequence(
            ...
            name=throwName)
        self.notify.debug(
            'duration of postThrowAttack = %f' %
            fullSequence.getDuration())
        self.activeIntervals[throwName] = fullSequence
        fullSequence.start()
        flyName = self.uniqueName('flyingEvidence')
        self.activeIntervals[flyName] = self.flyingEvidenceTrack
