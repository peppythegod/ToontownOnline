...
class CharLonelyStateAI(StateData.StateData):
    ...
    def enter(self):
        ...
        taskMgr.doMethodLater(duration, self._CharLonelyStateAI__doneHandler,
    ...
    def _CharLonelyStateAI__doneHandler(self, task):
        ...
        messenger.send(self._CharLonelyStateAI__doneEvent, [doneStatus])
        return Task.done
...
class CharChattyStateAI(StateData.StateData):
    ...
    def __init__(self, doneEvent, character):
        ...
        self.character = character
        self._CharChattyStateAI__chatTaskName = 'characterChat-' + \
            str(character)
    ...
    def blather(self, task):
        ...
        if category == self.lastMessage[0] and len(self.chatter[category]) > 1:
            ...
            if lastMsgIndex < len(
                    self.chatter[category]) and lastMsgIndex >= 0:
                while self.chatter[category][msg] == self.chatter[category][
                        lastMsgIndex]:
    ...
    def doneHandler(self, task):
        ...
        doneStatus['status'] = 'done'
        messenger.send(self._CharChattyStateAI__doneEvent, [doneStatus])
        return Task.done
...
class CharWalkStateAI(StateData.StateData):
    ...
    def doneHandler(self, task):
        ...
        doneStatus['status'] = 'done'
        messenger.send(self._CharWalkStateAI__doneEvent, [doneStatus])
        return Task.done


class CharFollowChipStateAI(StateData.StateData):
    notify = DirectNotifyGlobal.directNotify.newCategory(
        'CharFollowChipStateAI')
    ...
    def _CharFollowChipStateAI__doneHandler(self, task):
        ...
        doneStatus['state'] = 'walk'
        doneStatus['status'] = 'done'
        messenger.send(self._CharFollowChipStateAI__doneEvent, [doneStatus])
        return Task.done
