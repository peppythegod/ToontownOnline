...
class CharNeutralState(StateData.StateData):
    ...
    def enter(self, startTrack=None, playRate=None):
        ...
        self._CharNeutralState__neutralTrack = Sequence(
            name=self.character.getName() + '-neutral')
        if startTrack:
            self._CharNeutralState__neutralTrack.append(startTrack)

        if playRate:
            self._CharNeutralState__neutralTrack.append(
                Func(self.character.setPlayRate, playRate, 'neutral'))

        self._CharNeutralState__neutralTrack.append(
            Func(self.character.loop, 'neutral'))
