...
class DistributedDivingGameAI(DistributedMinigameAI):
    ...
    def setGameReady(self):
        ...
        self.sendUpdate('setTrolleyZone', [
            self.trolleyZone])
            ...
            self.spawnings.append(
                Sequence(
                    Func(
                        ...
                        i),
                    Wait(
                        self.SPAWNTIME + random.random()),
                    Func(
                        self.spawnFish,
                        i),
                    Wait(
                        (self.SPAWNTIME - 0.5) + random.random())))
    ...
    def treasureRecovered(self):
        ...
        self.sendUpdate('incrementScore', [
            avId,
            newSpot,
            timestamp])
    ...
    def gameOver(self):
        ...
        for avId in self.scoreTracking.keys():
            trackingString = trackingString + '\navId:%s fishHits:%s crabHits:%s treasureCatches:%s treasureDrops:%s treasureRecoveries:%s Score: %s' % (
                avId, self.scoreTracking[avId][0], self.scoreTracking[avId][1], self.scoreTracking[avId][2], self.scoreTracking[avId][3], self.scoreTracking[avId][4], self.scoreDict[avId])
    ...
    def handleCrabCollision(self, avId, status):
        ...
        timestamp = globalClockDelta.getFrameNetworkTime()
        self.sendUpdate('setTreasureDropped', [
            avId,
            timestamp])
        self.scoreTracking[avId][1] += 1
        if status == 'normal' or status == 'treasure':
            timestamp = globalClockDelta.getFrameNetworkTime()
            self.sendUpdate('performCrabCollision', [
                avId,
                timestamp])
            if status == 'treasure':
