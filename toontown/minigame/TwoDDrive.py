...
class TwoDDrive:
    ...
    def start(self):
        ...
        self._TwoDDrive__placeToonHOG(self.lt.getPos())
        base.localAvatar.enableAvatarControls()
        taskMgr.remove(TwoDDrive.TASK_NAME)
        taskMgr.add(
            self._TwoDDrive__update,
            TwoDDrive.TASK_NAME,
            priority=self.priority)

    def _TwoDDrive__placeToonHOG(self, pos, h=None):
        if h is None:
            h = self.lt.getH()
    ...
    def _TwoDDrive__update(self, task):
        vel = Vec3(0, 0, 0)
        xVel = 0
        yVel = 0
        ...
        if self.setHeading:
            self._TwoDDrive__handleHeading(xVel, yVel)
