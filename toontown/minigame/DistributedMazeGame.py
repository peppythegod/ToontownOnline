...
class DistributedMazeGame(DistributedMinigame):
    ...
    def onstage(self):
        ...
        self.goalBar = DirectWaitBar(
            ...
            pos=(
                0.83999999999999997,
                0,
                (0.5 - 0.28000000000000003 * self.numPlayers) + 0.050000000000000003),
            barColor=(
                0,
                0.69999999999999996,
                0,
    ...
    def enterPlay(self):
        ...
        orthoDrive = OrthoDrive(
            ...
            customCollisionCallback=self._DistributedMazeGame__doMazeCollisions,
            priority=1)
        self.orthoWalk = OrthoWalk(
            orthoDrive, broadcast=not self.isSinglePlayer())
        self.orthoWalk.start()
        self.accept(
            MazeSuit.COLLISION_EVENT_NAME,
    ...
    def _DistributedMazeGame__doMazeCollisions(self, oldPos, newPos):
        ...
        def calcFlushCoord(curTile, newTile, centerTile):
            EPSILON = 0.01
            if newTile > curTile:
                return (newTile - centerTile) * \
                    self.CELL_WIDTH - EPSILON - WALL_OFFSET
            else:
    ...
    def _DistributedMazeGame__spawnCameraTask(self):
        ...
        taskMgr.add(
            self._DistributedMazeGame__cameraTask,
            self.CAMERA_TASK,
            priority=45)
    ...
    def _DistributedMazeGame__spawnUpdateSuitsTask(self):
        ...
        taskMgr.add(
            self._DistributedMazeGame__updateSuitsTask,
    ...
    def enterShowScores(self):
        ...
        self.showScoreTrack = Parallel(
            lerpTrack, Sequence(
                Wait(
                    MazeGameGlobals.SHOWSCORES_DURATION), Func(
                    self.gameOver)))
    ...
    def getIntroTrack(self):
        ...
        lerpTrack.append(
            LerpPosHprInterval(
                iCamParent, lerpDur, pos=Point3(
                    0, 0, 0), hpr=Point3(
                    0, 0, 0), startHpr=startHpr, name=self.uniqueName('introLerpParent')))
        lerpTrack.append(
            LerpPosHprInterval(
                camera,
                lerpDur,
