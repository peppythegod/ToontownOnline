...
class DistributedRaceGame(DistributedMinigame):
    ...
    def load(self):
        ...
        for i in range(1, 5):
            ...
            diceButton = DirectButton(image=(button,
                                             ...
                                             None),
                                      relief=None,
                                      pos=(-0.90000000000000002 + (i - 1) * 0.20000000000000001,
                                           0.0,
                                           -0.84999999999999998),
                                      scale=0.25,
        ...
        self.waitingChoicesLabel = DirectLabel(text=TTLocalizer.RaceGameWaitingChoices, text_fg=VBase4(
            1, 1, 1, 1), relief=None, pos=(-0.59999999999999998, 0, -0.75), scale=0.074999999999999997)
    ...
    def showMatches(self, task):
        ...
        for i in range(len(task.choiceList)):
            ...
            if choice != 0:
                ...
                if freq == 1:
                    diceInstance.setColor(
                        0.20000000000000001, 1, 0.20000000000000001, 1)
                    if avId == self.localAvId:
                        base.playSfx(self.posBuzzer)

                else:
                    diceInstance.setColor(
                        1, 0.20000000000000001, 0.20000000000000001, 1)
                    if avId == self.localAvId:
    ...
    def walkInPlace(self, avatar, lane, place, time):
        ...
        def stopWalk(task, raceBoard=self.raceBoard, posH=posH):
            ...
            if raceBoard.isEmpty():
                ...
                task.avatar.setPosHpr(
                    raceBoard, posH[0], posH[1], posH[2], posH[3], 0, 0)
            return Task.done
    ...
    def runInPlace(self, avatar, lane, currentPlace, newPlace, time):
        ...
        def stopRun(task, raceBoard=self.raceBoard, pos3=pos3):
            task.avatar.setAnimState('neutral', 1)
            task.avatar.setPosHpr(
                raceBoard, pos3[0], pos3[1], pos3[2], pos3[3], 0, 0)
            return Task.done
