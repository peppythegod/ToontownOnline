...
class TwoDEnemy(DirectObject):
    ...
    def destroy(self):
        ...
        if self.shotTrack is not None:
            self.shotTrack.finish()
            self.shotTrack = None

        if self.deathTrack is not None:
            ...
            if abs(finalZ - initZ) > 0.0:
                ...
                if self.suit.prop is None:
                    self.suit.prop = BattleProps.globalPropPool.getProp(
                        'propeller')
                    self.suit.prop.setScale(1.1000000000000001)
                    self.suit.prop.setColor(1, 1, 0.59999999999999998, 1)
