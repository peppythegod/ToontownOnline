...
GOODROWS = 13
BADROWS = 4
RAYSPREADX = 0.080000000000000002
RAYSPREADY = 0.059999999999999998
ZOOMRATIO = 0.40000000000000002
ZOOMTIME = 0.5
WANTDOTS = 1
NUMSTARS = PhotoGameGlobals.NUMSTARS
STARSIZE = 0.059999999999999998
...
class DistributedPhotoGame(DistributedMinigame, PhotoGameBase.PhotoGameBase):
    ...
    def __init__(self, cr):
        ...
        self.gameFSM = ClassicFSM.ClassicFSM('DistributedPhotoGame', [
            ...
            State.State('showResults', self.enterShowResults,
                        self.exitShowResults, ['cleanup']),
    ...
    def load(self):
        ...
        self.filmPanel = DirectLabel(
            ...
            text_pos=(0.080000000000000002, -0.14999999999999999),
            text_font=ToontownGlobals.getSignFont(),
            image=self.filmImage,
            image_scale=Point3(1.0, 0.0, 0.84999999999999998))
        self.filmPanelTitle = DirectLabel(
            parent=self.filmPanel,
            relief=None,
            pos=(0.080000000000000002, 0, 0.040000000000000001),
        ...
        viewfinderImage.setScale(0.55000000000000004, 1.0, 0.55000000000000004)
        self.blackoutNode = base.aspect2d.attachNewNode('blackout node')
        self.blackoutNode.setP(90)
        BuildGeometry.addSquareGeom(self.blackoutNode,
                                    self.screenSizeX * 2.2000000000000002,
                                    self.screenSizeZ * 2.2000000000000002,
    ...
    def _DistributedPhotoGame__setupCapture(self):
        ...
        self.captureZoomFOV = (
            VIEWSIZEX / self.screenSizeX) * self.zoomFov * 0.5
        self.captureLens.setFov(self.captureOutFOV)
        self.captureLens.setAspectRatio(1.3300000000000001)
        self.captureCam.node().setLens(self.captureLens)

    def _DistributedPhotoGame__removeCapture(self):
        del self.captureCam
    ...
    def unload(self):
        ...
        self._DistributedPhotoGame__removeCapture()
    ...
    def onstage(self):
        ...
        self._DistributedPhotoGame__createTripod()
        self.tripod.reparentTo(render)
        self.tripod.hide()
        self._DistributedPhotoGame__loadToonInTripod(self.localAvId)
        camera.reparentTo(render)
        self._DistributedPhotoGame__oldCamFar = base.camLens.getFar()
        base.camLens.setFar(FAR_PLANE_DIST)
        self._DistributedPhotoGame__setupSubjects()
        self._DistributedPhotoGame__startIntro()
        ...
        self._DistributedPhotoGame__setupCapture()
    ...
    def offstage(self):
        ...
        self._DistributedPhotoGame__stopIntro()
        base.camLens.setFar(self._DistributedPhotoGame__oldCamFar)
    ...
    def _DistributedPhotoGame__setupCollisions(self):
        ...
        for row in range(-(GOODROWS / 2), GOODROWS / 2 + 1):
            for column in range(-(GOODROWS / 2), GOODROWS / 2 + 1):
                goodRange = range(-((GOODROWS - BADROWS) / 2),
                                  (GOODROWS - BADROWS) / 2 + 1)
                rayQuality = 'g'
                ...
                self.rayArray.append((row, column, pickerNode, pickerNP,
                                      pickerRay))
        ...
        if WANTDOTS:
            ...
            for rayEntry in self.rayArray:
                ...
                BuildGeometry.addSquareGeom(markerNode, 0.01, 0.01,
                                            Vec4(1.0, 1.0, 1.0, 1.0))
    ...
    def _DistributedPhotoGame__moveViewfinder(self, task):
        if base.mouseWatcherNode.hasMouse():
            mpos = base.mouseWatcherNode.getMouse()
            self.viewfinderNode.setX(mpos.getX() * self.screenSizeX)
            self.viewfinderNode.setZ(mpos.getY() * self.screenSizeZ)
            horzAngle = (self.viewfinderNode.getX() /
                         self.screenSizeX) * 0.5 * base.camLens.getFov()[0]
            vertAngle = (self.viewfinderNode.getZ() /
                         self.screenSizeZ) * 0.5 * base.camLens.getFov()[1]
            ...
            newUp = 0
            newDown = 0
            if self.viewfinderNode.getX(
            ) > self.screenSizeX * 0.94999999999999996:
                newRight = 1
            ...
            if self.viewfinderNode.getZ(
            ) > self.screenSizeZ * 0.94999999999999996:
                newUp = 1
            ...
            if not (self.edgeRight) and newRight:
                self.edgeRight = 1
                self._DistributedPhotoGame__rightPressed()
            elif self.edgeRight and not newRight:
                self.edgeRight = 0
                self._DistributedPhotoGame__rightReleased()

            if not (self.edgeLeft) and newLeft:
                self.edgeLeft = 1
                self._DistributedPhotoGame__leftPressed()
            elif self.edgeLeft and not newLeft:
                self.edgeLeft = 0
                self._DistributedPhotoGame__leftReleased()

            if not (self.edgeUp) and newUp:
                self.edgeUp = 1
                self._DistributedPhotoGame__upPressed()
            elif self.edgeUp and not newUp:
                self.edgeUp = 0
                self._DistributedPhotoGame__upReleased()

            if not (self.edgeDown) and newDown:
                self.edgeDown = 1
                self._DistributedPhotoGame__downPressed()
            elif self.edgeDown and not newDown:
                self.edgeDown = 0
                self._DistributedPhotoGame__downReleased()

        return task.cont

    def _DistributedPhotoGame__testCollisions(self):
        self.notify.debug('\nSnapping Photo')
        self.playSound('snap')
        if self.filmCount <= 0:
        ...
        for rayEntry in self.rayArray:
            posX = (self.viewfinderNode.getX() +
                    RAYSPREADX * rayEntry[0]) / self.screenSizeX
            posY = (self.viewfinderNode.getZ() +
                    RAYSPREADY * rayEntry[1]) / self.screenSizeZ
        ...
        for i in range(self.queue.getNumEntries()):
            ...
            object = None
            objectIndex = None
            subjectIndexString = entry.getIntoNodePath().getNetTag(
                'subjectIndex')
            sceneryIndexString = entry.getIntoNodePath().getNetTag(
                'sceneryIndex')
            ...
            if object:
                ...
                distance = Vec3(entry.getSurfacePoint(
        ...
        for key in hitDict:
            ...
            if superParent not in centerDict:
                centerDict[superParent] = (onCenter, overB, overT, overR,
                                           overL)
                continue
        ...
        hMDegree = toDegrees(hMRadian)
        vMDegree = toDegrees(vMRadian)
        self._DistributedPhotoGame__decreaseFilmCount()
        if self.filmCount == 0:
            self.sendUpdate('filmOut', [])

        self.notify.debug('Screen angles H:%s V:%s' % (self.swivel.getH(),
                                                       self.swivel.getP()))
        self.notify.debug(
            'Viewfinder to screen angles H:%s V:%s' % (horzAngle, vertAngle))
        self.notify.debug('Viewfinder to screen angles with math H:%s V:%s' %
                          (hMDegree, vMDegree))
    ...
    def judgePhoto(self, subject, centerDict):
        ...
        self.notify.debug('angle camera:%s subject:%s between:%s' %
        ...
        if isinstance(subject, type(self.subjectToon)):
            ...
            if (centering[1] or centering[2]) and centering[3]:
                pass
            tooClose = centering[4]
            if centering[1]:
                if not centering[2] and centering[3]:
                    pass
            portrait = not centering[4]
    ...
    def _DistributedPhotoGame__toggleView(self):
        ...
        if self.zoomedView:
            ...
            self.swivel.setHpr(self.swivel, hMove * -(self.zoomFlip),
                               vMove * self.zoomFlip, 0)
        else:
    ...
    def _DistributedPhotoGame__doZoom(self):
        self.notify.debug('Toggle View')
        self.playSound('zoom')
        hCam = self.swivel.getH()
        vCam = self.swivel.getP()
    ...
    def _DistributedPhotoGame__setupSubjects(self):
        self._DistributedPhotoGame__setupCollisions()
        self.subjects = []
        self.subjectTracks = {}
    ...
    def generateToonTrack(self, subject, path, pathIndex):
        ...
        subjectTrack.append(
            Func(self.regenerateToonTrack, subject, path, pathIndex))
    ...
    def _DistributedPhotoGame__createTripod(self):
    ...
    def setGameStart(self, timestamp):
        ...
        self._DistributedPhotoGame__stopIntro()
        self._DistributedPhotoGame__putCameraOnTripod()
        if not base.config.GetBool('endless-cannon-game', 0):
            self.timer.show()
            self.timer.countdown(self.data['TIME'],
                                 self._DistributedPhotoGame__gameTimerExpired)
        ...
        self.scorePanel.setPos(1.05, 0.0, -0.72499999999999998)
        self.updateAssignmentPanels()
        for subject in self.subjects:
            subject.useLOD(1000)
    ...
    def _DistributedPhotoGame__gameTimerExpired(self):
        ...
        for template in assignmentTemplates:
            ...
            if assignment not in self.assignments:
                ...
                self.assignmentDataDict[assignment] = [
                    score, panel, topScore, topScorerId, textureBuffer,
                    texturePanel, texture, panelToon
                ]
                continue
    ...
    def generateAssignmentPanels(self):
        ...
        spacing = (self.screenSizeX /
                   PhotoGameGlobals.ONSCREENASSIGNMENTS) * 1.6100000000000001
        index = 0
        Xoff = self.screenSizeX - 0.73499999999999999
        ...
        for assignment in self.assignments:
            ...
            panel.setZ(Zoff)
            texturePanel.setZ(0.065000000000000002)
            rot = random.choice([0.0, 2.0, -2.0, -4.0, 6.0])
            panel.setR(rot)
            textureBuffer = base.win.makeTextureBuffer('Photo Capture', 128,
                                                       128)
    ...
    def makeAssignmentPanel(self, assignment):
        ...
        if assignment[1] == 'swim':
            ...
            assignedToon.setZ(-0.59999999999999998)
        ...
        BuildGeometry.addSquareGeom(screen, 0.35999999999999999,
                                    0.27000000000000002,
                                    Vec4(1.0, 1.0, 1.0, 1.0))
        ...
        for index in range(NUMSTARS):
            ...
            star.setZ(-0.050000000000000003 - STARSIZE)
    ...
    def makeFrameModel(self, model):
        ...
        if model:
            ...
            scale.setScale(0.40000000000000002 * scaleFactor / max(
    ...
    def makeAssignmentFrame(self):
        ...
        frame = DirectFrame(
            ...
            image=photoImage,
            image_color=(1, 1, 1, 1),
            image_scale=Point3(1.6000000000000001 * assignmentScale, 0.0,
                               1.75 * assignmentScale),
            frameSize=(-size, size, -size, size),
            text='HC Score',
            textMayChange=1,
            text_wordwrap=9,
            text_pos=Point3(0.0, -0.13500000000000001, 0.0),
    ...
    def makeScoreFrame(self):
        ...
        frame = DirectFrame(
            ...
            image=scoreImage,
            image_color=(1, 1, 1, 1),
            image_scale=Point3(0.64000000000000001, 0.0, 0.64000000000000001),
            frameSize=(-size, size, -size, size),
            text='Score Frame',
            textMayChange=1,
            text_wordwrap=9,
    ...
    def enterAim(self):
        ...
        self._DistributedPhotoGame__enableAimInterface()
        taskMgr.add(self._DistributedPhotoGame__moveViewfinder,
                    'photo game viewfinder Task')
        self.accept('mouse1', self._DistributedPhotoGame__handleMouseClick)
        base.localAvatar.laffMeter.stop()
        base.transitions.noIris()

    def exitAim(self):
        self._DistributedPhotoGame__disableAimInterface()
    ...
    def enterZoom(self):
        ...
        taskMgr.add(self._DistributedPhotoGame__moveViewfinder,
                    'photo game viewfinder Task')
        self._DistributedPhotoGame__doZoom()
    ...
    def _DistributedPhotoGame__enableAimInterface(self):
        self.accept(self.FIRE_KEY, self._DistributedPhotoGame__fireKeyPressed)
        self.accept(self.UP_KEY, self._DistributedPhotoGame__upKeyPressed)
        self.accept(self.DOWN_KEY, self._DistributedPhotoGame__downKeyPressed)
        self.accept(self.LEFT_KEY, self._DistributedPhotoGame__leftKeyPressed)
        self.accept(self.RIGHT_KEY,
                    self._DistributedPhotoGame__rightKeyPressed)
        self._DistributedPhotoGame__spawnLocalPhotoMoveTask()

    def _DistributedPhotoGame__disableAimInterface(self):
        ...
        self._DistributedPhotoGame__killLocalPhotoMoveTask()

    def _DistributedPhotoGame__fireKeyPressed(self):
        self.ignore(self.FIRE_KEY)
        self.accept(self.FIRE_KEY + '-up',
                    self._DistributedPhotoGame__fireKeyReleased)
        self._DistributedPhotoGame__firePressed()

    def _DistributedPhotoGame__upKeyPressed(self):
        self.ignore(self.UP_KEY)
        self.accept(self.UP_KEY + '-up',
                    self._DistributedPhotoGame__upKeyReleased)
        self._DistributedPhotoGame__upPressed()

    def _DistributedPhotoGame__downKeyPressed(self):
        self.ignore(self.DOWN_KEY)
        self.accept(self.DOWN_KEY + '-up',
                    self._DistributedPhotoGame__downKeyReleased)
        self._DistributedPhotoGame__downPressed()

    def _DistributedPhotoGame__leftKeyPressed(self):
        self.ignore(self.LEFT_KEY)
        self.accept(self.LEFT_KEY + '-up',
                    self._DistributedPhotoGame__leftKeyReleased)
        self._DistributedPhotoGame__leftPressed()

    def _DistributedPhotoGame__rightKeyPressed(self):
        self.ignore(self.RIGHT_KEY)
        self.accept(self.RIGHT_KEY + '-up',
                    self._DistributedPhotoGame__rightKeyReleased)
        self._DistributedPhotoGame__rightPressed()

    def _DistributedPhotoGame__fireKeyReleased(self):
        self.ignore(self.FIRE_KEY + '-up')
        self.accept(self.FIRE_KEY, self._DistributedPhotoGame__fireKeyPressed)
        self._DistributedPhotoGame__fireReleased()

    def _DistributedPhotoGame__leftKeyReleased(self):
        self.ignore(self.LEFT_KEY + '-up')
        self.accept(self.LEFT_KEY, self._DistributedPhotoGame__leftKeyPressed)
        self._DistributedPhotoGame__leftReleased()

    def _DistributedPhotoGame__rightKeyReleased(self):
        self.ignore(self.RIGHT_KEY + '-up')
        self.accept(self.RIGHT_KEY,
                    self._DistributedPhotoGame__rightKeyPressed)
        self._DistributedPhotoGame__rightReleased()

    def _DistributedPhotoGame__upKeyReleased(self):
        self.ignore(self.UP_KEY + '-up')
        self.accept(self.UP_KEY, self._DistributedPhotoGame__upKeyPressed)
        self._DistributedPhotoGame__upReleased()

    def _DistributedPhotoGame__downKeyReleased(self):
        self.ignore(self.DOWN_KEY + '-up')
        self.accept(self.DOWN_KEY, self._DistributedPhotoGame__downKeyPressed)
        self._DistributedPhotoGame__downReleased()

    def _DistributedPhotoGame__firePressed(self):
        self.notify.debug('fire pressed')

    def _DistributedPhotoGame__fireReleased(self):
        self.gameFSM.request('zoom')

    def _DistributedPhotoGame__upPressed(self):
        self.notify.debug('up pressed')
        self.upPressed = self._DistributedPhotoGame__enterControlActive(
            self.upPressed)

    def _DistributedPhotoGame__downPressed(self):
        self.notify.debug('down pressed')
        self.downPressed = self._DistributedPhotoGame__enterControlActive(
            self.downPressed)

    def _DistributedPhotoGame__leftPressed(self):
        self.notify.debug('left pressed')
        self.leftPressed = self._DistributedPhotoGame__enterControlActive(
            self.leftPressed)

    def _DistributedPhotoGame__rightPressed(self):
        self.notify.debug('right pressed')
        self.rightPressed = self._DistributedPhotoGame__enterControlActive(
            self.rightPressed)

    def _DistributedPhotoGame__upReleased(self):
        self.notify.debug('up released')
        self.upPressed = self._DistributedPhotoGame__exitControlActive(
            self.upPressed)

    def _DistributedPhotoGame__downReleased(self):
        self.notify.debug('down released')
        self.downPressed = self._DistributedPhotoGame__exitControlActive(
            self.downPressed)

    def _DistributedPhotoGame__leftReleased(self):
        self.notify.debug('left released')
        self.leftPressed = self._DistributedPhotoGame__exitControlActive(
            self.leftPressed)

    def _DistributedPhotoGame__rightReleased(self):
        self.notify.debug('right released')
        self.rightPressed = self._DistributedPhotoGame__exitControlActive(
            self.rightPressed)

    def _DistributedPhotoGame__handleMouseClick(self):
        self.notify.debug('mouse click')
        self._DistributedPhotoGame__testCollisions()

    def _DistributedPhotoGame__enterControlActive(self, control):
        return control + 1

    def _DistributedPhotoGame__exitControlActive(self, control):
        return max(0, control - 1)

    def _DistributedPhotoGame__spawnLocalPhotoMoveTask(self):
        self.leftPressed = 0
        self.rightPressed = 0
        self.upPressed = 0
        self.downPressed = 0
        self.photoMoving = 0
        task = Task(self._DistributedPhotoGame__localPhotoMoveTask)
        task.lastPositionBroadcastTime = 0.0
        taskMgr.add(task, self.LOCAL_PHOTO_MOVE_TASK)

    def _DistributedPhotoGame__killLocalPhotoMoveTask(self):
        taskMgr.remove(self.LOCAL_PHOTO_MOVE_TASK)
        if self.photoMoving:
            self.sndPhotoMove.stop()

    def _DistributedPhotoGame__localPhotoMoveTask(self, task):
        if not hasattr(self, 'swivel'):
            return None
    ...
    def _DistributedPhotoGame__putCameraOnTripod(self):
        camera.setPosHpr(0, 0.0, 0, 0, 0, 0)
        camera.reparentTo(self.swivel)
        self.swivel.setHpr(self.data['START_HPR'])

    def _DistributedPhotoGame__loadToonInTripod(self, avId):
        toon = base.cr.doId2do.get(avId)
        if toon:
            toon.reparentTo(self.swivel)

    def _DistributedPhotoGame__toRadians(self, angle):
        return angle * 2.0 * math.pi / 360.0

    def _DistributedPhotoGame__toDegrees(self, angle):
        return angle * 360.0 / 2.0 * math.pi

    def _DistributedPhotoGame__decreaseFilmCount(self):
    ...
    def _DistributedPhotoGame__stopIntro(self):
    ...
    def _DistributedPhotoGame__startIntro(self):
        camera.reparentTo(render)
    ...
    def constructDD(self):
        ...
        self.boatTrack.append(
            LerpHprInterval(self.boat, 90.0, Point3(360, 0, 0)))
        self.boatTrack.loop()
        ...
        water.setColorScale(1.0, 1.0, 1.0, 0.80000000000000004)
    ...
    def constructDG(self):
        ...
        self.photoRoot.setPos(1.3899999999999999, 92.909999999999997, 2.0)
    ...
    def constructMM(self):
        ...
        self.photoRoot.setPos(103.59999999999999, -61, -4.4969999999999999)
    ...
    def constructBR(self):
        ...
        self.photoRoot.setPos(-110, -48, 8.5670000000000002)
        self.sky = loader.loadModel('phase_3.5/models/props/BR_sky')
        self.sky.reparentTo(render)
        self.sky.setBin('background', -100)
        self.scene.reparentTo(render)
    ...
    def constructDL(self):
        ...
        self.photoRoot.setPos(-70.227999999999994, 87.587999999999994,
                              4.3970000000000002)
