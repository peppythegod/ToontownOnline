...
class DistributedCogThiefGame(DistributedMinigame):
    ...
    def __init__(self, cr):
        ...
        self._DistributedCogThiefGame__textGen.setFont(
    ...
    def setGameReady(self):
        ...
        for avId in self.remoteAvIdList:
            ...
            if toon:
                ...
                self._DistributedCogThiefGame__placeToon(avId)
                toon.useLOD(1000)
                toonSD = CogThiefGameToonSD.CogThiefGameToonSD(avId, self)
                self.toonSDs[avId] = toonSD
                toonSD.load()
    ...
    def _DistributedCogThiefGame__placeToon(self, avId):
        toon = self.getAvatar(avId)
        if toon:
        ...
        if base.config.GetBool('cog-thief-check-barrels', 1):
            if not base.config.GetBool('cog-thief-endless', 0):
                if len(self.stolenBarrels) == len(self.barrels):
                    localStamp = globalClockDelta.networkToLocalTime(
                        timestamp, bits=32)
    ...
    def _DistributedCogThiefGame__gameTimerExpired(self):
    ...
    def _DistributedCogThiefGame__genText(self, text):
        self._DistributedCogThiefGame__textGen.setText(text)
        return self._DistributedCogThiefGame__textGen.generate()

    def getIntroTrack(self):
        base.camera.setPosHpr(0, -13.66, 13.59, 0, -51.600000000000001, 0)
        result = Sequence(
            Wait(2),
            LerpPosHprInterval(
                base.camera,
