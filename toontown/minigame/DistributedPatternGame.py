...
class DistributedPatternGame(DistributedMinigame):
    ...
    def unload(self):
        ...
        del self.music
        del self._DistributedPatternGame__otherToonIndex
    ...
    def onstage(self):
        ...
        camera.setPosHpr(0.0, -14.59, 10.56, 0.0, -16.390000000000001, 0.0)
        ...
        m = self.minnie
        m.reparentTo(render)
        m.setPos(-1.6000000000000001, 20, 0)
        m.setScale(1)
    ...
    def setGameReady(self):
        ...
        for avId in self.remoteAvIdList:
            ...
            if toon:
                ...
                toon.setScale(0.90000000000000002)
    ...
    def _DistributedPatternGame__initGameVars(self):
        self.round = 0
        self.perfectGame = 1

    def _DistributedPatternGame__numPingPongFrames(self, numFrames):
    ...
    def getWalkTrack(self,
        ...
        duration = (abs(endHeading - curHeading) / 180.0) * 0.29999999999999999
        track.extend([
            Func(toon.loop, 'walk'),
    ...
    def _DistributedPatternGame__getMinnieDanceStepAnimTrack(
        ...
        return self._DistributedPatternGame__getDanceStepAnimTrack(
            minnie, animName, self.minnieAnimSpeedMult[animName])

    def _DistributedPatternGame__getToonDanceStepAnimTrack(
            self, toon, direction):
    ...
    def formatStatusBalls(self, sb, jj):
        for x in range(0, self.totalMoves):
            ...
            sb[x].setPos(xpos, 0, 0.29999999999999999)

    def showStatusBalls(self, toonID):
        sb = self.arrowDict[toonID][2]
        for x in range(0, len(self._DistributedPatternGame__serverPattern)):
    ...
    def getDanceSequenceAnimTrack(self, toon, pattern):
        getDanceStepTrack = self._DistributedPatternGame__getToonDanceStepAnimTrack
        if toon == self.minnie:
            getDanceStepTrack = self._DistributedPatternGame__getMinnieDanceStepAnimTrack
    ...
    def _DistributedPatternGame__clearMinnieChat(self):
    ...
    def enterShowServerPattern(self):
        ...
        danceTrack = self.getDanceSequenceAnimTrack(
            self.minnie, self._DistributedPatternGame__serverPattern)
        arrowTrack = self.getDanceArrowAnimTrack(
            'm', self._DistributedPatternGame__serverPattern, 0)
    ...
    def _DistributedPatternGame__handleInputTimeout(self):
        self._DistributedPatternGame__doneGettingInput(
            self._DistributedPatternGame__localPattern)

    def _DistributedPatternGame__pressHandler(self, index):
        self._DistributedPatternGame__buttonPressed(index)

    def _DistributedPatternGame__releaseHandler(self, index):
        pass
    ...
    def _DistributedPatternGame__getButtonSound(self, index):
    ...
    def _DistributedPatternGame__buttonPressed(self, index):
        ...
        if len(self._DistributedPatternGame__localPattern) == len(
                self._DistributedPatternGame__serverPattern) or badd:
            self._DistributedPatternGame__doneGettingInput(
                self._DistributedPatternGame__localPattern)
    ...
    def enterPlayBackPatterns(self):
        ...
        if self.fastestAvId == self.localAvId:
            self.roundText.setScale(0.10000000000000001)
