...
class TownBattle(StateData.StateData):
    ...
    def __init__(self, doneEvent):
        ...
        self.runPanel = TTDialog.TTDialog(
            ...
            command=self._TownBattle__handleRunPanelDone)
        self.runPanel.hide()
    ...
    def enter(self,
        ...
        base.localAvatar.inventory.setBattleCreditMultiplier(
            self.creditMultiplier)
    ...
    def _TownBattle__enterPanels(self, num, localNum):
        self.notify.debug(
            'enterPanels() num: %d localNum: %d' % (num, localNum))
        for toonPanel in self.toonPanels:
            toonPanel.hide()
    ...
    def updateChosenAttacks(self, battleIndices, tracks, levels, targets):
        ...
        for i in range(4):
            ...
            if tracks[i] == BattleBase.NO_ATTACK:
                ...
                numTargets = 0
                target = -2
            elif tracks[i] == BattleBase.SOS and tracks[
                    i] == BattleBase.NPCSOS or tracks[i] == BattleBase.PETSOS:
                numTargets = 0
                target = -2
    ...
    def enterAttack(self):
        ...
        self.accept(self.attackPanelDoneEvent,
                    self._TownBattle__handleAttackPanelDone)
        for toonPanel in self.toonPanels:
    ...
    def _TownBattle__handleAttackPanelDone(self, doneStatus):
        ...
        if mode == 'Inventory':
            ...
            self.toonPanels[self.localNum].setValues(self.localNum, self.track,
                                                     self.level)
            if self.track == HEAL_TRACK:
                if self._TownBattle__isGroupHeal(self.level):
                    ...
                    if self.localNum == 0:
                        ...
                        self.notify.error(
                            'Bad localNum value: %s' % self.localNum)
                    messenger.send(self.battleEvent, [response])
                    self.fsm.request('AttackWait')
                else:
                    self.notify.error(
    ...
    def checkHealTrapLure(self):
        self.notify.debug('numToons: %s, numCogs: %s, lured: %s, trapped: %s' %
                          (self.numToons, self.numCogs, self.luredIndices,
                           self.trappedIndices))
        if len(PythonUtil.union(self.trappedIndices,
                                self.luredIndices)) == self.numCogs:
            canTrap = 0
        else:
            canTrap = 1
    ...
    def enterChooseCog(self):
        ...
        self.accept(self.chooseCogPanelDoneEvent,
                    self._TownBattle__handleChooseCogPanelDone)
    ...
    def enterAttackWait(self, chosenToon=-1):
        self.accept(self.waitPanelDoneEvent,
                    self._TownBattle__handleAttackWaitBack)
    ...
    def enterChooseToon(self):
        ...
        self.accept(self.chooseToonPanelDoneEvent,
                    self._TownBattle__handleChooseToonPanelDone)
    ...
    def enterFire(self):
        ...
        self.accept(self.fireCogPanelDoneEvent,
                    self._TownBattle__handleCogFireDone)
    ...
    def enterSOS(self):
        ...
        self.accept(self.SOSPanelDoneEvent,
                    self._TownBattle__handleSOSPanelDone)
    ...
    def enterSOSPetSearch(self):
        ...
        self.accept(self.proxyGenerateMessage,
                    self._TownBattle__handleProxyGenerated)
        self.accept(self.SOSPetSearchPanelDoneEvent,
                    self._TownBattle__handleSOSPetSearchPanelDone)
        messenger.send(self.battleEvent, [response])
    ...
    def enterSOSPetInfo(self):
        ...
        self.accept(self.SOSPetInfoPanelDoneEvent,
                    self._TownBattle__handleSOSPetInfoPanelDone)
    ...
    def _TownBattle__isCogChoiceNecessary(self):
        if self.numCogs > 1 and not self._TownBattle__isGroupAttack(
