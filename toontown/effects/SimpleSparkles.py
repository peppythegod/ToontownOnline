...
class SimpleSparkles(PooledEffect, EffectController):
    ...
    def createTrack(self):
        self.track = Sequence(
            Func(
                self.p0.setBirthRate, 0.029999999999999999), Func(
                self.p0.clearToInitial), Func(
                self.f.start, self, self), Wait(1.0), Func(
                    self.p0.setBirthRate, 100.0), Wait(2.5), Func(
                        self.cleanUpEffect))

    def setEffectScale(self, scale):
        self.effectScale = scale
        self.p0.renderer.setInitialXScale(
            0.80000000000000004 * self.cardScale * scale)
