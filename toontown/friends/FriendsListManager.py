...
class FriendsListManager:
    ...
    def load(self):
        ...
        self.accept(
            OTPGlobals.AvatarNewFriendAddEvent,
        ...
        if avatar.isPet():
            ...
            if hasattr(self, 'avatarPanel'):
                if self.avatarPanel:
                    if not hasattr(
                            self.avatarPanel,
                            'getAvId') or self.avatarPanel.getAvId() == avatar.doId:
        ...
        if showType == 1:
            if hasattr(self, 'avatarPanel'):
                if self.avatarPanel:
                    if not hasattr(
                            self.avatarPanel,
                            'getPlayerId') or self.avatarPanel.getPlayerId() == playerId:
                        if not self.avatarPanel.isHidden():
    ...
    def _FriendsListManager__handleGotoAvatar(
            self, avId, avName, avDisableName):
        ToonTeleportPanel.showTeleportPanel(avId, avName, avDisableName)

    def _FriendsListManager__handleFriendAvatar(
            self, avId, avName, avDisableName):
        FriendInviter.showFriendInviter(avId, avName, avDisableName)

    def _FriendsListManager__handleFriendInvitation(
            self, avId, avName, inviterDna, context):
        dna = ToonDNA.ToonDNA()
    ...
    def _FriendsListManager__processFriendRequest(
            self, avId, avName, inviterDna=None, context=None):
    ...
    def _FriendsListManager__friendAdded(self, avId):
        if FriendInviter.globalFriendInviter is not None:
            messenger.send('FriendsListManagerAddEvent', [
                avId])
        else:
