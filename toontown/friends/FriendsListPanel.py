...
def hideFriendsList():
    if globalFriendsList is not None:
...
class FriendsListPanel(DirectFrame, StateData.StateData):
    ...
    def load(self):
        ...
        clipper.setPlane(
            Plane(Vec3(-1, 0, 0), Point3(0.20000000000000001, 0, 0)))
    ...
    def _FriendsListPanel__friendPlayers(self, doId):
        if self.panelType == FLPPlayers:
            self._FriendsListPanel__updateScrollList()

    def _FriendsListPanel__friendsListChanged(self, arg1=None, arg2=None):
        if self.panelType != FLPEnemies:
            self._FriendsListPanel__updateScrollList()
