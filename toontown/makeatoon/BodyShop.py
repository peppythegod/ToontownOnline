...
class BodyShop(StateData.StateData):
    ...
    def enter(self, toon, shopsVisited=[]):
        ...
        self.accept('next', self._BodyShop__handleForward)
    ...
    def _BodyShop__swapHead(self, offset):
        ...
        self._BodyShop__updateHead()

    def _BodyShop__swapSpecies(self, offset):
        ...
        self.headList = ToonDNA.getHeadList(self.species)
        self._BodyShop__changeSpeciesName(self.species)
    ...
    def _BodyShop__updateHead(self):
        self._BodyShop__updateScrollButtons(self.headChoice, len(
    ...
    def restrictHeadType(self, head):
        if not base.cr.isPaid():
            if head[0] in ('h', 'p', 'b'):
                self.accept('next', self._BodyShop__restrictForward)
            else:
                self.accept('next', self._BodyShop__handleForward)

    def _BodyShop__restrictForward(self):
    ...
    def changeBody(self):
        ...
        self._BodyShop__swapSpecies(newSpeciesIndex - oldSpeciesIndex)
        self._BodyShop__swapHead(newHeadIndex - oldHeadIndex)
        self._BodyShop__swapTorso(newTorsoIndex - oldTorsoIndex)
        self._BodyShop__swapLegs(newLegsIndex - oldLegsIndex)
