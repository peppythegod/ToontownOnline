...
class NameShop(StateData.StateData):
    ...
    def enter(self, toon, usedNames, warp):
        ...
        self.acceptOnce('next', self._NameShop__handleDone)
        self.acceptOnce('last', self._NameShop__handleBackward)
        self.acceptOnce('skipTutorial', self._NameShop__handleSkipTutorial)
        self._NameShop__listsChanged()
    ...
    def titleToggle(self, value):
        ...
        if not self.titleActive and self.firstActive or self.lastActive:
            self.titleActive = 1
    ...
    def load(self):
        ...
        for x in range(0, 2):
            ...
            self.approvalDialog.buttonList[x]['text_scale'] = (
                0.040000000000000001, 0.059990000000000002)
    ...
    def _NameShop__handleDone(self):
        if self.fsm.getCurrentState().getName() == 'TypeAName':
            self._NameShop__typedAName()
        else:
            self._NameShop__isFirstTime()

    def _NameShop__handleSkipTutorial(self):
        self._NameShop__createAvatar(skipTutorial=True)
    ...
    def _NameShop__createAvatar(self, skipTutorial=False, *args):
    ...
    def promptTutorial(self):
        self.promptTutorialDialog = TTDialog.TTDialog(
            ...
            text_align=TextNode.ACenter,
            text_wordwrap=22,
            command=self._NameShop__openTutorialDialog,
            fadeScreen=0.5,
    ...
    def _NameShop__openTutorialDialog(self, choice=0):
        if choice == 1:
            ...
            self._NameShop__handleSkipTutorial()
