...
class DistributedLawbotChair(DistributedObject.DistributedObject, FSM.FSM):
    ...
    def loadSounds(self):
        ...
        if self.cogJurorSound is None:
            self.cogJurorSound = base.loadSfx(
                'phase_11/audio/sfx/LB_cog_jury.mp3')
    ...
    def attachColSphere(self):
        ...
        radius *= 0.65000000000000002
        adjustedZ = center[2]
        adjustedZ += 0.59999999999999998
    ...
    def putCogJurorOnSeat(self):
        ...
        if self.cogJuror and not self.cogJuror.isEmpty():
            ...
            self.cogJuror.setPos(0, -1.25, 0.94999999999999996)
            if self.toonJuror:
                self.toonJuror.hide()
    ...
    def putToonJurorOnSeat(self):
        if self.toonJuror and not self.toonJuror.isEmpty():
            ...
            self.toonJuror.setPos(0, -2.5, 0.94999999999999996)
    ...
    def _DistributedLawbotChair__touchedChair(self, entry):
        self.notify.debug('__touchedChair')
        self.notify.debug('self=%s entry=%s' % (self, entry))
        self.boss.touchedChair(self, entry)

    def _DistributedLawbotChair__touchedChairHandle(self, entry):
