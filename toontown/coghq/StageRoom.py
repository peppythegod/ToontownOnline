...
class StageRoom(DirectObject.DirectObject):
    ...
    def setGeom(self, geom):
        self._StageRoom__geom = geom
        ug = self._StageRoom__geom.find('**/underground')
        if not ug.isEmpty():
            ug.setBin('ground', -10)

    def getGeom(self):
        return self._StageRoom__geom

    def _getEntrances(self):
        return self._StageRoom__geom.findAllMatches('**/ENTRANCE*')

    def _getExits(self):
        return self._StageRoom__geom.findAllMatches('**/EXIT*')
