...
class DistributedStageRoomAI(DistributedLevelAI.DistributedLevelAI,
    ...
    def getBattleCreditMultiplier(self):
        return ToontownBattleGlobals.getStageCreditMultiplier(
            self.getFloorNum())
    ...
    def getFloorNum(self):
        ...
        if stage is None:
            self.notify.warning(
                'getFloorNum: could not find stage %s' % self.stageDoId)
            return 0
    ...
    def generate(self):
        ...
        self.d_setSuits()
        self.notify.debug(
            'finish stage room %s %s creation' % (self.roomId, self.doId))
    ...
    def allToonsGone(self, toonsThatCleared):
        DistributedLevelAI.DistributedLevelAI.allToonsGone(
            self, toonsThatCleared)
        if self.roomNum == 0:
            ...
            if stage is not None:
                ...
                self.notify.warning(
                    'no stage %s in allToonsGone' % self.stageDoId)
