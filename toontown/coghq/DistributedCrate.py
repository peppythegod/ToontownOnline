...
class DistributedCrate(DistributedCrushableEntity.DistributedCrushableEntity):
    ...
    ModelPaths = ('phase_9/models/cogHQ/woodCrateB',
    ...
    def __init__(self, cr):
        ...
        self.creakSound = loader.loadSfx(
            'phase_9/audio/sfx/CHQ_FACT_crate_effort.mp3')
        self.pushSound = loader.loadSfx(
            'phase_9/audio/sfx/CHQ_FACT_crate_sliding.mp3')
    ...
    def _DistributedCrate__listenForCollisions(self, on):
        if on:
            self.accept(
                self.uniqueName('entercrateCollision'), self.handleCollision)
        else:
    ...
    def handleCollision(self, collEntry=None):
        ...
        self.notify.debug('offsetDot = %s, world = %s, rel = %s' %
                          (offsetDot, worldVec, offsetVec))
        if relativeVec.getY() < - \
                0.69999999999999996 and offsetDot > 0.90000000000000002 and offsetVec.getZ() < 0.050000000000000003:
            self.getCrateSide(crateNormal)
            self.tContact = globalClock.getFrameTime()
            self._DistributedCrate__listenForCollisions(0)
            self._DistributedCrate__listenForCancelEvents(1)
            self._DistributedCrate__startStickTask(
                crateNormal, base.localAvatar.getPos(render))
    ...
    def _DistributedCrate__stickTask(self, task):
        ...
        if tElapsed > self.tStick:
            ...
            self.avPushTrack = Sequence(
                ...
                Func(self._DistributedCrate__sendPushRequest,
                     task.crateNormal),
    ...
    def setMoveTo(self, avId, x0, y0, z0, x1, y1, z1):
        ...
        if isLocal or self.stuckToCrate or not isLocal:
            self._DistributedCrate__moveAvTo(avId, Vec3(x0, y0, z0),
                                             Vec3(x1, y1, z1))
    ...
    def _DistributedCrate__moveCrateTo(self, startPos, endPos):
        ...
        self.moveTrack = Parallel(
            Sequence(
                LerpPosInterval(
                    self, T_PUSH, endPos, startPos=startPos, fluid=1)),
            SoundInterval(self.creakSound, node=self),
            SoundInterval(
                self.pushSound,
                node=self,
