...
class GameSprite:
    colorRed = Vec4(1, 0.20000000000000001, 0.20000000000000001, 1)
    colorBlue = Vec4(0.69999999999999996, 0.80000000000000004, 1, 1)
    colorGreen = Vec4(0, 1, 0, 1)
    colorGhostRed = Vec4(1, 0.20000000000000001, 0.20000000000000001, 0.5)
    colorGhostBlue = Vec4(0.69999999999999996, 0.80000000000000004, 1, 0.5)
    colorGhostGreen = Vec4(0, 1, 0, 0.5)
    colorDisolveRed = Vec4(1, 0.20000000000000001, 0.20000000000000001, 0.0)
    colorDisolveBlue = Vec4(0.69999999999999996, 0.80000000000000004, 1, 0.0)
    ...
    colorList = (colorRed, colorBlue, colorGreen, colorWhite, colorBlack,
                 colorPurple, colorYellow, colorOrange, colorAqua, colorSteel)
    disolveList = (colorDisolveRed, colorDisolveBlue, colorDisolveGreen,
                   colorDisolveWhite, colorDisolveBlack, colorDisolvePurple,
                   colorDisolveYellow, colorDisolveOrange, colorDisolveAqua,
                   colorSteelDissolve)
    ...
    def __init__(self, spriteBase, size, colorType=0, foundation=0, facing=0):
        ...
        self.multiColorStep = 0.025000000000000001
        self.facing = facing
        self.breakable = 1
        self.deleteFlag = 0
        self.nodeObj = None
        ...
        self.size = 0.40000000000000002 * self.sizeMult
        self.isQue = 0
    ...
    def setBallType(self, type, solidOverride=0):
        ...
        if not solid or colorType > 9:
            if colorType == 0:
                ...
                self.multiColorNext = 1
                self.multiColorLevel = 0.0
                self.multiColorStep = 0.10000000000000001
            elif colorType == 8:
            ...
            self.multiColorNext = 1
            self.multiColorLevel = 0.0
            self.multiColorStep = 0.14999999999999999
        elif colorType == 8:
            myColor = GameSprite.colorAqua
            self.multiColor = 1
            self.multiColorList = [0, 1, 2, 6]
    ...
    def runColor(self):
        if self.multiColor:
            c1 = GameSprite.colorList[self.multiColorList[
                self.multiColorIndex]]
    ...
    def warningBump(self):
        ...
        track = Sequence(
            Wait(num1 * 0.10000000000000001),
            LerpPosInterval(self.nodeObj, num2 * 0.10000000000000001,
                            Point3(0.0, 0.0, 0.5)),
            LerpPosInterval(self.nodeObj, num3 * 0.10000000000000001,
                            Point3(0.0, 0.0, 0.0)),
            LerpPosInterval(self.nodeObj, num2 * 0.10000000000000001,
                            Point3(0.0, 0.0, 0.5)),
            LerpPosInterval(self.nodeObj, num1 * 0.10000000000000001,
                            Point3(0.0, 0.0, 0.0)))
        track.start()
    ...
    def deathEffect(self):
        ...
        track = Sequence(
            Parallel(
                ProjectileInterval(
                    self.spriteBase,
                    startVel=Vec3(-20.0 + notNum3 * 40.0, -20.0 + num3 * 40.0,
                                  30),
            ...
            Func(self.removeDelay), Func(self.delete))
        track.start()
        ...
        track = Sequence(
            Parallel(
                ...
                LerpColorScaleInterval(
                    ...
                    duration=1.0,
                    startColorScale=GameSprite.colorList[self.colorType],
                    colorScale=Vec4(0, 0, 0, 0.0))), Func(self.removeDelay),
            Func(self.delete))
        track.start()
