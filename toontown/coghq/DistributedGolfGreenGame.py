...
class DistributedGolfGreenGame(BattleBlocker.BattleBlocker):
    ...
    def __init__(self, cr):
        ...
        self.wildIndex = 8
        self.bombIndex = 7
        self.sizeMult = 1.3999999999999999
        self.cellSizeX = 1.0 * self.sizeMult
        self.cellSizeZ = self.cellSizeX * 0.80000000000000004
        self.radiusBall = 0.5 * self.cellSizeX
        self.gridDimX = 9
        self.gridDimZ = 15
        self.minX = -1.0 * \
            (self.gridDimX + 0.37509999999999999) * 0.5 * self.cellSizeX
        self.minZ = -(self.gridDimZ) * 0.10000000000000001 * self.cellSizeZ
        self.newBallX = 0.0
        self.newBallZ = self.minZ + 0.10000000000000001 * self.sizeMult
        ...
        self.rollTrack = None
        self.zGap = 0.091999999999999998
        self.screenSizeX = base.a2dRight - base.a2dLeft
        self.screenSizeZ = base.a2dTop - base.a2dBottom
        self.XtoZ = self.screenSizeX / \
            self.screenSizeZ * (1.0 - self.zGap * 1.0)
        ...
        self.running = 0
        self.finished = 0
        self._DistributedGolfGreenGame__toonTracks = {}
    ...
    def load(self):
        ...
        self.soundBurst = base.loadSfx(
            'phase_5/audio/sfx/Toon_bodyfall_synergy.mp3')
        self.soundBomb = base.loadSfx(
            'phase_4/audio/sfx/MG_cannon_fire_alt.mp3')
        self.soundLose = base.loadSfx(
            'phase_11/audio/sfx/LB_capacitor_discharge_3.mp3')
        self.soundWin = base.loadSfx(
            'phase_4/audio/sfx/MG_pairing_match_bonus_both.mp3')
        self.soundDone = base.loadSfx(
        ...
        self.backBoard.setScale(0.29999999999999999, 0.20000000000000001, 0.25)
    ...
    def _DistributedGolfGreenGame__handleExit(self):
        self._DistributedGolfGreenGame__acceptExit()

    def _DistributedGolfGreenGame__startGame(self):
        if not self.setupFlag:
            self.setup()
    ...
    def startBoard(self, board, attackPattern):
        ...
        self.accept('mouse1', self._DistributedGolfGreenGame__handleMouseClick)
    ...
    def failBoard(self, task=None):
        self._DistributedGolfGreenGame__finishBoard(0)

    def _DistributedGolfGreenGame__handleWin(self):
        self._DistributedGolfGreenGame__handleExit()

    def _DistributedGolfGreenGame__finishBoard(self, success=1):
        if self.rollTrack:
    ...
    def _DistributedGolfGreenGame__removeGame(self):
        self.spriteNode.remove()
        self.setupFlag = 0

    def _DistributedGolfGreenGame__leaveGame(self):
    ...
    def explodeBombs(self):
        ...
        for column in self.grid:
            for cell in column:
                ...
                cell[0].colorType == self.bombIndex
    ...
    def bombNeighbors(self, cellX, cellZ):
        ...
        elif self.getColorType(cellX - 1, cellZ) != -1:
            self.addToMatchList(cellX - 1, cellZ)
    ...
    def doOnClearGrid(self):
        ...
        if self.winCounter > 4:
            self._DistributedGolfGreenGame__handleWin()
    ...
    def colorGrid(self):
        for row in self.grid:
            for cell in row:
                ...
                cell[0].colorType == 3
                if cell[4] <= self.spriteNotchPos:
                    cell[5].setColorScale(self.outColor)
                    continue
                if self.hasNeighbor(cell[3], cell[4]):
    ...
    def addControlSprite(self, x=0.0, z=0.0, color=None):
        newSprite = self.addSprite(
            self.block, posX=x, posZ=z, color=color, found=1)
        newSprite.spriteBase.reparentTo(self.frame)
        newSprite.spriteBase.setPos(0.0, 0.69999999999999996, -1.54)
    ...
    def _DistributedGolfGreenGame__handleMouseClick(self):
    ...
    def _DistributedGolfGreenGame__run(self, cont=1):
        ...
        if base.mouseWatcherNode.hasMouse():
            ...
            if inputX > 0.0:
                self.aimRadian = -1.0 * pi + \
                    math.atan(outputZ / inputX * self.XtoZ)
            elif inputX < 0.0:
            ...
            degrees = self._DistributedGolfGreenGame__toDegrees(self.aimRadian)
        ...
        self._DistributedGolfGreenGame__colTest()
        if self.hasChanged and self.running:
            ...
            if whiteCount == 0:
                self._DistributedGolfGreenGame__finishBoard()
                self.flagNextLevel = 0
                self.killSprite(self.controlSprite)
    ...
    def _DistributedGolfGreenGame__stop(self):
        taskMgr.remove('GolfGreenGameTask')
        self.running = 0

    def _DistributedGolfGreenGame__testWin(self):
        gridCount = 0
        for column in self.grid:
            for cell in column:
                if cell[0]:
    ...
    def _DistributedGolfGreenGame__toRadians(self, angle):
        return angle * 2.0 * math.pi / 360.0

    def _DistributedGolfGreenGame__toDegrees(self, angle):
        return angle * 360.0 / 2.0 * math.pi

    def _DistributedGolfGreenGame__colTest(self):
        ...
        for movingSpriteIndex in range(len(self.sprites)):
            for testSpriteIndex in range(movingSpriteIndex, len(self.sprites)):
                ...
                if testSprite and movingSprite:
                    if movingSpriteIndex != testSpriteIndex:
    ...
    def getSprite(self, spriteIndex):
        if spriteIndex >= len(
                self.sprites) or self.sprites[spriteIndex].markedForDeath:
            return None
        else:
    ...
    def _DistributedGolfGreenGame__collide(self, move, test):
        test.velX = 0
        test.velZ = 0
        move.velX = 0
        move.velZ = 0
    ...
    def announceGenerate(self):
        ...
        groundCircle.setScale(0.23999999999999999)
    ...
    def _DistributedGolfGreenGame__setCamera(self):
        ...
        base.camLens.setFov(60, 46.826500000000003)
    ...
    def signalDone(self, success):
        self.finished = 1
        self.soundDone.play()
        self._DistributedGolfGreenGame__leaveGame()
        self._DistributedGolfGreenGame__removeGame()
        self.scoreBoard.hide()
        self.cleanupTimer()
        if success:
    ...
    def storeToonTrack(self, avId, track):
        ...
        self._DistributedGolfGreenGame__toonTracks[avId] = track

    def clearToonTrack(self, avId):
        oldTrack = self._DistributedGolfGreenGame__toonTracks.get(avId)
        if oldTrack:
            oldTrack.pause()
            if self._DistributedGolfGreenGame__toonTracks.get(avId):
                DelayDelete.cleanupDelayDeletes(
                    self._DistributedGolfGreenGame__toonTracks[avId])
                del self._DistributedGolfGreenGame__toonTracks[avId]

    def clearToonTracks(self):
        keyList = []
        for key in self._DistributedGolfGreenGame__toonTracks:
            keyList.append(key)

        for key in keyList:
            if key in self._DistributedGolfGreenGame__toonTracks:
