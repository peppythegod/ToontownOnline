...
class DistributedSecurityCamera(BasicEntities.DistributedNodePathEntity):
    ...
    def __init__(self, cr):
        ...
        self.delta = 0.0
        self.Norm = {}
        self.Norm['Red'] = 0.20000000000000001
        self.Norm['Green'] = 0.20000000000000001
        self.Norm['Blue'] = 0.20000000000000001
        self.Norm['Alpha'] = 1.0
        self.Alert = {}
        self.Alert['Red'] = 1.0
    ...
    def _DistributedSecurityCamera__detect(self, task):
    ...
    def loadModel(self):
        ...
        taskMgr.add(
            self._DistributedSecurityCamera__updateTrack,
            self.taskName,
            priority=25)
    ...
    def _DistributedSecurityCamera__updateTrack(self, task):
        ...
        if trigDist < self.radius * 0.5 + 1.0:
            ...
            self.moveLoopTrack.finish()
        elif not self.moveLoopTrack.isPlaying():
    ...
    def genTrack(self):
        dist = self.getDist(base.localAvatar)
        draw = 1.0 / (0.01 + float(pow(dist, 0.40000000000000002)))
        self.trackShape = []
        wideX = 1
        wideY = 1
        ...
        beamAlpha = 1.0
        origin = {}
        origin['Red'] = 0.20000000000000001
        origin['Green'] = 0.20000000000000001
        origin['Blue'] = 0.20000000000000001
        origin['Alpha'] = 1.0
        if self.canDamage:
            origin = self.Norm
        else:
