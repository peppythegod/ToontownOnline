...
class DistributedLevelBattle(DistributedBattle.DistributedBattle):
    ...
    def lockLevelViz(self):
        ...
        if level:
            ...
            self.notify.warning(
                "lockLevelViz: couldn't find level %s" %
                self.levelDoId)
    ...
    def unlockLevelViz(self):
        ...
        if level:
            ...
            self.notify.warning(
                "unlockLevelViz: couldn't find level %s" %
                self.levelDoId)

    def onWaitingForJoin(self):
        self.lockLevelViz()

    def _DistributedLevelBattle__faceOff(self, ts, name, callback):
        if len(self.suits) == 0:
    ...
    def enterFaceOff(self, ts):
        ...
        self._DistributedLevelBattle__faceOff(
            ts, self.faceOffName, self._DistributedLevelBattle__handleFaceOffDone)

    def _DistributedLevelBattle__handleFaceOffDone(self):
    ...
    def _DistributedLevelBattle__playReward(self, ts, callback):
        toonTracks = Parallel()
        for toon in self.toons:
            toonTracks.append(
                Sequence(
    ...
    def enterReward(self, ts):
        ...
        self.delayDeleteMembers()
        self._DistributedLevelBattle__playReward(
            ts, self._DistributedLevelBattle__handleFloorRewardDone)

    def _DistributedLevelBattle__handleFloorRewardDone(self):
        pass
