...
class SuitPlannerInteriorAI:
    ...
    def _SuitPlannerInteriorAI__genJoinChances(self, num):
        joinChances = []
    ...
    def _genSuitInfos(self, numFloors, bldgLevel, bldgTrack):
        ...
        self.notify.debug('\n\ngenerating suitsInfos with numFloors (' +
                          str(numFloors) + ') bldgLevel (' + str(bldgLevel) +
                          '+1) and bldgTrack (' + str(bldgTrack) + ')')
        for currFloor in range(numFloors):
            infoDict = {}
            lvls = self._SuitPlannerInteriorAI__genLevelList(
                bldgLevel, currFloor, numFloors)
            activeDicts = []
            ...
            if len(bldgInfo) > SuitBuildingGlobals.SUIT_BLDG_INFO_REVIVES:
                revives = bldgInfo[
                    SuitBuildingGlobals.SUIT_BLDG_INFO_REVIVES][0]
            else:
                revives = 0
    ...
    def _SuitPlannerInteriorAI__genLevelList(self, bldgLevel, currFloor,
                                             numFloors):
        bldgInfo = SuitBuildingGlobals.SuitBuildingInfo[bldgLevel]
        ...
        if currFloor + 1 == numFloors:
            bossLvlRange = bldgInfo[
                SuitBuildingGlobals.SUIT_BLDG_INFO_BOSS_LVLS]
    ...
    def _SuitPlannerInteriorAI__setupSuitInfo(self, suit, bldgTrack, suitLevel,
                                              suitType):
    ...
    def _SuitPlannerInteriorAI__genSuitObject(self,
                                              suitZone,
                                              suitType,
                                              bldgTrack,
                                              suitLevel,
        ...
        skel = self._SuitPlannerInteriorAI__setupSuitInfo(
    ...
    def myPrint(self):
        ...
        for currInfo in suitInfos:
            ...
            self.notify.debug(' Floor ' + str(whichSuitInfo) + ' has ' +
                              str(len(currInfo[0])) + ' active suits.')
            for currActive in range(len(currInfo[0])):
                self.notify.debug(
                    '  Active suit ' + str(currActive + 1) + ' is of type ' +
                    str(currInfo[0][currActive][0]) + ' and of track ' +
                    str(currInfo[0][currActive][1]) + ' and of level ' +
                    str(currInfo[0][currActive][2]))

            self.notify.debug(' Floor ' + str(whichSuitInfo) + ' has ' +
                              str(len(currInfo[1])) + ' reserve suits.')
            for currReserve in range(len(currInfo[1])):
                self.notify.debug(
                    '  Reserve suit ' + str(currReserve + 1) + ' is of type ' +
                    str(currInfo[1][currReserve][0]) + ' and of track ' +
                    str(currInfo[1][currReserve][1]) + ' and of lvel ' +
                    str(currInfo[1][currReserve][2]) + ' and has ' +
                    str(currInfo[1][currReserve][3]) + '% join restriction.')
    ...
    def genFloorSuits(self, floor):
        ...
        for activeSuitInfo in floorInfo['activeSuits']:
            suit = self._SuitPlannerInteriorAI__genSuitObject(
                self.zoneId, activeSuitInfo['type'], activeSuitInfo['track'],
                activeSuitInfo['level'], activeSuitInfo['revives'])
        ...
        for reserveSuitInfo in floorInfo['reserveSuits']:
            suit = self._SuitPlannerInteriorAI__genSuitObject(
                self.zoneId, reserveSuitInfo['type'], reserveSuitInfo['track'],
                reserveSuitInfo['level'], reserveSuitInfo['revives'])
