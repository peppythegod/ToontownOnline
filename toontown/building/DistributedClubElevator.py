...
class DistributedClubElevator(DistributedElevatorFSM.DistributedElevatorFSM):
    ...
    def __init__(self, cr):
        ...
        self.leftDoor = None
        self.rightDoor = None
        self._DistributedClubElevator__toonTracks = {}
    ...
    def announceGenerate(self):
                        ...
                        self.wheels[2], 5.0, Vec3(
                            self.wheels[2].getH(), wheelAngle, self.wheels[2].getR())), LerpHprInterval(
                                self.wheels[3], 5.0, Vec3(
                                    self.wheels[3].getH(), wheelAngle, self.wheels[3].getR())), name='CogKartAnimate')
        ...
        self.closeDoors = Sequence(
            self.trolleyExitTrack, Func(
                self.onDoorCloseFinish))
        self.openDoors = Sequence(self.trolleyEnterTrack)
        self.setPos(0, 0, 0)

    def _DistributedClubElevator__placeElevator(self):
    ...
    def setEntranceId(self, entranceId):
        ...
        if self.entranceId == 0:
            self.elevatorModel.setPosHpr(
                62.740000000000002, -85.310000000000002, 0.0, 2.0, 0.0, 0.0)
    ...
    def enterClosed(self, ts):
        ...
        self._DistributedClubElevator__doorsClosed(self.getZoneId())
    ...
    def _DistributedClubElevator__doorsClosed(self, zoneId):
        pass
    ...
    def storeToonTrack(self, avId, track):
        ...
        self._DistributedClubElevator__toonTracks[avId] = track
        ...
        if oldTrack:
            ...
            if self._DistributedClubElevator__toonTracks.get(avId):
                DelayDelete.cleanupDelayDeletes(
                    self._DistributedClubElevator__toonTracks[avId])
                del self._DistributedClubElevator__toonTracks[avId]

    def clearToonTracks(self):
        keyList = []
        for key in self._DistributedClubElevator__toonTracks:
            keyList.append(key)

        for key in keyList:
            if key in self._DistributedClubElevator__toonTracks:
