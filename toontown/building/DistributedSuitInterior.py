...
class DistributedSuitInterior(DistributedObject.DistributedObject):
    ...
    def _DistributedSuitInterior__addToon(self, toon):
        self.accept(
            ...
            self._DistributedSuitInterior__handleUnexpectedExit,
            extraArgs=[toon])
    ...
    def enterElevator(self, ts=0):
        ...
        self._DistributedSuitInterior__playElevator(
            ts, self.elevatorName,
            self._DistributedSuitInterior__handleElevatorDone)
    ...
    def enterBattle(self, ts=0):
        if self.elevatorOutOpen == 1:
            self._DistributedSuitInterior__playCloseElevatorOut(
    ...
    def exitBattle(self):
        if self.elevatorOutOpen == 1:
            self._DistributedSuitInterior__finishInterval(
                self.uniqueName('close-out-elevator'))
            self.elevatorOutOpen = 0

    def _DistributedSuitInterior__playReservesJoining(self, ts, name,
                                                      callback):
        index = 0
    ...
    def enterReservesJoining(self, ts=0):
        self._DistributedSuitInterior__playReservesJoining(
            ts, self.uniqueName('reserves-joining'),
            self._DistributedSuitInterior__handleReserveJoinDone)
