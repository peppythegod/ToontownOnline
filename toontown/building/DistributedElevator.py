...
class DistributedElevator(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        self._DistributedElevator__toonTracks = {}
    ...
    def setupElevator(self):
        ...
        self.boardedAvIds = {}
        self.openDoors = getOpenInterval(self, self.leftDoor, self.rightDoor,
                                         self.openSfx, self.finalOpenSfx,
                                         self.type)
        self.closeDoors = getCloseInterval(self, self.leftDoor, self.rightDoor,
    ...
    def finishSetup(self):
        ...
        if self._DistributedElevator__preSetupState:
            self.fsm.request(self._DistributedElevator__preSetupState, [0])
            self._DistributedElevator__preSetupState = None
    ...
    def gotToon(self, index, avId, toonList):
        ...
        if request:
            ...
            self.notify.error(
                'gotToon: already had got toon in slot %s.' % index)
    ...
    def fillSlot(self, index, avId, wantBoardingShow=0):
        self.notify.debug(
            '%s.fillSlot(%s, %s, ...)' % (self.doId, index, avId))
        request = self.toonRequests.get(index)
        if request:
            self.cr.relatedObjectMgr.abortRequest(request)
            del self.toonRequests[index]
    ...
    def storeToonTrack(self, avId, track):
        ...
        self._DistributedElevator__toonTracks[avId] = track

    def clearToonTrack(self, avId):
        oldTrack = self._DistributedElevator__toonTracks.get(avId)
        if oldTrack:
            oldTrack.pause()
            if self._DistributedElevator__toonTracks.get(avId):
                DelayDelete.cleanupDelayDeletes(
                    self._DistributedElevator__toonTracks[avId])
                del self._DistributedElevator__toonTracks[avId]

    def clearToonTracks(self):
        keyList = []
        for key in self._DistributedElevator__toonTracks:
            keyList.append(key)

        for key in keyList:
            if key in self._DistributedElevator__toonTracks:
    ...
    def canHideBoardingQuitBtn(self, avId):
        ...
        ) and localAvatar.boardingParty and localAvatar.boardingParty.groupPanel:
            return True
        else:
            return False
