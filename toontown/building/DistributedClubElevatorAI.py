...
class DistributedClubElevatorAI(
        DistributedElevatorFSMAI.DistributedElevatorFSMAI):
    notify = DirectNotifyGlobal.directNotify.newCategory(
        'DistributedElevatorFloorAI')
    defaultTransitions = {
        'Off': ['Opening', 'Closed'],
    ...
    id = 0
    DoBlockedRoomCheck = simbase.config.GetBool('elevator-blocked-rooms-check',
                                                1)
    ...
    def acceptBoarder(self, avId, seatIndex):
        ...
        self.acceptOnce(
            ...
            self._DistributedClubElevatorAI__handleUnexpectedExit,
            extraArgs=[avId])
        if self.state == 'WaitEmpty' and self.countFullSeats(
        ) < self.countAvsInZone():
            ...
            taskMgr.doMethodLater(ElevatorConstants.TOON_BOARD_ELEVATOR_TIME,
                                  self.goAllAboard, self.quickBoardTask)
    ...
    def enterOpening(self):
        ...
        taskMgr.doMethodLater(
            ...
            ['openTime'], self.waitEmptyTask, self.uniqueName('opening-timer'))
    ...
    def enterWaitCountdown(self):
        self.lastState = self.state
        DistributedElevatorFSMAI.DistributedElevatorFSMAI.enterWaitCountdown(
            self)
    ...
    def enterAllAboard(self):
        ...
        waitTime = max(
            ElevatorConstants.TOON_BOARD_ELEVATOR_TIME - elapsedTime, 0)
    ...
    def enterClosing(self):
        ...
        taskMgr.doMethodLater(
            ...
            ['closeTime'], self.elevatorClosedTask,
