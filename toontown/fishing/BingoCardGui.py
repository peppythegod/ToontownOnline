...
class BingoCardGui(DirectFrame):
    ...
    def show(self):
        ...
        if self.game and self.game.checkForBingo():
            self._BingoCardGui__indicateBingo(True)
    ...
    def loadGameTimer(self):
        ...
        self.timer.setScale(0.17000000000000001)
        self.timer.setPos(0.23999999999999999, 0, -0.17999999999999999)
    ...
    def loadCard(self):
        ...
        for index in xrange(cardSize):
            ...
            if index == cardSize / 2:
                ...
                continue
            if self.game.getGameState() & 1 << index:
    ...
    def cellUpdateCheck(self, id, genus, species):
        ...
        if self.cellGuiList[id].getFishGenus(
        ) == genus or fishTuple == FishGlobals.BingoBoot:
    ...
    def getUnmarkedMatches(self, fish):
        ...
        for cell in self.cellGuiList:
            if cell['state'] == DGG.NORMAL:
                if cell.getFishGenus(
                ) == fish[0] or fish == FishGlobals.BingoBoot:
                    matches.append(cell)

            fish == FishGlobals.BingoBoot
    ...
    def _BingoCardGui__indicateMatches(self, bFlipFlop, fish):
    ...
    def fishCaught(self, fish):
        self.stopCellBlinking()
        self._BingoCardGui__indicateMatches(True, fish)
    ...
    def _BingoCardGui__indicateBingo(self, bFlipFlop):
        if not self.game:
            return Task.done
    ...
    def lightSwitch(self, bOn, lightIndex=-1):
        if lightIndex == -1:
            ...
            light['image'] = self.model.find(
                self.getLightName(lightIndex, bOn))
