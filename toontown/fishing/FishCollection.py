...
class FishCollection:
    ...
    def _FishCollection__collect(self, newFish, updateCollection):
        for fish in self.fishList:
            if fish.getGenus() == newFish.getGenus() and fish.getSpecies(
            ) == newFish.getSpecies():
            ...
            fish.getWeight() < newFish.getWeight()
    ...
    def collectFish(self, newFish):
        return self._FishCollection__collect(newFish, updateCollection=1)

    def getCollectResult(self, newFish):
        return self._FishCollection__collect(newFish, updateCollection=0)
