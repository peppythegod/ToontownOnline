...
class Place(StateData.StateData, FriendsListManager.FriendsListManager):
    ...
    def load(self):
        ...
        self.walkStateData = PublicWalk.PublicWalk(
            self.fsm, self.walkDoneEvent)
    ...
    def _getQZState(self):
        if hasattr(base, 'cr') and hasattr(base.cr, 'playGame'):
            if hasattr(
                    base.cr.playGame,
                    'quietZoneStateData') and base.cr.playGame.quietZoneStateData:
    ...
    def addLeftQuietZoneCallback(self, callback, priority=None):
        ...
        if qzsd:
            ...
            token = self._leftQuietZoneLocalCallbacks.add(
    ...
    def addSetZoneCompleteCallback(self, callback, priority=None):
        ...
        if qzsd:
            ...
            token = self._setZoneCompleteLocalCallbacks.add(
                callback, priority=priority)
        ...
        if self.isPeriodTimerEffective():
            if base.cr.periodTimerExpired:
                taskMgr.doMethodLater(
                    5, self.redoPeriodTimer, 'redoPeriodTimer')
    ...
    def periodTimerExpired(self):
        ...
        if base.localAvatar.book.isEntered:
            ...
            base.localAvatar.b_setAnimState(
                'CloseBook', 1, callback=self._Place__handlePeriodTimerBookClose)
        else:
            base.localAvatar.b_setAnimState(
                'TeleportOut', 1, self._Place__handlePeriodTimerExitTeleport)
    ...
    def enterWalk(self, teleportIn=0):
        ...
        if base.cr.productName in [
            'DisneyOnline-US',
                'ES'] and not base.cr.isPaid() and base.localAvatar.tutorialAck:
    ...
    def exitWalk(self):
        ...
        if base.cr.productName in [
                'DisneyOnline-US',
                'ES'] and not base.cr.isPaid() and base.localAvatar.tutorialAck and not (
    ...
    def enterStickerBook(self, page=None):
        ...
        base.localAvatar.b_setAnimState(
            'OpenBook', 1, self.enterStickerBookGUI)
    ...
    def _Place__handleFallingAsleep(self, task):
        base.localAvatar.book.exit()
        base.localAvatar.b_setAnimState(
            'CloseBook', 1, callback=self._Place__handleFallingAsleepBookClose)
        return Task.done
    ...
    def _Place__handleBook(self):
        ...
        if bookStatus['mode'] == 'close':
            ...
            base.localAvatar.collisionsOff()
            base.localAvatar.b_setAnimState(
                'CloseBook', 1, callback=self._Place__handleBookCloseExit)
            ...
            base.localAvatar.collisionsOff()
            base.localAvatar.b_setAnimState(
                'CloseBook', 1, callback=self.startPartyNow, extraArgs=[
                    firstStart, hostId])
        ...
        if doneStatus['mode'] == 'complete':
            if requestStatus.get('tutorial', 0):
                ...
                out = {
                    ...
                    'tunnelIn': 'tunnelOut',
                    'doorIn': 'doorOut'}
            teleportDebug(requestStatus, 'requesting %s, requestStatus=%s' % (
    ...
    def enterTunnelIn(self, requestStatus):
        self.notify.debug(
            'enterTunnelIn(requestStatus=' + str(requestStatus) + ')')
    ...
    def _placeTeleportInPostZoneComplete(self, requestStatus):
        teleportDebug(
            requestStatus, '_placeTeleportInPostZoneComplete(%s)' %
            (requestStatus,))
        NametagGlobals.setMasterArrowsOn(0)
        base.localAvatar.laffMeter.start()
        base.localAvatar.startQuestMap()
        ...
        globalClock.tick()
        base.localAvatar.b_setAnimState(
            'TeleportIn', 1, callback=self.teleportInDone)
    ...
    def enterBanking(self):
        ...
        base.localAvatar.startSleepWatch(
            self._Place__handleFallingAsleepBanking)
        self.enablePeriodTimer()

    def _Place__handleFallingAsleepBanking(self, arg):
        if hasattr(self, 'fsm'):
    ...
    def enterPhone(self):
        ...
        base.localAvatar.laffMeter.start()
        base.localAvatar.obscureMoveFurnitureButton(1)
        base.localAvatar.startSleepWatch(self._Place__handleFallingAsleepPhone)
        self.enablePeriodTimer()

    def _Place__handleFallingAsleepPhone(self, arg):
        if hasattr(self, 'fsm'):
            self.fsm.request('walk')
    ...
    def enterStopped(self):
        ...
        base.localAvatar.obscureMoveFurnitureButton(1)
        base.localAvatar.startSleepWatch(
            self._Place__handleFallingAsleepStopped)
        self.enablePeriodTimer()

    def _Place__handleFallingAsleepStopped(self, arg):
        if hasattr(self, 'fsm'):
            self.fsm.request('walk')
