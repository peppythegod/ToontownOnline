...
class DistributedPetAI(
    ...
    def __init__(self, air, dna=None):
        ...
        self._DistributedPetAI__funcsToDelete = []
        self._DistributedPetAI__generateDistTraitFuncs()
        self._DistributedPetAI__generateDistMoodFuncs()
        self.busy = 0
    ...
    def _DistributedPetAI__generateDistTraitFuncs(self):
        for i in xrange(PetTraits.PetTraits.NumTraits):
            ...
            self._DistributedPetAI__funcsToDelete.append(getterName)
            self._DistributedPetAI__funcsToDelete.append(b_setterName)
            self._DistributedPetAI__funcsToDelete.append(d_setterName)
            self._DistributedPetAI__funcsToDelete.append(setterName)
    ...
    def _DistributedPetAI__handleMoodSet(self, component, value):
        if self.isGenerated():
            self.mood.setComponent(component, value)
        else:
            self.requiredMoodComponents[component] = value

    def _DistributedPetAI__handleMoodGet(self, component):
        if self.isGenerated():
            return self.mood.getComponent(component)
        else:
            return 0.0

    def _DistributedPetAI__generateDistMoodFuncs(self):
        for compName in PetMood.PetMood.Components:
            ...
            def moodGetter(compName=compName):
                return self._DistributedPetAI__handleMoodGet(compName)
            ...
            self._DistributedPetAI__funcsToDelete.append(getterName)
            self._DistributedPetAI__funcsToDelete.append('b_%s' % setterName)
            self._DistributedPetAI__funcsToDelete.append('d_%s' % setterName)
            self._DistributedPetAI__funcsToDelete.append(setterName)
    ...
    def _doDeleteCleanup(self):
        ...
        try:
            for funcName in self._DistributedPetAI__funcsToDelete:
    ...
    def _DistributedPetAI__lockPetMoveTask(self, avId):
    ...
    def endLockPetMove(self, avId):
        ...
        self._DistributedPetAI__petMovieStart(avId)
