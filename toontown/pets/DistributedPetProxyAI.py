...
class DistributedPetProxyAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __init__(self, air):
        ...
        self._DistributedPetProxyAI__funcsToDelete = []
        self._DistributedPetProxyAI__generateDistTraitFuncs()
        self._DistributedPetProxyAI__generateDistMoodFuncs()
    ...
    def _DistributedPetProxyAI__generateDistTraitFuncs(self):
        for i in xrange(PetTraits.PetTraits.NumTraits):
            ...
            self._DistributedPetProxyAI__funcsToDelete.append(getterName)
            self._DistributedPetProxyAI__funcsToDelete.append(b_setterName)
            self._DistributedPetProxyAI__funcsToDelete.append(d_setterName)
            self._DistributedPetProxyAI__funcsToDelete.append(setterName)
    ...
    def _DistributedPetProxyAI__handleMoodSet(self, component, value):
        if self.isGenerated():
            self.mood.setComponent(component, value)
        else:
            self.requiredMoodComponents[component] = value

    def _DistributedPetProxyAI__handleMoodGet(self, component):
        if self.isGenerated():
            return self.mood.getComponent(component)
        else:
            return 0.0

    def _DistributedPetProxyAI__generateDistMoodFuncs(self):
        for compName in PetMood.PetMood.Components:
            ...
            def moodGetter(compName=compName):
                return self._DistributedPetProxyAI__handleMoodGet(compName)
            ...
            def moodSetter(value, compName=compName):
                self._DistributedPetProxyAI__handleMoodSet(compName, value)
            ...
            self._DistributedPetProxyAI__funcsToDelete.append(getterName)
            self._DistributedPetProxyAI__funcsToDelete.append(
                'b_%s' % setterName)
            self._DistributedPetProxyAI__funcsToDelete.append(
                'd_%s' % setterName)
            self._DistributedPetProxyAI__funcsToDelete.append(setterName)
    ...
    def delete(self):
        ...
        del self.mood
        del self.traits
        for funcName in self._DistributedPetProxyAI__funcsToDelete:
    ...
    def d_setDominantMood(self, dominantMood):
        self.sendUpdate('setDominantMood', [
            dominantMood])
