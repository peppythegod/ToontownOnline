...
class Pet(Avatar.Avatar):
    ...
    def __init__(self, forGui=0):
        ...
        self.forGui = forGui
        self.moodModel = None
        self._Pet__blinkName = 'petblink-' + str(self.this)
        self.track = None
    ...
    def generateMoods(self):
        ...
        self.moodIcons.setZ(3.6499999999999999)
    ...
    def showMood(self, mood):
        if hasattr(base.cr, 'newsManager') and base.cr.newsManager:
            ...
            if (ToontownGlobals.APRIL_FOOLS_COSTUMES in holidayIds or ToontownGlobals.SILLYMETER_EXT_HOLIDAY in holidayIds) and not (
                    mood == 'confusion'):
                self.speakMood(mood)
                return None
            else:
    ...
    def enterBeg(self):
        ...
        self.track = Sequence(
            Func(
                self.play, 'toBeg'), Wait(delay), Func(
                self.loop, 'beg'))
        self.track.start()
        ...
        track = Sequence(
            Wait(1.0), Parallel(
                self.getTeleportInSoundInterval(), Sequence(
                    Func(
                        self.showHole), ActorInterval(
                        self.teleportHole, 'hole', startFrame=81, endFrame=71), ActorInterval(
                        self, 'reappear'), ActorInterval(
                            self.teleportHole, 'hole', startFrame=71, endFrame=81), Func(
                                self.cleanupHole), Func(
                                    self.loop, 'neutral')), Sequence(
                                        Func(
                                            self.dropShadow.hide), Wait(1.0), Func(
                ...
                self.getTeleportOutSoundInterval(), Sequence(
                    ActorInterval(
                        ...
                        ActorInterval(
                            ...
                            self.showHole), ActorInterval(
                                self.teleportHole, 'hole', startFrame=81, endFrame=71)), ActorInterval(
                                    self, 'disappear'), ActorInterval(
                                        self.teleportHole, 'hole', startFrame=71, endFrame=81), Func(
                                            self.cleanupHole)), Sequence(
                                                Wait(1.0), Func(
    ...
    def trackAnimToSpeed(self, forwardVel, rotVel, inWater=0):
        ...
        if self.isInWater():
            action = 'swim'
        elif forwardVel > 0.10000000000000001 or abs(rotVel) > 0.10000000000000001:
            action = 'walk'
    ...
    def _Pet__blinkOpen(self, task):
        ...
        if r < 0.10000000000000001:
            ...
            t = r * 4.0 + 1
        taskMgr.doMethodLater(t, self._Pet__blinkClosed, self._Pet__blinkName)
        return Task.done

    def _Pet__blinkClosed(self, task):
        self.eyesClose()
        taskMgr.doMethodLater(
            0.125,
            self._Pet__blinkOpen,
            self._Pet__blinkName)
        return Task.done

    def startBlink(self):
        taskMgr.remove(self._Pet__blinkName)
        self.eyesOpen()
    ...
    def getInteractIval(self, interactId):
        ...
        if isinstance(anims, types.StringType):
