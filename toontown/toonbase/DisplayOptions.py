...
class DisplayOptions:
    ...
    def loadFromSettings(self):
        ...
        self.notify.debug(
            'before prc settings full screen mode=%s' %
            str(mode))
        if mode is None:
            mode = 1
        ...
        loadPrcFileData(
            ...
            'audio-master-music-volume %s' %
            musicVol)
        loadPrcFileData(
            'toonBase Settings Sfx Volume',
            'audio-master-sfx-volume %s' %
            sfxVol)
        loadPrcFileData(
    ...
    def isDefaultEmbedded():
        ...
        try:
            embedOption = int(
                base.launcher.getValue(
                    'GAME_DEFAULT_TO_EMBEDDED', None))
            if embedOption is not None:
    ...
    isDefaultEmbedded = staticmethod(isDefaultEmbedded)

    def isEmbeddedPossible():
        result = False

        try:
            showOption = base.launcher.getValue(
                'GAME_SHOW_EMBEDDED_OPTION', None)
            if showOption is not None:
    ...
    isEmbeddedPossible = staticmethod(isEmbeddedPossible)
    ...
    def isWindowedPossible():
        ...
        try:
            showOption = base.launcher.getValue(
                'GAME_SHOW_WINDOWED_OPTION', None)
            if showOption is not None:
                result = bool(int(showOption))
        except BaseException:
            pass
    ...
    isWindowedPossible = staticmethod(isWindowedPossible)
