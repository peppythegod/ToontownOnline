...
class CogdoFlyingGameIntro(CogdoGameMovie):
    ...
    def load(self):
        ...
        self.cogHead.setPosHprScale(-0.73999999999999999, 0, -1.79,
                                    180, 0, 0, 0.12, 0.14000000000000001, 0.14000000000000001)
        self.cogHead.reparentTo(hidden)
        ...
        flyThroughIval = Parallel(
            camera.posInterval(
                flyDur, self._exit.getPos(render) + Point3(
                    0, -22, 1), blendType='easeInOut'), camera.hprInterval(
                flyDur, Point3(
                    0, 5, 0), blendType='easeInOut'))
        self._ival = Sequence(
            Func(start), Parallel(
                flyThroughIval, Sequence(
                    Func(
                        self.displayLine, 'cog', self._getRandomLine(
                            dialogue[0])), Wait(waitDur), Func(
                        self.displayLine, 'toon', self._getRandomLine(
                            dialogue[1])), Wait(waitDur), Func(
                                self.displayLine, 'cog', self._getRandomLine(
                                    dialogue[2])), Wait(waitDur))), Func(end))
...
class CogdoFlyingGameFinish(CogdoGameMovie):
    ...
    def load(self):
        ...
        showExitIval = Sequence(
            Func(
                camera.wrtReparentTo, render), Parallel(
                camera.posInterval(
                    exitDur, Point3(
                        0, -55, 40), other=self._exit, blendType='easeInOut'), camera.hprInterval(
                    exitDur, Point3(
                        0, -45, 0), blendType='easeInOut')))
