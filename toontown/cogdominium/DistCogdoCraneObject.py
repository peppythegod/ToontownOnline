...
class DistCogdoCraneObject(
    ...
    def __init__(self, cr):
        ...
        self.hitBossSfx = loader.loadSfx(
            'phase_5/audio/sfx/AA_drop_safe_miss.mp3')
        ...
        self.touchedBossSoundInterval = SoundInterval(
            self.touchedBossSfx, duration=0.80000000000000004)
    ...
    def _DistCogdoCraneObject__hitFloor(self, entry):
        if self.state == 'Dropped' or self.state == 'LocalDropped':
            self.d_hitFloor()
            self.demand('SlidingFloor', localAvatar.doId)

    def _DistCogdoCraneObject__hitGoon(self, entry):
        if self.state == 'Dropped' or self.state == 'LocalDropped':
    ...
    def _DistCogdoCraneObject__hitDropPlane(self, entry):
    ...
    def _DistCogdoCraneObject__watchDrift(self, task):
