...
class CogdoMazeGame(DirectObject):
    ...
    def unload(self):
        ...
        self._CogdoMazeGame__stopUpdateTask()
        self.ignoreAll()
        self.maze.destroy()
        del self.maze
        self._exit.destroy()
    ...
    def start(self):
        ...
        self._CogdoMazeGame__startUpdateTask()
        for player in self.players:
    ...
    def _CogdoMazeGame__startUpdateTask(self):
        self._CogdoMazeGame__stopUpdateTask()
        taskMgr.add(self._CogdoMazeGame__updateTask,
                    CogdoMazeGame.UpdateTaskName, 45)

    def _CogdoMazeGame__stopUpdateTask(self):
        taskMgr.remove(CogdoMazeGame.UpdateTaskName)
        taskMgr.remove('loopSecondQuakeSound')

    def _CogdoMazeGame__updateTask(self, task):
        ...
        self._CogdoMazeGame__updateGags()
        if Globals.QuakeSfxEnabled:
            self._CogdoMazeGame__updateQuakeSound()
    ...
    def _CogdoMazeGame__updateGags(self):
        remove = []
    ...
    def handleLocalToonMeetsSuit(self, suitType, suitNum):
        if self.localPlayer.state == 'Normal' and not (
                self.localPlayer.invulnerable):
    ...
    def handleLocalToonMeetsGagPickup(self, collEntry):
        if self.localPlayer.equippedGag is not None:
            return None
    ...
    def handleLocalToonEntersDoor(self, door):
        ...
        elif localToonId not in self.toonsThatRevealedDoor:
