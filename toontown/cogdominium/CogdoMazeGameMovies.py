...
class CogdoMazeGameIntro(CogdoGameMovie):
    ...
    def load(self):
        ...
        self.cogHead.setPosHprScale(-0.72999999999999998, 0, -1.46, 180, 0, 0,
                                    0.14000000000000001, 0.14000000000000001,
                                    0.14000000000000001)
        self.cogHead.reparentTo(hidden)
    ...
    def _updateTask(self, task):
        ...
        if self._state == 1:
            self._camHelperNode.setPos(self._camTarget.getPos() +
                                       self._camOffset)
            camera.setPos(self._camHelperNode, 0, self._camDistance, 0)
    ...
    def load(self):
        ...
        if not self._exit.hasPlayer(self._localPlayer):
            ...
            self._ival.append(
                Sequence(
                    Func(loseSfx.play),
                    Func(self._localPlayer.toon.setAnimState, 'Sad')))
