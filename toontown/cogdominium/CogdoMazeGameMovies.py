...
class CogdoMazeGameIntro(CogdoGameMovie):
    ...
    def makeSuit(self, suitType):
        ...
        suit.generateSuit()
        suit.setScale(1, 1, 2)
        suit.setPos(0, 0, -4.4000000000000004)
    ...
    def load(self):
        ...
        self.cogHead.setPosHprScale(-0.72999999999999998, 0, -1.46, 180, 0,
                                    0, 0.14000000000000001, 0.14000000000000001, 0.14000000000000001)
        self.cogHead.reparentTo(hidden)
        ...
        def showExit():
            self._setCamTarget(
                self._exit, 10, offset=Point3(
                    0, 0, 0), angle=Point3(
                    0, 60, 0))
            self._exit.close()
            self._state = 2

        showExitIval = Parallel(camera.posInterval(waitDuration *
                                                   0.5, (10, -
                                                         25, 20), other=self._exit, blendType='easeInOut'), Sequence(Wait(waitDuration *
                                                                                                                          0.25), Func(bossSuit.play, 'effort'), camera.hprInterval(waitDuration *
                                                                                                                                                                                   0.25, (30, -
                                                                                                                                                                                          30, 0), blendType='easeInOut'), Func(self._exit.close), Wait(waitDuration *
                                                                                                                                                                                                                                                       0.5)))
    ...
    def _setCamTarget(
        self, targetNP, distance, offset=Point3(
            0, 0, 0), angle=Point3(
            0, 0, 0)):
        camera.wrtReparentTo(render)
    ...
    def _updateTask(self, task):
        ...
        if self._state == 1:
            self._camHelperNode.setPos(
                self._camTarget.getPos() + self._camOffset)
            camera.setPos(self._camHelperNode, 0, self._camDistance, 0)
            ...
            camera.setY(camera, 0.80000000000000004 * dt)
        ...
        if not self._exit.hasPlayer(self._localPlayer):
            ...
            self._ival.append(
                Sequence(
                    Func(
                        loseSfx.play), Func(
                        self._localPlayer.toon.setAnimState, 'Sad')))
