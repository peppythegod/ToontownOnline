...
class CogdoFlyingLocalPlayer(CogdoFlyingPlayer):
    ...
    def __init__(self, toon, game, level, guiMgr):
        ...
        self.defaultTransitions = {
            ...
            'FreeFly': [
                'Inactive', 'OutOfTime', 'Death', 'FlyingUp', 'Running',
                'HitWhileFlying', 'InWhirlwind'
            ],
            'FlyingUp': [
                'Inactive', 'OutOfTime', 'Death', 'FreeFly', 'Running',
                'HitWhileFlying', 'InWhirlwind'
            ],
            'InWhirlwind':
            ['Inactive', 'OutOfTime', 'Death', 'FreeFly', 'HitWhileFlying'],
            'HitWhileFlying':
            ['Inactive', 'OutOfTime', 'Death', 'FreeFly', 'InWhirlwind'],
            'Death': ['Inactive', 'OutOfTime', 'Spawn'],
            'Running': [
                'Inactive', 'OutOfTime', 'FreeFly', 'FlyingUp', 'Refuel',
                'WaitingForWin', 'HitWhileRunning'
            ],
            'HitWhileRunning':
            ['Inactive', 'OutOfTime', 'Death', 'Running', 'FreeFly'],
        ...
        self._cameraMgr = CogdoFlyingCameraManager(camera, render, self,
                                                   self._level)
    ...
    def initOrthoWalker(self):
        orthoDrive = OrthoDrive(
            9.7780000000000005, maxFrameMove=0.5, wantSound=True)
    ...
    def initLocalPlayerIntervals(self):
            ...
            Parallel(
                Parallel(
                    ...
                    self.toon.posInterval(
                        0.5, Vec3(0, 0, -25), other=self.toon)),
        ...
        self.waitingForWinSeq = Sequence(
            ...
            Wait(2.0),
            Func(self.game.distGame.d_sendRequestAction,
                 Globals.AI.GameActions.LandOnWinPlatform, 0),
        ...
        self.winInterval = Sequence(
            ...
            Wait(4.0),
            Func(self.game.distGame.d_sendRequestAction,
                 Globals.AI.GameActions.WinStateFinished, 0),
    ...
    def initCollisions(self):
        ...
        self.accept('Flyer.cRayNode-enter-%s' % floorColl,
                    self._CogdoFlyingLocalPlayer__handleRayCollisionEnterFloor)
        self.accept('Flyer.cRayNode-again-%s' % floorColl,
    ...
    def updateToonFlyingState(self, dt):
        ...
        if leftPressed and not rightPressed:
            self.toon.setH(self.toon,
                           Globals.Gameplay.ToonTurning['turningSpeed'] * dt)
            ...
            self.toon.setH(
                self.toon,
                -1.0 * Globals.Gameplay.ToonTurning['turningSpeed'] * dt)
    ...
    def updateControlVelocity(self, dt):
        ...
        if leftPressed:
            self.controlVelocity[
                0] -= Globals.Gameplay.ToonAcceleration['turning'] * dt
        ...
        if upPressed:
            self.controlVelocity[
                1] += Globals.Gameplay.ToonAcceleration['forward'] * dt

        if downPressed:
            self.controlVelocity[
                2] -= Globals.Gameplay.ToonAcceleration['activeDropDown'] * dt
            self.controlVelocity[
                1] -= Globals.Gameplay.ToonAcceleration['activeDropBack'] * dt

        if jumpPressed and self.isFuelLeft():
            self.controlVelocity[
                2] += Globals.Gameplay.ToonAcceleration['boostUp'] * dt
        ...
        if (not leftPressed or not rightPressed) and self.controlVelocity[
            ...
            x = self.dampenVelocityVal(self.controlVelocity[0], 'turning',
        ...
        if (not upPressed or not downPressed) and self.controlVelocity[
                1] > maxVal or self.controlVelocity[1] < minVal:
            y = self.dampenVelocityVal(self.controlVelocity[1], 'backward',
                                       'forward', minVal, maxVal, dt)
        ...
        if self.controlVelocity[2] > minVal:
            if (not self._inputMgr.arrowKeys.jumpPressed()
                ...
                self.controlVelocity[
                    2] -= Globals.Gameplay.ToonAcceleration['fall'] * dt
        ...
        minVal = -Globals.Gameplay.ToonVelMax['turning']
        maxVal = Globals.Gameplay.ToonVelMax['turning']
        self.controlVelocity[0] = clamp(self.controlVelocity[0], minVal,
                                        maxVal)
        minVal = -Globals.Gameplay.ToonVelMax['backward']
        maxVal = Globals.Gameplay.ToonVelMax['forward']
        self.controlVelocity[1] = clamp(self.controlVelocity[1], minVal,
                                        maxVal)
        if self.isFuelLeft():
            minVal = -Globals.Gameplay.ToonVelMax['fall']
        else:
            minVal = -Globals.Gameplay.ToonVelMax['fallNoFuel']
        maxVal = Globals.Gameplay.ToonVelMax['boost']
        self.controlVelocity[2] = clamp(self.controlVelocity[2], minVal,
                                        maxVal)
    ...
    def updateFanVelocity(self, dt):
        ...
        for fan in self.activeFans:
            ...
            if Globals.Gameplay.UseVariableFanPower:
                ...
                power = math.fabs(distance / fanHeight -
                                  1.0) * powerRange + min
    ...
    def dampenVelocityVal(self, velocityVal, typeNeg, typePos, minVal, maxVal,
                          dt):
        if velocityVal > 0.0:
    ...
    def updateToonPos(self, dt):
        ...
        toonPos = Vec3(
            clamp(toonPos[0], self._levelBounds[0][0],
                  self._levelBounds[0][1]),
            clamp(toonPos[1], self._levelBounds[1][0],
                  self._levelBounds[1][1]),
            clamp(toonPos[2], self._levelBounds[2][0],
                  self._levelBounds[2][1]))
        ...
        if self.propState != propState:
            ...
            if self.propState == CogdoFlyingLocalPlayer.PropStates.Normal:
                ...
                self._loopPropellerSfx(
                    playRate=0.69999999999999996, volume=0.80000000000000004)
    ...
    def enterInactive(self):
        CogdoFlyingLocalPlayer.notify.info(
            "enter%s: '%s' -> '%s'" % (self.newState, self.oldState,
                                       self.newState))
    ...
    def exitInactive(self):
        CogdoFlyingLocalPlayer.notify.debug(
            "exit%s: '%s' -> '%s'" % (self.oldState, self.oldState,
                                      self.newState))
    ...
    def startHitByWhirlwindInterval(self):
        def spinPlayer(t):
            ...
            self.toon.setPos(
                ...
                math.sin(angle) * 2, self.toon.getZ())
