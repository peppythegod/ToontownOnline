...
class CogdoFlyingLocalPlayer(CogdoFlyingPlayer):
    ...
    def __init__(self, toon, game, level, guiMgr):
        ...
        self._cameraMgr = CogdoFlyingCameraManager(
            camera, render, self, self._level)
    ...
    def initLocalPlayerIntervals(self):
        self.coolDownAfterHitInterval = Sequence(
            Wait(
                Globals.Gameplay.HitCooldownTime), Func(
                self.setEnemyHitting, False), name='coolDownAfterHitInterval-%i' %
            self.toon.doId)
        self.deathInterval = Sequence(
            Func(
                self.resetVelocities), Parallel(
                Parallel(
                    Func(
                        self._deathSfx.play), LerpHprInterval(
                        self.toon, 1.0, Vec3(
                            720, 0, 0)), LerpFunctionInterval(
                                self.toon.setScale, fromData=1.0, toData=0.10000000000000001, duration=1.0), self.toon.posInterval(
                                    0.5, Vec3(
                                        0, 0, -25), other=self.toon)), Sequence(
                                            Wait(0.5), Func(
                                                base.transitions.irisOut))), Func(
                                                    self.toon.stash), Wait(1.0), Func(
                                                        self.toonSpawnFunc), name='%s.deathInterval' %
            self.__class__.__name__)
        self.outOfTimeInterval = Sequence(
            Func(
                messenger.send, CogdoFlyingLocalPlayer.PlayWaitingMusicEventName), Func(
                self._loseSfx.play), Func(
                base.transitions.irisOut), Wait(1.0), Func(
                    self.resetVelocities), Func(
                        self._guiMgr.setMessage, '', transition=None), Func(
                            self.toon.stash), Func(
                                self.toonSpawnFunc), name='%s.outOfTimeInterval' %
            self.__class__.__name__)
        self.spawnInterval = Sequence(
            Func(
                self.resetToonFunc), Func(
                self._cameraMgr.update, 0.0), Func(
                self._level.update), Func(
                    self.toon.cnode.broadcastPosHprFull), Func(
                        base.transitions.irisIn), Wait(0.5), Func(
                            self.toon.setAnimState, 'TeleportIn'), Func(
                                self.toon.unstash), Wait(1.5), Func(
                                    self.requestPostSpawnState), name='%s.spawnInterval' %
            self.__class__.__name__)
        self.waitingForWinInterval = Sequence(
            Func(
                self._guiMgr.setMessage,
                TTLocalizer.CogdoFlyingGameWaiting %
                '.'),
            Wait(1.5),
            Func(
                self._guiMgr.setMessage,
        ...
        self.waitingForWinSeq = Sequence(
            ...
            Func(
                ...
                0),
            Func(
                self.playWaitingForWinInterval),
            name='%s.waitingForWinSeq' %
            self.__class__.__name__)
        ...
        self.introGuiSeq = Sequence(
            ...
            Parallel(
                ...
                Sequence(
                    Wait(1.0),
                    Func(
                        self._guiMgr.presentProgressGui))),
            Wait(5.0),
            Func(
                self._guiMgr.setMessage,
                TTLocalizer.CogdoFlyingGamePickUpAPropeller),
            name='%s.introGuiSeq' %
            self.__class__.__name__)
    ...
    def setWaitingForWinState(self):
        if self.didTimeRunOut:
            ...
            self._guiMgr.setMessage(
                TTLocalizer.CogdoFlyingGameOutOfTime,
                transition='blink')
        else:
            self._winSfx.play()
    ...
    def initCollisions(self):
        ...
        self.accept(
            'Flyer.cHeadCollSphere-enter-%s' %
            ceilingColl,
            self._CogdoFlyingLocalPlayer__handleHeadCollisionIntoCeiling)
        self.accept(
            'Flyer.cHeadCollSphere-exit-%s' %
            ceilingColl,
            self._CogdoFlyingLocalPlayer__handleHeadCollisionExitCeiling)
        self.accept(
            'Flyer.cFloorEventSphere-exit-%s' %
            floorColl,
            self._CogdoFlyingLocalPlayer__handleEventCollisionExitFloor)
        self.accept(
            'Flyer.cRayNode-enter-%s' %
            floorColl,
            self._CogdoFlyingLocalPlayer__handleRayCollisionEnterFloor)
        self.accept(
            'Flyer.cRayNode-again-%s' %
            floorColl,
            self._CogdoFlyingLocalPlayer__handleRayCollisionAgainFloor)
    ...
    def _CogdoFlyingLocalPlayer__handleHeadCollisionIntoCeiling(
            self, collEntry):
    ...
    def _CogdoFlyingLocalPlayer__handleHeadCollisionExitCeiling(
            self, collEntry):
        self.isHeadInCeiling = False
        self.surfacePoint = None
    ...
    def _CogdoFlyingLocalPlayer__handleRayCollisionEnterFloor(self, collEntry):
        ...
        if toonPos.getZ() < collPos.getZ() + Globals.Gameplay.RayPlatformCollisionThreshold:
            if not (self.isToonOnFloor) and self.state in [
                'FreeFly',
                    'FlyingUp']:
                self.landOnPlatform(collEntry)

    def _CogdoFlyingLocalPlayer__handleRayCollisionAgainFloor(self, collEntry):
        fromNodePath = collEntry.getFromNodePath()
        intoNodePath = collEntry.getIntoNodePath()
        intoName = intoNodePath.getName()
        fromName = fromNodePath.getName()
    ...
    def _CogdoFlyingLocalPlayer__handleEventCollisionExitFloor(
            self, collEntry):
        fromNodePath = collEntry.getFromNodePath()
        intoNodePath = collEntry.getIntoNodePath()
        intoName = intoNodePath.getName()
    ...
    def updateToonFlyingState(self, dt):
        ...
        if leftPressed and not rightPressed:
            self.toon.setH(
                self.toon,
                Globals.Gameplay.ToonTurning['turningSpeed'] *
                dt)
    ...
    def updateControlVelocity(self, dt):
        ...
        if (not leftPressed or not rightPressed) and self.controlVelocity[
                0] > maxVal or self.controlVelocity[0] < minVal:
        ...
        if (
                not upPressed or not downPressed) and self.controlVelocity[1] > maxVal or self.controlVelocity[1] < minVal:
        ...
        if self.controlVelocity[2] > minVal:
            if (not self._inputMgr.arrowKeys.jumpPressed()
                    or not self.isFuelLeft()) and not (self.isToonOnFloor):
        ...
        minVal = -Globals.Gameplay.ToonVelMax['turning']
        maxVal = Globals.Gameplay.ToonVelMax['turning']
        self.controlVelocity[0] = clamp(
            self.controlVelocity[0], minVal, maxVal)
        minVal = -Globals.Gameplay.ToonVelMax['backward']
        maxVal = Globals.Gameplay.ToonVelMax['forward']
        self.controlVelocity[1] = clamp(
            self.controlVelocity[1], minVal, maxVal)
        if self.isFuelLeft():
            minVal = -Globals.Gameplay.ToonVelMax['fall']
        else:
            minVal = -Globals.Gameplay.ToonVelMax['fallNoFuel']
        maxVal = Globals.Gameplay.ToonVelMax['boost']
        self.controlVelocity[2] = clamp(
            self.controlVelocity[2], minVal, maxVal)
    ...
    def updateFanVelocity(self, dt):
        ...
        for fan in self.activeFans:
            ...
            if Globals.Gameplay.UseVariableFanPower:
                ...
                power = math.fabs(
                    distance / fanHeight - 1.0) * powerRange + min
    ...
    def updateToonPos(self, dt):
        ...
        if not self.hasPickedUpFirstPropeller:
            if toonWorldY > -7.5999999999999996:
                self.toon.setY(-7.5999999999999996)
        ...
        if Globals.Dev.DisableDeath:
            pass
        1
        if toonPos[2] < 0.0 and self.state in [
            'FreeFly',
                'FlyingUp'] and self.allowFuelDeath():
            self.postSpawnState = 'Running'
        ...
        toonPos = Vec3(
            clamp(
                toonPos[0], self._levelBounds[0][0], self._levelBounds[0][1]), clamp(
                toonPos[1], self._levelBounds[1][0], self._levelBounds[1][1]), clamp(
                toonPos[2], self._levelBounds[2][0], self._levelBounds[2][1]))
    ...
    def bladeLost(self):
        ...
        self._bladeBreakSfx.play(volume=0.34999999999999998)
    ...
    def setPropellerState(self, propState):
        ...
        if self.propState != propState:
            ...
            if self.propState == CogdoFlyingLocalPlayer.PropStates.Normal:
                ...
                self._guiMgr.setPropellerSpinRate(
                    Globals.Gameplay.NormalPropSpeed)
                self._loopPropellerSfx(
                    playRate=0.69999999999999996,
                ...
                self._guiMgr.setPropellerSpinRate(
                    Globals.Gameplay.OverdrivePropSpeed)
                self._loopPropellerSfx(playRate=1.1000000000000001)
            elif self.propState == CogdoFlyingLocalPlayer.PropStates.Off:
                self.propellerSpinLerp.pause()
                self._propellerSfx.stop()

    def enterInactive(self):
        CogdoFlyingLocalPlayer.notify.info(
            "enter%s: '%s' -> '%s'" %
    ...
    def startHitByWhirlwindInterval(self):
        ...
        def spinPlayer(t):
            ...
            self.toon.setPos(
                self.activeWhirlwind.model.getX(
                    self.game.level.root) + math.cos(angle) * 2,
                self.activeWhirlwind.model.getY(
                    self.game.level.root) + math.sin(angle) * 2,
                self.toon.getZ())

        def movePlayerBack(t):
            self.toon.setY(
                self.activeWhirlwind.model.getY(
                    self.game.level.root) -
                t *
                Globals.Gameplay.WhirlwindMoveBackDist)

        self.startEventIval(
            Sequence(
                Func(
                    self._cameraMgr.freeze),
                Func(
                    self.activeWhirlwind.disable),
                LerpFunc(
                    spinPlayer,
                    fromData=0.0,
                    toData=1.0,
                ...
                LerpFunc(
                    ...
                    blendType='easeOut'),
                Func(
                    self.activeWhirlwind.enable),
                Func(
                    self._cameraMgr.unfreeze),
                Func(
                    self.request,
                    'FreeFly'),
                name='spinPlayerIval-%i' %
                self.toon.doId))
    ...
    def handleEnterMemo(self, gatherable):
        ...
        if self.score == 1:
            ...
            self._guiMgr.setTemporaryMessage(
                TTLocalizer.CogdoFlyingGameMemoIntro, 4.0)
    ...
    def handleEnterPropeller(self, gatherable):
        if self.fuel < 1.0:
            if not self.hasPickedUpFirstPropeller:
                ...
                self.setPropellerState(
                    CogdoFlyingLocalPlayer.PropStates.Normal)

            self.setFuel(1.0)
            self._guiMgr.update()
            self._refuelSfx.play()
            self._refuelSpinSfx.play(volume=0.14999999999999999)
