...
class CogdoMazeCameraManager:
    ...
    savedCamZ = 0.0
    savedCamX = 0.0
    shakeStrengthThreshold = 2.7999999999999998
    savedRumbleTimer = 0.0
    rumbleFreq = 0.10000000000000001
    rumbleSmall = 0.050000000000000003
    rumbleBig = 0.16
    ...
    def updateToonShake(self, dt):
        if self.toonIsShaking:
            newHeight = self.toon.getZ() + self.toonJumpDir * dt * \
                self.savedShakeStrength * self.toonJumpSpeed
    ...
    def updateRumble(self, dt):
        ...
        elif self.rumbleOnTimer > self.savedRumbleTimer + self.rumbleFreq:
