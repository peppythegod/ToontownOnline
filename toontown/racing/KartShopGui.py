...
class KartShopGuiMgr(object, DirectObject.DirectObject):
    ...
    class BuyAccessoryDlg(DirectFrame):
        ...
        def _BuyAccessoryDlg__handleAccessoryTypeChange(self, type):
    ...
    def _KartShopGuiMgr__destroyDialog(self):
    ...
    def _KartShopGuiMgr__removeDialog(self, dlgName=None):
        if self.dialog is not None:
            if dlgName is not None:
                for d in self.dialogStack:
                    if d == dlgName:
                        self.dialogStack.remove(d)
                        continue

    def _KartShopGuiMgr__popDialog(self):
        if self.dialog is not None:
            if dlgName is not None:
                for d in self.dialogStack:
                    if d == dlgName:
                ...
                if self.dialogStack:
                    self._KartShopGuiMgr__doDialog(self.dialogStack[-1])

    def _KartShopGuiMgr__doLastMenu(self):
        self._KartShopGuiMgr__doDialog(self.lastMenu)

    def _KartShopGuiMgr__doDialog(self, dialogType):
        self._KartShopGuiMgr__destroyDialog()
    ...
    def _KartShopGuiMgr__handleMainMenuDlg(self, exitType, args=[]):
        self.notify.debug(
    ...
    def _KartShopGuiMgr__handleTeaserPanelDlg(self):
        self._KartShopGuiMgr__doDialog(MENUS.TeaserPanel)

    def _KartShopGuiMgr__handleBuyKartDlg(self, exitType, args=[]):
        self.notify.debug(
        ...
        if exitType == BK_OPTIONS.Cancel:
            ...
            if base.localAvatar.hasKart():
                self._KartShopGuiMgr__doDialog(MENUS.ReturnKart)
            else:
                self._KartShopGuiMgr__doDialog(MENUS.ConfirmBuyKart)

    def _KartShopGuiMgr__handleBuyAccessoryDlg(self, exitType, args=[]):
        self.notify.debug(
            '__handleBuyKartDlg: Handling BuyKart Dialog Selection.')
        if exitType == BA_OPTIONS.Cancel:
            ...
            self.accID = exitType
            self._KartShopGuiMgr__doDialog(MENUS.ConfirmBuyAccessory)

    def _KartShopGuiMgr__handleReturnKartDlg(self, exitType, args=[]):
        self.notify.debug(
    ...
    def _KartShopGuiMgr__handleConfirmBuyKartDlg(self, exitType, args=[]):
        ...
        if __name__ == '__main__':
            ...
            class Main(DirectObject.DirectObject):
                ...
                def _Main__handleBuyAccessory(self, accID=-1):
                    requestAddOwnedAccessory(self, accID)

                def _Main__handleBuyKart(self, kartID=-1):
                    pass
