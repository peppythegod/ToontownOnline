...
class DistributedVehicle(DistributedSmoothNode.DistributedSmoothNode,
    ...
    turnRatio = (1.0 / 0.025000000000000001) * math.sqrt(cheatFactor)
    accelerationMult = 35
    accelerationBase = 20
    if proRacer:
        accelerationMult = 35
    ...
    getKartFromAvId = classmethod(getKartFromAvId)
    ...
    def __init__(self, cr):
        ...
        self.wheelMinTurn = 0.14999999999999999
        self.speedMaxTurn = 90
        self.speedMinTurn = 300
        self.wheelTurnTime = 0.59999999999999998
        self.wheelReturnTime = 0.29999999999999999
        self.wheelFightTime = 0.20000000000000001
        self.wheelPosition = 0.0
        self.outPutCounter = 0
        self.proCameraHeading = 0
        self.proCameraDummyNode = None
        self.cameraArmNode = None
        self.armSwingSpeedPerp = 0.20000000000000001
        self.armSwingSpeedPara = 0.69999999999999996
        ...
        for piece in range(self.numPieChunks):
            self.pieSlideSpeed.append(randFloat(0.0, 0.20000000000000001))
        ...
        self._DistributedVehicle__loadTextures()

    def _DistributedVehicle__loadTextures(self):
        self.pieSplatter = loader.loadModel(
            'phase_6/models/karting/pie_splat_1.bam')
    ...
    def announceGenerate(self):
        ...
        if self.localVehicle:
            self.setupPhysics()
            self.setupParticles()
            self._DistributedVehicle__enableCollisions()
    ...
    def setupPhysics(self):
        self._DistributedVehicle__setupCollisions()
        ...
        windResistance = LinearFrictionForce(0.20000000000000001)
    ...
    def disable(self):
        ...
        self.stopSmooth()
        if self.localVehicle:
            self._DistributedVehicle__disableCollisions()
            self._DistributedVehicle__undoCollisions()
    ...
    def _DistributedVehicle__updateWallCollision(self, entry=None):
        vol = self.curSpeed / 160
        for wallHit in self.wallHitsSfx:
            wallHit.setVolume(vol)

    def _DistributedVehicle__wallCollisionStart(self, entry):
        self.entry = entry
        ...
        self._DistributedVehicle__updateWallCollision()
    ...
    def _DistributedVehicle__wallCollisionStop(self, entry=None):
        self.hittingWall = 0
        if self.wallCollideTrack:
            self.wallCollideTrack.pause()

    def _DistributedVehicle__setupCollisions(self):
        ...
        self.wallHandler.setDynamicFrictionCoef(0.10000000000000001)
        ...
        self.lifter.setGravity(32.173999999999999 * 3.0)
    ...
    def _DistributedVehicle__undoCollisions(self):
        base.cTrav.removeCollider(self.cRayNodePath)
        base.localAvatar.collisionsOn()

    def _DistributedVehicle__groundCollision(self, entry):
    ...
    def _DistributedVehicle__enableCollisions(self):
        self.cQueue = []
    ...
    def _DistributedVehicle__disableCollisions(self):
        self.ignore('imIn-startLine')
        self.ignore('imIn-quarterLine')
        self.ignore('imIn-midLine')

    def _DistributedVehicle__handleCollisionSphereEnter(self, collEntry=None):
    ...
    def enterControlled(self, avId):
        ...
        if self.localVehicle:
            ...
            self._DistributedVehicle__enableControlInterface()
            self._DistributedVehicle__createPieWindshield()
            ...
            self.updateNonLocalTask = taskMgr.add(
                self._DistributedVehicle__updateNonlocalVehicle, taskName)

    def exitControlled(self):
        if self.localVehicle:
            self.stopPosHprBroadcast()
            self._DistributedVehicle__disableControlInterface()
    ...
    def _DistributedVehicle__createPieWindshield(self):
        self.piePieces = []
        for piece in range(self.numPieChunks):
    ...
    def showPieces(self):
        ...
        for piece in range(self.numPieChunks):
            self.pieSlideSpeed[piece] = randFloat(0.0, 0.20000000000000001)
    ...
    def _DistributedVehicle__countPies(self):
        if self.pieCount > 0:
            taskMgr.doMethodLater(
                1, self._DistributedVehicle__countPies, ' ', extraArgs=[])
            self.pieCount -= 1
    ...
    def _DistributedVehicle__slidePies(self, optional=None):
    ...
    def _DistributedVehicle__enableControlInterface(self):
        if self.canRace:
            self.enableControls()

        taskMgr.remove('watchVehicleControls')
        taskMgr.add(
            self._DistributedVehicle__watchControls,
            'watchVehicleControls',
            priority=25)
        if not self.speedometer:
            cm = CardMaker('speed')
            cm.setFrame(-0.5, 0.5, -0.5, 0.5)
            self.speedometerImages = aspect2d.attachNewNode(
                'SpeedometerImages')
            self.speedometerImages.setTransparency(True)
            self.speedometerImages.setPos(1.24, 0.0, -0.97999999999999998)
            ...
            self.speedometerImages.attachNewNode(
                m.find('**/*gauge').node()).setColorScale(1, 1, 1, 1)
            ...
            c.setW(1.0)
            self.speedometerImages.attachNewNode(
                m.find('**/*face').node()).setColorScale(c)
            c = (Vec4(2, 2, 2, 2) - accColor) / 2.0
            c = (bodyColor + Vec4(1, 1, 1, 1)) / 2.0
            c.setW(1.0)
            self.speedometerImages.attachNewNode(
                m.find('**/*tics').node()).setColorScale(c)
            c = (bodyColor + Vec4(1, 1, 1, 1)) / 2.0
            c.setW(1.0)
            self.speedometerImages.attachNewNode(
    ...
    def _DistributedVehicle__disableControlInterface(self):
        self.hideSpeedometer()
        self.disableControls()
        self.stopSmooth()
        taskMgr.remove('watchVehicleControls')

    def _DistributedVehicle__deleteControlInterface(self):
        if self.speedometer:
    ...
    def interruptTurbo(self):
        self._DistributedVehicle__stopTurbo()
        if self.cameraTrack:
    ...
    def _DistributedVehicle__controlPressed(self):
        self._DistributedVehicle__throwGag()

    def _DistributedVehicle__controlReleased(self):
        pass

    def _DistributedVehicle__upArrow(self, pressed):
        if pressed:
            self.arrowVert = 1
        elif self.arrowVert > 0:
            self.arrowVert = 0

    def _DistributedVehicle__downArrow(self, pressed):
        if pressed:
            self.arrowVert = -1
        elif self.arrowVert < 0:
            self.arrowVert = 0

    def _DistributedVehicle__rightArrow(self, pressed):
        if pressed:
            self.arrowHorz = 1
            self.rightHeld += 1
    ...
    def _DistributedVehicle__leftArrow(self, pressed):
        if pressed:
            self.arrowHorz = -1
            self.leftHeld += 1
        elif self.arrowHorz < 0:
    ...
    def _DistributedVehicle__updateNonlocalVehicle(self, task=None):
        ...
        self._DistributedVehicle__animate()
        return Task.cont

    def _DistributedVehicle__animate(self):
        speed = self.curSpeed
        ...
        if self.offGround == 0:
            modifier = self.surfaceModifiers[self.groundType]['shake'] * (
                speed / 50.0)
        else:
            modifier = 1
        roll = self.leanAmount * 1.5
        roll += (random.random() * 2 - 1) * modifier
        pitch = self.acceleration * -0.0050000000000000001
        pitch += (random.random() * 2 - 1) * modifier
        self.rollSuspension(roll)
        self.pitchSuspension(pitch)
        return Task.cont

    def _DistributedVehicle__clampPosition(self):
        retVal = False
        if self.getX() < -3276:
    ...
    def amIClampingPosition(self):
        if not self.getX() == -3276 and self.getX() == 3276 and self.getY() == - \
                3276 and self.getY() == 3276 and self.getZ() == -3276:
            pass
        return self.getZ() == 3276

    def _DistributedVehicle__computeTurnRatio(self, speed):
    ...
    def _DistributedVehicle__updateWheelPos(self, dt, curSpeed):
        ...
        maxWheelDeflection = self.wheelMaxTurn * ratioIntoMax + \
            self.wheelMinTurn * (1.0 - ratioIntoMax)
    ...
    def _DistributedVehicle__watchControls(self, task):
        dt = globalClock.getDt()
        ...
        if self.proRacer:
            self._DistributedVehicle__computeTurnRatio(curSpeed)
        ...
        if self.stopped:
            ...
            if self.proRacer:
                if self.skidding:
                    self.acceleration = self.arrowVert * \
                        self.accelerationMult * self.cheatFactor * 0.5
        ...
        for i in range(numFrames):
            ...
            driftVal = abs(self.leanAmount) * 16 / \
                self.cheatFactor + 15 / self.cheatFactor
            curVelocity = Vec3(
        ...
        if self.proRacer:
            ...
            if curSpeed < self.speedMaxTurn:
                ...
                speedProporation = (
                    curSpeed - self.speedMaxTurn) / self.speedMinTurn
            ...
            if curSpeed > self.speedMinTurn:
                ...
                cameraTightener = 1.0 + \
                    (curSpeed / self.speedMinTurn) * (self.cameraTightener - 1.0)
            swingSpeedRatio = speedDif / 90
            swingSpeed = self.armSwingSpeedPerp * swingSpeedRatio + \
        ...
        if self._DistributedVehicle__clampPosition():
    ...
    def enableControls(self):
        self.canRace = True
        self.accept('control', self._DistributedVehicle__controlPressed)
        self.accept('control-up', self._DistributedVehicle__controlReleased)
    ...
    def flattenMe(self):
        self.geom[0].setScale(3, 3, 0.10000000000000001)

    def unFlattenMe(self):
        self.geom[0].setScale(2)

    def startSkid(self):
        if not self.skidding:
            self.skidding = True
