...
class CatalogScreen(DirectFrame):
    ...
    def show(self):
        self.accept('CatalogItemPurchaseRequest',
                    self._CatalogScreen__handlePurchaseRequest)
        self.accept('CatalogItemGiftPurchaseRequest',
                    self._CatalogScreen__handleGiftPurchaseRequest)
        self.accept(
            localAvatar.uniqueName('moneyChange'),
            self._CatalogScreen__moneyChange)
        self.accept(
            localAvatar.uniqueName('bankMoneyChange'),
            self._CatalogScreen__bankMoneyChange)
        self.accept(
            localAvatar.uniqueName('emblemsChange'),
            self._CatalogScreen__emblemChange)
    ...
    def showPageItems(self):
        ...
        randGen.seed(base.localAvatar.catalogScheduleCurrentWeek +
                     (self.pageIndex << 8) + (newOrBackOrLoyalty << 16))
    ...
    def load(self, guiItems, guiButton, guiBack):
        ...
        maxSeries = ToontownGlobals.CatalogNumWeeks / \
    ...
    def reload(self):
        for panel in self.panelList + self.backPanelList + \
                self.loyaltyPanelList + self.emblemPanelList:
            panel.destroy()
    ...
    def _CatalogScreen__handlePurchaseRequest(self, item):
        item.requestPurchase(self['phone'],
                             self._CatalogScreen__handlePurchaseResponse)
        taskMgr.remove('clarabelleAskAnythingElse')
        ...
        if hasManager:
            for avatarId in base.cr.playerFriendsManager.getAllOnlinePlayerAvatars():
                ...
                playerId = base.cr.playerFriendsManager.findPlayerIdFromAvId(
                    avatarId)
                playerInfo = base.cr.playerFriendsManager.getFriendInfo(
                    playerId)
                ...
                if handle and not self.checkFamily(avatarId):
                    if hasattr(handle, 'getName'):
                        ...
                        self.notify.warning(
                            'Bad Handle for getName in makeFFlist')
                hasattr(handle, 'getName')
