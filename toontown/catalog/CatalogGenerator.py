...
class CatalogGenerator:
    ...
    def generateWeeklyCatalog(self, avatar, week, monthlyCatalog):
        ...
        if time.time() < 1096617600.0:
            ...
            if not hasPetTrick(weeklyCatalog) and not hasPetTrick(
                    avatar.weeklyCatalog) and not hasPetTrick(avatar.backCatalog):
    ...
    def _CatalogGenerator__getReleasedItemLists(self, dayNumber, weekStart):
        ...
        testDaysAhead = simbase.config.GetInt(
            'test-server-holiday-days-ahead', 0)
    ...
    def _CatalogGenerator__selectItem(
            self, avatar, item, duplicateItems, saleItem=0):
        chooseCount = 1
    ...
    def _CatalogGenerator__chooseFromList(self, avatar, list, duplicateItems):
        ...
        while item.notOfferedTo(avatar) and item.reachedPurchaseLimit(
                avatar) and item in duplicateItems and item in avatar.backCatalog or item in avatar.weeklyCatalog:
            if len(list) == 0:
                return None
    ...
    def outputSchedule(self, filename):
        ...
        for item in items:
            ...
            line = '"%s"\t"%s"\t"%s"\t%s\t"%s"\t"%s"\t"%s"\t"%s"\t"%s"' % (item.output(
                store=0),
                item.getTypeName(),
                item.getDisplayName(),
                item.getBasePrice(),
    ...
    def _CatalogGenerator__recordScheduleItem(
            self, sched, weekCode, maybeWeekCode, item):
