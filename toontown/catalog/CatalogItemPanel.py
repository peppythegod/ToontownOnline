...
class CatalogItemPanel(DirectFrame):
    ...
    def __init__(self, parent=aspect2d, parentCatalogScreen=None, **kw):
        optiondefs = (
            ('item',
             ...
             DGG.INITOPT),
            ('type',
             CatalogItem.CatalogTypeUnspecified,
             DGG.INITOPT),
            ('relief',
             None,
             None))
    ...
    def load(self):
            ...
            if typeCode == CatalogItemTypes.WALLPAPER_ITEM and typeCode == CatalogItemTypes.FLOORING_ITEM and typeCode == CatalogItemTypes.MOULDING_ITEM and typeCode == CatalogItemTypes.FURNITURE_ITEM and typeCode == CatalogItemTypes.WAINSCOTING_ITEM or typeCode == CatalogItemTypes.TOON_STATUE_ITEM:
                ...
                if self.numItems > 1:
                    ...
                    self.nextVariant = DirectButton(
                        parent=self, relief=None, image=(
                            nextUp, nextDown, nextRollover, nextUp), image3_color=(
                            1, 1, 1, 0.40000000000000002), pos=(
                            0.13, 0, 0), command=self.showNextVariant)
                    self.prevVariant = DirectButton(
                        parent=self, relief=None, image=(
                            prevUp, prevDown, prevRollover, prevUp), image3_color=(
                            1, 1, 1, 0.40000000000000002), pos=(
                            -0.13, 0, 0), command=self.showPrevVariant, state=DGG.DISABLED)
                ...
                (picture, self.ival) = self['item'].getPicture(
                    base.localAvatar)
                if picture:
        ...
        self.priceLabel = DirectLabel(
            parent=self, relief=None, pos=(
                0, 0, -0.29999999999999999), scale=priceScale, text=priceStr, text_fg=(
        ...
        self.soundOnButton = DirectButton(
            parent=self, relief=None, pos=(
                0.20000000000000001, 0, -0.14999999999999999), scale=(
                0.69999999999999996, 1, 0.80000000000000004), text_scale=buyTextScale, text_pos=(
                -0.0050000000000000001, -0.01), image=(
                    upButton, downButton, rolloverButton, upButton), image_color=(
                        0.20000000000000001, 0.5, 0.20000000000000001, 1), image0_color=Vec4(
                            0.40000000000000002, 0.5, 0.40000000000000002, 1), image3_color=Vec4(
                                0.40000000000000002, 0.5, 0.40000000000000002, 0.40000000000000002), command=self.handleSoundOnButton)
        ...
        self.soundOffButton = DirectButton(
            parent=self, relief=None, pos=(
                ...
                -0.0050000000000000001, -0.01), image=(
                    upButton, downButton, rolloverButton, upButton), image_color=(
                        0.20000000000000001, 1.0, 0.20000000000000001, 1), image0_color=Vec4(
                            0.40000000000000002, 1.0, 0.40000000000000002, 1), image3_color=Vec4(
                                0.40000000000000002, 1.0, 0.40000000000000002, 0.40000000000000002), command=self.handleSoundOffButton)
    ...
    def updateBuyButton(self):
        ...
        if orderCount > 0:
            if orderCount > 1:
                auxText = '%d %s' % (
                    orderCount, TTLocalizer.CatalogOnOrderText)
            else:
        ...
        if self['item'].replacesExisting() and self['item'].hasExisting():
            ...
            if emblemPrices:
                ...
                if price and silver and gold:
                    ...
                    self.notify.warning(
                        'is this a completely free item %s?' %
                        self['item'].getName())
                    message = TTLocalizer.CatalogVerifyPurchase % {
                        'item': self['item'].getName(),
                        'price': self['item'].getPrice(self['type'])}
    ...
    def updateGiftButton(self, giftUpdate=0):
        ...
        if numFriends > 0:
            ...
            if self['item'].isGift() <= 0:
                ...
                if (self['item'].forBoysOnly() or avatar.getStyle().getGender(
