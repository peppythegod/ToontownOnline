...
class DistributedNPCPartyPersonAI(DistributedNPCToonBaseAI):
    ...
    def avatarEnter(self):
        ...
        self.acceptOnce(
            ...
            self._DistributedNPCPartyPersonAI__handleUnexpectedExit,
            extraArgs=[avId])
        ...
        if not self.air.partyManager.canBuyParties():
            ...
            self.d_setMovie(avId, flag)
            taskMgr.doMethodLater(30.0, self.sendTimeoutMovie,
        ...
        if wantsToPlan:
            ...
            if av:
                if av.getGameAccess() != ToontownGlobals.AccessFull:
                    ...
                    self.d_setMovie(avId, NPCToons.PARTY_MOVIE_COMPLETE,
                                    [hoodId, zoneId])
    ...
    def _DistributedNPCPartyPersonAI__handleUnexpectedExit(self, avId):
