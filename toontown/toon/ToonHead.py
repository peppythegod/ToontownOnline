...
class ToonHead(Actor.Actor):
    ...
    def setupHead(self, dna, forGui = 0):
        self._ToonHead__height = self.generateToonHead(1, dna, ('1000',), forGui)
    ...
    def loadPumpkin(self, headStyle, lod, copy):
        if hasattr(base, 'launcher'):
            if (not (base.launcher) or base.launcher) and base.launcher.getPhaseComplete(4):
    ...
    def enablePumpkins(self, enable):
        if not hasattr(self, 'pumpkins'):
            if len(self._ToonHead__lods) == 1:
                pLoaded = self.loadPumpkin(self._ToonHead__headStyle[1], None, self._ToonHead__copy)
                if pLoaded:
                    self._ToonHead__fixPumpkin(self._ToonHead__style, None, self._ToonHead__copy)
                
            else:
                for lod in self._ToonHead__lods:
                    pLoaded = self.loadPumpkin(self._ToonHead__headStyle[1], lod, self._ToonHead__copy)
                    if pLoaded:
                        self._ToonHead__fixPumpkin(self._ToonHead__style, lod, self._ToonHead__copy)
                        continue
    ...
    def enableSnowMen(self, enable):
        if not hasattr(self, 'snowMen'):
            if len(self._ToonHead__lods) == 1:
                self.loadSnowMan(self._ToonHead__headStyle[1], None, self._ToonHead__copy)
            else:
                for lod in self._ToonHead__lds:
                    self.loadSnowMan(self._ToonHead__headStyle[1], lod, self._ToonHead__copy)
    ...
    def _ToonHead__fixEyes(self, style, forGui = 0):
        ...
        if self.hasLOD():
            ...
            if self._ToonHead__lod500Eyes.isEmpty():
                ...
                if base.config.GetBool('want-new-anims', 1):
                    if not self.find('**/joint_pupilL*').isEmpty():
                        self._ToonHead__lod500lPupil = self._ToonHead__lod500Eyes.find('**/joint_pupilL*')
                        self._ToonHead__lod500rPupil = self._ToonHead__lod500Eyes.find('**/joint_pupilR*')
                    else:
                        self._ToonHead__lod500lPupil = self._ToonHead__lod500Eyes.find('**/def_left_pupil*')
                        self._ToonHead__lod500rPupil = self._ToonHead__lod500Eyes.find('**/def_right_pupil*')
                else:
                    self._ToonHead__lod500lPupil = self._ToonHead__lod500Eyes.find('**/joint_pupilL*')
                    self._ToonHead__lod500rPupil = self._ToonHead__lod500Eyes.find('**/joint_pupilR*')
            if self._ToonHead__lod250Eyes.isEmpty():
                ...
                if base.config.GetBool('want-new-anims', 1):
                    if not self.find('**/joint_pupilL*').isEmpty():
                        self._ToonHead__lod250lPupil = self._ToonHead__lod250Eyes.find('**/joint_pupilL*')
                        self._ToonHead__lod250rPupil = self._ToonHead__lod250Eyes.find('**/joint_pupilR*')
                    else:
                        self._ToonHead__lod250lPupil = self._ToonHead__lod250Eyes.find('**/def_left_pupil*')
                        self._ToonHead__lod250rPupil = self._ToonHead__lod250Eyes.find('**/def_right_pupil*')
                else:
                    self._ToonHead__lod250lPupil = self._ToonHead__lod250Eyes.find('**/joint_pupilL*')
                    self._ToonHead__lod250rPupil = self._ToonHead__lod250Eyes.find('**/joint_pupilR*')
        else:
    ...
    def setupMuzzles(self, style):
        ...
        if self.hasLOD():
            for lodName in self.getLODNames():
                ...
                hideAddNonEmptyItemToList(surpriseMuzzle, self._ToonHead__surpriseMuzzles)
