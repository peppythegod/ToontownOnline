...
class DistributedNPCClerkAI(DistributedNPCToonBaseAI):
    ...
    def avatarEnter(self):
        ...
        self.acceptOnce(
            ...
            self._DistributedNPCClerkAI__handleUnexpectedExit,
            extraArgs=[avId])
        if self.isBusy():
    ...
    def sendStartMovie(self, avId):
        ...
        self.sendUpdate('setMovie', [
            NPCToons.PURCHASE_MOVIE_START, self.npcId, avId,
    ...
    def sendNoMoneyMovie(self, avId):
        self.busy = avId
        self.sendUpdate('setMovie', [
            NPCToons.PURCHASE_MOVIE_NO_MONEY, self.npcId, avId,
            ClockDelta.globalClockDelta.getRealNetworkTime()
        ])
    ...
    def setInventory(self, blob, newMoney, done):
        ...
        if self.busy != avId:
            if self.busy != 0:
                self.air.writeServerEvent(
                    'suspicious', avId,
                    'DistributedNPCClerkAI.setInventory busy with %s' %
                    self.busy)
                self.notify.warning(
                    'setInventory from unknown avId: %s busy: %s' %
                    (avId, self.busy))
        ...
        if self.air.doId2do.has_key(avId):
            ...
            if av.inventory.validatePurchase(newInventory, currentMoney,
                                             newMoney):
    ...
    def _DistributedNPCClerkAI__handleUnexpectedExit(self, avId):
