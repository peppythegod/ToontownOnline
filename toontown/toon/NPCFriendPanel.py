...
class NPCFriendPanel(DirectFrame):
    ...
    def __init__(self, parent=aspect2d, **kw):
        ...
        self.accept(
            localAvatar.uniqueName('maxNPCFriendsChange'), self.updateLayout)
    ...
    def updateLayout(self):
        ...
        if self.maxNPCFriends == 8:
            ...
            self.notify.error(
                'got wrong max SOS cards %s' % self.maxNPCFriends)
        count = 0
        for i in range(self.maxNPCFriends):
            card = NPCFriendCard(
                parent=self,
...
class NPCFriendCard(DirectFrame):
    ...
    def __init__(self, parent=aspect2dp, rotateCard=False, **kw):
        ...
        if rotateCard:
            self.front.component('image0').configure(
                pos=(0, 0, 0.22), hpr=(0, 0, -90), scale=1.3500000000000001)
            self.back.component('image0').configure(
                hpr=(0, 0, -90),
                scale=(-1.3500000000000001, 1.3500000000000001,
                       1.3500000000000001))
        ...
        for i in range(self.maxRarity):
            label = DirectLabel(
                ...
                pos=(1.1000000000000001 - i * 0.23999999999999999, 0,
                     rarityPosZ))
            label.hide()
    ...
    def update(self, NPCID, count=0, fCallable=0):
        ...
        if NPCID != oldNPCID:
            ...
            (track, level, hp,
             rarity) = NPCToons.getNPCTrackLevelHpRarity(NPCID)
        ...
        if fCallable:
            ...
            self.sosCountInfo.setPos(-0.40000000000000002, 0,
                                     self.sosCountInfoPosZ)
