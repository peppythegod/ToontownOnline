...
class DistributedNPCTailorAI(DistributedNPCToonBaseAI):
    ...
    def avatarEnter(self):
        ...
        self.acceptOnce(
            ...
            self._DistributedNPCTailorAI__handleUnexpectedExit,
            extraArgs=[avId])
    ...
    def isClosetAlmostFull(self, av):
        numClothes = len(av.clothesTopsList) / 4 + len(
            av.clothesBottomsList) / 2
    ...
    def sendTimeoutMovie(self, task):
        ...
        self.timedOut = 1
        self.sendUpdate('setMovie', [
            NPCToons.PURCHASE_MOVIE_TIMEOUT, self.npcId, self.busy,
            ClockDelta.globalClockDelta.getRealNetworkTime()
        ])
    ...
    def sendClearMovie(self, task):
        ...
        self.timedOut = 0
        self.sendUpdate('setMovie', [
            NPCToons.PURCHASE_MOVIE_CLEAR, self.npcId, 0,
            ClockDelta.globalClockDelta.getRealNetworkTime()
        ])
    ...
    def completePurchase(self, avId):
        self.busy = avId
        self.sendUpdate('setMovie', [
            NPCToons.PURCHASE_MOVIE_COMPLETE, self.npcId, avId,
            ClockDelta.globalClockDelta.getRealNetworkTime()
        ])
        self.sendClearMovie(None)
    ...
    def setDNA(self, blob, finished, which):
        ...
        if self.air.doId2do.has_key(avId):
            ...
            if finished == 2 and which > 0:
                if self.air.questManager.removeClothingTicket(
                        av, self) == 1 or self.freeClothes:
                    ...
                    self.air.writeServerEvent(
                        ...
                        '%s|%s|%s' % (self.doId, which,
                                      self.customerDNA.asTuple()))
                else:
                    self.air.writeServerEvent(
                        'suspicious', avId,
    ...
    def _DistributedNPCTailorAI__handleUnexpectedExit(self, avId):
        ...
        if self.customerId == avId:
            ...
            self.notify.warning('invalid customer avId: %s, customerId: %s ' %
