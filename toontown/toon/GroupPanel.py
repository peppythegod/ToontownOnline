...
class GroupPanel(DirectObject.DirectObject):
    ...
    def _GroupPanel__load(self):
        ...
        leaveImageList = (leaveButtonGui.find('**/tt_t_gui_brd_leaveUp'), leaveButtonGui.find('**/tt_t_gui_brd_leaveDown'), leaveButtonGui.find('**/tt_t_gui_brd_leaveHover'), leaveButtonGui.find('**/tt_t_gui_brd_leaveUp'))
        ...
        self.quitButton = DirectButton(parent = self.frame, relief = None, image = leaveImageList, image_scale = 0.065000000000000002, command = self._GroupPanel__handleLeaveButton, text = ('', quitText, quitText, ''), text_scale = 0.059999999999999998, text_fg = Vec4(1, 1, 1, 1), text_shadow = Vec4(0, 0, 0, 1), text_pos = (0.044999999999999998, 0.0), text_align = TextNode.ALeft, pos = (0.223, 0, quitButtonZPos), image3_color = self.disabledOrangeColor)
        ...
        self.hideButton = DirectButton(relief = None, text_pos = (0, 0.14999999999999999), text_scale = 0.059999999999999998, text_align = TextNode.ALeft, text_fg = Vec4(0, 0, 0, 1), text_shadow = Vec4(1, 1, 1, 1), image = hideImageList, image_scale = (-0.34999999999999998, 1, 0.5), pos = (-1.3081, 0, 0.029999999999999999), scale = 1.05, command = self.hide)
        self.showButton = DirectButton(relief = None, text = ('', TTLocalizer.BoardingGroupShow, TTLocalizer.BoardingGroupShow), text_pos = (0.029999999999999999, 0), text_scale = 0.059999999999999998, text_align = TextNode.ALeft, text_fg = Vec4(1, 1, 1, 1), text_shadow = Vec4(0, 0, 0, 1), image = showImageList, image_scale = (0.34999999999999998, 1, 0.5), pos = (-1.3081, 0, 0.029999999999999999), scale = 1.05, command = self.show)
        ...
        self.accept('ToonBattleIdUpdate', self._GroupPanel__possibleGroupUpdate)
    ...
    def _GroupPanel__defineConstants(self):
        ...
        self.textBgDisabledColor = Vec4(0.40000000000000002, 0.80000000000000004, 0.40000000000000002, 1)

    
    def _GroupPanel__handleLeaveButton(self):
        messenger.send('wakeup')
        if not base.cr.playGame.getPlace().getState() == 'elevator':
            self.confirmQuitDialog = TTDialog.TTDialog(style = TTDialog.YesNo, text = TTLocalizer.QuitBoardingPartyConfirm, command = self._GroupPanel__confirmQuitCallback)
    ...
    def _GroupPanel__makeAvatarNameScrolledList(self):
        ...
        self.scrollList = DirectScrolledList(parent = self.frame, relief = None, incButton_image = (friendsListGui.find('**/FndsLst_ScrollUp'), friendsListGui.find('**/FndsLst_ScrollDN'), friendsListGui.find('**/FndsLst_ScrollUp_Rllvr'), friendsListGui.find('**/FndsLst_ScrollUp')), incButton_pos = (0.0, 0.0, -0.34999999999999998), incButton_image1_color = Vec4(1.0, 0.90000000000000002, 0.40000000000000002, 0), incButton_image3_color = Vec4(1.0, 1.0, 0.59999999999999998, 0), incButton_scale = (1.0, 1.0, -1.0), incButton_relief = None, decButton_image = (friendsListGui.find('**/FndsLst_ScrollUp'), friendsListGui.find('**/FndsLst_ScrollDN'), friendsListGui.find('**/FndsLst_ScrollUp_Rllvr'), friendsListGui.find('**/FndsLst_ScrollUp')), decButton_pos = (0.0, 0.0, 0.10000000000000001), decButton_image1_color = Vec4(1.0, 1.0, 0.59999999999999998, 0), decButton_image3_color = Vec4(1.0, 1.0, 0.59999999999999998, 0), decButton_relief = None, itemFrame_pos = (-0.19500000000000001, 0.0, 0.185), itemFrame_borderWidth = (0.10000000000000001, 0.10000000000000001), numItemsVisible = 8, itemFrame_scale = 1.0, forceHeight = 0.070000000000000007, items = [], pos = (0, 0, 0.074999999999999997))
        clipper = PlaneNode('clipper')
        clipper.setPlane(Plane(Vec3(-1, 0, 0), Point3(0.23499999999999999, 0, 0)))
    ...
    def _GroupPanel__makeDestinationScrolledList(self):
        ...
        incrementImageList = (arrowGui.find('**/tt_t_gui_brd_arrowL_gotoUp'), arrowGui.find('**/tt_t_gui_brd_arrowL_gotoDown'), arrowGui.find('**/tt_t_gui_brd_arrowL_gotoHover'), arrowGui.find('**/tt_t_gui_brd_arrowL_gotoUp'))
        if self.boardingParty.maxSize == 4:
            zPos = -0.17708299999999999
        else:
            zPos = -0.46384300000000001
        bottomImage = self.guiBg.find('**/tt_t_gui_brd_memberListBtm_leader')
        self.destScrollList = DirectScrolledList(parent = self.frame, relief = None, image = bottomImage, image_scale = (0.5, 1, 0.5), incButton_image = incrementImageList, incButton_pos = (0.217302, 0, 0.070000000000000007), incButton_image3_color = Vec4(1.0, 1.0, 0.59999999999999998, 0.5), incButton_scale = (-0.5, 1, 0.5), incButton_relief = None, incButtonCallback = self._GroupPanel__informDestChange, decButton_image = incrementImageList, decButton_pos = (-0.217302, 0, 0.070000000000000007), decButton_scale = (0.5, 1, 0.5), decButton_image3_color = Vec4(1.0, 1.0, 0.59999999999999998, 0.5), decButton_relief = None, decButtonCallback = self._GroupPanel__informDestChange, itemFrame_pos = (0, 0, 0.059999999999999998), itemFrame_borderWidth = (0.10000000000000001, 0.10000000000000001), numItemsVisible = 1, itemFrame_scale = TTLocalizer.GPdestScrollList, forceHeight = 0.070000000000000007, items = [], pos = (0, 0, zPos), scrollSpeed = 0.10000000000000001)
        arrowGui.removeNode()
    ...
    def _GroupPanel__makeGoButton(self):
        ...
        self.goImageList = (goGui.find('**/tt_t_gui_brd_gotoUp'), goGui.find('**/tt_t_gui_brd_gotoDown'), goGui.find('**/tt_t_gui_brd_gotoHover'), goGui.find('**/tt_t_gui_brd_gotoUp'))
        self.cancelGoImageList = (goGui.find('**/tt_t_gui_brd_cancelGotoUp'), goGui.find('**/tt_t_gui_brd_cancelGotoDown'), goGui.find('**/tt_t_gui_brd_cancelGotoHover'), goGui.find('**/tt_t_gui_brd_cancelGotoUp'))
        if self.boardingParty.maxSize == 4:
            zPos = -0.028000000000000001
            zPos = -0.036048299999999998
        else:
            zPos = -0.035378699999999999
        self.goButton = DirectButton(parent = self.destScrollList, relief = None, image = self.goImageList, image_scale = (0.47999999999999998, 1, 0.47999999999999998), command = self._GroupPanel__handleGoButton, text = ('', TTLocalizer.BoardingGo, TTLocalizer.BoardingGo, ''), text_scale = TTLocalizer.GPgoButton, text_fg = Vec4(1, 1, 1, 1), text_shadow = Vec4(0, 0, 0, 1), text_pos = (0, -0.12), pos = (-0.0030000000000000001, 0, zPos))
        goGui.removeNode()
    ...
    def _GroupPanel__getAvatarButton(self, avId):
        ...
        return DirectButton(parent = self.frame, relief = None, image = buttonImage, image_scale = (0.059999999999999998, 1.0, 0.059999999999999998), text = toonName, text_align = TextNode.ALeft, text_wordwrap = 16, text_scale = 0.040000000000000001, text_pos = (0.050000000000000003, -0.014999999999999999), text_fg = self.textFgcolor, text1_bg = self.textBgDownColor, text2_bg = self.textBgRolloverColor, text3_fg = self.textBgDisabledColor, pos = (0, 0, 0.20000000000000001), command = self._GroupPanel__openToonAvatarPanel, extraArgs = [
            toon,
            avId])
    ...
    def changeDestination(self, offset):
        if localAvatar.doId != self.leaderId:
            ...
            if self.destFrame:
                self.destFrame['text'] = self._GroupPanel__getDestName(self.destIndexSelected)
    ...
    def _GroupPanel__makeGoingToLabel(self):
        if self.boardingParty.maxSize == 4:
            ...
            zPos = -0.331731
        self.goingToLabel = DirectLabel(parent = self.frame, relief = None, text = TTLocalizer.BoardingGoingTo, text_scale = 0.044999999999999998, text_align = TextNode.ALeft, text_fg = Vec4(0, 0, 0, 1), pos = (-0.1966, 0, zPos))
