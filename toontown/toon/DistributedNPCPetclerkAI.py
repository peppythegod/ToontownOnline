...
class DistributedNPCPetclerkAI(DistributedNPCToonBaseAI):
    ...
    def avatarEnter(self):
        ...
        self.acceptOnce(
            ...
            self._DistributedNPCPetclerkAI__handleUnexpectedExit,
            extraArgs=[avId])
    ...
    def petAdopted(self, petNum, nameIndex):
        ...
        if self.busy != avId:
            ...
            self.notify.warning(
                'somebody called petAdopted that I was not busy with! avId: %s'
                % avId)
            return None
        ...
        if av:
            ...
            if petNum not in range(0, len(self.petSeeds)):
                ...
                self.notify.warning(
                    'somebody called petAdopted on a non-existent pet! avId: %s'
                    % avId)
                return None
    ...
    def petReturned(self):
        ...
        if self.busy != avId:
            ...
            self.notify.warning(
                'somebody called petReturned that I was not busy with! avId: %s'
                % avId)
            return None
    ...
    def transactionDone(self):
        ...
        if self.busy != avId:
            self.air.writeServerEvent(
                'suspicious', avId,
                'DistributedNPCPetshopAI.transactionDone busy with %s' %
                self.busy)
            self.notify.warning(
    ...
    def _DistributedNPCPetclerkAI__handleUnexpectedExit(self, avId):
