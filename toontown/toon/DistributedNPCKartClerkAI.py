...
class DistributedNPCKartClerkAI(DistributedNPCToonBaseAI):
    ...
    def avatarEnter(self):
        ...
        self.acceptOnce(
            ...
            self._DistributedNPCKartClerkAI__handleUnexpectedExit,
            extraArgs=[avId])
        flag = NPCToons.SELL_MOVIE_START
        self.d_setMovie(avId, flag)
        taskMgr.doMethodLater(KartShopGlobals.KARTCLERK_TIMER,
    ...
    def d_setMovie(self, avId, flag, extraArgs=[]):
        self.sendUpdate('setMovie', [
            flag, self.npcId, avId, extraArgs,
    ...
    def buyKart(self, whichKart):
        ...
        if self.busy != avId:
            ...
            self.notify.warning(
                'somebody called buyKart that I was not busy with! avId: %s' %
                avId)
            return None
        ...
        if av:
            ...
            self.air.writeServerEvent('kartingKartPurchased', avId,
                                      '%s' % whichKart)
    ...
    def buyAccessory(self, whichAcc):
        ...
        if self.busy != avId:
            self.air.writeServerEvent(
                'suspicious', avId,
                'DistributedNPCKartClerkAI.buyAccessory busy with %s' %
                self.busy)
            self.notify.warning(
                'somebody called buyAccessory that I was not busy with! avId: %s'
                % avId)
            return None
        ...
        if av:
            ...
            self.air.writeServerEvent('kartingAccessoryPurchased', avId,
                                      '%s' % whichAcc)
    ...
    def transactionDone(self):
        ...
        if self.busy != avId:
            self.air.writeServerEvent(
                'suspicious', avId,
                'DistributedNPCKartClerkAI.transactionDone busy with %s' %
                self.busy)
            self.notify.warning(
    ...
    def _DistributedNPCKartClerkAI__handleUnexpectedExit(self, avId):
