...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI,
    ...
    partTypeIds = {
        ...
        (CogDisguiseGlobals.leftLegIndex, CogDisguiseGlobals.rightLegIndex,
         CogDisguiseGlobals.torsoIndex, CogDisguiseGlobals.leftArmIndex,
    ...
    DbCheckPeriodPaid = simbase.config.GetInt('toon-db-check-period-paid',
                                              10 * 60)
    DbCheckPeriodUnpaid = simbase.config.GetInt('toon-db-check-period-unpaid',
                                                1 * 60)
    ...
    DbCheckAccountDateBegin = config.GetString('account-blackout-start',
                                               '2013-08-20 12:30:00')
    DbCheckAccountDateDisconnect = config.GetBool(
        'account-blackout-disconnect', 1)
    ...
    def announceGenerate(self):
        ...
        if hasattr(self, 'gameAccess') and self.gameAccess != 2:
            if self.hat[0] != 0:
                self.replaceItemInAccessoriesList(ToonDNA.HAT, 0, 0, 0,
                                                  self.hat[0], self.hat[1],
                                                  self.hat[2])
            ...
            if self.glasses[0] != 0:
                self.replaceItemInAccessoriesList(
                    ToonDNA.GLASSES, 0, 0, 0, self.glasses[0], self.glasses[1],
                    self.glasses[2])
            ...
            if self.backpack[0] != 0:
                self.replaceItemInAccessoriesList(
                    ToonDNA.BACKPACK, 0, 0, 0, self.backpack[0],
                    self.backpack[1], self.backpack[2])
                self.b_setBackpackList(self.backpackList)
                self.b_setBackpack(0, 0, 0)

            if self.shoes[0] != 0:
                self.replaceItemInAccessoriesList(ToonDNA.SHOES, 0, 0, 0,
                                                  self.shoes[0], self.shoes[1],
                                                  self.shoes[2])
    ...
    def _handleDbCheckResult(self, result):
        ...
        if result is None:
            self._doDbCheckBan(
                'toon %s not present in the database' % self.doId)
        else:
            self.air.securityMgr.getAccountId(
                self.doId, self._handleDbCheckGetAccountResult)
    ...
    def _handleDbCheckGetAccountResult(self, accountId):
        ...
        if accountId is None:
            ...
            self.air.sendFieldQuery('AccountAI', 'ACCOUNT_AV_SET', accountId,
                                    self._handleDbCheckGetAvSetResult)
            if DistributedToonAI.DbCheckAccountDateEnable:
                self.air.sendFieldQuery(
                    'AccountAI', 'CREATED', accountId,
                    self._handleDbCheckAccountCreatedResult)
    ...
    def _handleDbCheckGetAvSetResult(self, avSet):
        ...
        if avSet is None:
            self._doDbCheckBan(
                "toon %s's account has no ACCOUNT_AV_SET in the DB" %
                self.doId)
            renewDoLater = False
        elif self.doId not in avSet:
            self._doDbCheckBan(
                'toon %s not in ACCOUNT_AV_SET in the DB' % self.doId)
            renewDoLater = False
    ...
    def _handleDbCheckAccountCreatedResult(self, created):
        ...
        if created is None:
            self._doDbCheckBan(
                "toon %s's account has no CREATED in the DB" % self.doId)
    ...
    def _renewDoLater(self, renew=True):
        if renew:
            ...
            self._dbCheckDoLater = taskMgr.doMethodLater(
                delay, self._doDbCheck, 'dbCheck-%s' % self.doId)
    ...
    def sendDeleteEvent(self):
        if simbase.wantPets:
            ...
            if isInEstate or wasInEstate:
                PetObserve.send(
                    self.estateZones,
                    PetObserve.PetActionObserve(PetObserve.Actions.LOGOUT,
                                                self.doId))
                if wasInEstate:
    ...
    def handleLogicalZoneChange(self, newZoneId, oldZoneId):
        ...
        if self.isPlayerControlled() and self.WantTpTrack:
            messenger.send(self.staticGetLogicalZoneChangeAllEvent(),
                           [newZoneId, oldZoneId, self])
        ...
        if self.cogIndex != -1 and not ToontownAccessAI.canWearSuit(
            ...
            if not simbase.air.cogSuitMessageSent:
                self.notify.warning('%s handleLogicalZoneChange as a suit: %s'
    ...
    def checkAccessorySanity(self, accessoryType, idx, textureIdx, colorIdx):
        ...
        try:
            styleStr = stylesDict.keys()[stylesDict.values().index(
                [idx, textureIdx, colorIdx])]
            accessoryItemId = 0
            for itemId in CatalogAccessoryItem.AccessoryTypes.keys():
                if styleStr == CatalogAccessoryItem.AccessoryTypes[itemId][
            ...
            if not simbase.config.GetBool('want-check-accessory-sanity',
                                          False):
                return 1
    ...
    def d_setHat(self, idx, textureIdx, colorIdx):
        if not self.checkAccessorySanity(ToonDNA.HAT, idx, textureIdx,
                                         colorIdx):
            pass
        1
    ...
    def d_setGlasses(self, idx, textureIdx, colorIdx):
        if not self.checkAccessorySanity(ToonDNA.GLASSES, idx, textureIdx,
                                         colorIdx):
            pass
        1
    ...
    def d_setBackpack(self, idx, textureIdx, colorIdx):
        if not self.checkAccessorySanity(ToonDNA.BACKPACK, idx, textureIdx,
                                         colorIdx):
            pass
        1
    ...
    def d_setShoes(self, idx, textureIdx, colorIdx):
        if not self.checkAccessorySanity(ToonDNA.SHOES, idx, textureIdx,
                                         colorIdx):
            pass
        1
    ...
    def setDNAString(self, string):
        ...
        if simbase.config.GetBool('adjust-dna',
                                  True) and self.verifyDNA() == False:
            logStr = 'AvatarHackWarning! invalid dna colors for %s old: %s new: %s' % (
                self.doId, str(ToonDNA.ToonDNA(string).asTuple()),
    ...
    def setInventory(self, inventoryNetString):
        if self.inventory:
            ...
            self.inventory = InventoryBase.InventoryBase(
                self, inventoryNetString)
        ...
        if lengthMatch != 0:
            ...
            if oldTracks == 0 and oldLevels == 0:
                self.notify.warning(
                    'reseting invalid inventory to MAX on toon: %s' %
                    self.doId)
                ...
                for indexTrack in range(0, oldTracks):
                    for indexGag in range(0, oldLevels):
                        newInventory.addItems(indexTrack, indexGag,
                                              oldList[indexTrack][indexGag])
    ...
    def setNPCFriendsDict(self, NPCFriendsList):
        ...
        self.notify.debug(
            'setting NPC friends dict to %s' % self.NPCFriendsDict)
    ...
    def attemptSubtractNPCFriend(self, npcFriend):
        if not self.NPCFriendsDict.has_key(npcFriend):
            self.notify.warning(
                'attemptSubtractNPCFriend: invalid NPC %s' % npcFriend)
            return 0
    ...
    def isTrunkFull(self, extraAccessories=0):
        numAccessories = (len(self.hatList) + len(self.glassesList) + len(
            self.backpackList) + len(self.shoesList)) / 3
    ...
    def addToAccessoriesList(self, accessoryType, geomIdx, texIdx, colorIdx):
        ...
        for i in range(0, len(itemList), 3):
            if itemList[i] == geomIdx and itemList[
                    i + 1] == texIdx and itemList[i + 2] == colorIdx:
                return 0
                continue
    ...
    def replaceItemInAccessoriesList(self, accessoryType, geomIdxA, texIdxA,
                                     colorIdxA, geomIdxB, texIdxB, colorIdxB):
        if accessoryType == ToonDNA.HAT:
            itemList = self.hatList
        elif accessoryType == ToonDNA.GLASSES:
            itemList = self.glassesList
        ...
        index = 0
        for i in range(0, len(itemList), 3):
            if itemList[i] == geomIdxA and itemList[
                    i + 1] == texIdxA and itemList[i + 2] == colorIdxA:
                if accessoryType == ToonDNA.HAT:
    ...
    def isValidAccessorySetting(self, accessoryType, geomIdx, texIdx,
                                colorIdx):
    ...
    def removeItemInAccessoriesList(self, accessoryType, geomIdx, texIdx,
                                    colorIdx):
        if accessoryType == ToonDNA.HAT:
            itemList = self.hatList
        elif accessoryType == ToonDNA.GLASSES:
        ...
        if listLen < 3:
            self.notify.warning(
                'Accessory list is not long enough to delete anything')
            return 0
    ...
    def isClosetFull(self, extraClothes=0):
        numClothes = len(self.clothesTopsList) / 4 + len(
            self.clothesBottomsList) / 2
    ...
    def addToClothesTopsList(self, topTex, topTexColor, sleeveTex,
                             sleeveTexColor):
        ...
        for i in range(0, len(self.clothesTopsList), 4):
            if self.clothesTopsList[i] == topTex and self.clothesTopsList[
                    i + 1] == topTexColor and self.clothesTopsList[
                        i + 2] == sleeveTex and self.clothesTopsList[
                            i + 3] == sleeveTexColor:
                return 0
                continue
    ...
    def replaceItemInClothesTopsList(self, topTexA, topTexColorA, sleeveTexA,
                                     sleeveTexColorA, topTexB, topTexColorB,
                                     sleeveTexB, sleeveTexColorB):
        index = 0
        for i in range(0, len(self.clothesTopsList), 4):
            if self.clothesTopsList[i] == topTexA and self.clothesTopsList[
                    i + 1] == topTexColorA and self.clothesTopsList[
                        i + 2] == sleeveTexA and self.clothesTopsList[
                            i + 3] == sleeveTexColorA:
    ...
    def removeItemInClothesTopsList(self, topTex, topTexColor, sleeveTex,
                                    sleeveTexColor):
        ...
        for i in range(0, listLen, 4):
            if self.clothesTopsList[i] == topTex and self.clothesTopsList[
                    i + 1] == topTexColor and self.clothesTopsList[
                        i + 2] == sleeveTex and self.clothesTopsList[
                            i + 3] == sleeveTexColor:
                self.clothesTopsList = self.clothesTopsList[
                    0:i] + self.clothesTopsList[i + 4:listLen]
                return 1
                continue
    ...
    def addToClothesBottomsList(self, botTex, botTexColor):
        ...
        for i in range(0, len(self.clothesBottomsList), 2):
            if self.clothesBottomsList[
                    i] == botTex and self.clothesBottomsList[i +
                                                             1] == botTexColor:
                return 0
                continue
    ...
    def replaceItemInClothesBottomsList(self, botTexA, botTexColorA, botTexB,
                                        botTexColorB):
        index = 0
        for i in range(0, len(self.clothesBottomsList), 2):
            if self.clothesBottomsList[
                    i] == botTexA and self.clothesBottomsList[
                        i + 1] == botTexColorA:
    ...
    def removeItemInClothesBottomsList(self, botTex, botTexColor):
        ...
        for i in range(0, len(self.clothesBottomsList), 2):
            if self.clothesBottomsList[
                    i] == botTex and self.clothesBottomsList[i +
                                                             1] == botTexColor:
                self.clothesBottomsList = self.clothesBottomsList[
                    0:i] + self.clothesBottomsList[i + 2:listLen]
                return 1
                continue
    ...
    getGoneSadMessageForAvId = staticmethod(getGoneSadMessageForAvId)
    ...
    def setZonesVisited(self, hoods):
        ...
        self.notify.debug(
            'setting safe zone list to %s' % self.safeZonesVisited)
    ...
    def setCogRadar(self, radar):
        if not radar:
            self.notify.warning(
                'cogRadar set to bad value: %s. Resetting to [0,0,0,0]' %
                radar)
            self.cogRadar = [0, 0, 0, 0]
        else:
            self.cogRadar = radar
    ...
    def setBuildingRadar(self, radar):
        if not radar:
            self.notify.warning(
                'buildingRadar set to bad value: %s. Resetting to [0,0,0,0]' %
                radar)
            self.buildingRadar = [0, 0, 0, 0]
        else:
    ...
    def setCogTypes(self, types):
        if not types:
            self.notify.warning(
                'cogTypes set to bad value: %s. Resetting to [0,0,0,0]' %
                types)
            self.cogTypes = [0, 0, 0, 0]
        else:
            self.cogTypes = types
    ...
    def setCogLevels(self, levels):
        if not levels:
            self.notify.warning(
                'cogLevels set to bad value: %s. Resetting to [0,0,0,0]' %
                levels)
    ...
    def incCogLevel(self, dept):
        ...
        if newLevel > maxLevel:
            if not lastCog:
                ...
                cogTypeStr = SuitDNA.suitHeadTypes[self.cogTypes[dept]]
                self.cogLevels[dept] = SuitBattleGlobals.SuitAttributes[
                    cogTypeStr]['level']
    ...
    def getNumPromotions(self, dept):
        if dept not in SuitDNA.suitDepts:
            self.notify.warning(
                'getNumPromotions: Invalid parameter dept=%s' % dept)
            return 0
    ...
    def setCogParts(self, parts):
        if not parts:
            self.notify.warning(
                'cogParts set to bad value: %s. Resetting to [0,0,0,0]' %
                parts)
            self.cogParts = [0, 0, 0, 0]
        else:
            self.cogParts = parts
    ...
    def giveGenericCogPart(self, factoryType, dept):
        for partTypeId in self.partTypeIds[factoryType]:
            nextPart = CogDisguiseGlobals.getNextPart(self.getCogParts(),
                                                      partTypeId, dept)
            if nextPart:
                break
                continue
    ...
    def setCogMerits(self, merits):
        if not merits:
            self.notify.warning(
                'cogMerits set to bad value: %s. Resetting to [0,0,0,0]' %
                merits)
    ...
    def setCogIndex(self, index):
        if index != -1 and not ToontownAccessAI.canWearSuit(
                self.doId, self.zoneId):
            if not simbase.air.cogSuitMessageSent:
                self.notify.warning(
                    '%s setCogIndex invalid: %s' % (self.doId, index))
                if simbase.config.GetBool('want-ban-wrong-suit-place', False):
                    commentStr = 'Toon %s trying to set cog index to %s in Zone: %s' % (
                        self.doId, index, self.zoneId)
                    simbase.air.banManager.ban(self.doId, self.DISLid,
                                               commentStr)
    ...
    def d_setFishCollection(self, genusList, speciesList, weightList):
        self.sendUpdate('setFishCollection',
    ...
    def setFishCollection(self, genusList, speciesList, weightList):
        ...
        self.fishCollection.makeFromNetLists(genusList, speciesList,
                                             weightList)
    ...
    def hasQuest(self, questId, visitNpcId=None, rewardId=None):
        if self.getQuest(
                questId, visitNpcId=visitNpcId, rewardId=rewardId) == None:
            return False
        else:
            return True
    ...
    def removeAllTracesOfQuest(self, questId, rewardId):
        self.notify.debug('removeAllTracesOfQuest: questId: %s rewardId: %s' %
                          (questId, rewardId))
        self.notify.debug(
            'removeAllTracesOfQuest: quests before: %s' % self.quests)
        removedQuest = self.removeQuest(questId)
        self.notify.debug(
            'removeAllTracesOfQuest: quests after: %s' % self.quests)
        self.notify.debug('removeAllTracesOfQuest: questHistory before: %s' %
                          self.questHistory)
        removedQuestHistory = self.removeQuestFromHistory(questId)
        self.notify.debug('removeAllTracesOfQuest: questHistory after: %s' %
                          self.questHistory)
        self.notify.debug('removeAllTracesOfQuest: reward history before: %s' %
                          self.rewardHistory)
        removedRewardHistory = self.removeRewardFromHistory(rewardId)
        self.notify.debug('removeAllTracesOfQuest: reward history after: %s' %
                          self.rewardHistory)
    ...
    def requestDeleteQuest(self, questDesc):
        ...
        if not self.hasQuest(questId, rewardId=rewardId):
            self.air.writeServerEvent(
                'suspicious', self.doId,
                "Toon tried to delete quest they don't have %s" %
                str(questDesc))
            self.notify.warning(
        ...
        if not Quests.isQuestJustForFun(questId, rewardId):
            self.air.writeServerEvent(
                'suspicious', self.doId,
                'Toon tried to delete non-Just For Fun quest %s' %
                str(questDesc))
            self.notify.warning(
    ...
    def setCheesyEffect(self, effect, hoodId, expireTime):
        if simbase.air.holidayManager and ToontownGlobals.WINTER_CAROLING not in simbase.air.holidayManager.currentHolidays and ToontownGlobals.WACKY_WINTER_CAROLING not in simbase.air.holidayManager.currentHolidays and effect == ToontownGlobals.CESnowMan:
            self.b_setCheesyEffect(ToontownGlobals.CENormal, hoodId,
                                   expireTime)
            return None
            ...
            if effect != ToontownGlobals.CENormal:
                ...
                if duration > 0:
                    taskMgr.doMethodLater(
                        duration, self._DistributedToonAI__undoCheesyEffect,
                        taskName)
                else:
                    self._DistributedToonAI__undoCheesyEffect(None)

    def getCheesyEffect(self):
        return (self.savedCheesyEffect, self.savedCheesyHoodId,
                self.savedCheesyExpireTime)

    def _DistributedToonAI__undoCheesyEffect(self, task):
    ...
    def fixTrackAccess(self):
        fixed = 0
        (healExp, trapExp, lureExp, soundExp, throwExp, squirtExp,
         dropExp) = self.experience.experience
        numTracks = reduce(lambda a, b: a + b, self.trackArray)
        if self.rewardTier in [0, 1, 2, 3]:
            if numTracks != 2:
                self.notify.warning('bad num tracks in tier: %s, %s' %
            ...
            if numTracks != 3:
                self.notify.warning('bad num tracks in tier: %s, %s' %
                                    (self.rewardTier, self.trackArray))
                if self.trackArray[ToontownBattleGlobals.
                                   SOUND_TRACK] and not self.trackArray[
                                       ToontownBattleGlobals.HEAL_TRACK]:
                    self.b_setTrackAccess([0, 0, 0, 1, 1, 1, 0])
                elif self.trackArray[ToontownBattleGlobals.
                                     HEAL_TRACK] and not self.trackArray[
            ...
            if numTracks != 5:
                ...
                if self.trackArray[ToontownBattleGlobals.
                                   SOUND_TRACK] and not self.trackArray[
                                       ToontownBattleGlobals.HEAL_TRACK]:
                    if self.trackArray[ToontownBattleGlobals.
                                       DROP_TRACK] and not self.trackArray[
    ...
    def checkTeleportAccess(self, zoneId):
        if zoneId not in self.getTeleportAccess():
            simbase.air.writeServerEvent(
                'suspicious', self.doId,
                'Toon teleporting to zone %s they do not have access to.' %
                zoneId)
            if simbase.config.GetBool('want-ban-teleport', False):
                commentStr = 'Toon %s teleporting to a zone %s they do not have access to' % (
                    self.doId, zoneId)
    ...
    def setRewardHistory(self, tier, rewardList):
        ...
        self.notify.debug(
            'setting reward history to tier %s, %s' % (tier, rewardList))
    ...
    def fixAvatar(self):
        ...
        if not self.emoteAccess[0] and self.emoteAccess[1] and self.emoteAccess[
                2] and self.emoteAccess[3] and self.emoteAccess[4]:
            ...
            self.notify.info('Changed avatar %d to have emoteAccess: %s' %
    ...
    def restockAllResistanceMessages(self, charges=1):
        ResistanceChat = ResistanceChat
        import toontown.chat
        msgs = []
    ...
    def setCatalogSchedule(self, currentWeek, nextTime):
        ...
        if self.air.doLiveUpdates:
            ...
            taskMgr.doMethodLater(
                duration, self._DistributedToonAI__deliverCatalog, taskName)

    def getCatalogSchedule(self):
        return (self.catalogScheduleCurrentWeek, self.catalogScheduleNextTime)

    def _DistributedToonAI__deliverCatalog(self, task):
    ...
    def d_setCatalog(self, monthlyCatalog, weeklyCatalog, backCatalog):
        self.sendUpdate('setCatalog', [
            monthlyCatalog.getBlob(),
            weeklyCatalog.getBlob(),
            backCatalog.getBlob()
        ])
    ...
    def getCatalog(self):
        return (self.monthlyCatalog.getBlob(), self.weeklyCatalog.getBlob(),
    ...
    def d_setDeliverySchedule(self, onOrder):
        self.sendUpdate('setDeliverySchedule', [
            onOrder.getBlob(store=CatalogItem.Customization
    ...
    def setDeliverySchedule(self, onOrder, doUpdateLater=True):
        ...
        if hasattr(self, 'name'):
            if doUpdateLater and self.air.doLiveUpdates and hasattr(
                    self, 'air'):
                taskName = self.uniqueName('next-delivery')
                taskMgr.remove(taskName)
                ...
                if nextTime != None:
                    duration = max(10.0, nextTime * 60 - time.time())
                    taskMgr.doMethodLater(
                        duration, self._DistributedToonAI__deliverPurchase,
                        taskName)

    def getDeliverySchedule(self):
        return self.onOrder.getBlob(store=CatalogItem.Customization
                                    | CatalogItem.DeliveryDate)
        ...
        if doUpdateLater and self.air.doLiveUpdates and hasattr(self, 'name'):
            ...
            if nextTime:
                ...
                if existingDuration and existingDuration >= newDuration:
                    ...
                    taskMgr.doMethodLater(
                        newDuration,
                        self._DistributedToonAI__deliverBothPurchases,
                        taskName)
    ...
    def _DistributedToonAI__deliverBothPurchases(self, task):
        now = int(time.time() / 60 + 0.5)
        (delivered, remaining) = self.onOrder.extractDeliveryItems(now)
        (deliveredGifts,
         remainingGifts) = self.onGiftOrder.extractDeliveryItems(now)
        ...
        if len(giftItem) > 0:
            self.air.writeServerEvent(
                'Getting Gift', self.doId, 'sender %s receiver %s gift %s' %
    ...
    def setGiftSchedule(self, onGiftOrder, doUpdateLater=True):
        ...
        self.onGiftOrder = CatalogItemList.CatalogItemList(
            ...
            store=CatalogItem.Customization | CatalogItem.DeliveryDate)
        if doUpdateLater and self.air.doLiveUpdates and hasattr(
                self, 'air') and hasattr(self, 'name'):
            ...
            if nextTime != None:
                ...
                duration += 30
                taskMgr.doMethodLater(
                    duration, self._DistributedToonAI__deliverGiftPurchase,
                    taskName)

    def getGiftSchedule(self):
        return self.onGiftOrder.getBlob(store=CatalogItem.Customization
                                        | CatalogItem.DeliveryDate)

    def _DistributedToonAI__deliverGiftPurchase(self, task):
        now = int(time.time() / 60 + 0.5)
    ...
    def _DistributedToonAI__deliverPurchase(self, task):
        now = int(time.time() / 60 + 0.5)
        (delivered, remaining) = self.onOrder.extractDeliveryItems(now)
        ...
        if self.houseId and hasattr(self, 'air'):
            if self.air:
                ...
                if house and house.mailbox:
                    if not len(
                            self.mailboxContents
                    ) != 0 and self.numMailItems and self.getNumInvitesToShowInMailbox(
                    ):
                        pass
    ...
    def takeMoney(self, deltaMoney, bUseBank=True):
        ...
        if deltaMoney > totalMoney:
            self.notify.warning('Not enough money! AvId: %s Has:%s Charged:%s'
    ...
    def tossPie(self, x, y, z, h, p, r, sequence, power, timestamp32):
        if not self.validate(self.doId, self.numPies > 0,
    ...
    def startToonUp(self, healFrequency):
        ...
        self._DistributedToonAI__waitForNextToonUp()

    def _DistributedToonAI__waitForNextToonUp(self):
    ...
    def makeBlackCat(self):
        ...
        black = 26
        newDna.updateToonProperties(
            armColor=black, legColor=black, headColor=black)
    ...
    def d_setSCResistance(self, msgIndex, nearbyPlayers):
        ...
        if hasattr(self,
                   'autoResistanceRestock') and self.autoResistanceRestock:
        ...
        if len(affectedPlayers) > 50:
            self.air.writeServerEvent(
                'suspicious', self.doId,
                'said resistance %s for %s toons.' % (msgIndex,
                                                      len(affectedPlayers)))
            self.notify.warning(
    ...
    if simbase.wantKarts:
        ...
        def removeOwnedAccessory(self, accessoryId):
            if AccessoryDict.has_key(accessoryId):
                if self.accessories.count(accessoryId) == 0:
                    ...
                    self.air.writeServerEvent('deletedKartingAccessory',
                                              self.doId, '%s' % accessoryId)
                    self.b_setKartAccessoriesOwned(accList)
            else:
                self.air.writeServerEvent(
                    'suspicious', self.doId,
        ...
        def updateKartDNAField(self, dnaField, fieldValue):
            ...
            if dnaField == KartDNA.bodyType:
                if fieldValue not in KartDict.keys(
                ) and fieldValue != InvalidEntry:
                    self.air.writeServerEvent(
                        'suspicious', self.doId,
                        'attempt to update kart body to invalid body %s.' %
                        fieldValue)
                    return None
                ...
                if dnaField in accFields:
                    if fieldValue == InvalidEntry:
                        self._DistributedToonAI__updateKartDNAField(
                            dnaField, fieldValue)
                    elif fieldValue not in self.accessories:
                        self.air.writeServerEvent(
                            'suspicious', self.doId,
                    ...
                    if fieldValue == InvalidEntry:
                        ...
                        if fieldValue != getDefaultColor():
                            self.air.writeServerEvent(
                                ...
                                % fieldValue)
                            return None
                        elif fieldValue == getDefaultColor(
        ...
        def _DistributedToonAI__updateKartDNAField(self, dnaField, fieldValue):
    ...
    if simbase.wantPets:
        ...
        def setPetMovie(self, petId, flag):
            self.notify.debug(
                'setPetMovie: petId: %s, flag: %s' % (petId, flag))
        ...
        def wasInEstate(self):
            if hasattr(self, '_wasInEstate'):
                pass
            return self._wasInEstate
        ...
        def setSC(self, msgId):
            ...
            if msgId in [21006]:
                self.setHatePets(1)
                    ...
                    21000, 21001, 21003, 21004, 21200, 21201, 21202, 21203,
                    21204, 21205, 21206
            ]:
                self.setHatePets(0)
    ...
    def takeOutKart(self, zoneId=None):
        if not self.kart:
            DistributedVehicleAI = DistributedVehicleAI
            import toontown.racing
            self.kart = DistributedVehicleAI.DistributedVehicleAI(
                self.air, self.doId)
            if zoneId:
    ...
    def doCogInvasion(self, suitIndex):
        ...
        if invMgr.getInvading():
            returnCode = 'busy'
        elif suitIndex >= len(SuitDNA.suitHeadTypes):
            self.notify.warning('Bad suit index: %s' % suitIndex)
            return ['badIndex', suitIndex, 0]
    ...
    def removeCogSummonsEarned(self, suitIndex, type):
        ...
        self.notify.warning("Toon %s doesn't have a %s summons for %s" %
                            (self.doId, type, suitIndex))
        return False
    ...
    def assignNewCogSummons(self, level=None, summonType=None, deptIndex=None):
        ...
        if summonType in ['single', 'building', 'invasion']:
            ...
            typeWeights = ['single'] * 70 + ['building'] * 25 + ['invasion'
                                                                 ] * 5
    ...
    def removeGardenItem(self, index, count):
        ...
        self.notify.warning(
            "removing garden item %d that toon doesn't have" % index)
    ...
    def addFlowerToBasket(self, species, variety):
        ...
        if numFlower >= self.maxFlowerBasket:
            self.notify.warning(
                'addFlowerToBasket: cannot add flower, basket is full')
            return 0
    ...
    def b_setShovelSkill(self, skillLevel):
        ...
        if skillLevel >= GardenGlobals.ShovelAttributes[
            ...
            if self.shovel < GardenGlobals.MAX_SHOVELS - 1:
                ...
                self.air.writeServerEvent('garden_new_shovel', self.doId,
                                          '%d' % self.shovel)
    ...
    def b_setWateringCanSkill(self, skillLevel):
        ...
        if skillLevel >= GardenGlobals.WateringCanAttributes[
            ...
            if self.wateringCan < GardenGlobals.MAX_WATERING_CANS - 1:
                ...
                self.air.writeServerEvent('garden_new_wateringCan', self.doId,
                                          '%d' % self.wateringCan)
            else:
                skillLevel = GardenGlobals.WateringCanAttributes[
                    self.wateringCan]['skillPts'] - 1
    ...
    def giveMeSpecials(self, id=None):
        print 'Specials Go!!'
        self.b_setGardenSpecials([(0, 3), (1, 2), (2, 3), (3, 2), (4, 3),
                                  (5, 2), (6, 3), (7, 2), (100, 1), (101, 3),
                                  (102, 1)])
    ...
    def tryToUseSpecial(self, special):
        ...
        if not self.doId == estateOwnerDoId:
            self.notify.warning(
                "how did this happen, planting an item you don't own")
            return response
    ...
    def sendGardenEvent(self):
        if hasattr(self, 'estateZones') and hasattr(self, 'doId'):
            if simbase.wantPets and self.hatePets:
                PetObserve.send(
                    self.estateZones,
                    PetObserve.PetActionObserve(PetObserve.Actions.GARDEN,
                                                self.doId))
    ...
    def setGolfHistory(self, history):
        ...
        self.golfTrophies = GolfGlobals.calcTrophyListFromHistory(
            self.golfHistory)
        ...
        for invite in self.invites:
            ...
            if appendInvite:
                ...
                if appendInvite:
                    ...
                    curDate = simbase.air.toontownTimeManager.getCurServerDateTime(
                    ).date()
                    if endDate < curDate:
                        appendInvite = False
    ...
    def canPlanParty(self):
        ...
        for partyInfo in self.hostedParties:
            if partyInfo.status not in (PartyGlobals.PartyStatus.Cancelled,
            ...
            nonCancelledPartiesInTheFuture >= PartyGlobals.MaxHostedPartiesPerToon
    ...
    def setPartyCanStart(self, partyId):
        ...
        if not found:
            self.notify.warning(
                "setPartyCanStart can't find partyId %s" % partyId)
    ...
    def setPartyStatus(self, partyId, newStatus):
        ...
        if not found:
            self.notify.warning(
                "setPartyCanStart can't find hosted or invitedTO partyId %s" %
                partyId)
    ...
    def setAwardMailboxContents(self, awardMailboxContents):
        self.notify.debug(
            'Setting awardMailboxContents to %s.' % awardMailboxContents)
        self.awardMailboxContents = CatalogItemList.CatalogItemList(
            awardMailboxContents, store=CatalogItem.Customization)
        self.notify.debug(
            'awardMailboxContents is %s.' % self.awardMailboxContents)
    ...
    def getAwardMailboxContents(self):
        return self.awardMailboxContents.getBlob(
            store=CatalogItem.Customization)
        ...
        if hasattr(self, 'name'):
            if doUpdateLater and self.air.doLiveUpdates and hasattr(
                ...
                if nextTime != None:
                    ...
                    taskMgr.doMethodLater(
                        duration,
                        self._DistributedToonAI__deliverAwardPurchase,
                        taskName)

    def _DistributedToonAI__deliverAwardPurchase(self, task):
        now = int(time.time() / 60 + 0.5)
    ...
    def setGM(self, type):
        ...
        if self._isGM:
            ...
            if self._gmType > MaxGMType:
                self.notify.warning('toon %s has invalid GM type: %s' %
                                    (self.doId, self._gmType))
        ...
        for obfuscatedModule in info:
            ...
            if module not in self.moduleWhitelist:
                if module in self.moduleBlacklist:
                    ...
                    if simbase.config.GetBool('want-ban-blacklist-module',
                                              False):
                        commentStr = 'User has blacklist module: %s attached to their game process' % module
                        dislId = self.DISLid
                        simbase.air.banManager.ban(self.doId, dislId,
                                                   commentStr)
            ...
            module in self.moduleBlacklist

    def teleportResponseToAI(self, toAvId, available, shardId, hoodId, zoneId,
                             fromAvId):
        ...
        if toAvId != self.doId:
            self.air.writeServerEvent(
                'suspicious', self.doId,
                'toAvId=%d is not equal to self.doId' % toAvId)
            return None

        if available != 1:
            self.air.writeServerEvent('suspicious', self.doId,
                                      'invalid availableValue=%d' % available)
            return None
        ...
        dg = self.dclass.aiFormatUpdate(
            'teleportResponse', fromAvId, fromAvId, self.doId,
    ...
    staticGetLogicalZoneChangeAllEvent = staticmethod(
        staticGetLogicalZoneChangeAllEvent)
    ...
    def flagAv(self, avId, reason, params):
        self.notify.debug(
            'reason: %s timepassed: %s' % (reason, globalClock.getFrameTime() -
                                           DistributedToonAI.lastFlagAvTime))
        if reason == AV_FLAG_REASON_TOUCH and globalClock.getFrameTime(
        ) - DistributedToonAI.lastFlagAvTime > AV_TOUCH_CHECK_DELAY_AI:
            ...
            self.notify.debug(
                ...
                (avId, reason, params))
            if av and otherAv and isinstance(
                    av, DistributedToonAI
            ) and isinstance(
                    otherAv, DistributedToonAI
            ) and av.zoneId == otherAv.zoneId and av.zoneId not in MinigameCreatorAI.MinigameZoneRefs:
                self.notify.debug('...in zone %s' % av.zoneId)
                componentNode = av.getParent().attachNewNode('blah')
                componentNode.setPos(av.getComponentX(), av.getComponentY(),
                                     av.getComponentZ())
                avPos = componentNode.getPos(av.getRender())
                componentNode.reparentTo(otherAv.getParent())
                componentNode.setPos(otherAv.getComponentX(),
                ...
                dist = moveVec.length()
                self.notify.debug('2d dist between avs is %s %s %s' %
                                  (dist, avPos, otherAvPos))
                if dist < AV_TOUCH_CHECK_DIST and zDist < AV_TOUCH_CHECK_DIST_Z:
                    ...
                    newP = avHpr.getY()
                    newR = avHpr.getZ()
                    av.setPosHpr(av.getRender(), newX, newY, newZ, newH, newP,
                                 newR)
                    newAvPos = av.getPos()
                    if newAvPos.getX() > 3000 and newAvPos.getX(
                    ) < -3000 and newAvPos.getY() > 3000 or newAvPos.getY(
                    ) < -3000:
                        return None
                    ...
                    avPairKey = str(min(av.doId, otherAv.doId)) + '+' + str(
                        max(av.doId, otherAv.doId))
                    ...
                    self.notify.debug('moving av %s, newPos: %s oldPos: %s' %
                                      (av.doId, prevCount[2][av.doId], avPos))
                    if prevCount[2][av.doId][0] == None or prevCount[2][
                            av.doId][1] == None:
                        pass
                    1
                    if prevCount[2][av.doId][0] != avPos.getX(
                    ...
                    if prevCount[0][av.doId] > AV_TOUCH_COUNT_LIMIT:
                        if globalClock.getFrameTime(
                        ) - prevCount[1] < AV_TOUCH_COUNT_TIME:
                            if not not hasattr(av, 'zoneId') or 'undef':
                                pass
                            zoneId = av.zoneId
                            if not not hasattr(av, 'battleId') or 'undef':
                                pass
                            battleId = av.battleId
                            if not not hasattr(av, 'animName') or 'undef':
                                pass
                            animName = av.animName
                            if not not hasattr(av, 'isInEstate') or 'undef':
                                pass
                            inEstate = av.isInEstate()
                            if not not hasattr(av, 'ghostMode') or 'undef':
                                pass
                            ghostMode = av.ghostMode
                            if not not hasattr(av, 'immortalMode') or 'undef':
                                pass
                            immortalMode = av.immortalMode
                            if not not hasattr(av, '_isGM') or 'undef':
                                pass
                            isGm = av._isGM
                            valStr = '%s %s %s %s %s %s %s %s' % (
                                otherAv.doId, zoneId, battleId, animName,
                                inEstate, ghostMode, immortalMode, isGm)
                            self.notify.info(
                            ...
                            av.handleHacking(response,
                                             'collision and position hacking',
                                             [otherAv])
    ...
    def requestPing(self, avId):
        ...
        if av:
            ...
            if isinstance(av, DistributedToonAI
                          ) and not DistributedToonAI.pingedAvs.has_key(avId):
                av.sendPing()
    ...
    def sendPing(self):
        def verify(theId):
            if self.air:
                ...
                self.cleanupPing()
                disconnect = simbase.config.GetBool('client-ping-disconnect',
                                                    True)
                if disconnect:
        ...
        DistributedToonAI.pingedAvs[self.doId] = [
            globalClock.getFrameTime(), val
        ]
        delay = simbase.config.GetInt('client-ping-timeout', 150)
        taskMgr.doMethodLater(
            delay,
            verify,
            'pingverify-' + str(self.doId),
            extraArgs=[self.doId])

    def pingresp(self, resp):
        senderId = self.air.getAvatarIdFromSender()
        if not DistributedToonAI.pingedAvs.has_key(
                senderId) or self.air == None:
            self.cleanupPing()
            return None
