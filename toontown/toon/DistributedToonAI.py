...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    maxCallsPerNPC = 100
    partTypeIds = {
        ToontownGlobals.FT_FullSuit: (CogDisguiseGlobals.leftLegIndex, CogDisguiseGlobals.rightLegIndex, CogDisguiseGlobals.torsoIndex, CogDisguiseGlobals.leftArmIndex, CogDisguiseGlobals.rightArmIndex),
    ...
    DbCheckAccountDateDisconnect = config.GetBool('account-blackout-disconnect', 1)
    ...
    def d_setMaxNPCFriends(self, max):
        self.sendUpdate('setMaxNPCFriends', [
            max])
    ...
    def d_setNPCFriendsDict(self, NPCFriendsDict):
        ...
        self.sendUpdate('setNPCFriendsDict', [
            NPCFriendsList])
    ...
    def d_setMaxAccessories(self, max):
        self.sendUpdate('setMaxAccessories', [
            self.maxAccessories])
    ...
    def d_setGlassesList(self, clothesList):
        self.sendUpdate('setGlassesList', [
            clothesList])
    ...
    def d_setBackpackList(self, clothesList):
        self.sendUpdate('setBackpackList', [
            clothesList])
    ...
    def d_setClothesTopsList(self, clothesList):
        self.sendUpdate('setClothesTopsList', [
            clothesList])
    ...
    def d_setClothesBottomsList(self, clothesList):
        self.sendUpdate('setClothesBottomsList', [
            clothesList])
    ...
    def d_catalogGenClothes(self):
        self.sendUpdate('catalogGenClothes', [
            self.doId])

    
    def d_catalogGenAccessories(self):
        self.sendUpdate('catalogGenAccessories', [
            self.doId])
    ...
    def d_setTutorialAck(self, tutorialAck):
        self.sendUpdate('setTutorialAck', [
            tutorialAck])
    ...
    def d_setEarnedExperience(self, earnedExp):
        self.sendUpdate('setEarnedExperience', [
            earnedExp])
    ...
    def d_setBuildingRadar(self, radar):
        self.sendUpdate('setBuildingRadar', [
            radar])
    ...
    def d_setDisguisePageFlag(self, flag):
        self.sendUpdate('setDisguisePageFlag', [
            flag])
    ...
    def d_setSosPageFlag(self, flag):
        self.sendUpdate('setSosPageFlag', [
            flag])
    ...
    def d_setMaxFishTank(self, maxTank):
        self.sendUpdate('setMaxFishTank', [
            maxTank])
    ...
    def d_setFishingTrophies(self, trophyList):
        self.sendUpdate('setFishingTrophies', [
            trophyList])
    ...
    def addQuest(self, quest, finalReward, recordHistory = 1):
        ...
        if recordHistory:
            if quest[0] != Quests.VISIT_QUEST_ID:
                ...
                if finalReward:
                    newRewardHistory = self.rewardHistory + [
                        finalReward]
    ...
    def d_setQuestCarryLimit(self, limit):
        self.sendUpdate('setQuestCarryLimit', [
            limit])
    ...
    def d_setCheesyEffect(self, effect, hoodId, expireTime):
        self.sendUpdate('setCheesyEffect', [
            effect,
            hoodId,
            expireTime])
    ...
    def setCheesyEffect(self, effect, hoodId, expireTime):
        ...
        if self.air.doLiveUpdates:
            ...
            if effect != ToontownGlobals.CENormal:
                ...
                if duration > 0:
                    taskMgr.doMethodLater(duration, self._DistributedToonAI__undoCheesyEffect, taskName)
                else:
    ...
    def d_setTrackAccess(self, trackArray):
        self.sendUpdate('setTrackAccess', [
            trackArray])
    ...
    def d_setTrackProgress(self, trackId, progress):
        self.sendUpdate('setTrackProgress', [
            trackId,
            progress])
    ...
    def d_setHoodsVisited(self, hoodsVisitedArray):
        self.sendUpdate('setHoodsVisited', [
            hoodsVisitedArray])
    ...
    def d_setTeleportAccess(self, teleportZoneArray):
        self.sendUpdate('setTeleportAccess', [
            teleportZoneArray])
    ...
    def d_setQuestHistory(self, questList):
        self.sendUpdate('setQuestHistory', [
            questList])
    ...
    def d_setRewardHistory(self, tier, rewardList):
        self.sendUpdate('setRewardHistory', [
            tier,
            rewardList])
    ...
    def d_setEmoteAccess(self, bits):
        self.sendUpdate('setEmoteAccess', [
            bits])
    ...
    def d_setCustomMessages(self, customMessages):
        self.sendUpdate('setCustomMessages', [
            customMessages])
    ...
    def d_setResistanceMessages(self, resistanceMessages):
        self.sendUpdate('setResistanceMessages', [
            resistanceMessages])
    ...
    def d_setCatalogSchedule(self, currentWeek, nextTime):
        self.sendUpdate('setCatalogSchedule', [
            currentWeek,
            nextTime])
    ...
    def setCatalogSchedule(self, currentWeek, nextTime):
        ...
        if self.air.doLiveUpdates:
            ...
            taskMgr.doMethodLater(duration, self._DistributedToonAI__deliverCatalog, taskName)
    ...
    def d_setCatalogNotify(self, catalogNotify, mailboxNotify):
        self.sendUpdate('setCatalogNotify', [
            catalogNotify,
            mailboxNotify])
    ...
    def setDeliverySchedule(self, onOrder, doUpdateLater = True):
        ...
        if hasattr(self, 'name'):
            if doUpdateLater and self.air.doLiveUpdates and hasattr(self, 'air'):
                ...
                if nextTime != None:
                    duration = max(10.0, nextTime * 60 - time.time())
                    taskMgr.doMethodLater(duration, self._DistributedToonAI__deliverPurchase, taskName)
        ...
        if doUpdateLater and self.air.doLiveUpdates and hasattr(self, 'name'):
            ...
            if nextTime:
                ...
                if existingDuration and existingDuration >= newDuration:
                    taskMgr.remove(taskName)
                    taskMgr.doMethodLater(newDuration, self._DistributedToonAI__deliverBothPurchases, taskName)
    ...
    def setGiftSchedule(self, onGiftOrder, doUpdateLater = True):
        ...
        if doUpdateLater and self.air.doLiveUpdates and hasattr(self, 'air') and hasattr(self, 'name'):
            ...
            if nextTime != None:
                duration = max(10.0, nextTime * 60 - time.time())
                duration += 30
                taskMgr.doMethodLater(duration, self._DistributedToonAI__deliverGiftPurchase, taskName)
    ...
    def d_setMailboxContents(self, mailboxContents):
        self.sendUpdate('setMailboxContents', [
        ...
        if self.houseId and hasattr(self, 'air'):
            if self.air:
                ...
                if house and house.mailbox:
                    if not len(self.mailboxContents) != 0 and self.numMailItems and self.getNumInvitesToShowInMailbox():
                        pass
                    house.mailbox.b_setFullIndicator(len(self.awardMailboxContents) != 0)
    ...
    def d_setTrophyScore(self, score):
        self.sendUpdate('setTrophyScore', [
            score])
    ...
    def d_setSCResistance(self, msgIndex, nearbyPlayers):
        ...
        self.sendUpdate('setSCResistance', [
            msgIndex,
            affectedPlayers])
    ...
    if simbase.wantKarts:
        ...
        def d_setKartingTrophies(self, trophyList):
            self.sendUpdate('setKartingTrophies', [
                trophyList])
        ...
        def d_setKartingHistory(self, history):
            self.sendUpdate('setKartingHistory', [
                history])
        ...
        def d_setKartingPersonalBest(self, bestTimes):
            best1 = bestTimes[0:6]
            best2 = bestTimes[6:]
            self.sendUpdate('setKartingPersonalBest', [
                best1])
            self.sendUpdate('setKartingPersonalBest2', [
                best2])
        ...
        def d_setKartBodyType(self, bodyType):
            self.sendUpdate('setKartBodyType', [
                bodyType])
        ...
        def d_setKartBodyColor(self, bodyColor):
            self.sendUpdate('setKartBodyColor', [
                bodyColor])
        ...
        def d_setKartAccessoryColor(self, accColor):
            self.sendUpdate('setKartAccessoryColor', [
                accColor])
        ...
        def d_setKartEngineBlockType(self, ebType):
            self.sendUpdate('setKartEngineBlockType', [
                ebType])
        ...
        def d_setKartSpoilerType(self, spType):
            self.sendUpdate('setKartSpoilerType', [
                spType])
        ...
        def d_setKartBackWheelWellType(self, bwwType):
            self.sendUpdate('setKartBackWheelWellType', [
                bwwType])
        ...
        def d_setKartRimType(self, rimsType):
            self.sendUpdate('setKartRimType', [
                rimsType])
        ...
        def d_setKartAccessoriesOwned(self, accessories):
            self.sendUpdate('setKartAccessoriesOwned', [
                accessories])
            ...
            if dnaField == KartDNA.bodyType:
                ...
                if dnaField in accFields:
                    if fieldValue == InvalidEntry:
                        self._DistributedToonAI__updateKartDNAField(dnaField, fieldValue)
    ...
    if simbase.wantPets:
        ...
        def d_setPetTrickPhrases(self, tricks):
            self.sendUpdate('setPetTrickPhrases', [
                tricks])
    ...
    def reqCogSummons(self, type, suitIndex):
        if type not in ('single', 'building', 'invasion'):
            ...
            self.sendUpdate('cogSummonsResponse', [
                'fail',
                suitIndex,
                0])
            return None
    ...
    def d_setCogSummonsEarned(self, cogSummonsEarned):
        self.sendUpdate('setCogSummonsEarned', [
            cogSummonsEarned])
    ...
    def d_setGardenTrophies(self, trophyList):
        self.sendUpdate('setGardenTrophies', [
            trophyList])
    ...
    def d_setGardenSpecials(self, specials):
        self.sendUpdate('setGardenSpecials', [
            specials])
    ...
    def d_setFlowerCollection(self, speciesList, varietyList):
        self.sendUpdate('setFlowerCollection', [
            speciesList,
            varietyList])
    ...
    def d_setMaxFlowerBasket(self, maxFlowerBasket):
        self.sendUpdate('setMaxFlowerBasket', [
            maxFlowerBasket])
    ...
    def d_setFlowerBasket(self, speciesList, varietyList):
        self.sendUpdate('setFlowerBasket', [
            speciesList,
            varietyList])
    ...
    def d_setShovelSkill(self, skillLevel):
        self.sendUpdate('setShovelSkill', [
            skillLevel])
    ...
    def d_setWateringCan(self, wateringCanId):
        self.sendUpdate('setWateringCan', [
            wateringCanId])
    ...
    def b_setWateringCanSkill(self, skillLevel):
        ...
        if skillLevel >= GardenGlobals.WateringCanAttributes[self.wateringCan]['skillPts']:
            if self.wateringCan < GardenGlobals.MAX_WATERING_CANS - 1:
                ...
                self.sendUpdate('promoteWateringCan', [
                    self.wateringCan])
    ...
    def d_setWateringCanSkill(self, skillLevel):
        self.sendUpdate('setWateringCanSkill', [
            skillLevel])
    ...
    def d_setTrackBonusLevel(self, trackBonusLevelArray):
        self.sendUpdate('setTrackBonusLevel', [
            trackBonusLevelArray])
    ...
    def reqUseSpecial(self, special):
        ...
        self.sendUpdate('useSpecialResponse', [
            response])
    ...
    def d_setGardenStarted(self, bStarted):
        self.sendUpdate('setGardenStarted', [
            bStarted])
    ...
    def d_setGolfHistory(self, history):
        self.sendUpdate('setGolfHistory', [
            history])
    ...
    def d_setGolfHoleBest(self, holeBest):
        ...
        self.sendUpdate('setPackedGolfHoleBest', [
            packed])
    ...
    def d_setGolfCourseBest(self, courseBest):
        self.sendUpdate('setGolfCourseBest', [
            courseBest])
    ...
    def d_setUnlimitedSwing(self, unlimitedSwing):
        self.sendUpdate('setUnlimitedSwing', [
            unlimitedSwing])
    ...
    def d_setNametagStyle(self, nametagStyle):
        self.sendUpdate('setNametagStyle', [
            nametagStyle])
    ...
    def d_setAwardMailboxContents(self, awardMailboxContents):
        self.sendUpdate('setAwardMailboxContents', [
    ...
    def setAwardSchedule(self, onAwardOrder, doUpdateLater = True):
        ...
        if hasattr(self, 'name'):
            if doUpdateLater and self.air.doLiveUpdates and hasattr(self, 'air'):
                ...
                if nextTime != None:
                    duration = max(10.0, nextTime * 60 - time.time())
                    taskMgr.doMethodLater(duration, self._DistributedToonAI__deliverAwardPurchase, taskName)
    ...
    def d_setAwardNotify(self, awardMailboxNotify):
        self.sendUpdate('setAwardNotify', [
            awardMailboxNotify])
    ...
    staticGetLogicalZoneChangeAllEvent = staticmethod(staticGetLogicalZoneChangeAllEvent)
        ...
        if reason == AV_FLAG_REASON_TOUCH and globalClock.getFrameTime() - DistributedToonAI.lastFlagAvTime > AV_TOUCH_CHECK_DELAY_AI:
            ...
            if av and otherAv and isinstance(av, DistributedToonAI) and isinstance(otherAv, DistributedToonAI) and av.zoneId == otherAv.zoneId and av.zoneId not in MinigameCreatorAI.MinigameZoneRefs:
                ...
                if dist < AV_TOUCH_CHECK_DIST and zDist < AV_TOUCH_CHECK_DIST_Z:
                    ...
                    newAvPos = av.getPos()
                    if newAvPos.getX() > 3000 and newAvPos.getX() < -3000 and newAvPos.getY() > 3000 or newAvPos.getY() < -3000:
                        return None
                    ...
                    prevCount = DistributedToonAI.flagCounts.setdefault(avPairKey, [
                        { },
                    ...
                    if prevCount[0][av.doId] > AV_TOUCH_COUNT_LIMIT:
                        if globalClock.getFrameTime() - prevCount[1] < AV_TOUCH_COUNT_TIME:
                            ...
                            av.handleHacking(response, 'collision and position hacking', [
                                otherAv])
    ...
    def sendPing(self):
        ...
        self.sendUpdateToAvatarId(self.doId, 'ping', [
            val])
        DistributedToonAI.pingedAvs[self.doId] = [
            globalClock.getFrameTime(),
            val]
        delay = simbase.config.GetInt('client-ping-timeout', 150)
        taskMgr.doMethodLater(delay, verify, 'pingverify-' + str(self.doId), extraArgs = [
            self.doId])
    ...
    def startPing(self):
        ...
        taskMgr.doMethodLater(delay, self.requestPing, 'requestping-' + str(self.doId), extraArgs = [
            self.doId])
