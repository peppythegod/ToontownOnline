...
class DistributedNPCTailor(DistributedNPCToonBase):
    ...
    def _DistributedNPCTailor__handleUnexpectedExit(self):
    ...
    def setMovie(self, mode, npcId, avId, timestamp):
        ...
        if mode == NPCToons.PURCHASE_MOVIE_TIMEOUT:
            ...
            if self.clothesGUI:
                ...
                self._DistributedNPCTailor__handlePurchaseDone(timeout = 1)
            
            self.setChatAbsolute(TTLocalizer.STOREOWNER_TOOKTOOLONG, CFSpeech | CFTimeout)
            self.resetTailor()
        elif mode == NPCToons.PURCHASE_MOVIE_START and mode == NPCToons.PURCHASE_MOVIE_START_BROWSE or mode == NPCToons.PURCHASE_MOVIE_START_NOROOM:
            ...
            if self.av is None:
                ...
                self.accept(self.av.uniqueName('disable'), self._DistributedNPCTailor__handleUnexpectedExit)
    ...
    def popupPurchaseGUI(self, task):
        ...
        self.swapEvent = 'swap'
        self.acceptOnce(self.purchaseDoneEvent, self._DistributedNPCTailor__handlePurchaseDone)
        self.accept(self.swapEvent, self._DistributedNPCTailor__handleSwap)
        ...
        if self.browsing == 0:
            self.button = DirectButton(relief = None, image = (self.gui.find('**/CrtAtoon_Btn1_UP'), self.gui.find('**/CrtAtoon_Btn1_DOWN'), self.gui.find('**/CrtAtoon_Btn1_RLLVR')), pos = (-0.14999999999999999, 0, -0.84999999999999998), command = self._DistributedNPCTailor__handleButton, text = ('', TTLocalizer.MakeAToonDone, TTLocalizer.MakeAToonDone), text_font = ToontownGlobals.getInterfaceFont(), text_scale = 0.080000000000000002, text_pos = (0, -0.029999999999999999), text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1))
        else:
            self.button = None
        self.cancelButton = DirectButton(relief = None, image = (self.gui.find('**/CrtAtoon_Btn2_UP'), self.gui.find('**/CrtAtoon_Btn2_DOWN'), self.gui.find('**/CrtAtoon_Btn2_RLLVR')), pos = (0.14999999999999999, 0, -0.84999999999999998), command = self._DistributedNPCTailor__handleCancel, text = ('', TTLocalizer.MakeAToonCancel, TTLocalizer.MakeAToonCancel), text_font = ToontownGlobals.getInterfaceFont(), text_scale = 0.080000000000000002, text_pos = (0, -0.029999999999999999), text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1))
        camera.setPosHpr(base.localAvatar, -4.1600000000000001, 8.25, 2.4700000000000002, -152.88999999999999, 0.0, 0.0)
    ...
    def _DistributedNPCTailor__handleButton(self):
        messenger.send('next')

    
    def _DistributedNPCTailor__handleCancel(self):
        self.clothesGUI.resetClothes(self.oldStyle)
        messenger.send('last')

    
    def _DistributedNPCTailor__handleSwap(self):
        self.d_setDNA(self.av.getStyle().makeNetString(), 0)

    
    def _DistributedNPCTailor__handlePurchaseDone(self, timeout = 0):
            ...
            if self.roomAvailable == 0:
                if self.isLocalToon:
                    if (self.av.isClosetFull() or which & ClosetGlobals.SHIRT) and which & ClosetGlobals.SHORTS:
                        self._DistributedNPCTailor__enterConfirmLoss(2, which)
                        self.clothesGUI.hideButtons()
                        self.button.hide()
    ...
    def _DistributedNPCTailor__enterConfirmLoss(self, finished, which):
        if self.popupInfo == None:
            buttons = loader.loadModel('phase_3/models/gui/dialog_box_buttons_gui')
            okButtonImage = (buttons.find('**/ChtBx_OKBtn_UP'), buttons.find('**/ChtBx_OKBtn_DN'), buttons.find('**/ChtBx_OKBtn_Rllvr'))
            cancelButtonImage = (buttons.find('**/CloseBtn_UP'), buttons.find('**/CloseBtn_DN'), buttons.find('**/CloseBtn_Rllvr'))
            self.popupInfo = DirectFrame(parent = hidden, relief = None, state = 'normal', text = TTLocalizer.STOREOWNER_CONFIRM_LOSS, text_wordwrap = 10, textMayChange = 0, frameSize = (-1, 1, -1, 1), text_pos = (0, -0.050000000000000003), geom = DGG.getDefaultDialogGeom(), geom_color = ToontownGlobals.GlobalDialogColor, geom_scale = (0.88, 1, 0.55000000000000004), geom_pos = (0, 0, -0.17999999999999999), text_scale = 0.080000000000000002)
            DirectButton(self.popupInfo, image = okButtonImage, relief = None, text = TTLocalizer.STOREOWNER_OK, text_scale = 0.050000000000000003, text_pos = (0.0, -0.10000000000000001), textMayChange = 0, pos = (-0.080000000000000002, 0.0, -0.31), command = self._DistributedNPCTailor__handleConfirmLossOK, extraArgs = [
                finished,
                which])
            DirectButton(self.popupInfo, image = cancelButtonImage, relief = None, text = TTLocalizer.STOREOWNER_CANCEL, text_scale = 0.050000000000000003, text_pos = (0.0, -0.10000000000000001), textMayChange = 0, pos = (0.080000000000000002, 0.0, -0.31), command = self._DistributedNPCTailor__handleConfirmLossCancel)
            buttons.removeNode()
        
        self.popupInfo.reparentTo(aspect2d)

    
    def _DistributedNPCTailor__handleConfirmLossOK(self, finished, which):
        self.d_setDNA(self.av.getStyle().makeNetString(), finished, which)
        self.popupInfo.reparentTo(hidden)

    
    def _DistributedNPCTailor__handleConfirmLossCancel(self):
        self.d_setDNA(self.oldStyle.makeNetString(), 1)
        self.popupInfo.reparentTo(hidden)
