...
class DistributedNPCTailor(DistributedNPCToonBase):
        ...
        if mode == NPCToons.PURCHASE_MOVIE_TIMEOUT:
            ...
            if self.av is None:
                ...
                self.accept(
                    ...
                    self._DistributedNPCTailor__handleUnexpectedExit)
    ...
    def popupPurchaseGUI(self, task):
        ...
        self.acceptOnce(self.purchaseDoneEvent,
                        self._DistributedNPCTailor__handlePurchaseDone)
        ...
        if self.browsing == 0:
            self.button = DirectButton(
                ...
                command=self._DistributedNPCTailor__handleButton,
        ...
        self.cancelButton = DirectButton(
            ...
            pos=(0.14999999999999999, 0, -0.84999999999999998),
            command=self._DistributedNPCTailor__handleCancel,
            ...
            text_fg=(1, 1, 1, 1),
            text_shadow=(0, 0, 0, 1))
        camera.setPosHpr(base.localAvatar, -4.1600000000000001, 8.25,
                         2.4700000000000002, -152.88999999999999, 0.0, 0.0)
    ...
    def _DistributedNPCTailor__handlePurchaseDone(self, timeout=0):
        if self.clothesGUI.doneStatus == 'last' or timeout == 1:
            ...
            print 'setDNA: which = %d, top = %d, bot = %d' % (
                which, self.clothesGUI.topChoice, self.clothesGUI.bottomChoice)
            if self.roomAvailable == 0:
                if self.isLocalToon:
                    if (self.av.isClosetFull() or which & ClosetGlobals.SHIRT
                        ...
                        self.d_setDNA(self.av.getStyle().makeNetString(), 2,
                                      which)
    ...
    def _DistributedNPCTailor__enterConfirmLoss(self, finished, which):
        if self.popupInfo == None:
            ...
            self.popupInfo = DirectFrame(
                ...
                geom_pos=(0, 0, -0.17999999999999999),
                text_scale=0.080000000000000002)
            DirectButton(
                self.popupInfo,
                image=okButtonImage,
                ...
                textMayChange=0,
                pos=(-0.080000000000000002, 0.0, -0.31),
                command=self._DistributedNPCTailor__handleConfirmLossOK,
            ...
            DirectButton(
                ...
                textMayChange=0,
                pos=(0.080000000000000002, 0.0, -0.31),
                command=self._DistributedNPCTailor__handleConfirmLossCancel)
            buttons.removeNode()
            ...
            if av:
                if self.av == av:
                    ...
                    if len(oldTorso) == 2 and len(
                            self.av.style.torso
                    ) == 2 and self.av.style.torso[1] != oldTorso[1]:
