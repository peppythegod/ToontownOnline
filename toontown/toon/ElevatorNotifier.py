...
class ElevatorNotifier:
    ...
    def createFrame(self, message, framePos = None, withStopping = True, ttDialog = False):
        ...
        if not ttDialog:
            self.frame = DirectFrame(relief = None, image = DGG.getDefaultDialogGeom(), image_color = ToontownGlobals.GlobalDialogColor, image_scale = (1.0, 1.0, 0.40000000000000002), text = message, text_wordwrap = 16, text_scale = 0.059999999999999998, text_pos = (-0.0, 0.10000000000000001), pos = framePos)
        else:
            self.frame = TTDialog.TTDialog(relief = None, image = DGG.getDefaultDialogGeom(), image_color = ToontownGlobals.GlobalDialogColor, image_scale = (1.0, 1.0, 0.40000000000000002), text = message, text_wordwrap = 16, text_scale = 0.059999999999999998, text_pos = (-0.0, 0.10000000000000001), pos = framePos)
        ...
        self.doneButton = DirectButton(parent = self.frame, relief = None, image = self.cancelImageList, command = self.handleButton, pos = (0, 0, -0.14000000000000001))
        if not withStopping:
            self.doneButton['command'] = self._ElevatorNotifier__handleButtonWithoutStopping
