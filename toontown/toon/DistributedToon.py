...
class DistributedToon(DistributedPlayer.DistributedPlayer, Toon.Toon,
                      DistributedSmoothNode.DistributedSmoothNode,
                      DelayDeletable):
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedToon')
    partyNotify = DirectNotifyGlobal.directNotify.newCategory(
        'DistributedToon_Party')
    ...
    def __init__(self, cr, bFake=False):
        ...
        self._DistributedToon__currentDialogue = None
        self.mail = None
        self.invites = []
        self.hostedParties = []
    ...
    def setDNA(self, dna):
        if base.cr.newsManager:
            if base.cr.newsManager.isHolidayRunning(
                    ToontownGlobals.SPOOKY_BLACK_CAT):
                black = 26
    ...
    def setSCSinging(self, msgIndex):
        self.sendUpdate('logSuspiciousEvent', [
            'invalid msgIndex in setSCSinging: %s from %s' %
            (msgIndex, self.doId)
        ])
    ...
    def setDefaultZone(self, zoneId):
        ...
        if (not base.cr.isPaid()
                or launcher) and not launcher.getPhaseComplete(hoodPhase):
            self.defaultZone = ToontownCentral
        else:
    ...
    def refreshName(self):
        ...
        if hasattr(self, 'nametag') and self.gmNameTagEnabled:
            ...
            taskMgr.add(self._DistributedToon__refreshNameCallBack,
                        self.uniqueName('refreshNameCallBack'))

    def _DistributedToon__starSpin1(self, task):
        now = globalClock.getFrameTime()
        r = now * 90 % 360.0
        self.trophyStar1.setH(r)
        return Task.cont

    def _DistributedToon__refreshNameCallBack(self, task):
    ...
    def setTalk(self, fromAV, fromAC, avatarName, chat, mods, flags):
        ...
        if base.config.GetBool('want-sleep-reply-on-regular-chat', 0):
            if base.localAvatar.sleepFlag == 1:
                self.sendUpdate('setSleepAutoReply', [base.localAvatar.doId],
                                fromAV)

        (newText, scrubbed) = self.scrubTalk(chat, mods)
        self.displayTalk(newText)
        base.talkAssistant.receiveOpenTalk(fromAV, avatarName, fromAC, None,
                                           newText)

    def isAvFriend(self, avId):
        if not base.cr.isFriend(avId):
            pass
        return base.cr.playerFriendsManager.isAvatarOwnerPlayerFriend(avId)
    ...
    def _isValidWhisperSource(self, source):
        if not isinstance(source, FriendHandle.FriendHandle):
            pass
        return isinstance(source, DistributedToon)
    ...
    def setWhisperSCToontaskFrom(self, fromId, taskId, toNpcId, toonProgress,
                                 msgIndex):
    ...
    def setHoodsVisited(self, hoods):
        ...
        if ToontownGlobals.SellbotHQ in hoods and ToontownGlobals.CashbotHQ in hoods or ToontownGlobals.LawbotHQ in hoods:
    ...
    def getTunnelInToonTrack(self, endX, tunnelOrigin):
        ...
        return Sequence(
            ...
            LerpPosInterval(
                self,
                straightLerpDur,
                pos=Point3(endX, endY, 0.10000000000000001),
                other=tunnelOrigin,
    ...
    def handleTunnelIn(self, startTime, endX, x, y, z, h):
        ...
        self.tunnelTrack = Sequence(
            ...
            Func(tunnelOrigin.removeNode), Func(self.startSmooth))
    ...
    def b_setTunnelOut(self, startX, startY, tunnelOrigin):
        ...
        self.d_setTunnelOut(timestamp, startX, startY, pos[0], pos[1], pos[2],
                            h)
    ...
    def getTunnelOutToonTrack(self, startX, startY, tunnelOrigin):
        ...
        def getTargetPos(self=self):
            pos = self.getPos()
            return Point3(
                self.tunnelCenterOffset + (pos[0] - self.tunnelCenterOffset) *
                (1.0 - self.tunnelCenterInfluence), pos[1], pos[2])
        ...
        return Sequence(
            Parallel(
                LerpPosInterval(
                    self,
                    straightLerpDur,
                    pos=Point3(startX, pivotY, 0.10000000000000001),
                    startPos=startPos,
                    other=tunnelOrigin,
                    name=self.uniqueName('tunnelOutStraightLerp')),
                LerpHprInterval(
                    self,
                    straightLerpDur * 0.80000000000000004,
    ...
    def b_setAnimState(self,
        ...
        self.setAnimState(animName, animMultiplier, None, None, callback,
                          extraArgs)

    def d_setAnimState(self,
                       animName,
                       animMultiplier=1.0,
                       timestamp=None,
                       extraArgs=[]):
    ...
    def setGhostMode(self, flag):
        if self.ghostMode != flag:
            ...
            if self.activeState <= DistributedObject.ESDisabled:
                self.notify.debug(
                    'not applying cheesy effect to disabled Toon')
                ...
                self.notify.warning(
                    'unknown activeState: %s' % self.activeState)
            self.showNametag2d()
            self.showNametag3d()
    ...
    def setCatalogSchedule(self, currentWeek, nextTime):
        ...
        if self.isLocal():
            self.notify.debug(
                'setCatalogSchedule(%s, %s)' % (currentWeek, nextTime))
            if nextTime:
                serverTime = time.time() + self.cr.getServerDelta()
                duration = nextTime * 60 - serverTime
                self.notify.debug('next catalog in %s.' %
                                  PythonUtil.formatElapsedSeconds(duration))
    ...
    def setDeliverySchedule(self, onOrder):
        ...
        if self == base.localAvatar:
            ...
            if nextTime != None:
                serverTime = time.time() + self.cr.getServerDelta()
                duration = nextTime * 60 - serverTime
                self.notify.debug('next delivery in %s.' %
                                  PythonUtil.formatElapsedSeconds(duration))
    ...
    def playSplashEffect(self, x, y, z):
        if localAvatar.zoneId not in [
                ToontownGlobals.DonaldsDock, ToontownGlobals.OutdoorZone
        ]:
            if not hasattr(localAvatar,
                           'inEstate') or localAvatar.inEstate != 1:
                if random.random() < 0.10000000000000001:
    ...
    def b_setSpeedChatStyleIndex(self, index):
        realIndexToSend = 0
        if type(index) == type(
                0) and 0 <= index and index < len(speedChatStyles):
    ...
    def setSpeedChatStyleIndex(self, index):
        ...
        (nameKey, arrowColor, rolloverColor,
         frameColor) = speedChatStyles[realIndexToUse]
    ...
    def pieSplat(self, x, y, z, sequence, pieCode, timestamp32):
        ...
        splat = Sequence(
            Func(messenger.send, 'pieSplat', [self, pieCode]), splat)
        if ts > 0:
            startTime = ts
            lastPieTrack.finish()
        else:
    ...
    def setTrophyScore(self, score):
        ...
        if self.trophyScore >= ToontownGlobals.TrophyStarLevels[4]:
            self.trophyStar = loader.loadModel(
                'phase_3.5/models/gui/name_star')
            ...
            if self.trophyScore >= ToontownGlobals.TrophyStarLevels[5]:
                taskMgr.add(self._DistributedToon__starSpin,
                            self.uniqueName('starSpin'))
    ...
    if hasattr(base, 'wantPets') and base.wantPets:
        ...
        def lookupPetDNA(self):
            if self.petId and not (self.petDNA):
                ...
                PetDetail.PetDetail(self.petId,
                                    self._DistributedToon__petDetailsLoaded)
    ...
    def getShovelModel(self):
        ...
        shovel.setH(-90)
        shovel.setP(216)
        shovel.setX(0.20000000000000001)
        shovel.detachNode()
        shovels.removeNode()
        return shovel
    ...
    def getWateringCanModel(self):
        scalePosHprsTable = ((0.25, 0.10000000000000001, 0,
                              0.20000000000000001, -90, -125, -45),
                             (0.20000000000000001, 0.0, 0.25,
                              0.20000000000000001, -90, -125, -45),
                             (0.20000000000000001, 0.20000000000000001,
                              0.10000000000000001, 0.20000000000000001, -90,
                              -125, -45), (0.20000000000000001, 0.0, 0.25,
                                           0.20000000000000001, -90, -125,
                                           -45))
    ...
    def getMyTrees(self):
        treeDict = self.cr.getObjectsOfClass(
    ...
    def setDisplayName(self, str):
        if self.getGameAccess() == OTPGlobals.AccessFull and not (
                self.isDisguised):
    ...
    def setFancyNametag(self, name=None):
        ...
        if self.getNametagStyle() == 100:
            ...
            self.setFont(
                ToontownGlobals.getNametagFont(self.getNametagStyle()))
    ...
    def playCurrentDialogue(self, dialogue, chatFlags, interrupt=1):
        if interrupt and self._DistributedToon__currentDialogue is not None:
            self._DistributedToon__currentDialogue.stop()

        self._DistributedToon__currentDialogue = dialogue
        if dialogue:
            ...
            if self.nametag.getNumChatPages() > 0:
                ...
                self.playDialogueForString(self.nametag.getStompText(),
    ...
    def playDialogue(self, type, length, delay=0.0):
        ...
        if sfxIndex != None and sfxIndex < len(
                dialogueArray) and dialogueArray[sfxIndex] != None:
        ...
        for invite in self.invites:
            ...
            if appendInvite:
                ...
                if appendInvite:
                    ...
                    curDate = base.cr.toontownTimeManager.getCurServerDateTime(
                    ).date()
                    if endDate < curDate:
                        appendInvite = False
    ...
    def setHostedParties(self, hostedParties):
        DistributedToon.partyNotify.debug(
            'setHostedParties called passing in %d parties.' %
            len(hostedParties))
        self.hostedParties = []
    ...
    def setPartiesInvitedTo(self, partiesInvitedTo):
        DistributedToon.partyNotify.debug(
            'setPartiesInvitedTo called passing in %d parties.' %
            len(partiesInvitedTo))
        self.partiesInvitedTo = []
    ...
    def setPartyCanStart(self, partyId):
        ...
        for partyInfo in self.hostedParties:
            if partyInfo.partyId == partyId:
                ...
                if hasattr(
                        self, 'eventsPage'
                ) and base.localAvatar.book.entered and base.localAvatar.book.isOnPage(
                        self.eventsPage) and self.eventsPage.getMode(
                        ) == EventsPage.EventsPage_Host:
    ...
    def setPartyStatus(self, partyId, newStatus):
                ...
                if newStatus == PartyStatus.Started and hasattr(
                    ...
                    if invite:
                        ...
                        if invite.status == InviteStatus.Accepted:
                            displayStr = TTLocalizer.PartyHasStartedAcceptedInvite % TTLocalizer.GetPossesive(
                                name)
                            self.displaySystemClickableWhisper(
                                -1,
                                displayStr,
                            ...
                            displayStr = TTLocalizer.PartyHasStartedNotAcceptedInvite % TTLocalizer.GetPossesive(
                                name)
                            self.setSystemMessage(
                                partyInfo.hostId,
                                displayStr,
        ...
        if not found:
            self.notify.warning(
                "setPartyCanStart can't find partyId=% status=%d" %
                (partyId, newStatus))
    ...
    def scrubTalk(self, message, mods):
        ...
        for word in words:
            if word == '':
                ...
                continue
            if (word[0] == '\x7'
                    or len(word) > 1) and word[0] == '.' and word[1] == '\x7':
    ...
    def toonUp(self, hpGained, hasInteractivePropBonus=False):
        ...
        if hpGained > 0:
            self.showHpText(
                hpGained, hasInteractivePropBonus=hasInteractivePropBonus)
            ...
            if number != 0:
                ...
                seq = Task.sequence(
                    ...
                    self.hpText.lerpColor(
                        ...
                        0.10000000000000001), Task(self.hideHpTextTask))
    ...
    def setGMIcon(self, gmType=None):
        ...
        iconInfo = (('phase_3.5/models/gui/tt_m_gui_gm_toontroop_whistle',
                     '**/*whistleIcon*',
                     4), ('phase_3.5/models/gui/tt_m_gui_gm_toontroop_whistle',
                          '**/*whistleIcon*', 4),
        ...
        self.gmIconInterval = LerpHprInterval(self.gmIcon, 3.0, Point3(
            0, 0, 0), Point3(-360, 0, 0))
        self.gmIconInterval.loop()

    def setGMPartyIcon(self):
        gmType = self._gmType
        iconInfo = ('phase_3.5/models/gui/tt_m_gui_gm_toonResistance_fist',
    ...
    def _startZombieCheck(self):
        ...
        taskMgr.doMethodLater(2.0 + 60.0 * random.random(),
                              self._doZombieCheck,
    ...
    def _doZombieCheck(self, task=None):
        ...
        taskMgr.doMethodLater(60.0, self._doZombieCheck,
                              self._getZombieCheckTaskName())
    ...
    def _zombieCheckResult(self, context, present):
        if context == self._lastZombieContext:
            ...
            if not present:
                self.notify.warning(
                    'hiding av %s because they are not on the district!' %
                    self.doId)
