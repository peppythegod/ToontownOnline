...
class DistributedToon(DistributedPlayer.DistributedPlayer, Toon.Toon, DistributedSmoothNode.DistributedSmoothNode, DelayDeletable):
    ...
    def setBattleId(self, battleId):
        ...
        messenger.send('ToonBattleIdUpdate', [
            self.doId])
    ...
    def refreshName(self):
        ...
        if hasattr(self, 'nametag') and self.gmNameTagEnabled:
            ...
            taskMgr.add(self._DistributedToon__refreshNameCallBack, self.uniqueName('refreshNameCallBack'))
    ...
    def setTalk(self, fromAV, fromAC, avatarName, chat, mods, flags):
        ...
        if base.config.GetBool('want-sleep-reply-on-regular-chat', 0):
            if base.localAvatar.sleepFlag == 1:
                self.sendUpdate('setSleepAutoReply', [
                    base.localAvatar.doId], fromAV)
    ...
    def setAnimState(self, animName, animMultiplier = 1.0, timestamp = None, animType = None, callback = None, extraArgs = []):
        ...
        if base.config.GetBool('check-invalid-anims', True):
            if animMultiplier > 1.0 and animName in [
                'neutral']:
                animMultiplier = 1.0
    ...
    def d_requestDeleteQuest(self, questDesc):
        self.sendUpdate('requestDeleteQuest', [
            list(questDesc)])
    ...
    def playSplashEffect(self, x, y, z):
        if localAvatar.zoneId not in [
            ...
            if not hasattr(localAvatar, 'inEstate') or localAvatar.inEstate != 1:
    ...
    def setEmblems(self, emblems):
        if self.emblems != emblems:
            ...
            messenger.send(self.uniqueName('emblemsChange'), [
                self.emblems])
    ...
    if base.wantKarts:
        ...
        def requestKartDNAFieldUpdate(self, dnaField, fieldValue):
            ...
            self.sendUpdate('updateKartDNAField', [
                dnaField,
                fieldValue])
        ...
        def requestAddOwnedAccessory(self, accessoryId):
            ...
            self.sendUpdate('addOwnedAccessory', [
                accessoryId])
        ...
        def requestRemoveOwnedAccessory(self, accessoryId):
            ...
            self.sendUpdate('removeOwnedAccessory', [
                accessoryId])
    ...
    def d_reqCogSummons(self, type, suitIndex):
        ...
        if type == 'invasion':
            pass
        1
        self.sendUpdate('reqCogSummons', [
            type,
            suitIndex])

    
    def cogSummonsResponse(self, returnCode, suitIndex, doId):
        messenger.send('cog-summons-response', [
            returnCode,
            suitIndex,
            doId])
    ...
    def getWateringCanModel(self):
        scalePosHprsTable = ((0.25, 0.10000000000000001, 0, 0.20000000000000001, -90, -125, -45), (0.20000000000000001, 0.0, 0.25, 0.20000000000000001, -90, -125, -45), (0.20000000000000001, 0.20000000000000001, 0.10000000000000001, 0.20000000000000001, -90, -125, -45), (0.20000000000000001, 0.0, 0.25, 0.20000000000000001, -90, -125, -45))
    ...
    def useSpecialResponse(self, returnCode):
        messenger.send('use-special-response', [
            returnCode])
    ...
    def showHpText(self, number, bonus = 0, scale = 1, hasInteractivePropBonus = False):
        if self.HpTextEnabled and not (self.ghostMode):
            if number != 0:
                ...
                seq = Task.sequence(self.hpText.lerpPos(Point3(0, 0, self.height + 1.5), 1.0, blendType = 'easeOut'), Task.pause(0.84999999999999998), self.hpText.lerpColor(Vec4(r, g, b, a), Vec4(r, g, b, 0), 0.10000000000000001), Task(self.hideHpTextTask))
    ...
    def setGMIcon(self, gmType = None):
        ...
        iconInfo = (('phase_3.5/models/gui/tt_m_gui_gm_toontroop_whistle', '**/*whistleIcon*', 4), ('phase_3.5/models/gui/tt_m_gui_gm_toontroop_whistle', '**/*whistleIcon*', 4), ('phase_3.5/models/gui/tt_m_gui_gm_toonResistance_fist', '**/*fistIcon*', 4), ('phase_3.5/models/gui/tt_m_gui_gm_toontroop_getConnected', '**/*whistleIcon*', 4))
