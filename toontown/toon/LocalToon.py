...
class LocalToon(DistributedToon.DistributedToon, LocalAvatar.LocalAvatar):
    ...
    def __init__(self, cr):
        ...
        try:
            ...
            if Settings.getAcceptingNewFriends():
                pass
            self.acceptingNewFriends = base.config.GetBool('accepting-new-friends-default', True)
            if Settings.getAcceptingNonFriendWhispers():
                pass
            self.acceptingNonFriendWhispers = base.config.GetBool('accepting-non-friend-whispers-default', True)
    ...
    def _tmdcc(self, toon, checks = []):
        result = None
        if isinstance(toon, DistributedNPCToonBase) and toon is localAvatar and toon.isEmpty() and toon.bFake or toon._delayDeleteForceAllow:
            return result
        ...
        def sendT(header, msg, sToon, sendFooter = False, sendLs = True):
            ...
            def sendAsParts(message, counter):
                for currBlock in range(0, len(message) / msgSize + 1):
                    ...
                    taskMgr.doMethodLater(0.080000000000000002 * (currBlock + counter), delayedSend, uname + str(currBlock + counter), extraArgs = [
                        sToon,
                        fmsg])
            ...
            if sendFooter:
                ...
                if hasattr(sToon, 'suitGeom'):
                    pass
                footer = 'loc: %s dna: %s gmname: %s ntag: %s ceffect: %s disguise: %s sstyle: %s sgeom: %s %s' % (str(sToon.getLocation()), str(sToon.style.asTuple()), sToon.gmNameTagEnabled, str(sToon.nametag.getContents()), str(sToon.cheesyEffect), sToon.isDisguised, str(sToon.suit.style), sstream.getData(), bs)
                currCounter = sendAsParts(footer, currCounter)
            

        self.sendUpdate('requestPing', [
            toon.doId])
        if not checks:
            numChecks = 6
            checks = [
                random.choice(range(1, numChecks + 1))]
        ...
        def hacker_detect_immediate(cbdata):
            ...
            node = cbdata.getNode()
            np = NodePath(node)
            if not (self.cr) and not (self.cr.distributedDistrict) or not self.cr.distributedDistrict.getAllowAHNNLog():
        ...
        if 6 in checks:
            if base.config.GetBool('tmdcc-colorcheck', 1):
                ...
                if (filter(lambda x: if x.hasColor():
passx.getColor() not in ToonDNA.allowedColors, torsoPieces) and filter(lambda x: if x.hasColor():
passx.getColor() not in ToonDNA.allowedColors, legPieces) or filter(lambda x: if x.hasColor():
passx.getColor() not in ToonDNA.allowedColors, headPieces)) and toon.cheesyEffect == ToontownGlobals.CENormal:
                    torsoColors = str(map(lambda x: if not not x.hasColor() or 'clear':
if not x.getColor() in ToonDNA.allowedColors or 'ok':
passx.getColor(), torsoPieces))
                    legColors = str(map(lambda x: if not not x.hasColor() or 'clear':
if not x.getColor() in ToonDNA.allowedColors or 'ok':
passx.getColor(), legPieces))
                    headColors = str(map(lambda x: if not not x.hasColor() or 'clear':
if not x.getColor() in ToonDNA.allowedColors or 'ok':
passx.getColor(), headPieces))
    ...
    def initInterface(self):
        ...
        if not base.cr.isPaid():
            ...
            self.purchaseButton = DirectButton(parent = aspect2d, relief = None, image = (guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale = 0.90000000000000002, text = TTLocalizer.OptionsPagePurchase, text_scale = 0.050000000000000003, text_pos = (0, -0.01), textMayChange = 0, pos = (0.88500000000000001, 0, -0.93999999999999995), sortOrder = 100, command = self._LocalToon__handlePurchase)
    ...
    def makePiePowerMeter(self):
        ...
        if self._LocalToon__piePowerMeter == None:
            self._LocalToon__piePowerMeter = DirectWaitBar(frameSize = (-0.20000000000000001, 0.20000000000000001, -0.029999999999999999, 0.029999999999999999), relief = DGG.SUNKEN, borderWidth = (0.0050000000000000001, 0.0050000000000000001), barColor = (0.40000000000000002, 0.59999999999999998, 1.0, 1), pos = (0, 0.10000000000000001, 0.80000000000000004))
            self._LocalToon__piePowerMeter.hide()
    ...
    def updatePieButton(self):
        ...
        if self._LocalToon__pieButton == None:
            ...
            self._LocalToon__pieButton = DirectButton(image = (inv.upButton, inv.downButton, inv.rolloverButton), geom = pieGui, text = '50', text_scale = 0.040000000000000001, text_align = TextNode.ARight, geom_scale = pieScale, geom_pos = (-0.01, 0, 0), text_fg = Vec4(1, 1, 1, 1), text_pos = (0.070000000000000007, -0.040000000000000001), relief = None, image_color = (0, 0.59999999999999998, 1, 1), pos = (0, 0.10000000000000001, 0.90000000000000002))
            self._LocalToon__pieButton.bind(DGG.B1PRESS, self._LocalToon__beginTossPieMouse)
            self._LocalToon__pieButton.bind(DGG.B1RELEASE, self._LocalToon__endTossPieMouse)
    ...
    def showClarabelleGui(self, mailboxItems):
        ...
        if mailboxItems:
            ...
            self._LocalToon__clarabelleButton['text'] = [
                ...
                TTLocalizer.CatalogNewCatalogButton]
        if not (self.mailboxNotify) and not (self.awardNotify) and self.catalogNotify == ToontownGlobals.OldItems:
    ...
    def loadGardeningGui(self):
        ...
        self._LocalToon__gardeningGui = DirectFrame(relief = None, geom = gardenGuiCard, geom_color = GlobalDialogColor, geom_scale = (0.17000000000000001, 1.0, 0.29999999999999999), pos = (-1.2, 0, 0.5), scale = 1.0)
        self._LocalToon__gardeningGui.setName('gardeningFrame')
        self._LocalToon__gardeningGuiFake = DirectFrame(relief = None, geom = None, geom_color = GlobalDialogColor, geom_scale = (0.17000000000000001, 1.0, 0.29999999999999999), pos = (-1.2, 0, 0.5), scale = 1.0)
        self._LocalToon__gardeningGuiFake.setName('gardeningFrameFake')
        iconScale = 1
        iconColorWhite = Vec4(1.0, 1.0, 1.0, 1.0)
        iconColorGrey = Vec4(0.69999999999999996, 0.69999999999999996, 0.69999999999999996, 1.0)
        iconColorBrown = Vec4(0.69999999999999996, 0.40000000000000002, 0.29999999999999999, 1.0)
        iconColorBlue = Vec4(0.20000000000000001, 0.29999999999999999, 1.0, 1.0)
        ...
        self._LocalToon__shovelButtonFake = DirectLabel(parent = self._LocalToon__gardeningGuiFake, relief = None, text = self.shovelText, text_align = TextNode.ALeft, text_pos = (0.0, -0.0), text_scale = 0.070000000000000007, text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1), image_scale = (0.17999999999999999, 1.0, 0.35999999999999999), geom = None, geom_scale = iconScale, geom_color = iconColorWhite, pos = (0.14999999999999999, 0, 0.20000000000000001), scale = 0.77500000000000002)
        self.shovelButtonFake = self._LocalToon__shovelButtonFake
        self.shovelText = ('', '', buttonText, '')
        self._LocalToon__shovelButton = DirectButton(parent = self._LocalToon__gardeningGui, relief = None, text = self.shovelText, text_align = TextNode.ACenter, text_pos = (0.0, -0.0), text_scale = 0.10000000000000001, text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1), image = (shovelCardP, shovelCardY, shovelCardY, shovelCardY), image_scale = (0.17999999999999999, 1.0, 0.35999999999999999), geom = None, geom_scale = iconScale, geom_color = iconColorWhite, pos = (0, 0, 0.20000000000000001), scale = 0.77500000000000002, command = self._LocalToon__shovelButtonClicked)
        ...
        self._LocalToon__wateringCanButtonFake = DirectLabel(parent = self._LocalToon__gardeningGuiFake, relief = None, text = self.waterText, text_align = TextNode.ALeft, text_pos = (0.0, -0.0), text_scale = 0.070000000000000007, text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1), image_scale = (0.17999999999999999, 1.0, 0.35999999999999999), geom = None, geom_scale = iconScale, geom_color = iconColorWhite, pos = (0.14999999999999999, 0, 0.01), scale = 0.77500000000000002)
        self.wateringCanButtonFake = self._LocalToon__wateringCanButtonFake
        self._LocalToon__wateringCanButton = DirectButton(parent = self._LocalToon__gardeningGui, relief = None, text = self.waterText, text_align = TextNode.ACenter, text_pos = (0.0, -0.0), text_scale = 0.10000000000000001, text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1), image = (wateringCanCardP, wateringCanCardY, wateringCanCardY, wateringCanCardY), image_scale = (0.17999999999999999, 1.0, 0.35999999999999999), geom = None, geom_scale = iconScale, geom_color = iconColorWhite, pos = (0, 0, 0.01), scale = 0.77500000000000002, command = self._LocalToon__wateringCanButtonClicked)
        self.wateringCanButton = self._LocalToon__wateringCanButton
        self.basketText = '%s / %s' % (self.numFlowers, self.maxFlowerBasket)
        self.basketButton = DirectLabel(parent = self._LocalToon__gardeningGui, relief = None, text = self.basketText, text_align = TextNode.ALeft, text_pos = (0.81999999999999995, -1.3999999999999999), text_scale = 0.20000000000000001, text_fg = (1, 1, 1, 1), text_shadow = (0, 0, 0, 1), image = None, image_scale = iconScale, geom = None, geom_scale = iconScale, geom_color = iconColorWhite, pos = (-0.34000000000000002, 0, 0.16), scale = 0.29999999999999999, textMayChange = 1)
    ...
    def verboseState(self):
        ...
        taskMgr.add(self._LocalToon__expressState, 'expressState', extraArgs = [])
        ...
        if plant:
            ...
            if not plant.canBePicked():
                ...
                if plant.getShovelCommand():
                    ...
                    self._LocalToon__shovelButton['command'] = self._LocalToon__shovelButtonClicked
    ...
    def lockGardeningButtons(self, task = None):
        ...
        self.accept('endPlantInteraction', self._LocalToon__handleEndPlantInteraction)
    ...
    def promoteShovelSkill(self, shovelLevel, shovelSkill):
        ...
        if doPartyBall:
            go = Fanfare.makeFanfareWithMessageImage(0, base.localAvatar, 1, message, Vec2(0, 0.20000000000000001), 0.080000000000000002, image, Vec3(0, 0, -0.10000000000000001), Vec3(0.34999999999999998, 0, 0.69999999999999996), wordwrap = 23)
    ...
    def promoteWateringCan(self, wateringCanlevel = 0):
        ...
        if wateringCanlevel >= GardenGlobals.MAX_WATERING_CANS - 1:
            go = Fanfare.makeFanfareWithMessageImage(0, base.localAvatar, 1, message, Vec2(0, 0.20000000000000001), 0.080000000000000002, image, Vec3(0, 0, -0.10000000000000001), Vec3(0.34999999999999998, 0, 0.69999999999999996))
    ...
    def swimTimeoutAction(self):
        ...
        self.b_setAnimState('TeleportOut', 1, self._LocalToon__handleSwimExitTeleport, [
            0])
        return Task.done
            ...
            if maze:
                if kindOfCheat == 0:
                    ...
                    for joke in maze.game.pickups:
                        maze.sendUpdate('requestPickUp', [
                            joke.serialNum])
                    
                
            
        else:
            self.sendUpdate('logSuspiciousEvent', [
                'cheatCogdoMazeGame'])
