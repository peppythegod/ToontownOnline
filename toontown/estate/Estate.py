...
class Estate(Place.Place):
    ...
    def load(self):
        ...
        taskMgr.add(
            self._Estate__checkCameraUnderwater,
    ...
    def startAprilFoolsControls(self):
        if isinstance(
                base.localAvatar.controlManager.currentControls,
                GravityWalker):
            base.localAvatar.controlManager.currentControls.setGravity(
                32.173999999999999 * 0.75)
    ...
    def exitPetTutorial(self):
        ...
        if hasattr(self, 'fsm'):
            taskMgr.add(
                self._Estate__checkToonUnderwater,
    ...
    def enterMailbox(self):
        ...
        base.localAvatar.startSleepWatch(
            self._Estate__handleFallingAsleepMailbox)
        self.enablePeriodTimer()

    def _Estate__handleFallingAsleepMailbox(self, arg):
        if hasattr(self, 'fsm'):
            self.fsm.request('walk')
    ...
    def enterTeleportOut(self, requestStatus):
        Place.Place.enterTeleportOut(
            self, requestStatus, self._Estate__teleportOutDone)
    ...
    def _Estate__submergeToon(self):
        ...
        base.localAvatar.d_playSplashEffect(
            pos[0], pos[1], -2.2999999999999998)
        self.toonSubmerged = 1
