...
class DistributedBank(DistributedFurnitureItem.DistributedFurnitureItem):
    ...
    def announceGenerate(self):
        ...
        DistributedFurnitureItem.DistributedFurnitureItem.announceGenerate(
            self)
        self.accept(self.bankSphereEnterEvent,
                    self._DistributedBank__handleEnterSphere)

    def loadModel(self):
        model = DistributedFurnitureItem.DistributedFurnitureItem.loadModel(
            self)
    ...
    def disable(self):
        ...
        self._DistributedBank__removeToonJar()
        self.ignoreAll()
        ...
        if mode == BANK_MOVIE_CLEAR:
            ...
            if isLocalToon:
                self.bankDialog = TTDialog.TTDialog(
                    ...
                    text_wordwrap=15,
                    fadeScreen=1,
                    command=self._DistributedBank__clearDialog)
    ...
    def _DistributedBank__attachToonJar(self, toon):
        self._DistributedBank__removeToonJar()
        for hand in toon.getRightHands():
            self.jarLods.append(toon.jar.instanceTo(hand))

    def _DistributedBank__removeToonJar(self):
