...
class GardenDropGame(DirectObject.DirectObject):
    def __init__(self):
        ...
        self.minZ = -0.10000000000000001
        self.newBallX = 0.0
        self.newBallZ = 0.59999999999999998
        ...
        for countX in range(self.gridDimX):
            ...
            for countZ in range(self.gridDimZ):
                ...
                newRow.append([
                    None, countX * size + self.minX + offset,
        ...
        for countX in range(self.gridDimX):
            for countZ in range(self.gridDimZ):
                ...
                if self.grid[countX][countZ][
                    ...
                    if force or self.hasNeighbor(countX, countZ):
    ...
    def fillMatchList(self, cellX, cellZ):
        ...
        elif self.getColorType(cellX - 1, cellZ) == colorType:
            self.fillMatchList(cellX - 1, cellZ)
    ...
    def doOnClearGrid(self):
        secondSprite = self.addSprite(
            self.block, posX=self.newBallX, posZ=0.0, found=1)
    ...
    def load(self):
        ...
        self.itemBoard = DirectFrame(
            ...
            image=itemBoard,
            image_scale=0.050000000000000003,
            image_color=(0.92200000000000004, 0.92200000000000004, 0.753, 1),
            relief=None,
            pos=(0, 1, 0))
        ...
        self.quitButton = DirectButton(
            ...
            pos=(0.5, 1.0, -0.41999999999999998),
            ...
            text_pos=(0, -0.01),
            command=self._GardenDropGame__handleExit)
    ...
    def _GardenDropGame__handleExit(self):
        self._GardenDropGame__acceptExit()

    def _GardenDropGame__acceptExit(self, buttonValue=None):
    ...
    def _GardenDropGame__run(self, cont=1):
        ...
        for sprite in self.sprites:
            ...
            if sprite.isActive:
                sprite.addForce(timeDelta * 0.90000000000000002, pi * 1.5)
                continue

        self.queBall.velX = (
            self.queBall.getX() - self.queBall.prevX) / timeDelta
        self.queBall.velZ = (
            self.queBall.getZ() - self.queBall.prevZ) / timeDelta
        self._GardenDropGame__colTest()
        for sprite in self.sprites:
    ...
    def _GardenDropGame__collide(self, move, test):
        queHit = 0
        if move.isQue:
            que = move
            hit = test
    ...
    def push(self, move, test):
        ...
        if queHit:
            ...
            if abs(dist) < self.queExtent * que.size and abs(dist) > 0:
                ...
                force = ((scaleSize - distFromPara) / scaleSize) * (
                    dist / abs(dist))
