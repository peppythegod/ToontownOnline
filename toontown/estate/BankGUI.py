...
class BankGui(DirectFrame):
    ...
    def __init__(self, doneEvent, allowWithdraw=1):
        ...
        self.okButton = DirectButton(
            ...
            text_pos=(
                0,
                -0.10000000000000001),
            command=self._BankGui__requestTransaction)
        ...
        self.bankDisplay = DirectLabel(
            parent=self, relief=None, pos=(
                0.40000000000000002, 0, 0), scale=0.90000000000000002, text=str(
                base.localAvatar.getBankMoney()), text_scale=0.20000000000000001, text_fg=(
                0.94999999999999996, 0.94999999999999996, 0, 1), text_shadow=(
                    0, 0, 0, 1), text_pos=(
                        0, -0.10000000000000001, 0), geom=bankModel, geom_scale=0.080000000000000002, geom_pos=(
                            0, 10, -0.26000000000000001), geom_hpr=(
                                0, 0, 0), text_font=ToontownGlobals.getSignFont())
        self.depositArrow = DirectButton(
            parent=self, relief=None, image=arrowImageList, image_scale=(
                1, 1, 1), image3_color=Vec4(
                0.59999999999999998, 0.59999999999999998, 0.59999999999999998, 0.25), pos=(
                0.01, 0, 0.14999999999999999))
        self.withdrawArrow = DirectButton(parent=self,
                                          ...
                                          pos=(-0.01,
                                               0,
                                               -0.14999999999999999))
        self.depositArrow.bind(DGG.B1PRESS, self._BankGui__depositButtonDown)
        self.depositArrow.bind(DGG.B1RELEASE, self._BankGui__depositButtonUp)
        self.withdrawArrow.bind(DGG.B1PRESS, self._BankGui__withdrawButtonDown)
        self.withdrawArrow.bind(DGG.B1RELEASE, self._BankGui__withdrawButtonUp)
        self.accept('bankAsleep', self._BankGui__cancel)
        self.accept(
            localAvatar.uniqueName('moneyChange'),
            self._BankGui__moneyChange)
        self.accept(
            localAvatar.uniqueName('bankMoneyChange'),
            self._BankGui__bankMoneyChange)
        if allowWithdraw:
    ...
    def _BankGui__updateTransaction(self, amount):
        ...
        self._BankGui__transactionAmount = min(
            self._BankGui__transactionAmount, jarMoney)
        self._BankGui__transactionAmount = min(
            self._BankGui__transactionAmount,
            maxBankMoney - bankMoney)
        self._BankGui__transactionAmount = - \
            min(-(self._BankGui__transactionAmount), maxJarMoney - jarMoney)
        self._BankGui__transactionAmount = - \
            min(-(self._BankGui__transactionAmount), bankMoney)
