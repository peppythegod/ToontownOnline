...
class DistributedGardenPlot(DistributedLawnDecor.DistributedLawnDecor):
    ...
    def _DistributedGardenPlot__handleFlowerPlantingDone(
        ...
        if willPlant:
            ...
            if recipeKey >= 0:
                ...
                self.notify.debug(
                    '%s %d is not a valid recipe' % (recipeStr, special))
    ...
    def _DistributedGardenPlot__handleTreePlantingDone(self,
                                                       willPlant=False,
                                                       gagTrack=None,
                                                       gagLevel=None):
        self.ignore(self.plantingGuiDoneEvent)
    ...
    def doPlaceItemTrack(self, avId, item=None):
        ...
        self.movie = Sequence(
            self.startCamIval(avId), moveTrack, Func(shovel.show),
            placeItemTrack)
        if avId == localAvatar.doId:
        ...
        if item:
            placeItemTrack.append(
                Sequence(
                    ...
                    Wait(0.55000000000000004), Func(item.wrtReparentTo,
                                                    render),
                    Parallel(
                        LerpHprInterval(
                            item, hpr=self.getHpr(render), duration=1.2),
                        ProjectileInterval(
                            item,
