...
class DistributedTrunk(DistributedCloset.DistributedCloset):
    ...
    def load(self):
        ...
        if not lNode.isEmpty():
            self.scale = lLid.getScale()[0] * 0.59999999999999998
    ...
    def _DistributedTrunk__handleDelete(self, which):
    ...
    def _DistributedTrunk__proceedToCheckout(self):
        if self.hatDeleted and self.glassesDeleted and self.backpackDeleted or self.shoesDeleted:
            self._DistributedTrunk__popupAreYouSurePanel()
        else:
            self._handlePurchaseDone()
    ...
    def _DistributedTrunk__popupNotOwnerPanel(self):
        if self.popupInfo is not None:
            self.popupInfo.destroy()
            self.popupInfo = None
    ...
    def _DistributedTrunk__popupAreYouSurePanel(self):
        if self.popupInfo is not None:
            self.popupInfo.destroy()
            self.popupInfo = None
