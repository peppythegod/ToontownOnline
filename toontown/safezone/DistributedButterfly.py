...
class DistributedButterfly(DistributedObject.DistributedObject):
    ...
    def generate(self):
        ...
        for type in self.wingTypes:
            ...
            if index == 0 or index == 1:
                ...
                color = self.paleYellowColors[self.doId % len(
                    self.paleYellowColors)]
            else:
                color = Vec4(1, 1, 1, 1)
            wing.setColor(color)
    ...
    def _DistributedButterfly__detectAvatars(self):
        self.accept('enter' + self.cSphereNode.getName(),
                    self._DistributedButterfly__handleCollisionSphereEnter)
    ...
    def enterFlying(self, ts):
        ...
        if ts <= self.time:
            flyTime = self.time - \
                ButterflyGlobals.BUTTERFLY_TAKEOFF[self.playground] + ButterflyGlobals.BUTTERFLY_LANDING[self.playground]
            ...
            takeoffShadowT = 0.20000000000000001 * \
