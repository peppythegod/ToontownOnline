...
class DistributedTrolley(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        self._DistributedTrolley__toonTracks = {}
    ...
    def generate(self):
        ...
        self.trolleySphereNode = self.trolleyStation.find(
            '**/trolley_sphere').node()
    ...
    def storeToonTrack(self, avId, track):
        ...
        self._DistributedTrolley__toonTracks[avId] = track

    def clearToonTrack(self, avId):
        oldTrack = self._DistributedTrolley__toonTracks.get(avId)
        if oldTrack:
            oldTrack.pause()
            DelayDelete.cleanupDelayDeletes(oldTrack)
            del self._DistributedTrolley__toonTracks[avId]

    def clearToonTracks(self):
        keyList = []
        for key in self._DistributedTrolley__toonTracks:
            keyList.append(key)

        for key in keyList:
            if key in self._DistributedTrolley__toonTracks:
