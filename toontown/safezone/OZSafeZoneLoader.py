...
class OZSafeZoneLoader(SafeZoneLoader):
    ...
    def load(self):
        ...
        if self.geyserModel:
            ...
            self.geyserActor.setPlayRate(8.5999999999999996, 'idle')
            ...
            self.geyserPoolSfx = loader.loadSfx(
                'phase_6/audio/sfx/OZ_Geyser_BuildUp_Loop.wav')
    ...
    def startGeyser(self, task=None):
        if hasattr(base.cr, 'DTimer') and base.cr.DTimer:
            ...
            taskMgr.doMethodLater(timeToNextGeyser, self.doGeyser,
                                  'geyser Task')
        else:
    ...
    def setGeyserAnim(self, task=None):
        ...
        maxSize = 0.40000000000000002 * random.random() + 0.75
        time = 1.0
        ...
        for av in avList:
            ...
            if distance < 7.0:
                ...
                animTrack.append(
                    ActorInterval(
                        av, 'jump-idle', loop=1, endTime=11.5 * time))
                animTrack.append(
        ...
        self.geyserTrack.append(
            Parallel(
                ...
                LerpPosInterval(
                    self.geyserActor, time, pos=upPos, startPos=downPos)))
        self.geyserTrack.append(
            Parallel(
                LerpScaleInterval(self.geyserActor, 2.0 * time, 0.75, maxSize),
                LerpPosInterval(
                    self.geyserActor, 2.0 * time, pos=downPos,
                    startPos=upPos)))
        self.geyserTrack.append(
            Parallel(
                LerpScaleInterval(self.geyserActor, time, maxSize, 0.75),
        ...
        self.geyserTrack.append(
            Parallel(
                ...
                LerpPosInterval(
                    self.geyserActor, 4.0 * time, pos=downPos,
                    startPos=upPos)))
        self.geyserTrack.append(
            Func(self.geyserNodePath.setPos, self.geyserPos[0],
    ...
    def enterGolfCourse(self, requestStatus):
        ...
        base.transitions.irisOut(t=0.20000000000000001)
    ...
    def storeToonTrack(self, avId, track):
        ...
        self._OZSafeZoneLoader__toonTracks[avId] = track
    ...
    def clearToonTracks(self):
        keyList = []
        for key in self._OZSafeZoneLoader__toonTracks:
            keyList.append(key)

        for key in keyList:
            if key in self._OZSafeZoneLoader__toonTracks:
