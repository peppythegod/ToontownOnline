...
class DistributedFindFourAI(DistributedNodeAI):
    ...
    def requestWin(self, pieceNum):
        ...
        if self.checkWin(x, y, playerNum):
            self.sendUpdate('announceWinnerPosition',
                            [x, y, self.winDirection, playerNum])
            winnersSequence = Sequence(
                Wait(5.0), Func(self.fsm.request, 'gameOver'),
    ...
    def requestMove(self, moveColumn):
        ...
        if self.checkForTie():
            self.sendUpdate('setGameState',
                            [self.board, moveColumn, movePos, turn])
            self.sendUpdate('tie', [])
            winnersSequence = Sequence(
                Wait(8.0), Func(self.fsm.request, 'gameOver'))
            winnersSequence.start()
            return None
    ...
    def tempSetBoardState(self):
        self.board = [[0, 0, 0, 0, 0, 0, 0], [1, 2, 1, 2, 2, 2, 1],
                      [2, 2, 1, 2, 1, 2, 1], [2, 1, 1, 2, 2, 1, 2],
