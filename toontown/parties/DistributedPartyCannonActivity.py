...
class DistributedPartyCannonActivity(DistributedPartyActivity):
    ...
    def load(self):
        ...
        self.accept(
            FireworksStartedEvent,
            self._DistributedPartyCannonActivity__handleFireworksStarted)
        self.accept(
            FireworksFinishedEvent,
            self._DistributedPartyCannonActivity__handleFireworksFinished)
    ...
    def unload(self):
        ...
        if self.localFlyingToon:
            self._DistributedPartyCannonActivity__resetToon(
                self.localFlyingToon)
    ...
    def disable(self):
        ...
        if self.localFlyingToonId:
            self._DistributedPartyCannonActivity__stopCollisionHandler(
                self.localFlyingToon)
            self._DistributedPartyCannonActivity__stopLocalFlyTask(
    ...
    def _DistributedPartyCannonActivity__handleAvatarGone(self):
    ...
    def setCannonWillFire(self, cannonId, zRot, angle):
        ...
        task = Task(self._DistributedPartyCannonActivity__fireCannonTask)
        task.toonId = toonId
        task.cannon = cannon
        taskMgr.add(task, self.taskNameFireCannon)
        self.toonIds.append(toonId)

    def _DistributedPartyCannonActivity__fireCannonTask(self, task):
        launchTime = 0.0
        toonId = task.toonId
        cannon = task.cannon
        toon = cannon.getToonInside()
        self.notify.debug(str(self.doId) +
                          ' FIRING CANNON FOR TOON ' +
                          str(toonId))
        ...
        if self.isLocalToonId(toonId):
            self.inWater = 0
            flightResults = self._DistributedPartyCannonActivity__calcFlightResults(
        ...
        shootTask = Task(
            self._DistributedPartyCannonActivity__shootTask,
            self.taskNameShoot)
        shootTask.info = {
            'toonId': toonId,
            'cannon': cannon}
        if self.isLocalToonId(toonId):
            ...
            flyTask = Task(
                self._DistributedPartyCannonActivity__localFlyTask,
                self.taskNameFly)
            flyTask.info = info
            seqTask = Task.sequence(shootTask, flyTask)
            self._DistributedPartyCannonActivity__startCollisionHandler()
    ...
    def setToonTrajectory(
        ...
        startT = globalClockDelta.networkToLocalTime(
            launchTime, bits=31) + 0.20000000000000001
    ...
    def _DistributedPartyCannonActivity__calcFlightResults(
            self, cannon, toonId, launchTime):
    ...
    def _DistributedPartyCannonActivity__shootTask(self, task):
        ...
        if toon:
            ...
            self.notify.debug(
                '__shootTask avoided a crash, toon %d not found' %
                toonId)
    ...
    def landToon(self, toonId):
        ...
        if toon is not None:
            ...
            self._DistributedPartyCannonActivity__setToonUpright(toon)
            toon.setPlayRate(1.0, 'run')
            toon.startSmooth()
            toon.setScale(1.0)
            self.ignore(toon.uniqueName('disable'))
            self._DistributedPartyCannonActivity__cleanupFlyingToonData(toon)
        ...
        if toon is not None and toon == base.localAvatar:
            self._DistributedPartyCannonActivity__localDisplayLandedResults()

    def _DistributedPartyCannonActivity__localDisplayLandedResults(self):
        if self.flyingToonCloudsHit > 0:
            self._doneCannons = True
        else:
            self._DistributedPartyCannonActivity__localToonDoneLanding()
    ...
    def _DistributedPartyCannonActivity__localToonDoneLanding(self):
    ...
    def _DistributedPartyCannonActivity__setToonUpright(self, toon, pos=None):
        if toon:
            if self.inWater:
                toon.setP(0)
                toon.setR(0)
            ...
            toon.loop('neutral')
            if self.localFiringCannon and hasattr(
                    self.localFiringCannon, 'cannonNode'):
    ...
    def _DistributedPartyCannonActivity__resetToonToCannon(self, avatar):
        ...
        if avatar:
            self._DistributedPartyCannonActivity__resetToon(avatar)

    def _DistributedPartyCannonActivity__resetToon(self, avatar, pos=None):
        self.notify.debug('__resetToon')
        if avatar:
            self._DistributedPartyCannonActivity__stopCollisionHandler(avatar)
            self._DistributedPartyCannonActivity__setToonUpright(avatar, pos)
    ...
    def _DistributedPartyCannonActivity__updateFlightVelocity(
            self, trajectory):
    ...
    def _DistributedPartyCannonActivity__isFlightKeyPressed(self):
        if not self.gui.leftPressed and self.gui.rightPressed and self.gui.upPressed:
            pass
        return self.gui.downPressed

    def _DistributedPartyCannonActivity__moveFlyingToon(self, toon):
    ...
    def _DistributedPartyCannonActivity__stopLocalFlyTask(self, toonId):
        taskMgr.remove(self.taskName('flyingToon') + '-' + str(toonId))
        self.gui.disableAimKeys()

    def _DistributedPartyCannonActivity__localFlyTask(self, task):
        toon = task.info['toon']
        if toon.isEmpty():
            self._DistributedPartyCannonActivity__resetToonToCannon(
                self.localFlyingToon)
            return Task.done
        ...
        if self.hitBumper:
            ...
            cameraView = 0
        elif not (self.hitCloud) and self._DistributedPartyCannonActivity__isFlightKeyPressed():
            self._DistributedPartyCannonActivity__moveFlyingToon(toon)
            self._DistributedPartyCannonActivity__updateFlightVelocity(
                task.info['trajectory'])
        ...
        toon.setFluidPos(
            toon,
            self.flyingToonOffsetX,
            self.flyingToonOffsetY,
            0)
        vel = task.info['trajectory'].getVel(t)
        toon.lookAt(toon.getPos() + Vec3(vel[0], vel[1], vel[2]))
        ...
        if pos.getZ() < -20 or pos.getZ() > 1000:
            self.notify.debug(
                'stopping fly task toon.getZ()=%.2f' %
                pos.getZ())
            self._DistributedPartyCannonActivity__resetToonToCannon(
                self.localFlyingToon)
            return Task.done

        self._DistributedPartyCannonActivity__setFlyingCameraView(
            task.info['toon'], cameraView, deltaT)
        return Task.cont

    def _DistributedPartyCannonActivity__setFlyingCameraView(
            self, toon, view, deltaT):
        if toon != base.localAvatar:
            return None
    ...
    def _DistributedPartyCannonActivity__cleanupFlyingToonData(self, toon):
        self.notify.debug('__cleanupFlyingToonData')
        if toon:
            toon.dropShadow.show()
            self.toonIds.remove(toon.doId)
            if self.isLocalToon(toon):
                if self.localFlyingDropShadow is not None:
    ...
    def _DistributedPartyCannonActivity__startCollisionHandler(self):
        ...
        taskMgr.add(
            self._checkFlyingToonCollision,
            self._flyingCollisionTaskName)
        base.cTrav.addCollider(self.flyColNodePath, self.handler)

    def _DistributedPartyCannonActivity__stopCollisionHandler(self, avatar):
    ...
    def _checkFlyingToonCollision(self, task=None):
        ...
        if self.handler.getNumEntries():
            ...
            if activeEntry is not None:
                self._DistributedPartyCannonActivity__handleFlyingToonCollision(
                    activeEntry)
    ...
    def _DistributedPartyCannonActivity__handleFlyingToonCollision(
            self, collisionEntry):
        ...
        if hitNode.find('cSphere') == 0 and hitNode.find('treasureSphere') == 0 and hitNode.find('prop') == 0 and hitNode.find('distAvatarCollNode') == 0 and hitNode.find('CannonSphere') == 0 and hitNode.find('plotSphere') == 0 and hitNode.find(
                'flySphere') == 0 and hitNode.find('FishingSpotSphere') == 0 and hitNode.find('TrampolineTrigger') == 0 and hitNode == 'gagtree_collision' and hitNode == 'sign_collision' and hitNode == 'FlowerSellBox' or hitPylonBelowWater:
            self.notify.debug('--------------hit and ignoring %s' % hitNode)
            return None

        if vel.dot(hitNormal) > 0 and not (
                hitNode == 'collision_roof') and not (
                hitNode == 'collision_fence'):
            self.notify.debug(
        ...
        if hitNode in bumperNodes and hitNode.find(
                'cogPie') == 0 or PartyCannonCollisions['trampoline_bounce'] in hitNode:
            if hitNode == 'sky_collision' and hitNode in PartyCannonCollisions['fence'] or hitNode.find(
                    'cogPie') == 0:
                self._DistributedPartyCannonActivity__hitFence(
                    self.localFlyingToon, collisionEntry)
                ...
                self.hitCloud = 1
                self._DistributedPartyCannonActivity__hitBumper(
                    self.localFlyingToon, collisionEntry, hitSound, kr=0.089999999999999997, angVel=5)
                self.hitBumper = 0
            elif hitNode in cloudBumpers:
                self._DistributedPartyCannonActivity__hitCloudPlatform(
                    self.localFlyingToon, collisionEntry)
            elif hitNode == 'statuaryCol':
                self._DistributedPartyCannonActivity__hitStatuary(
                    self.localFlyingToon, collisionEntry)
            else:
                self.notify.debug(
                    '*************** hit something else ************')
        ...
        if hitNode in PartyCannonCollisions['ground']:
            track.append(
                Func(
                    self._DistributedPartyCannonActivity__hitGround,
                    self.localFlyingToon,
                    pos))
            track.append(Wait(1.0))
            track.append(
                Func(
                    self._DistributedPartyCannonActivity__setToonUpright,
                    self.localFlyingToon,
                    self.landingPos))
            ...
            track.append(
                Func(
                    self._DistributedPartyCannonActivity__hitWater,
                    self.localFlyingToon,
                    pos,
                    collisionEntry))
    ...
    def _DistributedPartyCannonActivity__hitBumper(
            self,
            avatar,
            collisionEntry,
            sound,
            kr=0.59999999999999998,
            angVel=1):
        self.hitBumper = 1
        base.playSfx(sound)
        ...
        if self.hitCloud:
            ...
            centerVec.normalize()
            newVel = centerVec * d + normal * 0.20000000000000001
    ...
    def _DistributedPartyCannonActivity__hitGround(
            self, avatar, pos, extraArgs=[]):
        ...
        self.notify.debug(
            'pos = %s, hpr = %s' %
            (avatar.getPos(render),
             avatar.getHpr(render)))
        self._DistributedPartyCannonActivity__playDustCloud(avatar, pos)
        base.playSfx(self.sndHitGround)
        avatar.setPlayRate(2.0, 'run')
        avatar.loop('run')

    def _DistributedPartyCannonActivity__playDustCloud(self, toon, pos):
    ...
    def _DistributedPartyCannonActivity__hitFence(
            self, avatar, collisionEntry, extraArgs=[]):
        self.notify.debug('__hitFence')
        self._DistributedPartyCannonActivity__hitBumper(
            avatar, collisionEntry, self.sndHitHouse, kr=0.20000000000000001, angVel=3)

    def _DistributedPartyCannonActivity__hitWater(
            self, avatar, pos, collisionEntry, extraArgs=[]):
        hitP = avatar.getPos(render)
        if hitP[2] > ToontownGlobals.EstateWakeWaterHeight:
            self.notify.debug('we hit the ground before we hit water')
            self._DistributedPartyCannonActivity__hitGround(
                avatar, pos, extraArgs)
            return None
    ...
    def _DistributedPartyCannonActivity__hitStatuary(
            self, avatar, collisionEntry, extraArgs=[]):
        self._DistributedPartyCannonActivity__hitBumper(
            avatar, collisionEntry, self.sndHitHouse, kr=0.40000000000000002, angVel=5)
    ...
    def d_requestCloudHit(self, cloudNumber, color):
        self.sendUpdate('requestCloudHit', [
            cloudNumber,
            color.getX(),
            color.getY(),
            color.getZ()])
    ...
    def _DistributedPartyCannonActivity__hitCloudPlatform(
            self, avatar, collisionEntry, extraArgs=[]):
        if not (self.hitBumper) and not (self.hitCloud):
            self.hitCloud = 1
            self._DistributedPartyCannonActivity__hitBumper(
                avatar, collisionEntry, self.sndBounce1, kr=0.34999999999999998, angVel=5)
            self.hitBumper = 0
            if self._lastCloudHit is None:
                ...
                self._DistributedPartyCannonActivity__playDustCloud(
                    avatar, collisionEntry.getSurfacePoint(render))
                self.flyingToonCloudsHit += 1
                taskMgr.doMethodLater(
                    0.25,
                    self._DistributedPartyCannonActivity__reactivateLastCloudHit,
                    DistributedPartyCannonActivity.REACTIVATE_CLOUD_TASK)

    def _DistributedPartyCannonActivity__reactivateLastCloudHit(self, task):
        self._lastCloudHit = None
        return Task.done

    def _DistributedPartyCannonActivity__handleFireworksStarted(self):
        self.notify.debug('__handleFireworksStarted')
        base.cr.playGame.hood.loader.fadeClouds()

    def _DistributedPartyCannonActivity__handleFireworksFinished(self):
        self.notify.debug('__handleFireworksFinished')
        if self._DistributedPartyCannonActivity__checkHoodValidity():
            base.cr.playGame.hood.loader.fadeClouds()
        else:
            self.notify.debug('Toon has left the party')

    def _DistributedPartyCannonActivity__checkHoodValidity(self):
        if hasattr(
                base.cr.playGame,
