...
class PartyCog(FSM):
    ...
    def load(self):
        ...
        self.arm1Coll = CollisionSphere(
            1.6499999999999999, 0, 3.9500000000000002, 1.0)
        ...
        self.arm2Coll = CollisionSphere(-1.6499999999999999,
                                        0, 3.4500000000000002, 1.0)
    ...
    def respondToPieHit(self, timestamp, position, hot=False, direction=1.0):
        if self.netTimeSentToStartByHit < timestamp:
            self._PartyCog__showSplat(position, direction, hot)
            if self.netTimeSentToStartByHit < timestamp:
    ...
    def _PartyCog__showSplat(self, position, direction, hot=False):
        if self.kaboomTrack is not None and self.kaboomTrack.isPlaying():
            self.kaboomTrack.finish()
    ...
    def showHitScore(self, number, scale=1):
        ...
        seq = Task.sequence(
            ...
            self.hpText.lerpColor(
                ...
                0.10000000000000001),
            Task.Task(
                self._PartyCog__hideHitScoreTask))
