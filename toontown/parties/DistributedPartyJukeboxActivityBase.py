...
class DistributedPartyJukeboxActivityBase(DistributedPartyActivity):
    ...
    def activate(self):
        self.accept(
            'enter' + self.getCollisionName(),
            self._DistributedPartyJukeboxActivityBase__handleEnterCollision)

    def _DistributedPartyJukeboxActivityBase__handleEnterCollision(
            self, collisionEntry):
    ...
    def _DistributedPartyJukeboxActivityBase__localUseJukebox(self):
        ...
        self.accept(
            JukeboxGui.CLOSE_EVENT,
            self._DistributedPartyJukeboxActivityBase__handleGuiClose)
        taskMgr.doMethodLater(
            0.5,
            self._DistributedPartyJukeboxActivityBase__localToonWillExitTask,
            self.uniqueName('toonWillExitJukeboxOnTimeout'),
            extraArgs=None)
        self.accept(JukeboxGui.ADD_SONG_CLICK_EVENT,
                    self._DistributedPartyJukeboxActivityBase__handleQueueSong)
        if self.isUserHost():
            self.accept(
                JukeboxGui.MOVE_TO_TOP_CLICK_EVENT,
                self._DistributedPartyJukeboxActivityBase__handleMoveSongToTop)

    def _DistributedPartyJukeboxActivityBase__localToonWillExitTask(
            self,
            task):
    ...
    def queuedSongsResponse(self, songInfoList, index):
        if self.gui.isLoaded():
            for i in range(len(songInfoList)):
                ...
                if index >= 0:
                    pass
                self._DistributedPartyJukeboxActivityBase__addSongToQueue(
                    songInfo, isLocalQueue=i == index)
    ...
    def _DistributedPartyJukeboxActivityBase__handleQueueSong(
            self, name, values):
        ...
        if self.gui.isLoaded():
            ...
            if data:
                if self.localQueuedSongListItem is not None:
                    ...
                    self._DistributedPartyJukeboxActivityBase__addSongToQueue(
                        songInfo, isLocalQueue=True)

    def _DistributedPartyJukeboxActivityBase__addSongToQueue(
            self, songInfo, isLocalQueue=False):
        if isLocalQueue:
            pass
    ...
    def _DistributedPartyJukeboxActivityBase__play(
            self, phase, filename, length):
        self.music = base.loadMusic((MUSIC_PATH + '%s') % (phase, filename))
        if self.music:
            if self._DistributedPartyJukeboxActivityBase__checkPartyValidity() and hasattr(
    ...
    def setSongPlaying(self, songInfo, toonId):
        ...
        data = self.getMusicData(phase, filename)
        if data:
            self._DistributedPartyJukeboxActivityBase__play(
                phase, filename, data[1])
