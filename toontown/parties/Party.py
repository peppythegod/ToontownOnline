...
class Party(Place.Place):
    ...
    def _Party__setZoneId(self, zoneId):
        self.zoneId = zoneId
    ...
    def handlePartyPlanningDone(self):
        ...
        self.partyPlanner.close()
        del self.partyPlanner
        messenger.send('deallocateZoneIdFromPlannedParty',
    ...
    def enterTeleportIn(self, requestStatus):
        ...
        if hasattr(base, 'distributedParty'):
            self._Party__updateLocalAvatarTeleportIn(requestStatus)
        elif hasattr(base.localAvatar,
                     'aboutToPlanParty') and base.localAvatar.aboutToPlanParty:
            self._Party__updateLocalAvatarTeleportIn(requestStatus)
        else:
            self.acceptOnce(DistributedParty.generatedEvent,
                            self._Party__updateLocalAvatarTeleportIn,
                            [requestStatus])
    ...
    def _Party__updateLocalAvatarTeleportIn(self, requestStatus):
        ...
        if hasattr(base, 'distributedParty'):
            ...
            self.accept(
                ...
                self._Party__setPartyHat)
            self._Party__setPartyHat()
        else:
            (x, y, z) = (0.0, 0.0, 0.10000000000000001)
        base.localAvatar.detachNode()
        base.localAvatar.setPos(render, x, y, z)
        base.localAvatar.lookAt(0.0, 0.0, 0.10000000000000001)
        ...
        if hasattr(base.localAvatar,
            ...
            self._partyTiToken = self.addSetZoneCompleteCallback(
                Functor(self._partyTeleportInPostZoneComplete, requestStatus),
                150)

    def _partyTeleportInPostZoneComplete(self, requestStatus):
        self.nextState = 'partyPlanning'

    def _Party__setPartyHat(self, doId=None):
        if hasattr(base, 'distributedParty'):
    ...
    def _Party__removePartyHat(self):
        if hasattr(base, 'distributedParty'):
            if base.distributedParty.partyInfo.hostId in base.cr.doId2do:
                host = base.cr.doId2do[base.distributedParty.partyInfo.hostId]
                if hasattr(host, 'gmIcon') and host.gmIcon:
    ...
    def enterTeleportOut(self, requestStatus):
        Place.Place.enterTeleportOut(self, requestStatus,
                                     self._Party__teleportOutDone)

    def _Party__teleportOutDone(self, requestStatus):
        if hasattr(self, 'fsm'):
            self.fsm.requestFinalState()
        ...
        if hoodId == ToontownGlobals.PartyHood and zoneId == self.getZoneId(
        ) and shardId is None:
    ...
    def handleTeleportQuery(self, fromAvatar, toAvatar):
        if self.isPartyEnding:
            ...
            fromAvatar.d_teleportResponse(toAvatar.doId, 0,
            ...
            if toAvatar == localAvatar:
                localAvatar.doTeleportResponse(
                    fromAvatar, toAvatar, toAvatar.doId, 1,
