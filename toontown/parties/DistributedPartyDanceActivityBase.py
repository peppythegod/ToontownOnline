...
class DistributedPartyDanceActivityBase(DistributedPartyActivity):
    ...
    def generateInit(self):
        ...
        self._DistributedPartyDanceActivityBase__initOrthoWalk()
    ...
    def load(self):
        ...
        self.danceFloorSequence = Sequence(
            Wait(0.29999999999999999), Func(floor.setH, floor, 36))
        ...
        self.discoBallSequence = Parallel(
            ...
            Sequence(
                discoBall.posInterval(
                    3, Point3(0, 0, 1), blendType='easeInOut'),
                discoBall.posInterval(
                    3, Point3(0, 0, 0), blendType='easeInOut')))
    ...
    def unload(self):
        ...
        if self.localToonDancing:
            self._DistributedPartyDanceActivityBase__localStopDancing()
        ...
        self._DistributedPartyDanceActivityBase__destroyOrthoWalk()
    ...
    def startActive(self):
        self.accept(
            ...
            self._DistributedPartyDanceActivityBase__handleEnterDanceFloor)
        self.accept(
            'exit' + DANCE_FLOOR_COLLISION,
            self._DistributedPartyDanceActivityBase__handleExitDanceFloor)
    ...
    def _DistributedPartyDanceActivityBase__destroyOrthoWalk(self):
        self.notify.debug('Destroy Ortho Walk')
        self.orthoWalk.stop()
        self.orthoWalk.destroy()
        del self.orthoWalk

    def _DistributedPartyDanceActivityBase__disableLocalControl(self):
        self.orthoWalk.stop()
        self.keyCodes.disable()
        self.keyCodesGui.disable()

    def _DistributedPartyDanceActivityBase__enableLocalControl(self):
        self.orthWalk.start()
        self.keyCodes.enable()
        self.keyCodesGui.enable()
        self.keyCodesGui.hideAll()

    def _DistributedPartyDanceActivityBase__handleEnterDanceFloor(
            self, collEntry):
    ...
    def handleToonJoined(self, toonId, h):
        self.notify.debug(
            'handleToonJoined( toonId=%d, h=%.2f )' % (toonId, h))
        if toonId in base.cr.doId2do:
            ...
            if toonId == base.localAvatar.doId:
                self._DistributedPartyDanceActivityBase__localStartDancing(h)

    def _DistributedPartyDanceActivityBase__localStartDancing(self, h):
        if not self.localToonDancing:
            place = base.cr.playGame.getPlace()
            if place and hasattr(place, 'fsm'):
                self.localToonDancing = True
                place.fsm.request('activity')
                self._DistributedPartyDanceActivityBase__updateLocalToonState(
                    ToonDancingStates.Run)
                self._DistributedPartyDanceActivityBase__setViewMode(
                    DanceViews.Dancing)
                self.gui.load()
                self.startRules()
                self._DistributedPartyDanceActivityBase__localEnableControls()
            else:
                self.notify.warning(
    ...
    def _DistributedPartyDanceActivityBase__localEnableControls(self):
        if base.localAvatar.doId not in self.dancingToonFSMs:
            self.notify.debug(
                'no dancing FSM for local avatar, not enabling controls')
            return None

        self.accept(KeyCodes.PATTERN_MATCH_EVENT,
                    self._DistributedPartyDanceActivityBase__doDanceMove)
        self.accept(KeyCodes.PATTERN_NO_MATCH_EVENT,
                    self._DistributedPartyDanceActivityBase__noDanceMoveMatch)
    ...
    def _DistributedPartyDanceActivityBase__localDisableControls(self):
        self.orthoWalk.stop()
        self.keyCodes.disable()
        self.gui.disable()
    ...
    def _DistributedPartyDanceActivityBase__handleExitDanceFloor(
            self, collEntry):
        if self.localToonDanceSequence is not None:
    ...
    def _DistributedPartyDanceActivityBase__localStopDancing(self):
        if self.localToonDancing:
            self._DistributedPartyDanceActivityBase__localDisableControls()
            self.gui.unload()
            self._DistributedPartyDanceActivityBase__setViewMode(
                DanceViews.Normal)
            self._DistributedPartyDanceActivityBase__updateLocalToonState(
                ToonDancingStates.Cleanup)
    ...
    def _DistributedPartyDanceActivityBase__doDanceMove(self, pattern):
        self.notify.debug('Dance move! %s' % pattern)
        anim = self.dancePatternToAnims.get(pattern)
        if anim:
            self._DistributedPartyDanceActivityBase__updateLocalToonState(
                ToonDancingStates.DanceMove, anim)
            ...
            if pattern not in self.localPatternsMatched:
                ...
                h = node2.getH() * (
                    camera.getH(camNode) / abs(camera.getH(camNode)))
                del node2
                hpr = camera.getHpr()
                pos = camera.getPos()
                ...
                self.localToonDanceSequence = Sequence(
                    Func(
                        self.
                        _DistributedPartyDanceActivityBase__localDisableControls
                    ),
                    Parallel(
                        camera.posInterval(
                            0.5, Point3(0, 15, 10), blendType='easeIn'),
                        camera.hprInterval(
                            0.5, Point3(h, -20, 0), blendType='easeIn')),
                    ...
                    Func(
                        self.
                        _DistributedPartyDanceActivityBase__localEnableControls
                    ))
            else:
                self.localToonDanceSequence = Sequence(
                    Func(
    ...
    def _DistributedPartyDanceActivityBase__noDanceMoveMatch(self):
        self.gui.setColor(1, 0, 0)
        self.gui.showText('No Match!')
        self._DistributedPartyDanceActivityBase__updateLocalToonState(
            ToonDancingStates.DanceMove)
        self.localToonDanceSequence = Sequence(
            Func(
                self._DistributedPartyDanceActivityBase__localDisableControls),
            Wait(1.0),
            Func(self._DistributedPartyDanceActivityBase__localEnableControls))
        self.localToonDanceSequence.start()
    ...
    def _DistributedPartyDanceActivityBase__updateLocalToonState(
            self, state, anim=''):
    ...
    def _requestToonState(self, toonId, state, anim):
        if toonId in self.dancingToonFSMs:
            ...
            if state == ToonDancingStates.getString(ToonDancingStates.Cleanup):
                self.notify.debug(
                    'deleting this fsm %s' % self.dancingToonFSMs[toonId])
                del self.dancingToonFSMs[toonId]
