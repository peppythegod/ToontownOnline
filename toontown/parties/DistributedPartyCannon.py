...
class DistributedPartyCannon(DistributedObject, Cannon):
    ...
    def disable(self):
        ...
        self.ignoreAll()
        self._DistributedPartyCannon__disableCannonControl()
    ...
    def activate(self):
        self.accept(
            ...
            self._DistributedPartyCannon__handleToonCollisionWithCannon)
        Cannon.show(self)
        self.active = True
    ...
    def setMovie(self, mode, avId):
        ...
        if mode == PartyGlobals.CANNON_MOVIE_CLEAR:
            ...
            self.notify.error(
                'setMovie Unhandled case mode=%d avId=%d' % (mode, avId))

    def _DistributedPartyCannon__handleToonCollisionWithCannon(
            self, collEntry):
        self.notify.debug('collEntry: %s' % collEntry)
        if base.cr.playGame.getPlace().getState(
        ) == 'walk' and self.toonInsideAvId == 0:
    ...
    def _DistributedPartyCannon__avatarGone(self, avId):
        ...
        if avId == base.localAvatar.doId:
            ...
            if not self.activity.hasPlayedBefore():
                ...
                self.acceptOnce(
                    ...
                    self._DistributedPartyCannon__enableCannonControl)
            else:
                self._DistributedPartyCannon__enableCannonControl()
        ...
        if avId in self.cr.doId2do:
            ...
            if toon:
                self.acceptOnce(
                    ...
                    self._DistributedPartyCannon__avatarGone,
                    extraArgs=[avId])
                toon.stopSmooth()
                toon.dropShadow.hide()
                self.placeToonInside(toon)
            else:
                self._DistributedPartyCannon__avatarGone(avId)
        else:
            self.notify.warning(
                'Unknown avatar %d in cannon %d' % (avId, self.doId))
    ...
    def resetToon(self, pos=None):
        ...
        self.notify.debug('%d resetToon self.toonInsideAvId=%d' %
                          (self.doId, self.toonInsideAvId))
        self.removeToonDidNotFire()
        self._DistributedPartyCannon__setToonUpright(toon, pos)
        if toonInsideAvId == base.localAvatar.doId:
            ...
            self.notify.debug('currentState=%s, requesting walk' %
                              base.cr.playGame.getPlace().getState())
            base.cr.playGame.getPlace().setState('walk')
            self.notify.debug('after request walk currentState=%s,' %
                              base.cr.playGame.getPlace().getState())

        toon.dropShadow.show()
        self.d_setLanded()

    def _DistributedPartyCannon__setToonUpright(self, toon, pos=None):
        if not pos:
    ...
    def removeAvFromCannon(self, avId):
        ...
        if place:
            ...
            if placeState != 'fishing':
                if av is not None:
                    av.startSmooth()
                    self._DistributedPartyCannon__destroyToonModels(avId)
                    return None
        ...
        if av is not None:
            ...
            if av.getParent().getName() == 'toonOriginChange':
                ...
                self._DistributedPartyCannon__setToonUpright(av)
    ...
    def _DistributedPartyCannon__destroyToonModels(self, avId):
        av = base.cr.doId2do.get(avId)
        if not av:
            return None
    ...
    def setClear(self):
        ...
        if toon and self.isToonInside():
            ...
            if toon.getParent().getName() == 'toonOriginChange':
                ...
                self._DistributedPartyCannon__setToonUpright(toon)
            ...
            self.notify.debug('setClear in else toon=%s, self.isToonInsde()=%s'
                              % (toonName, self.isToonInside()))
        self.toonInsideAvId = 0
        self.notify.debug(
            'setClear self.toonInsideAvId=%d' % self.toonInsideAvId)
    ...
    def _DistributedPartyCannon__enableCannonControl(self):
        if not self.gui:
        ...
        self.d_setTimeout()
        self.accept(CannonGui.FIRE_PRESSED,
                    self._DistributedPartyCannon__handleFirePressed)
        self._DistributedPartyCannon__startLocalCannonMoveTask()

    def d_setTimeout(self):
        self.sendUpdate('setTimeout')

    def _DistributedPartyCannon__disableCannonControl(self):
        if self.gui:
            self.gui.unload()

        self.ignore(CannonGui.FIRE_PRESSED)
        self._DistributedPartyCannon__stopLocalCannonMoveTask()

    def _DistributedPartyCannon__startLocalCannonMoveTask(self):
        self.localCannonMoving = False
        task = Task(self._DistributedPartyCannon__localCannonMoveTask)
        task.lastPositionBroadcastTime = 0.0
        taskMgr.add(task, self.LOCAL_CANNON_MOVE_TASK)

    def _DistributedPartyCannon__stopLocalCannonMoveTask(self):
        taskMgr.remove(self.LOCAL_CANNON_MOVE_TASK)
        if self.localCannonMoving:
            self.localCannonMoving = False
            self.stopMovingSound()

    def _DistributedPartyCannon__localCannonMoveTask(self, task):
        rotVel = 0
        ...
        if self.hasMoved():
            ...
            if task.time - task.lastPositionBroadcastTime > CANNON_MOVE_UPDATE_FREQ:
                self.notify.debug(
                    'Broadcast local cannon %s position' % self.doId)
                task.lastPositionBroadcastTime = task.time
                self._DistributedPartyCannon__broadcastLocalCannonPosition()
    ...
    def _DistributedPartyCannon__broadcastLocalCannonPosition(self):
    ...
    def updateCannonPosition(self, avId, zRot, angle):
        if avId and avId == self.toonInsideAvId and avId != base.localAvatar.doId:
            self.notify.debug('update cannon %s position zRot = %d, angle = %d'
                              % (self.doId, zRot, angle))
            self.setRotation(zRot)
            self.setAngle(angle)
            self.updateModel()

    def _DistributedPartyCannon__handleFirePressed(self):
