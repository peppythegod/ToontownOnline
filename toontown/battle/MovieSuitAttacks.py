...
def doSuitAttack(attack):
    ...
    if groupStatus == ATK_TGT_SINGLE:
        ...
        toonHprTrack = Sequence(
            Func(
                toon.headsUp, battle, MovieUtil.PNT3_ZERO), Func(
                toon.loop, 'neutral'))
    else:
...
def getResetTrack(suit, battle):
    ...
    walkTrack = Sequence(
        ...
        ActorInterval(
            ...
            endTime=1.0000000000000001e-005),
        Func(
            suit.loop,
            'neutral'))
...
def getSuitAnimTrack(attack, delay=0):
    ...
    return Sequence(
        Wait(delay), Func(
            suit.setChatAbsolute, taunt, CFSpeech | CFTimeout), ActorInterval(
            attack['suit'], attack['animName']), Func(
                suit.clearChat))
...
def doClipOnTie(attack):
    ...
    tiePropTrack.append(getPropThrowTrack(attack,
                                          tie,
                                          [__toonFacePoint(toon)],
                                          [__toonGroundPoint(attack,
                                                             toon,
                                                             0.10000000000000001)],
...
def doPoundKey(attack):
    ...
    BattleParticles.setEffectTexture(
        particleEffect, 'poundsign', color=Vec4(
            0, 0, 0, 1))
    suitTrack = getSuitTrack(attack)
    ...
    propTrack = Sequence(
        Wait(0.29999999999999999), Func(
            __showProp, phone, suit.getLeftHand(), phonePosPoints[0], phonePosPoints[1]), Func(
            __showProp, receiver, suit.getLeftHand(), receiverPosPoints[0], receiverPosPoints[1]), LerpScaleInterval(
                phone, 0.5, MovieUtil.PNT3_ONE, MovieUtil.PNT3_NEARZERO), Wait(0.73999999999999999), Func(
                    receiver.wrtReparentTo, suit.getRightHand()), LerpPosHprInterval(
                        receiver, 0.0001, Point3(
                            -0.45000000000000001, 0.47999999999999998, -0.62), VBase3(
                                -87.469999999999999, -18.210000000000001, 7.8200000000000003)), Wait(3.1400000000000001), Func(
                                    receiver.wrtReparentTo, phone), Wait(0.62), LerpScaleInterval(
                                        phone, 0.5, MovieUtil.PNT3_NEARZERO), Func(
...
def doFillWithLead(attack):
    ...
    headTrack = getPartTrack(headSmotherEffect, partDelay, partDuration, [
        ...
        toon,
        0])
    torsoTrack = getPartTrack(torsoSmotherEffect, partDelay +
                              partIvalDelay, partDuration, [torsoSmotherEffect, toon, 0])
    legsTrack = getPartTrack(legsSmotherEffect,
...
def doFountainPen(attack):
    ...
    def hitPoint(toon=toon): return __toonFacePoint(toon)

    def missPoint(
        prop=pen,
        toon=toon): return __toonMissPoint(
        prop,
        toon,
        0,
        parent=render)
    ...
    if dmg > 0:
        ...
        splashTrack = Sequence(
            Func(
                battle.movie.needRestoreRenderProp,
                splash),
    ...
    penSpillTrack = getPartTrack(
        penSpill, 1.3999999999999999, 0.69999999999999996, [
            penSpill, pen, 0])
...
def doRubOut(attack):
    ...
    headTrack = getPartTrack(headEffect, partDelay + 0, 0.5, [
        ...
        toon,
        0])
    torsoTrack = getPartTrack(torsoEffect, partDelay +
                              1.1000000000000001, 0.5, [torsoEffect, toon, 0])
...
def doFingerWag(attack):
    ...
    BattleParticles.setEffectTexture(
        particleEffect, 'blah', color=Vec4(
            0.55000000000000004, 0, 0.55000000000000004, 1))
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'a':
        partDelay = 1.3
...
def doWriteOff(attack):
    ...
    pencilPropTrack = Sequence(
        Wait(0.5), Func(
            __showProp, *extraArgsForShowProp), LerpScaleInterval(
            pencil, 0.5, Point3(
                1.5, 1.5, 1.5), startScale=Point3(0.01)), Wait(2), Func(
                    battle.movie.needRestoreRenderProp, checkmark), Func(
                        checkmark.reparentTo, render), Func(
                            checkmark.setScale, 1.6000000000000001), Func(
                                checkmark.setPosHpr, pencil, 0, 0, 0, 0, 0, 0), Func(
                                    checkmark.setP, 0), Func(
                                        checkmark.setR, 0))
...
def doRubberStamp(attack):
    ...
    propTrack = Sequence(
        Func(
            __showProp, stamp, suit.getRightHand(), stampPosPoints[0], stampPosPoints[1]), LerpScaleInterval(
            stamp, 0.5, MovieUtil.PNT3_ONE), Wait(2.6000000000000001), Func(
                battle.movie.needRestoreRenderProp, cancelled), Func(
                    cancelled.reparentTo, render), Func(
                        cancelled.setScale, 0.59999999999999998), Func(
                            cancelled.setPosHpr, stamp, 0.81000000000000005, -1.1100000000000001, -0.16, 0, 0, 90), Func(
                                cancelled.setP, 0), Func(
                                    cancelled.setR, 0))
...
def doRazzleDazzle(attack):
    ...
    if hitSuit:
        ...
        def hitPoint(
            ...
            toon=toon,
            suit=suit): return __toonMissPoint(
            particleEffect,
            toon,
            parent=suit.getRightHand())
    signPropTrack = Sequence(
        Wait(0.5), Func(
            __showProp, sign, suit.getRightHand(), signPosPoints[0], signPosPoints[1]), LerpScaleInterval(
            sign, 0.5, Point3(
                1.3899999999999999, 1.3899999999999999, 1.3899999999999999)), Wait(0.5), Func(
                    battle.movie.needRestoreParticleEffect, particleEffect), Func(
                        particleEffect.start, sign), Func(
                            particleEffect.wrtReparentTo, render), LerpPosInterval(
                                particleEffect, 2.0, pos=hitPoint), Func(
                                    particleEffect.cleanup), Func(
                                        battle.movie.clearRestoreParticleEffect, particleEffect))
    signPropAnimTrack = ActorInterval(sign, 'smile', duration=4, startTime=0)
    toonTrack = getToonTrack(attack, 2.6000000000000001, [
        'cringe'], 1.8999999999999999, [
        'sidestep'])
    soundTrack = getSoundTrack(
        'SA_razzle_dazzle.mp3',
        delay=1.6000000000000001,
        node=suit)
    return Sequence(
        Parallel(
...
def doSynergy(attack):
    ...
    partTrack = getPartTrack(particleEffect, 1.0, 1.8999999999999999, [
        ...
        suit,
        0])
    waterfallTrack = getPartTrack(
        waterfallEffect, 0.80000000000000004, 1.8999999999999999, [
    ...
    synergySoundTrack = Sequence(Wait(0.90000000000000002), SoundInterval(
...
def doTeeOff(attack):
    ...
    ballPropTrack = Sequence(
        getPropAppearTrack(
            ball, suit, ballPosPoints, 1.7, Point3(
                1.5, 1.5, 1.5)), Func(
            battle.movie.needRestoreRenderProp, ball), Func(
                    ball.wrtReparentTo, render), Wait(2.1499999999999999))

    def missPoint(ball=ball, toon=toon): return __toonMissPoint(ball, toon)
...
def doBrainStorm(attack):
    ...
    cloudPropTrack.append(
        Parallel(
            ParticleInterval(
                snowEffect, cloud, worldRelative=0, duration=2.2000000000000002, cleanup=True), Sequence(
                Wait(0.5), ParticleInterval(
                    snowEffect2, cloud, worldRelative=0, duration=1.7, cleanup=True)), Sequence(
                        Wait(1.0), ParticleInterval(
                            snowEffect3, cloud, worldRelative=0, duration=1.2, cleanup=True)), Sequence(
                                ActorInterval(
                                    cloud, 'stormcloud', startTime=3, duration=0.5), ActorInterval(
                                        cloud, 'stormcloud', startTime=2.5, duration=0.5), ActorInterval(
                                            cloud, 'stormcloud', startTime=1, duration=1.5))))
...
def doBuzzWord(attack):
    ...
    for i in range(0, 5):
        ...
        if random.random() > 0.5:
            BattleParticles.setEffectTexture(
                effect, texturesList[i], color=Vec4(
                    1, 0.93999999999999995, 0.02, 1))
        else:
            BattleParticles.setEffectTexture(
...
def doDemotion(attack):
    ...
    partTrack = getPartTrack(
        sprayEffect, 0.69999999999999996, 1.1000000000000001, [
            sprayEffect, suit, 0])
    partTrack2 = getPartTrack(
        freezeEffect, 1.3999999999999999, 2.8999999999999999, [
            freezeEffect, toon, 0])
...
def doCanned(attack):
    ...
    if dmg > 0:
        ...
        hprTrack = Sequence(
            Wait(
                propDelay + suitDelay), LerpHprInterval(
                can, throwDuration, canHpr), Wait(0.40000000000000002), LerpHprInterval(
                can, 0.40000000000000002, Point3(
                    83.269999999999996, 19.52, -177.91999999999999)), LerpHprInterval(
                        can, 0.29999999999999999, Point3(
                            95.239999999999995, -72.090000000000003, 88.650000000000006)), LerpHprInterval(
                                can, 0.20000000000000001, Point3(
                                    -96.340000000000003, -2.6299999999999999, 179.88999999999999)))
        soundTrack = getSoundTrack(
            'SA_canned_tossup_only.mp3',
            delay=2.6000000000000001,
            node=suit)
    canTrack = Sequence(
        Parallel(
            throwTrack, scaleTrack, hprTrack), Func(
            MovieUtil.removeProp, can), Func(
                battle.movie.clearRenderProp, can))
    damageAnims = [
        [
            'struggle',
...
def doDownsize(attack):
    ...
    sprayTrack = getPartTrack(sprayEffect, 1.0, 1.28, [
        ...
        suit,
        0])
    cloudTrack = getPartTrack(
        cloudEffect, 2.1000000000000001, 1.8999999999999999, [
            cloudEffect, toon, 0])
    if dmg > 0:
        ...
        shrinkTrack = Sequence(
            Wait(
                damageDelay + 0.5),
            Func(
                battle.movie.needRestoreToonScale),
            LerpScaleInterval(
                toon,
                1.0,
                downScale * 1.1000000000000001),
            LerpScaleInterval(
                toon,
                0.10000000000000001,
            ...
            LerpScaleInterval(
                ...
                initialScale),
            Func(
                battle.movie.clearRestoreToonScale))
    ...
    if dmg > 0:
        ...
        paperAppearTrack.append(
            LerpPosInterval(
                ...
                1.1000000000000001,
                landPoint))
        paperSpinTrack = Sequence(Wait(throwDelay), LerpHprInterval(paper, throwDuration, VBase3(
            300, 0, 0)), Wait(paperPause), LerpHprInterval(paper, 1.3, VBase3(-200, 100, 100)))
    else:
        slidePoint = Point3(
            hitPoint.getX(),
            hitPoint.getY() - 5,
        ...
        paperSpinTrack = Sequence(
            Wait(throwDelay), LerpHprInterval(
                paper, throwDuration, VBase3(
                    300, 0, 0)), LerpHprInterval(
                paper, 0.5, VBase3(
                    10, 0, 0)))
    propTrack = Sequence()
...
def doReOrg(attack):
    ...
    if dmg > 0:
        ...
        def getChestTrack(part, attackDelay=attackDelay):
            ...
            return Sequence(
                Wait(attackDelay), LerpHprInterval(
                    part, 1.1000000000000001, VBase3(
                        180, 0, 0)), Wait(1.1000000000000001), LerpHprInterval(
                    part, 1.1000000000000001, part.getHpr()))
...
def doSacked(attack):
    ...
    sackHpr = VBase3(-154.33000000000001, -
                     6.3300000000000001, 163.80000000000001)
    suitTrack = getSuitTrack(attack)
    posPoints = [
    ...
    if dmg > 0:
        ...
        sackTrack = Sequence(
            Parallel(
                sackAppearTrack, scaleTrack, hprTrack), Func(
                MovieUtil.removeProp, sack), Func(
                battle.movie.clearRenderProp, sack))
    else:
...
def doHalfWindsor(attack):
    ...
    tiePropTrack = getPropAppearTrack(
        tie, suit.getRightHand(), posPoints, 0.5, Point3(
            7, 7, 7), scaleUpTime=0.5)
    tiePropTrack.append(Wait(throwDelay))
...
def doHeadShrink(attack):
    ...
    sprayTrack = getPartTrack(
        shrinkSpray, 0.29999999999999999, 1.3999999999999999, [
            shrinkSpray, suit, 0])
    ...
    cloudTrack.append(
        Func(
            battle.movie.needRestoreParticleEffect,
            shrinkCloud))
    ...
    cloudTrack.append(
        Func(
            battle.movie.clearRestoreParticleEffect,
            shrinkCloud))
...
def doEvilEye(attack):
    ...
    eyePropTrack = Sequence(
        eyeAppearTrack, Parallel(
            eyeMoveTrack, eyeRollTrack), Func(
            battle.movie.clearRenderProp, eye), Func(
                MovieUtil.removeProp, eye))
    damageAnims = [
        [
            'duck',
            0.01,
...
def doFilibuster(attack):
    ...
    sprayTrack = getPartTrack(sprayEffect, partDelay, partDuration, [
        ...
        suit,
        0])
    sprayTrack2 = getPartTrack(sprayEffect2,
                               partDelay + 0.80000000000000004,
                               partDuration,
                               [sprayEffect2,
                                suit,
                                0])
    sprayTrack3 = getPartTrack(sprayEffect3,
                               partDelay + 1.6000000000000001,
                               partDuration,
                               [sprayEffect3,
                                suit,
                                0])
    sprayTrack4 = getPartTrack(sprayEffect4,
                               partDelay + 2.3999999999999999,
                               partDuration,
                               [sprayEffect4,
                                suit,
                                0])
...
def doSchmooze(attack):
    ...
    for i in range(0, 4):
        ...
        BattleParticles.setEffectTexture(
            upperEffect, textureNames[i], color=Vec4(
                0, 0, 1, 1))
        BattleParticles.setEffectTexture(
            lowerEffect, textureNames[i], color=Vec4(
                0, 0, 1, 1))
...
def doHangUp(attack):
    ...
    propTrack = Sequence(
        Wait(0.29999999999999999), Func(
            __showProp, phone, suit.getLeftHand(), phonePosPoints[0], phonePosPoints[1]), Func(
            __showProp, receiver, suit.getLeftHand(), receiverPosPoints[0], receiverPosPoints[1]), LerpScaleInterval(
                phone, 0.5, scaleUpPoint, MovieUtil.PNT3_NEARZERO), Wait(pickupDelay), Func(
                    receiver.wrtReparentTo, suit.getRightHand()), LerpScaleInterval(
                        receiver, 0.01, receiverAdjustScale), LerpPosHprInterval(
                            receiver, 0.0001, Point3(
                                -0.53000000000000003, 0.20999999999999999, -0.54000000000000004), VBase3(
                                    -99.489999999999995, -35.270000000000003, 1.8400000000000001)), Wait(dialDuration), Func(
                                        receiver.wrtReparentTo, phone), Wait(finalPhoneDelay), LerpScaleInterval(
                                            phone, 0.5, MovieUtil.PNT3_NEARZERO), Func(
                                                MovieUtil.removeProps, [
                                                    receiver, phone]))
...
def doRedTape(attack):
    ...
    def hitPoint(toon=toon): return __toonTorsoPoint(toon)
...
def doParadigmShift(attack):
    ...
    suitName = suit.getStyleName()
    if suitName == 'm':
        sprayEffect.setPos(Point3(-5.2000000000000002,
                                  4.5999999999999996, 2.7000000000000002))
    elif suitName == 'sd':
        sprayEffect.setPos(Point3(-5.2000000000000002,
                                  4.5999999999999996, 2.7000000000000002))
    else:
    ...
    for t in targets:
        ...
        if dmg > 0:
            ...
            liftTracks.append(
                getPartTrack(
                    liftEffect, 1.1000000000000001, 4.0999999999999996, [
                        liftEffect, battle, 0]))
    ...
    if hitAtleastOneToon == 1:
        soundTrack = getSoundTrack(
            'SA_paradigm_shift.mp3',
            delay=2.1000000000000001,
            node=suit)
        return Parallel(
            suitTrack,
            sprayTrack,
...
def doPowerTrip(attack):
    ...
    def getPowerTrack(effect, suit=suit, battle=battle):
        partTrack = Sequence(
            Wait(1.0), Func(
                battle.movie.needRestoreParticleEffect, effect), Func(
                effect.start, suit), Wait(0.40000000000000002), LerpPosInterval(
                effect, 1.0, Point3(
                    0, 15, 0.40000000000000002)), LerpFunctionInterval(
                        effect.setAlphaScale, fromData=1, toData=0, duration=0.40000000000000002), Func(
                            effect.cleanup), Func(
                                battle.movie.clearRestoreParticleEffect, effect))
        return partTrack
...
def doBounceCheck(attack):
    ...
    def bounce1Point(
        suit=suit,
        toon=toon,
        battle=battle): return getThrowEndPoint(
        suit,
        toon,
        battle,
        'one')
...
def doFired(attack):
    ...
    baseFlameSmallTrack = getPartTrack(
        baseFlameSmall, 1.0, 1.8999999999999999, [
...
def doAudit(attack):
    ...
    suitTrack = getSuitTrack(attack)
    partTrack = getPartTrack(
        particleEffect, 2.1000000000000001, 1.8999999999999999, [
    ...
    partTrack3 = getPartTrack(
        particleEffect3, 2.2999999999999998, 2.1000000000000001, [
            particleEffect3, suit, 0])
    partTrack4 = getPartTrack(
        particleEffect4, 2.3999999999999999, 2.2000000000000002, [
    ...
    if suitName == 'nc':
        ...
        calcPosPoints = [
            Point3(
                0.34999999999999998, 0.52000000000000002, 0.029999999999999999), VBase3(
                1.3520000000000001, -6.5179999999999998, -6.0449999999999999)]
...
def doCrunch(attack):
    ...
    numberSpillTrack1 = getPartTrack(
        numberSpill1, 1.1000000000000001, 2.2000000000000002, [
            numberSpill1, suit, 0])
    ...
    for i in range(0, numOfNumbers - 1):
        ...
        nextDuration = random.random() * 0.40000000000000002 + 1.3999999999999999
    ...
    for i in range(0, numOfNumbers):
        ...
        next.setPos(
            ...
            0.59999999999999998 -
            0.29999999999999999)
        next.setHpr(VBase3(-1.1499999999999999,
                           86.579999999999998, -76.780000000000001))
        numberTrack = Sequence(
            Wait(0.90000000000000002), LerpScaleInterval(
                next, 0.59999999999999998, MovieUtil.PNT3_ONE), Wait(1.7), Func(
...
def doLiquidate(attack):
    ...
    cloudPropTrack.append(
        Parallel(
            Sequence(
                ParticleInterval(
                    rainEffect, cloud, worldRelative=0, duration=2.1000000000000001, cleanup=True)), Sequence(
                Wait(0.10000000000000001), ParticleInterval(
                    rainEffect2, cloud, worldRelative=0, duration=2.0, cleanup=True)), Sequence(
                Wait(0.10000000000000001), ParticleInterval(
                    rainEffect3, cloud, worldRelative=0, duration=2.0, cleanup=True)), Sequence(
                ActorInterval(
                    cloud, 'stormcloud', startTime=3, duration=0.10000000000000001), ActorInterval(
                    cloud, 'stormcloud', startTime=1, duration=2.2999999999999998))))
    cloudPropTrack.append(Wait(0.40000000000000002))
    cloudPropTrack.append(
        LerpScaleInterval(
            cloud,
    ...
    if dmg > 0:
        ...
        puddleTrack = Sequence(
            Func(
                ...
                puddle),
            Wait(
                damageDelay - 0.69999999999999996),
            Func(
                puddle.reparentTo,
                battle),
            Func(
...
def doMarketCrash(attack):
    ...
    spinTrack = Sequence(Wait(propDelay + suitDelay + 0.20000000000000001),
...
def doBite(attack):
    ...
    if dmg > 0:
        ...
        scaleTrack = Sequence(
            Wait(throwDelay), LerpScaleInterval(
                teeth, throwDuration, Point3(
                    8, 8, 8)), Wait(0.90000000000000002), LerpScaleInterval(
                teeth, 0.20000000000000001, Point3(
                    14, 14, 14)), Wait(1.2), LerpScaleInterval(
                        teeth, 0.29999999999999999, MovieUtil.PNT3_NEARZERO))
        hprTrack = Sequence(
            Wait(throwDelay), LerpHprInterval(
                teeth, 0.29999999999999999, Point3(
                    180, 0, 0)), Wait(0.20000000000000001), LerpHprInterval(
                teeth, 0.40000000000000002, Point3(
                    180, -35, 0), startHpr=Point3(
                        180, 0, 0)), Wait(0.59999999999999998), LerpHprInterval(
                            teeth, 0.10000000000000001, Point3(
                                180, -75, 0), startHpr=Point3(
                                    180, -35, 0)))
        animTrack = Sequence(
            Wait(throwDelay), ActorInterval(
                teeth, 'teeth', duration=throwDuration), ActorInterval(
                teeth, 'teeth', duration=0.29999999999999999), Func(
                teeth.pose, 'teeth', 1), Wait(0.69999999999999996), ActorInterval(
                    teeth, 'teeth', duration=0.90000000000000002))
        propTrack = Sequence(
            Parallel(
                teethAppearTrack, scaleTrack, hprTrack, animTrack), Func(
                MovieUtil.removeProp, teeth), Func(
                battle.movie.clearRenderProp, teeth))
    else:
...
def doChomp(attack):
    ...
    if dmg > 0:
        ...
        scaleTrack = Sequence(
            Wait(throwDelay), LerpScaleInterval(
                teeth, throwDuration, Point3(
                    6, 6, 6)), Wait(0.90000000000000002), LerpScaleInterval(
                teeth, 0.20000000000000001, Point3(
                    10, 10, 10)), Wait(1.2), LerpScaleInterval(
                        teeth, 0.29999999999999999, MovieUtil.PNT3_NEARZERO))
        hprTrack = Sequence(
            Wait(throwDelay), LerpHprInterval(
                teeth, 0.29999999999999999, Point3(
        ...
        hprTrack = Sequence(
            Wait(throwDelay), LerpHprInterval(
                teeth, 0.29999999999999999, Point3(
                    180, 0, 0)), Wait(0.5), LerpHprInterval(
                teeth, 0.40000000000000002, Point3(
                    80, 0, 0), startHpr=Point3(
                        180, 0, 0)), LerpHprInterval(
                            teeth, 0.80000000000000004, Point3(
                                -10, 0, 0), startHpr=Point3(
                                    80, 0, 0)))
        animTrack = Sequence(
            Wait(throwDelay),
            ActorInterval(
        ...
        propTrack = Sequence(
            Parallel(
                teethAppearTrack, hprTrack, animTrack), Func(
                MovieUtil.removeProp, teeth), Func(
                battle.movie.clearRenderProp, teeth))
    damageAnims = [
        [
...
def doWithdrawal(attack):
    ...
    suitTrack = getSuitAnimTrack(attack)
    partTrack = getPartTrack(particleEffect,
                             1.0000000000000001e-005,
...
def doGuiltTrip(attack):
    ...
    partTrack2 = getPowerTrack(powerBar2)
    waterfallTrack = getPartTrack(
        waterfallEffect, 0.59999999999999998, 0.59999999999999998, [
            waterfallEffect, suit, 0])
...
def doSpin(attack):
    ...
    height1 = toon.getHeight() * (random.random() *
                                  0.20000000000000001 + 0.69999999999999996)
    height2 = toon.getHeight() * (random.random() *
                                  0.20000000000000001 + 0.40000000000000002)
    height3 = toon.getHeight() * (random.random() *
    ...
    sprayTrack = getPartTrack(sprayEffect, 1.0, 1.8999999999999999, [
        ...
        suit,
        0])
    spinTrack1 = getPartTrack(
        spinEffect1, 2.1000000000000001, 3.8999999999999999, [
            spinEffect1, battle, 0])
    spinTrack2 = getPartTrack(
        spinEffect2, 2.1000000000000001, 3.8999999999999999, [
            spinEffect2, battle, 0])
    spinTrack3 = getPartTrack(
        spinEffect3, 2.1000000000000001, 3.8999999999999999, [
            spinEffect3, battle, 0])
    damageAnims = []
    damageAnims.append([
        'duck',
        0.01,
    ...
    toonTrack = getToonTrack(
        ...
        showMissedExtraTime=1.0)
    if dmg > 0:
        toonSpinTrack = Sequence(Wait(damageDelay + 0.90000000000000002),
    ...
    for i in range(0, numBirds):
        ...
        if dmg > 0:
            hitPoint = Point3(
                random.random() * 5 - 2.5,
                random.random() * 2 - 1 - 6,
                (random.random() * 3 - 1.5) + toon.getHeight() - 0.90000000000000002)
        else:
            hitPoint = Point3(
                random.random() * 2 - 1,
        ...
        birdTrack = Sequence(
            Wait(throwDelay), Func(
                battle.movie.needRestoreRenderProp, next), Func(
                next.wrtReparentTo, battle), Func(
                next.setHpr, Point3(
                    90, 20, 0)), LerpPosInterval(
                        next, 1.1000000000000001, hitPoint))
        scaleTrack = Sequence(
            Wait(throwDelay), LerpScaleInterval(
                next, 0.14999999999999999, Point3(
                    9, 9, 9)))
        birdTracks.append(
            Sequence(
                Parallel(
