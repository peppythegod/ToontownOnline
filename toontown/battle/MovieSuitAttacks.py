...
def getResetTrack(suit, battle):
    ...
    walkTrack = Sequence(
        ...
        ActorInterval(
            ...
            endTime=1.0000000000000001e-005), Func(suit.loop, 'neutral'))
...
def getToonTrack(attack,
                 damageDelay=9.9999999999999995e-007,
    ...
    if dmg > 0:
        animTrack.append(
            getToonTakeDamageTrack(toon, target['died'], dmg, damageDelay,
                                   damageAnimNames, splicedDamageAnims,
                                   showDamageExtraTime))
        return animTrack
    else:
        animTrack.append(
            getToonDodgeTrack(target, dodgeDelay, dodgeAnimNames,
...
def getToonTracks(attack,
                  damageDelay=9.9999999999999995e-007,
                  damageAnimNames=None,
                  dodgeDelay=9.9999999999999995e-007,
                  dodgeAnimNames=None,
                  splicedDamageAnims=None,
                  splicedDodgeAnims=None,
                  showDamageExtraTime=0.01,
...
def getToonDodgeTrack(target, dodgeDelay, dodgeAnimNames, splicedDodgeAnims,
                      showMissedExtraTime):
    toon = target['toon']
    toonTrack = Sequence()
...
def getAllyToonsDodgeParallel(target):
    ...
    toonTracks.append(
        Sequence(
            ActorInterval(toon, sidestepAnim), Func(toon.loop, 'neutral')))
    toonTracks.append(
        Sequence(Wait(0.5), SoundInterval(soundEffect, node=toon)))
    return toonTracks


def getPropTrack(prop,
                 parent,
                 posPoints,
                 appearDelay,
                 remainDelay,
    ...
    if anim == 1:
        track = Sequence(
            Wait(appearDelay), Func(__showProp, prop, parent, *posPoints),
            LerpScaleInterval(
                prop, scaleUpTime, scaleUpPoint, startScale=startScale),
            ActorInterval(
                prop, propName, duration=animDuration,
                startTime=animStartTime), Wait(remainDelay),
...
def getPropThrowTrack(attack,
    ...
    def getLambdas(list, prop, toon):
        for i in range(len(list)):
            ...
            if list[i] == 'miss':
                ...
                list[i] = lambda prop=prop, toon=toon: __toonMissPoint(
                    prop, toon)
                continue
            if list[i] == 'bounceHit':

                list[i] = lambda prop=prop, toon=toon: __throwBounceHitPoint(
                    prop, toon)
                continue
    ...
    if dmg > 0:
        ...
        if missScaleDown:
            propTrack.append(
                LerpScaleInterval(prop, missScaleDown,
...
def getToonTakeDamageTrack(toon,
                           died,
                           dmg,
                           delay,
                           damageAnimNames=None,
    ...
    if damageAnimNames:
        ...
        indicatorTrack = Sequence(
            Wait(delay + showDamageExtraTime), Func(__doDamage, toon, dmg,
                                                    died))
    else:
...
def getSplicedAnimsTrack(anims, actor=None):
    ...
    for nextAnim in anims:
        ...
        if len(nextAnim) == 3:
            track.append(Wait(delay))
            track.append(
                ActorInterval(actor, nextAnim[0], startTime=nextAnim[2]))
            continue
        if len(nextAnim) == 4:
            track.append(Wait(delay))
            duration = nextAnim[3]
...
def getSplicedLerpAnims(animName,
                        origDuration,
                        newDuration,
                        startTime=0,
                        fps=30,
    ...
    for i in range(0, numAnims):
        anims.append(
            [animName, timeInterval, startTime + addition, animInterval])
...
def doClipOnTie(attack):
    ...
    tiePropTrack.append(
        getPropThrowTrack(
            attack,
            tie, [__toonFacePoint(toon)],
            [__toonGroundPoint(attack, toon, 0.10000000000000001)],
            hitDuration=0.40000000000000002,
            missDuration=0.80000000000000004,
            missScaleDown=1.2))
    toonTrack = getToonTrack(attack, damageDelay, ['conked'], dodgeDelay,
                             ['sidestep'])
    throwSound = getSoundTrack(
        'SA_powertie_throw.mp3', delay=throwDelay + 1, node=suit)
...
def doPoundKey(attack):
    ...
    propTrack = Sequence(
        ...
        Func(__showProp, phone, suit.getLeftHand(), phonePosPoints[0],
             phonePosPoints[1]),
        Func(__showProp, receiver, suit.getLeftHand(), receiverPosPoints[0],
             receiverPosPoints[1]),
        LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_ONE,
                          MovieUtil.PNT3_NEARZERO), Wait(0.73999999999999999),
        Func(receiver.wrtReparentTo, suit.getRightHand()),
        LerpPosHprInterval(
            receiver, 0.0001,
            Point3(-0.45000000000000001, 0.47999999999999998, -0.62),
            VBase3(-87.469999999999999, -18.210000000000001,
                   7.8200000000000003)), Wait(3.1400000000000001),
        Func(receiver.wrtReparentTo, phone), Wait(0.62),
        LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_NEARZERO),
        Func(MovieUtil.removeProps, [receiver, phone]))
    toonTrack = getToonTrack(attack, 2.7000000000000002, ['cringe'],
                             1.8999999999999999, ['sidestep'])
...
def doShred(attack):
    ...
    paperPropTrack = getPropTrack(
        ...
        scaleUpTime=0.20000000000000001,
        anim=1,
        propName='shredder-paper',
        animDuration=1.5,
        animStartTime=2.7999999999999998)
    shredderPosPoints = [
    ...
    shredderPropTrack = getPropTrack(
        ...
        1,
        3,
        scaleUpPoint=Point3(4.8099999999999996, 4.8099999999999996,
                            4.8099999999999996))
    toonTrack = getToonTrack(
        attack,
        suitTrack.getDuration() - 1.1000000000000001, ['conked'],
        suitTrack.getDuration() - 3.1000000000000001, ['sidestep'])
    soundTrack = getSoundTrack(
        'SA_shred.mp3', delay=3.3999999999999999, node=suit)
...
def doFillWithLead(attack):
    ...
    BattleParticles.loadParticles()
    sprayEffect = BattleParticles.createParticleEffect(
        file='fillWithLeadSpray')
    headSmotherEffect = BattleParticles.createParticleEffect(
        file='fillWithLeadSmother')
    torsoSmotherEffect = BattleParticles.createParticleEffect(
        file='fillWithLeadSmother')
    legsSmotherEffect = BattleParticles.createParticleEffect(
        file='fillWithLeadSmother')
    BattleParticles.setEffectTexture(
    ...
    pencilPropTrack = getPropTrack(
        ...
        scaleUpTime=0.20000000000000001)
    sharpenerPosPoints = [
        Point3(0.0, 0.0, -0.029999999999999999), MovieUtil.PNT3_ZERO
    ]
    ...
    damageAnims.append([
        'conked',
        suitTrack.getDuration() - 1.5, 1.0000000000000001e-005,
        1.3999999999999999
    ])
    damageAnims.append([
        'conked', 1.0000000000000001e-005, 0.69999999999999996,
        0.69999999999999996
    ])
    damageAnims.append([
        'conked', 1.0000000000000001e-005, 0.69999999999999996,
    ...
    damageAnims.append(['conked', 1.0000000000000001e-005, 1.3999999999999999])
    toonTrack = getToonTrack(
        attack,
        splicedDamageAnims=damageAnims,
        dodgeDelay=suitTrack.getDuration() - 3.1000000000000001,
        dodgeAnimNames=['sidestep'],
        showDamageExtraTime=4.5,
        showMissedExtraTime=1.6000000000000001)
    ...
    torsoEffectHeight = ToontownGlobals.torsoHeightDict[toon.style.torso] * \
        bodyScale / 2 + legsHeight
    ...
    legsSmotherEffect.setPos(effectX, effectY - 0.59999999999999998,
                             legsEffectHeight)
    partDelay = 3.5
    ...
    torsoTrack = getPartTrack(torsoSmotherEffect, partDelay + partIvalDelay,
                              partDuration, [torsoSmotherEffect, toon, 0])
    legsTrack = getPartTrack(legsSmotherEffect, partDelay + partIvalDelay * 2,
                             partDuration, [legsSmotherEffect, toon, 0])
...
def doFountainPen(attack):
    ...
    def hitPoint(toon=toon):
        return __toonFacePoint(toon)

    def missPoint(prop=pen, toon=toon):
        return __toonMissPoint(prop, toon, 0, parent=render)
    ...
    penSpillTrack = getPartTrack(penSpill, 1.3999999999999999,
                                 0.69999999999999996, [penSpill, pen, 0])
    toonTrack = getToonTrack(
        attack,
        1.8100000000000001, ['conked'],
        dodgeDelay=0.11,
        splicedDodgeAnims=[['duck', 0.01, 0.59999999999999998]],
        showMissedExtraTime=1.6599999999999999)
    soundTrack = getSoundTrack(
        'SA_fountain_pen.mp3', delay=1.6000000000000001, node=suit)
    return Parallel(suitTrack, toonTrack, propTrack, soundTrack, penSpillTrack,
                    splashTrack)
...
def doRubOut(attack):
    ...
    suitTrack = getSuitTrack(attack)
    padPosPoints = [
        Point3(-0.66000000000000003, 0.81000000000000005,
               -0.059999999999999998),
        VBase3(14.93, -2.29, 180.0)
    ]
    padPropTrack = getPropTrack(pad, suit.getLeftHand(), padPosPoints, 0.5,
                                2.5699999999999998)
    pencilPosPoints = [
        Point3(0.040000000000000001, -0.38, -0.10000000000000001),
        VBase3(-170.22300000000001, -3.762, -62.929000000000002)
    ]
    pencilPropTrack = getPropTrack(pencil, suit.getRightHand(),
                                   pencilPosPoints, 0.5, 2.5699999999999998)
    toonTrack = getToonTrack(attack, 2.2000000000000002, ['conked'], 2.0,
                             ['jump'])
    hideTrack = Sequence()
    headParts = toon.getHeadParts()
    torsoParts = toon.getTorsoParts()
    ...
    if dmg > 0:
        ...
        return Parallel(suitTrack, toonTrack, padPropTrack, pencilPropTrack,
                        soundTrack, hideTrack, headTrack, torsoTrack,
                        legsTrack)
    else:
        return Parallel(suitTrack, toonTrack, padPropTrack, pencilPropTrack,
                        soundTrack)
...
def doFingerWag(attack):
    ...
    BattleParticles.setEffectTexture(
        particleEffect,
        'blah',
        color=Vec4(0.55000000000000004, 0, 0.55000000000000004, 1))
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'a':
        partDelay = 1.3
    ...
    if suitName == 'mm':
        ...
        particleEffect.setPos(0.16700000000000001, 1.8999999999999999,
                              suit.getHeight() - 1.8)
...
def doWriteOff(attack):
    ...
    padPropTrack = getPropTrack(
        pad, suit.getLeftHand(), padPosPoints, 0.5, 2.5699999999999998,
        Point3(1.8899999999999999, 1.8899999999999999, 1.8899999999999999))

    def missPoint(checkmark=checkmark, toon=toon):
        return __toonMissPoint(checkmark, toon)
    ...
    pencilPropTrack = Sequence(
        Wait(0.5), Func(__showProp, *extraArgsForShowProp),
        LerpScaleInterval(
            pencil, 0.5, Point3(1.5, 1.5, 1.5), startScale=Point3(0.01)),
        Wait(2), Func(battle.movie.needRestoreRenderProp, checkmark),
        Func(checkmark.reparentTo, render),
        Func(checkmark.setScale, 1.6000000000000001),
        Func(checkmark.setPosHpr, pencil, 0, 0, 0, 0, 0, 0),
        Func(checkmark.setP, 0), Func(checkmark.setR, 0))
    pencilPropTrack.append(
        getPropThrowTrack(attack, checkmark, [__toonFacePoint(toon)],
                          [missPoint]))
    ...
    toonTrack = getToonTrack(attack, 3.3999999999999999, ['slip-forward'],
                             2.3999999999999999, ['sidestep'])
    soundTrack = Sequence(
        Wait(2.2999999999999998),
        SoundInterval(
            globalBattleSoundCache.getSound('SA_writeoff_pen_only.mp3'),
            duration=0.90000000000000002,
            node=suit),
        SoundInterval(
            globalBattleSoundCache.getSound('SA_writeoff_ding_only.mp3'),
            node=suit))
    return Parallel(suitTrack, toonTrack, padPropTrack, pencilPropTrack,
                    soundTrack)
...
def doRubberStamp(attack):
    ...
    if suitType == 'a':
        padPosPoints = [
            Point3(-0.65000000000000002, 0.82999999999999996,
                   -0.040000000000000001),
            VBase3(5.625, 4.4560000000000004, -165.125)
        ]
        stampPosPoints = [
            Point3(-0.64000000000000001, -0.17000000000000001,
                   -0.029999999999999999), MovieUtil.PNT3_ZERO
        ]
    elif suitType == 'c':
        padPosPoints = [
            Point3(0.19, -0.55000000000000004, -0.20999999999999999),
            VBase3(-166.75999999999999, -4.0010000000000003,
                   -1.6579999999999999)
        ]
        stampPosPoints = [
    ...
    padPropTrack = getPropTrack(pad, suit.getLeftHand(), padPosPoints,
                                9.9999999999999995e-007, 3.2000000000000002)

    def missPoint(cancelled=cancelled, toon=toon):
        return __toonMissPoint(cancelled, toon)

    propTrack = Sequence(
        Func(__showProp, stamp, suit.getRightHand(), stampPosPoints[0],
             stampPosPoints[1]),
        LerpScaleInterval(stamp, 0.5, MovieUtil.PNT3_ONE),
        Wait(2.6000000000000001),
        Func(battle.movie.needRestoreRenderProp, cancelled),
        Func(cancelled.reparentTo, render),
        Func(cancelled.setScale, 0.59999999999999998),
        Func(cancelled.setPosHpr, stamp, 0.81000000000000005,
             -1.1100000000000001, -0.16, 0, 0, 90), Func(cancelled.setP, 0),
        Func(cancelled.setR, 0))
    propTrack.append(
        getPropThrowTrack(attack, cancelled, [__toonFacePoint(toon)],
                          [missPoint]))
    ...
    toonTrack = getToonTrack(attack, 3.3999999999999999, ['conked'],
                             1.8999999999999999, ['sidestep'])
    soundTrack = getSoundTrack(
        'SA_rubber_stamp.mp3',
        delay=1.3,
...
def doRazzleDazzle(attack):
    ...
    if hitSuit:
        ...
        def hitPoint(particleEffect=particleEffect, toon=toon, suit=suit):
            return __toonMissPoint(
                particleEffect, toon, parent=suit.getRightHand())

    signPropTrack = Sequence(
        Wait(0.5),
        Func(__showProp, sign, suit.getRightHand(), signPosPoints[0],
             signPosPoints[1]),
        LerpScaleInterval(
            sign, 0.5,
            Point3(1.3899999999999999, 1.3899999999999999,
                   1.3899999999999999)), Wait(0.5),
    ...
    toonTrack = getToonTrack(attack, 2.6000000000000001, ['cringe'],
                             1.8999999999999999, ['sidestep'])
    soundTrack = getSoundTrack(
        'SA_razzle_dazzle.mp3', delay=1.6000000000000001, node=suit)
    return Sequence(
        Parallel(suitTrack, signPropTrack, signPropAnimTrack, toonTrack,
                 soundTrack), Func(MovieUtil.removeProp, sign))
...
def doSynergy(attack):
    ...
    partTrack = getPartTrack(particleEffect, 1.0, 1.8999999999999999,
                             [particleEffect, suit, 0])
    waterfallTrack = getPartTrack(waterfallEffect, 0.80000000000000004,
                                  1.8999999999999999,
                                  [waterfallEffect, suit, 0])
    damageAnims = [['slip-forward']]
    dodgeAnims = []
    dodgeAnims.append(['jump', 0.01, 0, 0.59999999999999998])
    dodgeAnims.extend(
        getSplicedLerpAnims('jump', 0.31, 1.3, startTime=0.59999999999999998))
    dodgeAnims.append(['jump', 0, 0.91000000000000003])
    toonTracks = getToonTracks(
        attack,
        damageDelay=damageDelay,
        damageAnimNames=['slip-forward'],
        dodgeDelay=0.91000000000000003,
        splicedDodgeAnims=dodgeAnims,
        showMissedExtraTime=1.0)
    synergySoundTrack = Sequence(
        Wait(0.90000000000000002),
        SoundInterval(
            globalBattleSoundCache.getSound('SA_synergy.mp3'), node=suit))
...
def doTeeOff(attack):
    ...
    clubPropTrack = getPropTrack(
        club, suit.getLeftHand(), clubPosPoints, 0.5, 5.2000000000000002,
        Point3(1.1000000000000001, 1.1000000000000001, 1.1000000000000001))
    suitName = attack['suitName']
    if suitName == 'ym':
        ballPosPoints = [Point3(2.1000000000000001, 0, 0.10000000000000001)]
    elif suitName == 'tbc':
        ballPosPoints = [Point3(4.0999999999999996, 0, 0.10000000000000001)]
    elif suitName == 'm':
        ballPosPoints = [Point3(3.2000000000000002, 0, 0.10000000000000001)]
    elif suitName == 'rb':
        ballPosPoints = [Point3(4.2000000000000002, 0, 0.10000000000000001)]
    else:
        ballPosPoints = [Point3(2.1000000000000001, 0, 0.10000000000000001)]
    ballPropTrack = Sequence(
        getPropAppearTrack(ball, suit, ballPosPoints, 1.7, Point3(
            1.5, 1.5, 1.5)), Func(battle.movie.needRestoreRenderProp, ball),
        Func(ball.wrtReparentTo, render), Wait(2.1499999999999999))

    def missPoint(ball=ball, toon=toon):
        return __toonMissPoint(ball, toon)
    ...
    toonTrack = getToonTrack(
        attack,
        suitTrack.getDuration() - 2.25, ['conked'],
        dodgeDelay, ['duck'],
        showMissedExtraTime=1.7)
    soundTrack = getSoundTrack(
        'SA_tee_off.mp3', delay=4.0999999999999996, node=suit)
    return Parallel(suitTrack, toonTrack, clubPropTrack, ballPropTrack,
                    soundTrack)
...
def doBrainStorm(attack):
    ...
    effectColor = Vec4(0.65000000000000002, 0.79000000000000004,
                       0.93000000000000005, 0.84999999999999998)
    BattleParticles.setEffectTexture(
    ...
    cloudPropTrack.append(
        getPropAppearTrack(
            ...
            Point3(3, 3, 3),
            scaleUpTime=0.69999999999999996))
    ...
    cloudPropTrack.append(
        Parallel(
            ...
            Sequence(
                ...
                ActorInterval(
                    cloud, 'stormcloud', startTime=2.5, duration=0.5),
    ...
    damageAnims = [['cringe', 0.01, 0.40000000000000002, 0.80000000000000004],
                   ['duck', 9.9999999999999995e-007, 1.6000000000000001]]
    toonTrack = getToonTrack(
        attack,
        damageDelay=damageDelay,
        splicedDamageAnims=damageAnims,
        ...
        dodgeAnimNames=['sidestep'],
        showMissedExtraTime=1.1000000000000001)
    soundTrack = getSoundTrack(
        'SA_brainstorm.mp3', delay=2.6000000000000001, node=suit)
...
def doBuzzWord(attack):
    ...
    for i in range(0, 5):
        ...
        if random.random() > 0.5:
            BattleParticles.setEffectTexture(
                effect,
                texturesList[i],
                color=Vec4(1, 0.93999999999999995, 0.02, 1))
        else:
            BattleParticles.setEffectTexture(
    ...
    if suitName == 'm':
        ...
        for effect in particleEffects:
            effect.setPos(0, 2.1000000000000001,
                          suit.getHeight() - 0.80000000000000004)
    ...
    toonTrack = getToonTrack(
        ...
        splicedDodgeAnims=[['duck', dodgeDelay, 1.3999999999999999]],
        showMissedExtraTime=dodgeDelay + 0.5)
    soundTrack = getSoundTrack(
        'SA_buzz_word.mp3', delay=3.8999999999999999, node=suit)
...
def doDemotion(attack):
    ...
    unFreezeEffect = BattleParticles.createParticleEffect(
        file='demotionUnFreeze')
    ...
    suitTrack = getSuitTrack(attack)
    partTrack = getPartTrack(sprayEffect, 0.69999999999999996,
                             1.1000000000000001, [sprayEffect, suit, 0])
    partTrack2 = getPartTrack(freezeEffect, 1.3999999999999999,
                              2.8999999999999999, [freezeEffect, toon, 0])
    partTrack3 = getPartTrack(unFreezeEffect, 6.6500000000000004, 0.5,
                              [unFreezeEffect, toon, 0])
    dodgeAnims = [['duck', 9.9999999999999995e-007, 0.80000000000000004]]
    damageAnims = []
    damageAnims.append(['cringe', 0.01, 0, 0.5])
    damageAnims.extend(
        getSplicedLerpAnims('cringe', 0.40000000000000002, 0.5, startTime=0.5))
    damageAnims.extend(
        getSplicedLerpAnims(
            'cringe', 0.29999999999999999, 0.5, startTime=0.90000000000000002))
    damageAnims.extend(
        getSplicedLerpAnims(
            'cringe', 0.29999999999999999, 0.59999999999999998, startTime=1.2))
    damageAnims.append(['cringe', 2.6000000000000001, 1.5])
    toonTrack = getToonTrack(
        attack,
        damageDelay=1.0,
        splicedDamageAnims=damageAnims,
...
def doCanned(attack):
    ...
    torso = torso[0]
    if torso == 's':
        scaleUpPoint = Point3(scale * 2.6299999999999999,
                              scale * 2.6299999999999999,
                              scale * 1.9975000000000001)
    elif torso == 'm':
        scaleUpPoint = Point3(scale * 2.6299999999999999,
                              scale * 2.6299999999999999,
                              scale * 1.7975000000000001)
    elif torso == 'l':
        scaleUpPoint = Point3(scale * 2.6299999999999999,
                              scale * 2.6299999999999999,
                              scale * 2.3100000000000001)

    canHpr = VBase3(-173.47, -0.41999999999999998, 162.09)
    suitTrack = getSuitTrack(attack)
    posPoints = [
        Point3(-0.14000000000000001, 0.14999999999999999,
    ...
    if dmg > 0:
        ...
        can2Point = Point3(hitPoint.getX(),
                           hitPoint.getY() + 6.4000000000000004,
                           hitPoint.getZ())
        ...
        soundTrack = Sequence(
            Wait(2.6000000000000001),
            SoundInterval(
                globalBattleSoundCache.getSound('SA_canned_tossup_only.mp3'),
                node=suit),
            SoundInterval(
                globalBattleSoundCache.getSound('SA_canned_impact_only.mp3'),
                node=suit))
    else:
        land = toon.getPos(battle)
        land.setZ(land.getZ() + 0.69999999999999996)
        bouncePoint1 = Point3(land.getX(),
                              land.getY() - 1.5,
                              land.getZ() + 2.5)
        bouncePoint2 = Point3(land.getX(),
                              land.getY() - 2.1000000000000001,
                              land.getZ() - 0.20000000000000001)
        bouncePoint3 = Point3(land.getX(),
                              land.getY() - 3.1000000000000001,
                              land.getZ() + 1.5)
        bouncePoint4 = Point3(land.getX(),
                              land.getY() - 4.0999999999999996,
                              land.getZ() + 0.29999999999999999)
        throwTrack.append(LerpPosInterval(can, 0.40000000000000002, land))
        throwTrack.append(
            LerpPosInterval(can, 0.40000000000000002, bouncePoint1))
        throwTrack.append(
            LerpPosInterval(can, 0.29999999999999999, bouncePoint2))
        throwTrack.append(
            LerpPosInterval(can, 0.29999999999999999, bouncePoint3))
        throwTrack.append(
            LerpPosInterval(can, 0.29999999999999999, bouncePoint4))
        throwTrack.append(Wait(1.1000000000000001))
        throwTrack.append(
            LerpScaleInterval(can, 0.29999999999999999,
                              MovieUtil.PNT3_NEARZERO))
        scaleTrack = Sequence(
            Wait(propDelay + suitDelay),
        ...
        hprTrack = Sequence(
            ...
            LerpHprInterval(
                can, 0.40000000000000002,
                Point3(83.269999999999996, 19.52, -177.91999999999999)),
            LerpHprInterval(
                can, 0.29999999999999999,
                Point3(95.239999999999995, -72.090000000000003,
                       88.650000000000006)),
            LerpHprInterval(
                can, 0.20000000000000001,
                Point3(-96.340000000000003, -2.6299999999999999,
                       179.88999999999999)))
        soundTrack = getSoundTrack(
            'SA_canned_tossup_only.mp3', delay=2.6000000000000001, node=suit)
    canTrack = Sequence(
        Parallel(throwTrack, scaleTrack, hprTrack),
        Func(MovieUtil.removeProp, can), Func(battle.movie.clearRenderProp,
                                              can))
    damageAnims = [[
        'struggle', propDelay + suitDelay + throwDuration, 0.01,
        0.69999999999999996
    ], ['slip-backward', 0.01, 0.45000000000000001]]
    toonTrack = getToonTrack(
        attack,
        splicedDamageAnims=damageAnims,
        dodgeDelay=dodgeDelay,
        dodgeAnimNames=['sidestep'],
        showDamageExtraTime=propDelay + suitDelay + 2.3999999999999999)
...
def doDownsize(attack):
    ...
    cloudPos = Point3(toonPos.getX(), toonPos.getY(),
                      toonPos.getZ() + toon.getHeight() * 0.55000000000000004)
    cloudEffect.setPos(cloudPos)
    suitTrack = getSuitTrack(attack)
    sprayTrack = getPartTrack(sprayEffect, 1.0, 1.28, [sprayEffect, suit, 0])
    cloudTrack = getPartTrack(cloudEffect, 2.1000000000000001,
                              1.8999999999999999, [cloudEffect, toon, 0])
    if dmg > 0:
        initialScale = toon.getScale()
        downScale = Vec3(0.40000000000000002, 0.40000000000000002,
                         0.40000000000000002)
        shrinkTrack = Sequence(
            Wait(damageDelay + 0.5), Func(battle.movie.needRestoreToonScale),
            LerpScaleInterval(toon, 1.0, downScale * 1.1000000000000001),
            LerpScaleInterval(toon, 0.10000000000000001,
                              downScale * 0.90000000000000002),
            LerpScaleInterval(toon, 0.10000000000000001, downScale * 1.05),
            LerpScaleInterval(toon, 0.10000000000000001,
                              downScale * 0.94999999999999996),
            LerpScaleInterval(toon, 0.10000000000000001, downScale),
            Wait(2.1000000000000001),
            ...
            LerpScaleInterval(toon, 0.14999999999999999,
                              initialScale * 0.80000000000000004),
    ...
    damageAnims.append(['lose', 0.01, 2.1699999999999999, 0.93000000000000005])
    damageAnims.append(
        ['lose', 0.01, 3.1000000000000001, -0.93000000000000005])
    damageAnims.append(['struggle', 0.01, 0.80000000000000004, 1.8])
    damageAnims.append(['sidestep-right', 0.01, 2.9700000000000002, 1.49])
    toonTrack = getToonTrack(
        attack,
        damageDelay=damageDelay,
        splicedDamageAnims=damageAnims,
        dodgeDelay=0.59999999999999998,
        dodgeAnimNames=['sidestep'])
    if dmg > 0:
        return Parallel(suitTrack, sprayTrack, cloudTrack, shrinkTrack,
                        toonTrack)
    else:
...
def doPinkSlip(attack):
    ...
    suitTrack = getSuitTrack(attack)
    posPoints = [
        Point3(0.070000000000000007, -0.059999999999999998,
               -0.17999999999999999),
    ...
    hitPoint = __toonGroundPoint(
        attack, toon, 0.20000000000000001, parent=battle)
    ...
    if dmg > 0:
        paperPause = 0.01
        slidePoint = Point3(hitPoint.getX(),
                            hitPoint.getY() - 5,
                            hitPoint.getZ() + 4)
        landPoint = Point3(hitPoint.getX(),
                           hitPoint.getY() - 5, hitPoint.getZ())
        paperAppearTrack.append(Wait(paperPause))
        paperAppearTrack.append(
            LerpPosInterval(paper, 0.20000000000000001, slidePoint))
        paperAppearTrack.append(
            LerpPosInterval(paper, 1.1000000000000001, landPoint))
    ...
    damageAnims = [['jump', 0.01, 0.29999999999999999, 0.69999999999999996],
                   ['slip-forward', 0.01]]
    toonTrack = getToonTrack(
        attack,
        damageDelay=2.8100000000000001,
        splicedDamageAnims=damageAnims,
        dodgeDelay=2.7999999999999998,
        dodgeAnimNames=['jump'],
        showDamageExtraTime=0.90000000000000002)
    soundTrack = getSoundTrack(
        'SA_pink_slip.mp3',
        delay=2.8999999999999999,
        duration=1.1000000000000001,
        node=suit)
...
def doReOrg(attack):
    ...
    if dmg > 0:
        ...
        for partNum in range(0, headParts.getNumPaths()):
            ...
            headTracks.append(
                Sequence(
                    Wait(attackDelay),
                    LerpPosInterval(
                        part, 0.10000000000000001,
                        Point3(x - 0.20000000000000001, y,
                               z - 0.029999999999999999)),
                    LerpPosInterval(
                        part, 0.10000000000000001,
                        Point3(x + 0.40000000000000002, y,
                               z - 0.029999999999999999)),
                    LerpPosInterval(
                        part, 0.10000000000000001,
                        Point3(x - 0.40000000000000002, y,
                    ...
                    LerpPosInterval(
                        part, 0.10000000000000001,
                        Point3(x - 0.20000000000000001, y,
                               z - 0.040000000000000001)),
                    LerpPosInterval(part, 0.25,
                                    Point3(x, y, z + 2.2000000000000002)),
                    LerpHprInterval(part, 0.40000000000000002,
                                    VBase3(360, 0, 180)),
                    LerpPosInterval(part, 0.29999999999999999,
                                    Point3(x, y, z + 3.1000000000000001)),
                    LerpPosInterval(part, 0.14999999999999999,
                                    Point3(x, y, z + 0.29999999999999999)),
                    Wait(0.14999999999999999),
                    LerpHprInterval(
                        part,
                        0.59999999999999998,
                        VBase3(-745, 0, 180),
                    ...
                    LerpPosInterval(part, 0.14999999999999999,
                                    Point3(x, y, z + 1)),
                    LerpHprInterval(part, 0.29999999999999999, VBase3(h, p,
                                                                      r)),
                    Wait(0.20000000000000001),
                    LerpPosInterval(part, 0.10000000000000001, Point3(x, y,
                                                                      z)),
                    Wait(0.90000000000000002)))
        ...
        def getChestTrack(part, attackDelay=attackDelay):
            ...
            return Sequence(
                Wait(attackDelay),
                LerpHprInterval(part, 1.1000000000000001, VBase3(180, 0, 0)),
                Wait(1.1000000000000001),
    ...
    damageAnims = [['neutral', 0.01, 0.01, 0.5], ['juggle', 0.01, 0.01, 1.48],
                   ['think', 0.01, 2.2799999999999998]]
    dodgeAnims = []
    dodgeAnims.append(['think', 0.01, 0, 0.59999999999999998])
    toonTrack = getToonTrack(
        attack,
        damageDelay=damageDelay,
        splicedDamageAnims=damageAnims,
        dodgeDelay=0.01,
        dodgeAnimNames=['duck'],
        showDamageExtraTime=2.1000000000000001,
        showMissedExtraTime=2.0)
    if dmg > 0:
        return Parallel(suitTrack, partTrack, toonTrack, headTracks,
                        chestTracks)
    else:
...
def doSacked(attack):
    ...
    scaleUpPoint = Point3(1.05, 1.6699999999999999,
                          0.97999999999999998) * 4.0999999999999996
    sackHpr = VBase3(-154.33000000000001, -6.3300000000000001,
                     163.80000000000001)
    suitTrack = getSuitTrack(attack)
    posPoints = [
    ...
    if dmg > 0:
        ...
        scaleTrack = Sequence(
            Wait(propDelay + suitDelay),
            LerpScaleInterval(sack, throwDuration, scaleUpPoint), Wait(1.8),
            LerpScaleInterval(sack, 0.29999999999999999,
                              MovieUtil.PNT3_NEARZERO))
        hprTrack = Sequence(
            Wait(propDelay + suitDelay),
        ...
        sackTrack = Sequence(sackAppearTrack, Func(MovieUtil.removeProp, sack),
                             Func(battle.movie.clearRenderProp, sack))
    damageAnims = [['struggle', 0.01, 0.01, 0.69999999999999996],
                   ['slip-backward', 0.01, 0.45000000000000001]]
    toonTrack = getToonTrack(
        attack,
        ...
        showMissedExtraTime=0.80000000000000004)
...
def doGlowerPower(attack):
    ...
    if suitName == 'hh':
        leftPosPoints = [
            Point3(0.29999999999999999, 4.2999999999999998,
                   5.2999999999999998), MovieUtil.PNT3_ZERO
        ]
        rightPosPoints = [
            Point3(-0.29999999999999999, 4.2999999999999998,
                   5.2999999999999998), MovieUtil.PNT3_ZERO
        ...
        leftPosPoints = [
            Point3(0.59999999999999998, 4.5, 6), MovieUtil.PNT3_ZERO
        ]
        rightPosPoints = [
            Point3(-0.59999999999999998, 4.5, 6), MovieUtil.PNT3_ZERO
        ]
        ...
        leftPosPoints = [
            Point3(0.40000000000000002, 3.7999999999999998,
                   3.7000000000000002), MovieUtil.PNT3_ZERO
        ]
        rightPosPoints = [
            Point3(-0.40000000000000002, 3.7999999999999998,
                   3.7000000000000002), MovieUtil.PNT3_ZERO
    ...
    for i in range(0, 3):
        ...
        leftTrack.append(
            getPropAppearTrack(
                ...
                leftPosPoints,
                9.9999999999999995e-007,
                Point3(0.40000000000000002, 0.40000000000000002,
                       0.40000000000000002),
                scaleUpTime=0.10000000000000001))
        leftTrack.append(
            getPropThrowTrack(
                attack,
                leftKnives[i],
                hitPointNames=['face'],
                missPointNames=['miss'],
                hitDuration=0.29999999999999999,
                missDuration=0.29999999999999999))
    ...
    damageAnims = [['slip-backward', 0.01, 0.34999999999999998]]
    toonTrack = getToonTrack(
        attack,
        damageDelay=1.6000000000000001,
        splicedDamageAnims=damageAnims,
        dodgeDelay=0.69999999999999996,
        dodgeAnimNames=['sidestep'])
    soundTrack = getSoundTrack(
        'SA_glower_power.mp3', delay=1.1000000000000001, node=suit)
    return Parallel(suitTrack, toonTrack, soundTrack, leftKnifeTracks,
                    rightKnifeTracks)
...
def doHalfWindsor(attack):
    ...
    tiePropTrack.append(
        getPropThrowTrack(
            attack,
            tie, [hitPoint], [missPoint],
            hitDuration=0.40000000000000002,
            missDuration=0.80000000000000004,
            missScaleDown=0.29999999999999999,
            parent=battle))
    damageAnims = [['conked', 0.01, 0.01, 0.40000000000000002],
                   ['cringe', 0.01, 0.69999999999999996]]
    toonTrack = getToonTrack(
        attack,
        damageDelay=damageDelay,
        splicedDamageAnims=damageAnims,
...
def doHeadShrink(attack):
    ...
    suitTrack = getSuitTrack(attack)
    sprayTrack = getPartTrack(shrinkSpray, 0.29999999999999999,
                              1.3999999999999999, [shrinkSpray, suit, 0])
    ...
    for point in cloudPoints:
        circleTrack.append(
            LerpPosInterval(
                shrinkCloud, 0.14000000000000001, point, other=battle))
    ...
    if dmg > 0:
        ...
        def scaleHeadParallel(scale, duration, headParts=headParts):
            ...
            for partNum in range(0, headParts.getNumPaths()):
                nextPart = headParts.getPath(partNum)
                headTracks.append(
                    LerpScaleInterval(nextPart, duration,
        ...
        shrinkTrack.append(
            scaleHeadParallel(0.59999999999999998, shrinkDuration))
        shrinkTrack.append(Wait(1.6000000000000001))
        shrinkTrack.append(
            scaleHeadParallel(initialScale * 3.2000000000000002,
                              0.40000000000000002))
        shrinkTrack.append(
            scaleHeadParallel(initialScale * 0.69999999999999996,
                              0.40000000000000002))
        shrinkTrack.append(
            scaleHeadParallel(initialScale * 2.5, 0.29999999999999999))
        shrinkTrack.append(
            scaleHeadParallel(initialScale * 0.80000000000000004,
                              0.29999999999999999))
        shrinkTrack.append(
            scaleHeadParallel(initialScale * 1.8999999999999999,
                              0.20000000000000001))
        shrinkTrack.append(
            scaleHeadParallel(initialScale * 0.84999999999999998,
                              0.20000000000000001))
        shrinkTrack.append(
            scaleHeadParallel(initialScale * 1.7, 0.14999999999999999))
        shrinkTrack.append(
            scaleHeadParallel(initialScale * 0.90000000000000002,
                              0.14999999999999999))
        shrinkTrack.append(
            scaleHeadParallel(initialScale * 1.3, 0.10000000000000001))
        shrinkTrack.append(
            scaleHeadParallel(initialScale, 0.10000000000000001))
    ...
    damageAnims = []
    damageAnims.append(
        ['cringe', 0.01, 0.65000000000000002, 0.20000000000000001])
    damageAnims.extend(
        getSplicedLerpAnims(
            'cringe', 0.64000000000000001, 1.0, startTime=0.84999999999999998))
    damageAnims.append(['cringe', 0.40000000000000002, 1.49])
    damageAnims.append(
        ['conked', 0.01, 3.6000000000000001, -1.6000000000000001])
    damageAnims.append(
        ['conked', 0.01, 3.1000000000000001, 0.40000000000000002])
    toonTrack = getToonTrack(
        attack,
        damageDelay=damageDelay,
        splicedDamageAnims=damageAnims,
        ...
        dodgeAnimNames=['sidestep'])
    if dmg > 0:
        shrinkSound = globalBattleSoundCache.getSound(
            'SA_head_shrink_only.mp3')
        growSound = globalBattleSoundCache.getSound(
            'SA_head_grow_back_only.mp3')
        soundTrack = Sequence(
            Wait(2.1000000000000001),
            SoundInterval(shrinkSound, duration=2.1000000000000001, node=suit),
            Wait(1.6000000000000001), SoundInterval(growSound, node=suit))
...
def doRolodex(attack):
    ...
    particleEffect2 = BattleParticles.createParticleEffect(
        file='rollodexWaterfall')
    particleEffect3 = BattleParticles.createParticleEffect(
        file='rollodexStream')
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'a':
        propPosPoints = [
            Point3(-0.51000000000000001, -0.029999999999999999,
                   -0.10000000000000001),
        ...
        propScale = Point3(0.91000000000000003, 0.91000000000000003,
                           0.91000000000000003)
    ...
    toonTrack = getToonTrack(attack, damageDelay, ['conked'], dodgeDelay,
                             ['sidestep'])
    soundTrack = getSoundTrack(
        'SA_rolodex.mp3', delay=2.7999999999999998, node=suit)
    return Parallel(suitTrack, toonTrack, propTrack, soundTrack, partTrack2,
                    partTrack3)
...
def doEvilEye(attack):
    ...
    if suitName == 'cr':
        posPoints = [
            Point3(-0.46000000000000002, 4.8499999999999996,
                   5.2800000000000002),
            VBase3(-155.0, -20.0, 0.0)
        ]
    elif suitName == 'tf':
        posPoints = [
            Point3(-0.40000000000000002, 3.6499999999999999,
                   5.0099999999999998),
            VBase3(-155.0, -20.0, 0.0)
        ]
    elif suitName == 'le':
        posPoints = [
            Point3(-0.64000000000000001, 4.4500000000000002,
                   5.9100000000000001),
            VBase3(-155.0, -20.0, 0.0)
        ]
    else:
    ...
    eyeAppearTrack = Sequence(
        ...
        Wait(eyeHoldDuration * 0.29999999999999999),
        LerpHprInterval(eye, 0.02, Point3(205, 40, 0)),
        Wait(eyeHoldDuration * 0.69999999999999996),
    ...
    if dmg > 0:
        ...
        lerpInterval = LerpPosInterval(
            eye, moveDuration,
            Point3(toonFace.getX(),
                   toonFace.getY() - 5,
                   toonFace.getZ() - 2))
    eyeMoveTrack = lerpInterval
    eyeRollTrack = LerpHprInterval(eye, moveDuration, Point3(0, 0, -180))
    eyePropTrack = Sequence(eyeAppearTrack, Parallel(eyeMoveTrack,
                                                     eyeRollTrack),
                            Func(battle.movie.clearRenderProp, eye),
                            Func(MovieUtil.removeProp, eye))
    damageAnims = [['duck', 0.01, 0.01, 1.3999999999999999],
                   ['cringe', 0.01, 0.29999999999999999]]
    toonTrack = getToonTrack(
        attack,
        splicedDamageAnims=damageAnims,
        damageDelay=damageDelay,
...
def doPlayHardball(attack):
    ...
    if dmg > 0:
        ...
        propTrack.append(
            LerpPosInterval(ball, 0.40000000000000002, Point3(x, y + 5,
                                                              z + 2)))
        propTrack.append(
            LerpPosInterval(ball, 0.29999999999999999, Point3(x, y + 6, z)))
        propTrack.append(
            LerpPosInterval(ball, 0.10000000000000001, Point3(x, y + 7,
                                                              z + 1)))
        propTrack.append(
            LerpPosInterval(ball, 0.10000000000000001, Point3(x, y + 8, z)))
        propTrack.append(
            LerpPosInterval(ball, 0.10000000000000001,
                            Point3(x, y + 8.5, z + 0.59999999999999998)))
        propTrack.append(
            LerpPosInterval(ball, 0.10000000000000001,
                            Point3(x, y + 9, z + 0.20000000000000001)))
        propTrack.append(Wait(0.40000000000000002))
        soundTrack = getSoundTrack(
            'SA_hardball_impact_only.mp3', delay=2.7999999999999998, node=suit)
    else:
        propTrack.append(LerpPosInterval(ball, 0.5, Point3(x, y + 2, z)))
        propTrack.append(
            LerpPosInterval(ball, 0.40000000000000002, Point3(x, y - 1,
                                                              z + 2)))
        propTrack.append(
            LerpPosInterval(ball, 0.29999999999999999, Point3(x, y - 3, z)))
        propTrack.append(
            LerpPosInterval(ball, 0.10000000000000001, Point3(x, y - 4,
                                                              z + 1)))
        propTrack.append(
            LerpPosInterval(ball, 0.10000000000000001, Point3(x, y - 5, z)))
        propTrack.append(
            LerpPosInterval(ball, 0.10000000000000001,
                            Point3(x, y - 5.5, z + 0.59999999999999998)))
        propTrack.append(
            LerpPosInterval(ball, 0.10000000000000001,
                            Point3(x, y - 6, z + 0.20000000000000001)))
        propTrack.append(Wait(0.40000000000000002))
        soundTrack = getSoundTrack(
            'SA_hardball.mp3', delay=3.1000000000000001, node=suit)
    propTrack.append(
        LerpScaleInterval(ball, 0.29999999999999999, MovieUtil.PNT3_NEARZERO))
    ...
    damageAnims = [['conked', damageDelay, 0.01, 0.5],
                   ['slip-backward', 0.01, 0.69999999999999996]]
    toonTrack = getToonTrack(
        attack,
        splicedDamageAnims=damageAnims,
        dodgeDelay=dodgeDelay,
        dodgeAnimNames=['sidestep'],
        showDamageExtraTime=3.8999999999999999)
    return Parallel(suitTrack, toonTrack, propTrack, soundTrack)
...
def doPowerTie(attack):
    ...
    toonTrack = getToonTrack(attack, damageDelay, ['conked'], dodgeDelay,
                             ['sidestep'])
    throwSound = getSoundTrack(
        'SA_powertie_throw.mp3', delay=2.2999999999999998, node=suit)
    if dmg > 0:
        hitSound = getSoundTrack(
            'SA_powertie_impact.mp3', delay=2.8999999999999999, node=suit)
        return Parallel(suitTrack, toonTrack, tiePropTrack, throwSound,
                        hitSound)
    else:
        return Parallel(suitTrack, toonTrack, tiePropTrack, throwSound)
...
def doDoubleTalk(attack):
    ...
    partTrack2 = getPartTrack(particleEffect2, partDelay, 1.8,
                              [particleEffect2, suit, 0])
    damageAnims = [['duck', 0.01, 0.40000000000000002, 1.05],
                   ['cringe', 9.9999999999999995e-007, 0.80000000000000004]]
    toonTrack = getToonTrack(
        attack,
        damageDelay=damageDelay,
        splicedDamageAnims=damageAnims,
        dodgeDelay=dodgeDelay,
        splicedDodgeAnims=[['duck', 0.01, 1.3999999999999999]],
        showMissedExtraTime=0.90000000000000002,
        showDamageExtraTime=0.80000000000000004)
...
def doHotAir(attack):
    ...
    BattleParticles.setEffectTexture(
        flecksEffect,
        'roll-o-dex',
        color=Vec4(0.94999999999999996, 0.94999999999999996, 0.0, 1))
    sprayDelay = 1.3
    ...
    damageAnims = []
    damageAnims.append(['cringe', 0.01, 0.69999999999999996, 0.62])
    damageAnims.append(['slip-forward', 0.01, 0.40000000000000002, 1.2])
    ...
    toonTrack = getToonTrack(
        ...
        dodgeAnimNames=['sidestep'])
    soundTrack = getSoundTrack(
        'SA_hot_air.mp3', delay=1.6000000000000001, node=suit)
    if dmg > 0:
...
def doPickPocket(attack):
    ...
    billPropTrack = getPropTrack(
        ...
        0.59999999999999998,
        0.55000000000000004,
        scaleUpPoint=Point3(1.4099999999999999, 1.4099999999999999,
                            1.4099999999999999))
    toonTrack = getToonTrack(attack, 0.59999999999999998, ['cringe'], 0.01,
                             ['sidestep'])
    multiTrackList = Parallel(suitTrack, toonTrack)
    if dmg > 0:
        soundTrack = getSoundTrack(
            'SA_pick_pocket.mp3', delay=0.20000000000000001, node=suit)
...
def doFilibuster(attack):
    ...
    BattleParticles.setEffectTexture(
        sprayEffect, 'filibuster-cut', color=color)
    BattleParticles.setEffectTexture(
        sprayEffect2, 'filibuster-fiscal', color=color)
    BattleParticles.setEffectTexture(
        sprayEffect3, 'filibuster-impeach', color=color)
    BattleParticles.setEffectTexture(
        sprayEffect4, 'filibuster-inc', color=color)
    partDelay = 1.3
    ...
    sprayTrack = getPartTrack(sprayEffect, partDelay, partDuration,
                              [sprayEffect, suit, 0])
    sprayTrack2 = getPartTrack(sprayEffect2, partDelay + 0.80000000000000004,
                               partDuration, [sprayEffect2, suit, 0])
    sprayTrack3 = getPartTrack(sprayEffect3, partDelay + 1.6000000000000001,
                               partDuration, [sprayEffect3, suit, 0])
    sprayTrack4 = getPartTrack(sprayEffect4, partDelay + 2.3999999999999999,
                               partDuration, [sprayEffect4, suit, 0])
    damageAnims = []
    for i in range(0, 4):
        damageAnims.append([
            'cringe', 1.0000000000000001e-005, 0.29999999999999999,
            0.80000000000000004
        ])
    ...
    toonTrack = getToonTrack(
        ...
        dodgeAnimNames=['sidestep'])
    soundTrack = getSoundTrack(
        'SA_filibuster.mp3', delay=1.1000000000000001, node=suit)
    if dmg > 0:
        return Parallel(suitTrack, toonTrack, soundTrack, sprayTrack,
                        sprayTrack2, sprayTrack3, sprayTrack4)
    else:
        return Parallel(suitTrack, toonTrack, soundTrack, sprayTrack,
                        sprayTrack2, sprayTrack3)
...
def doSchmooze(attack):
    ...
    lowerEffects = []
    textureNames = [
        'schmooze-genius', 'schmooze-instant', 'schmooze-master',
        'schmooze-viz'
    ]
    for i in range(0, 4):
    ...
    for i in range(0, 4):
        upperPartTracks.append(
            getPartTrack(upperEffects[i], partDelay + i * 0.65000000000000002,
                         0.80000000000000004, [upperEffects[i], suit, 0]))
        lowerPartTracks.append(
            getPartTrack(
                lowerEffects[i],
                partDelay + i * 0.65000000000000002 + 0.69999999999999996, 1.0,
                [lowerEffects[i], suit, 0]))

    damageAnims = []
    for i in range(0, 3):
        damageAnims.append(
            ['conked', 0.01, 0.29999999999999999, 0.70999999999999996])

    damageAnims.append(['conked', 0.01, 0.29999999999999999])
    dodgeAnims = []
    dodgeAnims.append(['duck', 0.01, 0.20000000000000001, 2.7000000000000002])
    dodgeAnims.append(['duck', 0.01, 1.22, 1.28])
    dodgeAnims.append(['duck', 0.01, 3.1600000000000001])
    toonTrack = getToonTrack(
        attack,
        damageDelay=damageDelay,
        splicedDamageAnims=damageAnims,
        dodgeDelay=dodgeDelay,
        splicedDodgeAnims=dodgeAnims,
        showMissedExtraTime=1.8999999999999999,
        showDamageExtraTime=1.1000000000000001)
...
def doQuake(attack):
    ...
    toonTracks = getToonTracks(
        ...
        dodgeDelay=1.1000000000000001,
        splicedDodgeAnims=dodgeAnims,
        showMissedExtraTime=2.7999999999999998,
        showDamageExtraTime=1.1000000000000001)
...
def doShake(attack):
    ...
    toonTracks = getToonTracks(
        attack,
        damageDelay=1.1000000000000001,
        splicedDamageAnims=damageAnims,
        dodgeDelay=0.69999999999999996,
        splicedDodgeAnims=dodgeAnims,
        showMissedExtraTime=2.7999999999999998,
...
def doTremor(attack):
    ...
    toonTracks = getToonTracks(
        ...
        showDamageExtraTime=1.1000000000000001)
    soundTrack = getSoundTrack(
        'SA_tremor.mp3', delay=0.90000000000000002, node=suit)
...
def doHangUp(attack):
    ...
    if suitName == 'tf':
        ...
        receiverPosPoints = [
            ...
            VBase3(-1.8540000000000001, 2.4340000000000002,
                   -177.57900000000001)
        ]
        receiverAdjustScale = Point3(0.80000000000000004, 0.80000000000000004,
                                     0.80000000000000004)
        pickupDelay = 0.44
    ...
    propTrack = Sequence(
        ...
        Wait(pickupDelay), Func(receiver.wrtReparentTo, suit.getRightHand()),
        LerpScaleInterval(receiver, 0.01, receiverAdjustScale),
        LerpPosHprInterval(
            receiver, 0.0001,
            Point3(-0.53000000000000003, 0.20999999999999999,
                   -0.54000000000000004),
            VBase3(-99.489999999999995, -35.270000000000003,
                   1.8400000000000001)), Wait(dialDuration),
        Func(receiver.wrtReparentTo, phone), Wait(finalPhoneDelay),
        LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_NEARZERO),
        Func(MovieUtil.removeProps, [receiver, phone]))
    toonTrack = getToonTrack(attack, 5.5, ['slip-backward'],
                             4.7000000000000002, ['jump'])
    soundTrack = getSoundTrack('SA_hangup.mp3', delay=1.3, node=suit)
    return Parallel(suitTrack, toonTrack, propTrack, soundTrack)
...
def doRedTape(attack):
    ...
    if suitName == 'tf' or suitName == 'nc':
        tapePosPoints = [
            ...
            VBase3(-1.1519999999999999, 86.581000000000003,
                   -76.784000000000006)
        ]
    else:
        tapePosPoints = [
    ...
    tapeScaleUpPoint = Point3(0.90000000000000002, 0.90000000000000002,
                              0.23999999999999999)
    propTrack = Sequence(
        getPropAppearTrack(
            tape,
    ...
    def hitPoint(toon=toon):
        return __toonTorsoPoint(toon)

    propTrack.append(
        getPropThrowTrack(
            attack, tape, [hitPoint],
    ...
    if torso == 's':
        scaleUpPoint = Point3(scale * 2.0299999999999998,
                              scale * 2.0299999999999998,
                              scale * 0.79749999999999999)
    elif torso == 'm':
        scaleUpPoint = Point3(scale * 2.0299999999999998,
                              scale * 2.0299999999999998,
                              scale * 0.79749999999999999)
        ...
        scaleUpPoint = Point3(scale * 2.0299999999999998,
                              scale * 2.0299999999999998,
                              scale * 1.1100000000000001)

    if animal == 'h' or animal == 'd':
        tubeHeight = -0.87
        scaleUpPoint = Point3(scale * 1.6899999999999999,
                              scale * 1.6899999999999999,
    ...
    toonTrack = getToonTrack(attack, 3.3999999999999999, ['struggle'],
                             2.7999999999999998, ['jump'])
    soundTrack = getSoundTrack(
        'SA_red_tape.mp3', delay=2.8999999999999999, node=suit)
    if dmg > 0:
        return Parallel(suitTrack, toonTrack, propTrack, soundTrack,
                        tubeTracks)
    else:
...
def doParadigmShift(attack):
    ...
    if suitName == 'm':
        sprayEffect.setPos(
            Point3(-5.2000000000000002, 4.5999999999999996,
                   2.7000000000000002))
    elif suitName == 'sd':
        sprayEffect.setPos(
            Point3(-5.2000000000000002, 4.5999999999999996,
        ...
        sprayEffect.setPos(
            Point3(0.10000000000000001, 4.5999999999999996,
                   2.7000000000000002))
    suitTrack = getSuitAnimTrack(attack)
    ...
    for t in targets:
        ...
        if dmg > 0:
            ...
            liftTracks.append(
                getPartTrack(liftEffect, 1.1000000000000001,
                             4.0999999999999996, [liftEffect, battle, 0]))
            ...
            shakeTrack.append(
                LerpPosInterval(toon, 1.1000000000000001, risePoint))
            for i in range(0, 17):
                shakeTrack.append(
                    LerpPosInterval(toon, 0.029999999999999999, shakeLeft))
                shakeTrack.append(
                    LerpPosInterval(toon, 0.029999999999999999, shakeRight))

            shakeTrack.append(
                LerpPosInterval(toon, 0.10000000000000001, risePoint))
            shakeTrack.append(
                LerpPosInterval(toon, 0.10000000000000001, groundPoint))
            ...
            shadowTrack.append(
                LerpScaleInterval(
                    fakeShadow, 0.40000000000000002,
                    Point3(0.17000000000000001, 0.17000000000000001,
                           0.17000000000000001)))
            shadowTrack.append(Wait(1.8100000000000001))
            shadowTrack.append(
                LerpScaleInterval(
                    fakeShadow, 0.10000000000000001,
                    Point3(0.27000000000000002, 0.27000000000000002,
                           0.27000000000000002)))
    ...
    damageAnims.extend(
        getSplicedLerpAnims(
            ...
            0.66000000000000003,
            1.8999999999999999,
            startTime=2.0600000000000001))
    damageAnims.append(['slip-backward', 0.01, 0.5])
    dodgeAnims = []
    dodgeAnims.append(['jump', 0.01, 0, 0.59999999999999998])
    dodgeAnims.extend(
        getSplicedLerpAnims('jump', 0.31, 1.0, startTime=0.59999999999999998))
    dodgeAnims.append(['jump', 0, 0.91000000000000003])
    toonTracks = getToonTracks(
        attack,
        damageDelay=damageDelay,
        ...
        dodgeDelay=dodgeDelay,
        splicedDodgeAnims=dodgeAnims,
        showDamageExtraTime=2.7000000000000002)
    if hitAtleastOneToon == 1:
        soundTrack = getSoundTrack(
            'SA_paradigm_shift.mp3', delay=2.1000000000000001, node=suit)
        return Parallel(suitTrack, sprayTrack, soundTrack, liftTracks,
                        toonTracks, toonRiseTracks)
    else:
        return Parallel(suitTrack, sprayTrack, liftTracks, toonTracks,
                        toonRiseTracks)
...
def doPowerTrip(attack):
    suit = attack['suit']
    battle = attack['battle']
    centerColor = Vec4(0.10000000000000001, 0.10000000000000001,
                       0.10000000000000001, 0.40000000000000002)
    edgeColor = Vec4(0.40000000000000002, 0.10000000000000001,
    ...
    def getPowerTrack(effect, suit=suit, battle=battle):
        partTrack = Sequence(
            Wait(1.0), Func(battle.movie.needRestoreParticleEffect, effect),
            Func(effect.start, suit), Wait(0.40000000000000002),
            LerpPosInterval(effect, 1.0, Point3(0, 15, 0.40000000000000002)),
            LerpFunctionInterval(
                effect.setAlphaScale,
                fromData=1,
                toData=0,
                duration=0.40000000000000002), Func(effect.cleanup),
    ...
    return Parallel(suitTrack, partTrack1, partTrack2, waterfallTrack,
                    toonTracks)
...
def doBounceCheck(attack):
    ...
    def bounce1Point(suit=suit, toon=toon, battle=battle):
        return getThrowEndPoint(suit, toon, battle, 'one')

    def bounce2Point(suit=suit, toon=toon, battle=battle):
        return getThrowEndPoint(suit, toon, battle, 'two')

    def hit3Point(suit=suit, toon=toon, battle=battle):
        return getThrowEndPoint(suit, toon, battle, 'threeHit')

    def miss3Point(suit=suit, toon=toon, battle=battle):
        return getThrowEndPoint(suit, toon, battle, 'threeMiss')

    def bounce4Point(suit=suit, toon=toon, battle=battle):
        return getThrowEndPoint(suit, toon, battle, 'four')
    ...
    checkPropTrack.append(
        getThrowTrack(
            check, bounce2Point, duration=0.90000000000000002, parent=toon))
    if hitSuit:
        checkPropTrack.append(
            getThrowTrack(
                check, hit3Point, duration=0.69999999999999996, parent=toon))
    else:
        checkPropTrack.append(
            getThrowTrack(
                check, miss3Point, duration=0.69999999999999996, parent=toon))
        checkPropTrack.append(
            getThrowTrack(
                check, bounce4Point, duration=0.69999999999999996,
                parent=toon))
        checkPropTrack.append(
            LerpScaleInterval(check, 0.29999999999999999,
                              MovieUtil.PNT3_NEARZERO))
...
def doWatercooler(attack):
    ...
    def missPoint(prop=watercooler, toon=toon):
        return __toonMissPoint(prop, toon, 0, parent=render)

    hitSprayTrack = MovieUtil.getSprayTrack(
        battle,
        Point4(0.75, 0.75, 1.0, 0.80000000000000004),
        getCoolerSpout,
        hitPoint,
        0.20000000000000001,
        0.20000000000000001,
        0.20000000000000001,
        horizScale=0.29999999999999999,
        vertScale=0.29999999999999999)
    missSprayTrack = MovieUtil.getSprayTrack(
        battle,
        Point4(0.75, 0.75, 1.0, 0.80000000000000004),
        getCoolerSpout,
    ...
    propTrack = Sequence(
        Wait(1.01),
        Func(__showProp, watercooler, suit.getLeftHand(), posPoints[0],
             posPoints[1]),
        LerpScaleInterval(
            watercooler, 0.5,
            Point3(1.1499999999999999, 1.1499999999999999,
                   1.1499999999999999)), Wait(1.6000000000000001))
    if dmg > 0:
        propTrack.append(hitSprayTrack)
    else:
        propTrack.append(missSprayTrack)
    ...
    toonTrack = getToonTrack(attack,
                             suitTrack.getDuration() - 1.5, ['cringe'],
                             2.3999999999999999, ['sidestep'])
    soundTrack = Sequence(
        Wait(1.1000000000000001),
        SoundInterval(
            globalBattleSoundCache.getSound('SA_watercooler_appear_only.mp3'),
            node=suit,
            duration=1.4722), Wait(0.40000000000000002),
        SoundInterval(
            globalBattleSoundCache.getSound('SA_watercooler_spray_only.mp3'),
            node=suit,
            duration=2.3130000000000002))
...
def doFired(attack):
    ...
    BattleParticles.setEffectTexture(
        flecksEffect,
        'roll-o-dex',
        color=Vec4(0.80000000000000004, 0.80000000000000004,
                   0.80000000000000004, 1))
    baseFlameSmall = BattleParticles.createParticleEffect(
        file='firedBaseFlame')
    ...
    damageAnims.append(['cringe', 0.01, 0.69999999999999996, 0.62])
    damageAnims.append(
        ['slip-forward', 1.0000000000000001e-005, 0.40000000000000002, 1.2])
    damageAnims.extend(
        getSplicedLerpAnims(
            'slip-forward', 0.31, 0.80000000000000004, startTime=1.2))
    toonTrack = getToonTrack(
        attack,
        damageDelay=1.5,
        splicedDamageAnims=damageAnims,
        dodgeDelay=0.29999999999999999,
        dodgeAnimNames=['sidestep'])
...
def doAudit(attack):
    ...
    suitTrack = getSuitTrack(attack)
    partTrack = getPartTrack(particleEffect, 2.1000000000000001,
                             1.8999999999999999, [particleEffect, suit, 0])
    partTrack2 = getPartTrack(particleEffect2, 2.2000000000000002, 2.0,
                              [particleEffect2, suit, 0])
    partTrack3 = getPartTrack(particleEffect3, 2.2999999999999998,
                              2.1000000000000001, [particleEffect3, suit, 0])
    partTrack4 = getPartTrack(particleEffect4, 2.3999999999999999,
                              2.2000000000000002, [particleEffect4, suit, 0])
    partTrack5 = getPartTrack(particleEffect5, 2.5, 2.2999999999999998,
                              [particleEffect5, suit, 0])
    suitName = attack['suitName']
    if suitName == 'nc':
        calcPosPoints = [
            Point3(-0.14999999999999999, 0.37, 0.029999999999999999),
            VBase3(1.3520000000000001, -6.5179999999999998,
                   -6.0449999999999999)
        ]
        calcDuration = 0.76000000000000001
        scaleUpPoint = Point3(1.1000000000000001, 1.8500000000000001,
                              1.8100000000000001)
    else:
        calcPosPoints = [
            Point3(0.34999999999999998, 0.52000000000000002,
    ...
    calcPropTrack = getPropTrack(
        ...
        propName='calculator',
        animStartTime=0.5,
        animDuration=3.3999999999999999)
    toonTrack = getToonTrack(
        attack,
        3.2000000000000002, ['conked'],
        0.90000000000000002, ['duck'],
        showMissedExtraTime=2.2000000000000002)
    soundTrack = getSoundTrack(
        'SA_audit.mp3', delay=1.8999999999999999, node=suit)
    return Parallel(suitTrack, toonTrack, calcPropTrack, soundTrack, partTrack,
...
def doCalculate(attack):
    ...
    calcPropTrack = getPropTrack(
        ...
        animStartTime=0.5,
        animDuration=3.3999999999999999)
    toonTrack = getToonTrack(attack, 3.2000000000000002, ['conked'], 1.8,
                             ['sidestep'])
...
def doCrunch(attack):
    ...
    numberSpillTrack1 = getPartTrack(numberSpill1, 1.1000000000000001,
                                     2.2000000000000002,
                                     [numberSpill1, suit, 0])
    ...
    for i in range(0, numOfNumbers - 1):
        ...
        nextStartTime = random.random() * 0.59999999999999998 + throwDuration
        nextDuration = random.random(
        ) * 0.40000000000000002 + 1.3999999999999999
    ...
    for i in range(0, numOfNumbers):
        ...
        next = MovieUtil.copyProp(
            BattleParticles.getParticle('audit-' + texture))
        next.reparentTo(suit.getRightHand())
        next.setScale(0.01, 0.01, 0.01)
        next.setColor(Vec4(0.0, 0.0, 0.0, 1.0))
        next.setPos(
            random.random() * 0.59999999999999998 - 0.29999999999999999,
            random.random() * 0.59999999999999998 - 0.29999999999999999,
            random.random() * 0.59999999999999998 - 0.29999999999999999)
        next.setHpr(
            VBase3(-1.1499999999999999, 86.579999999999998,
                   -76.780000000000001))
        numberTrack = Sequence(
            Wait(0.90000000000000002),
            LerpScaleInterval(next, 0.59999999999999998, MovieUtil.PNT3_ONE),
            Wait(1.7), Func(MovieUtil.removeProp, next))
        numberTracks.append(numberTrack)

    damageAnims = []
    damageAnims.append(
        ['cringe', 0.01, 0.14000000000000001, 0.28000000000000003])
    damageAnims.append(['cringe', 0.01, 0.16, 0.29999999999999999])
    damageAnims.append(['cringe', 0.01, 0.13, 0.22])
    damageAnims.append(['slip-forward', 0.01, 0.59999999999999998])
    toonTrack = getToonTrack(
        attack,
        damageDelay=4.7000000000000002,
        splicedDamageAnims=damageAnims,
        dodgeDelay=3.6000000000000001,
        dodgeAnimNames=['sidestep'])
    return Parallel(suitTrack, toonTrack, numberSpillTrack1, numberSpillTrack2,
...
def doLiquidate(attack):
    ...
    cloudPropTrack.append(Func(MovieUtil.removeProp, cloud))
    cloudPropTrack.append(Func(battle.movie.clearRenderProp, cloud))
    damageAnims = [['melt'], ['jump', 1.5, 0.40000000000000002]]
    toonTrack = getToonTrack(
        attack,
        damageDelay=damageDelay,
        splicedDamageAnims=damageAnims,
    ...
    if dmg > 0:
        ...
        puddleTrack = Sequence(
            ...
            Wait(damageDelay - 0.69999999999999996),
            ...
            LerpScaleInterval(
                ...
                1.7,
                Point3(1.7, 1.7, 1.7),
                startScale=MovieUtil.PNT3_NEARZERO), Wait(3.2000000000000002),
            LerpFunctionInterval(
                puddle.setAlphaScale,
                fromData=1,
                toData=0,
...
def doMarketCrash(attack):
    ...
    movePoint = Point3(hitPoint.getX(),
                       hitPoint.getY() - 1.8,
                       hitPoint.getZ() + 0.20000000000000001)
    ...
    damageAnims = []
    damageAnims.append(
        ['cringe', 0.01, 0.20999999999999999, 0.080000000000000002])
    damageAnims.append(
        ['slip-forward', 0.01, 0.59999999999999998, 0.84999999999999998])
    damageAnims.extend(
        getSplicedLerpAnims(
            'slip-forward', 0.31, 0.94999999999999996, startTime=1.2))
    ...
    toonTrack = getToonTrack(
        ...
        dodgeDelay=2.3999999999999999,
        dodgeAnimNames=['sidestep'],
        showDamageExtraTime=0.40000000000000002,
...
def doBite(attack):
    ...
    suitTrack = getSuitTrack(attack)
    posPoints = [
        Point3(-0.050000000000000003, 0.40999999999999998,
               -0.54000000000000004),
    ...
    if dmg > 0:
        ...
        flyPoint = Point3(x, y + 2.7000000000000002,
                          toonHeight * 0.80000000000000004)
        teethAppearTrack.append(
            LerpPosInterval(teeth, throwDuration, pos=flyPoint))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.40000000000000002,
                pos=Point3(x, y + 3.2000000000000002,
                           toonHeight * 0.69999999999999996)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.29999999999999999,
                pos=Point3(x, y + 4.7000000000000002, toonHeight * 0.5)))
        teethAppearTrack.append(Wait(0.20000000000000001))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.10000000000000001,
                pos=Point3(x, y - 0.20000000000000001,
                           toonHeight * 0.90000000000000002)))
        ...
        scaleTrack = Sequence(
            ...
            Wait(1.2),
            LerpScaleInterval(teeth, 0.29999999999999999,
                              MovieUtil.PNT3_NEARZERO))
        hprTrack = Sequence(
            Wait(throwDelay),
            LerpHprInterval(teeth, 0.29999999999999999, Point3(180, 0, 0)),
            Wait(0.20000000000000001),
            LerpHprInterval(
                teeth,
                0.40000000000000002,
                Point3(180, -35, 0),
                startHpr=Point3(180, 0, 0)), Wait(0.59999999999999998),
            LerpHprInterval(
                teeth,
                0.10000000000000001,
                Point3(180, -75, 0),
                startHpr=Point3(180, -35, 0)))
        animTrack = Sequence(
            Wait(throwDelay),
            ActorInterval(teeth, 'teeth', duration=throwDuration),
            ActorInterval(teeth, 'teeth', duration=0.29999999999999999),
            Func(teeth.pose, 'teeth', 1), Wait(0.69999999999999996),
    ...
    damageAnims = [['cringe', 0.01, 0.69999999999999996, 1.2],
                   ['conked', 0.01, 0.20000000000000001, 2.1000000000000001],
                   ['conked', 0.01, 3.2000000000000002]]
    dodgeAnims = [['cringe', 0.01, 0.69999999999999996, 0.20000000000000001],
                  ['duck', 0.01, 1.6000000000000001]]
    toonTrack = getToonTrack(
        attack,
        damageDelay=3.2000000000000002,
        splicedDamageAnims=damageAnims,
        dodgeDelay=2.8999999999999999,
        splicedDodgeAnims=dodgeAnims,
        showDamageExtraTime=2.3999999999999999)
    return Parallel(suitTrack, toonTrack, propTrack)
...
def doChomp(attack):
    ...
    if dmg > 0:
        ...
        teethAppearTrack.append(
            LerpPosInterval(
                teeth, 0.10000000000000001, pos=Point3(x, y, toonHeight + 3)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.10000000000000001,
                pos=Point3(x, y - 1.2, toonHeight * 0.69999999999999996)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.10000000000000001,
                pos=Point3(x, y - 0.69999999999999996,
                           toonHeight * 0.40000000000000002)))
        teethAppearTrack.append(Wait(0.40000000000000002))
        scaleTrack = Sequence(
            Wait(throwDelay),
        ...
        teethAppearTrack.append(
            LerpPosInterval(
                teeth, 0.20000000000000001, pos=Point3(x + 0.5, y - 2.5, z)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.20000000000000001,
                pos=Point3(x + 1.0, y - 3.0, z + 0.40000000000000002)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.20000000000000001,
                pos=Point3(x + 1.3, y - 3.6000000000000001, z)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.20000000000000001,
                pos=Point3(x + 0.90000000000000002, y - 3.1000000000000001,
                           z + 0.40000000000000002)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.20000000000000001,
                pos=Point3(x + 0.29999999999999999, y - 2.6000000000000001,
                           z)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.20000000000000001,
                pos=Point3(x - 0.10000000000000001, y - 2.2000000000000002,
                           z + 0.40000000000000002)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.20000000000000001,
                pos=Point3(x - 0.40000000000000002, y - 1.8999999999999999,
                           z)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.20000000000000001,
                pos=Point3(x - 0.69999999999999996, y - 2.1000000000000001,
                           z + 0.40000000000000002)))
        teethAppearTrack.append(
            LerpPosInterval(
                teeth,
                0.20000000000000001,
                pos=Point3(x - 0.80000000000000004, y - 2.2999999999999998,
                           z)))
        teethAppearTrack.append(
            LerpScaleInterval(teeth, 0.59999999999999998,
                              MovieUtil.PNT3_NEARZERO))
        hprTrack = Sequence(
            Wait(throwDelay),
            LerpHprInterval(teeth, 0.29999999999999999, Point3(180, 0, 0)),
            ...
            LerpHprInterval(
                ...
                0.80000000000000004,
                Point3(-10, 0, 0),
                startHpr=Point3(80, 0, 0)))
        animTrack = Sequence(
            Wait(throwDelay),
            ActorInterval(teeth, 'teeth', duration=3.6000000000000001))
        propTrack = Sequence(
            Parallel(teethAppearTrack, hprTrack, animTrack),
            Func(MovieUtil.removeProp, teeth),
            Func(battle.movie.clearRenderProp, teeth))
    damageAnims = [['cringe', 0.01, 0.69999999999999996, 1.2],
                   ['spit', 0.01, 2.9500000000000002, 1.47],
                   ['spit', 0.01, 4.4199999999999999, 0.070000000000000007],
                   [
                       'spit', 0.080000000000000002, 4.4900000000000002,
                       -0.070000000000000007
                   ],
                   [
                       'spit', 0.080000000000000002, 4.4199999999999999,
                       0.070000000000000007
                   ],
                   [
                       'spit', 0.080000000000000002, 4.4900000000000002,
    ...
    toonTrack = getToonTrack(
        ...
        dodgeDelay=2.75,
        splicedDodgeAnims=dodgeAnims,
        showDamageExtraTime=1.3999999999999999)
    return Parallel(suitTrack, toonTrack, propTrack)
...
def doEvictionNotice(attack):
    ...
    suitTrack = getSuitTrack(attack)
    posPoints = [
        Point3(-0.040000000000000001, 0.14999999999999999,
               -1.3799999999999999),
    ...
    missPoint = __toonGroundPoint(
        attack, toon, 0.69999999999999996, parent=battle)
    missPoint.setX(missPoint.getX() - 1.1000000000000001)
    propTrack.append(
        getPropThrowTrack(
            attack, paper, [hitPoint], [missPoint], parent=battle))
    toonTrack = getToonTrack(attack, 3.3999999999999999, ['conked'],
                             2.7999999999999998, ['jump'])
    return Parallel(suitTrack, toonTrack, propTrack)
...
def doWithdrawal(attack):
    ...
    suitTrack = getSuitAnimTrack(attack)
    partTrack = getPartTrack(particleEffect, 1.0000000000000001e-005,
    ...
    toonTrack = getToonTrack(
        ...
        0.20000000000000001,
        splicedDodgeAnims=[[
            'duck', 1.0000000000000001e-005, 0.80000000000000004
        ]],
        showMissedExtraTime=0.80000000000000004)
    headParts = toon.getHeadParts()
    torsoParts = toon.getTorsoParts()
    ...
    soundTrack = getSoundTrack(
        'SA_withdrawl.mp3', delay=1.3999999999999999, node=suit)
    if dmg > 0:
        colorTrack = Sequence()
        ...
        colorTrack.append(
            Parallel(
                changeColor(headParts), changeColor(torsoParts),
...
def doJargon(attack):
    ...
    partTrack3 = getPartTrack(particleEffect3, partDelay + partInterval * 2, 2,
                              [particleEffect3, suit, 0])
    partTrack4 = getPartTrack(particleEffect4, partDelay + partInterval * 3,
                              1.0, [particleEffect4, suit, 0])
    damageAnims = []
    damageAnims.append(['conked', 0.0001, 0, 0.40000000000000002])
    damageAnims.append(
        ['conked', 0.0001, 2.7000000000000002, 0.84999999999999998])
    damageAnims.append(
        ['conked', 0.0001, 0.40000000000000002, 0.089999999999999997])
    damageAnims.append(
        ['conked', 0.0001, 0.40000000000000002, 0.089999999999999997])
    damageAnims.append(
        ['conked', 0.0001, 0.40000000000000002, 0.66000000000000003])
    damageAnims.append(
        ['conked', 0.0001, 0.40000000000000002, 0.089999999999999997])
    damageAnims.append(
        ['conked', 0.0001, 0.40000000000000002, 0.089999999999999997])
    damageAnims.append(
        ['conked', 0.0001, 0.40000000000000002, 0.85999999999999999])
    damageAnims.append(
        ['conked', 0.0001, 0.40000000000000002, 0.14000000000000001])
    damageAnims.append(
        ['conked', 0.0001, 0.40000000000000002, 0.14000000000000001])
    damageAnims.append(['conked', 0.0001, 0.40000000000000002])
    ...
    toonTrack = getToonTrack(
        ...
        splicedDodgeAnims=dodgeAnims,
        showMissedExtraTime=1.6000000000000001,
        showDamageExtraTime=0.69999999999999996)
    soundTrack = getSoundTrack(
        'SA_jargon.mp3', delay=2.1000000000000001, node=suit)
    return Parallel(suitTrack, toonTrack, soundTrack, partTrack, partTrack2,
                    partTrack3, partTrack4)
...
def doMumboJumbo(attack):
    ...
    dmg = target['hp']
    BattleParticles.loadParticles()
    particleEffect = BattleParticles.createParticleEffect(
        file='mumboJumboSpray')
    particleEffect2 = BattleParticles.createParticleEffect(
        file='mumboJumboSpray')
    particleEffect3 = BattleParticles.createParticleEffect(
        file='mumboJumboSmother')
    particleEffect4 = BattleParticles.createParticleEffect(
        file='mumboJumboSmother')
    particleEffect5 = BattleParticles.createParticleEffect(
        file='mumboJumboSmother')
    BattleParticles.setEffectTexture(
    ...
    partTrack3 = getPartTrack(particleEffect3, 3.2999999999999998, 1.7,
                              [particleEffect3, toon, 0])
    partTrack4 = getPartTrack(particleEffect4, 3.2999999999999998, 1.7,
                              [particleEffect4, toon, 0])
    partTrack5 = getPartTrack(particleEffect5, 3.2999999999999998, 1.7,
                              [particleEffect5, toon, 0])
    toonTrack = getToonTrack(attack, 3.2000000000000002, ['cringe'],
                             2.2000000000000002, ['sidestep'])
...
def doGuiltTrip(attack):
    suit = attack['suit']
    battle = attack['battle']
    centerColor = Vec4(1.0, 0.20000000000000001, 0.20000000000000001,
                       0.90000000000000002)
    edgeColor = Vec4(0.90000000000000002, 0.90000000000000002,
    ...
    partTrack1 = getPowerTrack(powerBar1)
    partTrack2 = getPowerTrack(powerBar2)
    waterfallTrack = getPartTrack(waterfallEffect, 0.59999999999999998,
                                  0.59999999999999998,
                                  [waterfallEffect, suit, 0])
    toonTracks = getToonTracks(attack, 1.5, ['slip-forward'],
                               0.85999999999999999, ['jump'])
    soundTrack = getSoundTrack(
        'SA_guilt_trip.mp3', delay=1.1000000000000001, node=suit)
...
def doRestrainingOrder(attack):
    ...
    propTrack.append(
        getPropThrowTrack(
            attack, paper, [hitPoint], [missPoint], parent=battle))
    damageAnims = [['conked', 0.01, 0.29999999999999999, 0.20000000000000001],
                   ['struggle', 0.01, 0.20000000000000001]]
    toonTrack = getToonTrack(
        attack,
        damageDelay=3.3999999999999999,
        splicedDamageAnims=damageAnims,
    ...
    if dmg > 0:
        ...
        restraintCloud.setPos(hitPoint.getX(),
                              hitPoint.getY() + 0.5, hitPoint.getZ())
        cloudTrack = getPartTrack(restraintCloud, 3.5, 0.20000000000000001,
                                  [restraintCloud, battle, 0])
...
def doSpin(attack):
    ...
    height1 = toon.getHeight() * (
        random.random() * 0.20000000000000001 + 0.69999999999999996)
    height2 = toon.getHeight() * (
        random.random() * 0.20000000000000001 + 0.40000000000000002)
    height3 = toon.getHeight() * (
        random.random() * 0.20000000000000001 + 0.10000000000000001)
    spinEffect1.setPos(0.80000000000000004, -0.69999999999999996, height1)
    spinEffect1.setHpr(0, 0, -random.random() * 10 - 85)
    spinEffect1.setHpr(spinEffect1, 0, 50, 0)
    spinEffect2.setPos(0.80000000000000004, -0.69999999999999996, height2)
    spinEffect2.setHpr(0, 0, -random.random() * 10 - 85)
    spinEffect2.setHpr(spinEffect2, 0, 50, 0)
    spinEffect3.setPos(0.80000000000000004, -0.69999999999999996, height3)
    ...
    sprayTrack = getPartTrack(sprayEffect, 1.0, 1.8999999999999999,
                              [sprayEffect, suit, 0])
    spinTrack1 = getPartTrack(spinEffect1, 2.1000000000000001,
                              3.8999999999999999, [spinEffect1, battle, 0])
    spinTrack2 = getPartTrack(spinEffect2, 2.1000000000000001,
                              3.8999999999999999, [spinEffect2, battle, 0])
    spinTrack3 = getPartTrack(spinEffect3, 2.1000000000000001,
                              3.8999999999999999, [spinEffect3, battle, 0])
    damageAnims = []
    damageAnims.append(['duck', 0.01, 0.01, 1.1000000000000001])
    damageAnims.extend(
        getSplicedLerpAnims(
            'think',
            0.66000000000000003,
            1.1000000000000001,
            startTime=2.2599999999999998))
    damageAnims.extend(
        getSplicedLerpAnims(
            'think',
            0.66000000000000003,
    ...
    if dmg > 0:
        toonSpinTrack = Sequence(
            Wait(damageDelay + 0.90000000000000002),
...
def doLegalese(attack):
    ...
    toonTrack = getToonTrack(
        ...
        dodgeAnimNames=['sidestep'],
        showMissedExtraTime=0.80000000000000004)
    return Parallel(suitTrack, toonTrack, sprayTrack1, sprayTrack2,
                    sprayTrack3)
...
def doPeckingOrder(attack):
    ...
    for i in range(0, numBirds):
        ...
        if dmg > 0:
            hitPoint = Point3(random.random() * 5 - 2.5,
                              random.random() * 2 - 1 - 6,
                              (random.random() * 3 - 1.5) + toon.getHeight() -
                              0.90000000000000002)
        else:
            hitPoint = Point3(random.random() * 2 - 1,
                              random.random() * 4 - 2 - 15,
                              (random.random() * 4 - 2) + 2.2000000000000002)
        birdTrack = Sequence(
            Wait(throwDelay), Func(battle.movie.needRestoreRenderProp, next),
    ...
    damageAnims = []
    damageAnims.append(
        ['cringe', 0.01, 0.14000000000000001, 0.20999999999999999])
    damageAnims.append(['cringe', 0.01, 0.14000000000000001, 0.13])
    damageAnims.append(['cringe', 0.01, 0.42999999999999999])
    toonTrack = getToonTrack(
        attack,
        damageDelay=4.2000000000000002,
        splicedDamageAnims=damageAnims,
        dodgeDelay=2.7999999999999998,
        dodgeAnimNames=['sidestep'],
        showMissedExtraTime=1.1000000000000001)
