...
class DistributedBattleBldgAI(DistributedBattleBaseAI.DistributedBattleBaseAI):
        ...
        if not self.ignoreFaceOffDone:
            if self.allToonsResponded():
                ...
                self.timer.startCallback(
                    TIMEOUT_PER_USER,
                    self._DistributedBattleBldgAI__serverFaceOffDone)
    ...
    def enterFaceOff(self):
        ...
        self.timer.startCallback(
            self.calcToonMoveTime(self.pos, self.elevatorPos) + FACEOFF_TAUNT_T
            + SERVER_BUFFER_TIME,
            self._DistributedBattleBldgAI__serverFaceOffDone)

    def _DistributedBattleBldgAI__serverFaceOffDone(self):
    ...
    def _DistributedBattleBldgAI__goToResumeState(self, task):
        self.b_setState('Resume')
