...
from otp.distributed.MessageTypes import *
...
def msgpack_length(
    ...
    if length < maxfix:
        ...
        dg.addUint8(tag8)
        dg.addUint8(length)
    elif tag16 is not None and length < 1 << 0x10:
        dg.addUint8(tag16)
        dg.addBeUint16(length)
    elif tag32 is not None and length < 1 << 0x20:
        dg.addUint8(tag32)
...
def msgpack_encode(dg, element):
    if element == None:
        ...
        if -0x20 <= element < 0x80:
            dg.addInt8(element)
        elif 0x80 <= element < 256:
            dg.addUint8(0xcc)
            dg.addUint8(element)
        elif 256 <= element < 65536:
            dg.addUint8(0xcd)
            dg.addBeUint16(element)
        elif 65536 <= element < 1 << 0x20:
            dg.addUint8(0xce)
            dg.addBeUint32(element)
        elif 1 << 0x20 <= element < 1 << 64:
            dg.addUint8(0xcf)
            dg.addBeUint64(element)
        elif -0x80 <= element < -0x20:
            dg.addUint8(0xd0)
            dg.addInt8(element)
        elif -32768 <= element < -0x80:
            dg.addUint8(0xd1)
            ...
            raise ValueError('int out of range for msgpack: %d'
                             % element)
        ...
        raise TypeError('Encountered non-MsgPack-packable value: %r'
                        % element)
...
class OTPInternalRepository(ConnectionRepository):
    ...
    def __init__(
        ...
        ConnectionRepository.__init__(self, connectMethod, config,
                hasOwnerView=False, threadedNet=threadedNet)
        ...
        maxChannels = self.config.GetInt('air-channel-allocation',
                1000000)
        self.channelAllocator = UniqueIdAllocator(baseChannel,
                baseChannel + maxChannels - 1)
        ...
        self.eventLogId = self.config.GetString('eventlog-id', 'AIR:%d'
                % self.ourChannel)
    ...
    def clearPostRemove(self):
        ...
        dg = PyDatagram()
        self.addServerControlHeader(dg, CONTROL_CLEAR_POST_REMOVE)
    ...
    def handleDatagram(self, di):
        ...
        if msgType in (STATESERVER_OBJECT_ENTER_AI_WITH_REQUIRED,
            ...
            self.handleObjEntry(di, msgType
                                == STATESERVER_OBJECT_ENTER_AI_WITH_REQUIRED_OTHER)
    ...
    def handleObjLocation(self, di):
        ...
        if not do:
            self.notify.warning('Received location for unknown doId=%d!'
                                 % doId)
            return
    ...
    def handleObjEntry(self, di, other):
        doId = di.getUint64()
        parentId = di.getUint64()
        zoneId = di.getUint32()
        classId = di.getUint16()

        if classId not in self.dclassesByNumber:
            self.notify.warning('Received entry for unknown dclass=%d! (Object %d)'
                                 % (classId, doId))
            return
    ...
    def handleGetActivatedResp(self, di):
        ...
        if ctx not in self.__callbacks:
            self.notify.warning('Received unexpected DBSS_OBJECT_GET_ACTIVATED_RESP (ctx: %d)'
                                 % ctx)
            return
    ...
    def handleGetLocationResp(self, di):
        ...
        if ctx not in self.__callbacks:
            self.notify.warning('Received unexpected STATESERVER_OBJECT_GET_LOCATION_RESP (ctx: %d)'
                                 % ctx)
            return
    ...
    def handleGetObjectResp(self, di):
        ...
        if ctx not in self.__callbacks:
            self.notify.warning('Received unexpected STATESERVER_OBJECT_GET_ALL_RESP (ctx: %d)'
                                 % ctx)
            return

        if classId not in self.dclassesByNumber:
            self.notify.warning('Received STATESERVER_OBJECT_GET_ALL_RESP for unknown dclass=%d! (Object %d)'
                                 % (classId, doId))
            return
        ...
        try:
            self.__callbacks[ctx](doId, parentId, zoneId, dclass,
                                  fields)
        finally:
            del self.__callbacks[ctx]
    ...
    def getNetworkAddress(self, clientId, callback):
        ...
        self.__callbacks[ctx] = callback
        dg = PyDatagram()
        dg.addServerHeader(clientId, self.ourChannel,
    ...
    def handleGetNetworkAddressResp(self, di):
        ...
        if ctx not in self.__callbacks:
            self.notify.warning('Received unexpected CLIENTAGENT_GET_NETWORK_ADDRESS_RESP (ctx: %d)'
                                 % ctx)
            return

        try:
            self.__callbacks[ctx](remoteIp, remotePort, localIp,
                                  localPort)
        finally:
            del self.__callbacks[ctx]
    ...
    def sendUpdateToChannel(
        ...
        dg = field.aiFormatUpdate(do.doId, channelId, self.ourChannel,
                                  args)
        self.send(dg)
    ...
    def sendActivate(
        ...
        if dclass and fields:
            for (k, v) in fields.items():
                ...
                if not field:
                    self.notify.error('Activation request for %s object contains invalid field named %s'
                             % (dclass.getName(), k))
    ...
    def generateWithRequiredAndId(
        ...
        do.doId = doId
        self.addDOToTables(do, location=(parentId, zoneId))
        do.sendGenerateWithRequired(self, parentId, zoneId,
                                    optionalFields)
    ...
    def __connectFailed(
        ...
        self.notify.warning('Failed to connect! (code=%s; %r)' % (code,
                            explanation))

        # Try again...

        retryInterval = config.GetFloat('air-reconnect-delay', 5.0)
        taskMgr.doMethodLater(retryInterval, self.connect,
                              'Reconnect delay', extraArgs=[host, port])
    ...
    def setEventLogHost(self, host, port=7197):
        ...
        if not address.setHost(host, port):
            self.notify.warning('Invalid Event Log host specified: %s:%s'
                                 % (host, port))
            self.eventSocket = None
        else:
