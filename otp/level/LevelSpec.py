    ...
    if __dev__:
        ...
        def getPrettyString(self):
            ...
            def getPrettyEntityDictStr(name, dict, tabs=0):
                ...
                for type in types:
                    str += t(1) + '# %s\n' % string.upper(type)
                    entIds = sorted(entType2ids[type])
                    for entId in entIds:
                        str += t(1) + '%s: {\n' % entId
                        spec = dict[entId]
        ...
        def _recurKeyTest(self, dict1, dict2):
            ...
            for key in dict1:
                ...
                self._recurKeyTest(dict1[key], dict2[key])
        ...
        def __repr__(self):
            return 'LevelSpec(%s, scenario=%s)' % (repeatableRepr(
                self.specDict), repeatableRepr(self.scenario))
