...
def irisInOut(self, track, subjectNodePath, duration):
    track.append(
        Sequence(
            ...
            Func(base.transitions.irisIn, 1.5), Wait(duration),
...
class CutScene(BasicEntities.NodePathEntity, DirectObject.DirectObject):
    ...
    def startOrStop(self, start):
        ...
        if start:
            ...
            track = self.getEffect(self, track, self.subjectNodePath,
                                   self.duration)
            track = self.getMotion(self, track, self.subjectNodePath,
                                   self.duration)
